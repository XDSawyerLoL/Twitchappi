...
// NOTE: In some builds there is no pre-existing openMarketOverlay(). We provide a safe implementation here.
async function ensureTwitchConnected(){
  try{
    const r = await fetch('/twitch_user_status', { credentials:'include' });
    const j = await r.json().catch(()=>null);
    if(j && j.is_connected){
      // standardize global state for other modules
      window.currentUser = window.currentUser || {};
      window.currentUser.is_connected = true;
      window.currentUser.display_name = j.display_name || window.currentUser.display_name;
      window.currentUser.login = j.login || window.currentUser.login;
      window.currentUser.id = j.id || window.currentUser.id;
      window.currentUser.profile_image_url = j.profile_image_url || window.currentUser.profile_image_url;
      return true;
    }
  }catch(e){}
  return false;
...
window.openMarketOverlay = async function(){
  ...
  const ok = await ensureTwitchConnected();
  if(!ok){
...
window.closeMarketOverlay = function(){
  ...
  function lockSidePanelToPlayer(){
    ...
    const apply = ()=>{
      ...
      // keep layout stable; only constrain height, allow internal scrolling
