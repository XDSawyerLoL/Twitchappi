...
nav.querySelectorAll('.u-tab-btn').forEach(b=>b.classList.remove('active'));
    ...
    async function checkAuth() {
      ...
      if (data.is_connected) {
        ...
        // Expose auth state globally for modules (Market, etc.)
        window.currentUser = {
          is_connected: true,
    ...
    window.startAuth = function startAuth() {
    ...
    window.logout = function logout() { fetch(`${API_BASE}/twitch_logout`, { method:'POST' }).then(()=>location.reload()); }
