...
// Serve UI from /public (Option A)
app.use(express.static(path.join(__dirname, 'public')));
app.use(express.static(path.join(__dirname)));

// Page principale (UI)
app.get('/', (req, res) => {
  // Prefer /public UI (Option A)
  const candidates = [
    process.env.UI_FILE,
    path.join('public','NicheOptimizer.html'),
    path.join('public','NicheOptimizer_v56.html'),
    'NicheOptimizer.html',
...
// Pricing (extension directe)
app.get('/pricing', (req, res) => {
  const f = path.join(__dirname, 'public', 'pricing.html');
  if (!fs.existsSync(f)) return res.status(404).send('pricing.html introuvable');
...
// =========================================================
// 3.B BILLING (Crédits + Premium) — Firestore source unique
// =========================================================
...
function requireTwitch(req, res, next){
  const u = req.session?.twitchUser;
  if (!u || (u.expiry && u.expiry <= Date.now())) return res.status(401).json({ error: 'Twitch login required' });
  return next();
}
...
async function getOrInitBilling(twitchId){
  ...
  const init = { plan: 'free', credits: FREE_START_CREDITS, createdAt: Date.now(), updatedAt: Date.now() };
  await ref.set(init, { merge: true });
  await db.collection(BILLING_LEDGER).add({ twitchId: String(twitchId), type: 'grant', amount: FREE_START_CREDITS, reason: 'free_start', ts: Date.now() });
  return { ref, data: init };
}

function creditsToActions(credits){
...
app.get('/api/billing/status', requireTwitch, async (req, res) => {
  try{
    ...
    const { data } = await getOrInitBilling(twitchId);
    const plan = (data.plan || 'free');
    const credits = Number(data.credits || 0);
    const actions = plan === 'premium' ? Infinity : creditsToActions(credits);
    return res.json({ ok:true, plan, credits, actions });
  }catch(e){
    return res.status(500).json({ error: e.message || 'billing_status_error' });
  }
});

app.get('/api/billing/packs', (req, res) => {
  // Static catalog (Stripe can map these to price IDs later)
  return res.json([
...
// DEV fallback (no payment) — enable with BILLING_DEMO_MODE=1
app.post('/api/billing/buy-pack', requireTwitch, express.json(), async (req, res) => {
  try{
    if (String(process.env.BILLING_DEMO_MODE || '0') !== '1') {
      return res.status(403).json({ error:'billing_demo_disabled' });
    }
    const packId = String((req.body||{}).packId || '');
    const packs = { starter:200, core:500, power:1200 };
    const add = packs[packId];
    if (!add) return res.status(400).json({ error:'pack_invalid' });

    const twitchId = req.session.twitchUser.id;
    const { ref, data } = await getOrInitBilling(twitchId);
    const credits = Number(data.credits || 0) + add;
    const plan = data.plan || 'free';
    await ref.set({ credits, plan: plan === 'free' ? 'credits' : plan, updatedAt: Date.now() }, { merge:true });
    await db.collection(BILLING_LEDGER).add({ twitchId: String(twitchId), type:'pack', amount:add, packId, ts: Date.now() });
    return res.json({ ok:true, credits });
  }catch(e){
    return res.status(500).json({ error: e.message || 'buy_pack_error' });
  }
});
...
app.post('/api/billing/subscribe-premium', requireTwitch, async (req, res) => {
  try{
    ...
    const twitchId = req.session.twitchUser.id;
    const { ref } = await getOrInitBilling(twitchId);
    await ref.set({ plan:'premium', updatedAt: Date.now() }, { merge:true });
    await db.collection(BILLING_LEDGER).add({ twitchId: String(twitchId), type:'premium', ts: Date.now() });
    return res.json({ ok:true, plan:'premium' });
  }catch(e){
    return res.status(500).json({ error: e.message || 'subscribe_error' });
  }
});

app.post('/api/billing/spend', requireTwitch, express.json(), async (req, res) => {
  try{
    const twitchId = req.session.twitchUser.id;
    const reason = String((req.body||{}).reason || 'action');
    const { ref, data } = await getOrInitBilling(twitchId);
    const plan = (data.plan || 'free');
    if (plan === 'premium') return res.json({ ok:true, plan, credits: Number(data.credits||0), actions: Infinity });

    const credits = Number(data.credits || 0);
    if (credits < ACTION_COST) return res.status(402).json({ error:'insufficient_credits', credits });

    const nextCredits = credits - ACTION_COST;
    await ref.set({ credits: nextCredits, updatedAt: Date.now(), plan: plan === 'free' ? 'credits' : plan }, { merge:true });
    await db.collection(BILLING_LEDGER).add({ twitchId: String(twitchId), type:'spend', amount: ACTION_COST, reason, ts: Date.now() });

    return res.json({ ok:true, plan: plan === 'free' ? 'credits' : plan, credits: nextCredits, actions: creditsToActions(nextCredits) });
  }catch(e){
    return res.status(500).json({ error: e.message || 'spend_error' });
  }
});
