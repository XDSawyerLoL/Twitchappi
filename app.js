/**
 * STREAMER & NICHE AI HUB - BACKEND (V50 - DATA & UI FIX + CHAT HUB PATCH)
 * =======================================================
 * - Moteur IA : @google/genai (Gemini 2.5 Flash)
 * - Scan : Enrichi via endpoint /channels
 * - Raid : Correction images
 * - Planning : Prompt IA forcé
 * - CHAT HUB : Ajout du support Socket.io (Lignes 58-75)
 */
...
const http = require('http'); // [AJOUT VITAL] Module HTTP natif
const { Server } = require("socket.io"); // [AJOUT VITAL] Module Socket.io
...
// =========================================================
// [MODIFICATION VITAL] CONFIGURATION SERVEUR & SOCKET.IO
// =========================================================
const server = http.createServer(app); // On crée un serveur HTTP combiné
const io = new Server(server, { 
    cors: { origin: "*" } // Autorise toutes les connexions (sécurité dev)
});

// LOGIQUE DU CHAT (Back-end)
const chatHistory = []; // Mémoire tampon
io.on('connection', (socket) => {
    // 1. Envoyer l'historique au nouvel arrivant
...
// [MODIFICATION VITAL] Utiliser "server.listen" pour Socket.io
server.listen(PORT, () => console.log(`🚀 SERVER V50 (CHAT HUB ENABLED) ON PORT ${PORT}`));
