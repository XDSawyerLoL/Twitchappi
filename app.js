...
// ---------------------------------------------------------
// Billing API (aliases for front-end compatibility)
// - /api/billing/status
// - /api/billing/packs
...
app.get('/api/billing/status', async (req,res)=>{
  try{
    ...
    if(!tu) return;
    const b = await getBillingDoc(tu);
    res.json({ plan: (b.plan || 'free'), credits: Number(b.credits||0) });
  }catch(e){
...
app.get('/api/billing/packs', async (req,res)=>{
  try{
    const tu = requireTwitchSession(req, res);
    if(!tu) return;
    // Keep in sync with server Stripe SKUs
    res.json([
      { id:'credits_500',  name:'Starter', credits:500,  actions: Math.floor(500/20),  price:'9,99€',  isBest:false },
      { id:'credits_1250', name:'Boost',   credits:1250, actions: Math.floor(1250/20), price:'19,99€', isBest:true  }
    ]);
  }catch(e){
    res.status(500).json({ error:e.message });
  }
...
app.post('/api/billing/buy-pack', async (req,res)=>{
  try{
    ...
    if(stripe && process.env.STRIPE_SECRET_KEY){
      ...
      const item = map[packId];
      if(!item || !item.price) return res.status(400).json({ error:'SKU invalide ou Price ID manquant côté serveur.' });
    ...
    // Dev / no Stripe: grant credits directly
    const credits = (packId === 'credits_1250') ? 1250 : 500;
    await updateBillingCredits(tu, credits);
    const b = await getBillingDoc(tu);
    res.json({ ok:true, plan: b.plan || 'free', credits: Number(b.credits||0) });
  }catch(e){
    res.status(500).json({ error:e.message });
  }
});
...
app.post('/api/billing/subscribe-premium', async (req,res)=>{
  try{
    ...
    if(stripe && process.env.STRIPE_SECRET_KEY){
      ...
      if(!price) return res.status(400).json({ error:'Price ID premium manquant côté serveur.' });
    ...
    // Dev / no Stripe: set premium directly
...
// Uses req.rawBody captured by bodyParser verify
app.post('/api/billing/webhook', async (req,res)=>{
  try{
    ...
    if(event.type === 'checkout.session.completed'){
      ...
      // Apply credits / plan
      if(userId){
        const fakeTU = { id: String(userId) };
        const credits = Number(s?.metadata?.credits || 0);
        const plan = s?.metadata?.plan;

        if(credits) await updateBillingCredits(fakeTU, credits);
        if(plan) await setBillingPlan(fakeTU, plan);
      }
    }
