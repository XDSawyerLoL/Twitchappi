...
/**
 * Firebase Admin credentials loader
 * Supports:
 *  - FIREBASE_SERVICE_ACCOUNT_JSON (recommended)
 *  - FIREBASE_SERVICE_KEY (legacy)
 *  - ./serviceAccountKey.json (local fallback)
 */
function loadFirebaseServiceAccount() {
  const envRaw = process.env.FIREBASE_SERVICE_ACCOUNT_JSON || process.env.FIREBASE_SERVICE_KEY;
  if (envRaw) {
    try {
      let rawJson = String(envRaw);
...
');
  ...
  try { return require('./serviceAccountKey.json'); } catch (e) { return null; }
}
...
// =========================================================
// STRIPE WEBHOOK (DOIT être déclaré AVANT bodyParser.json)
// =========================================================
app.post('/api/billing/webhook', bodyParser.raw({ type: 'application/json' }), async (req, res) => {
  try {
    if (!process.env.STRIPE_SECRET_KEY) return res.status(400).send('Stripe not configured');
    if (!process.env.STRIPE_WEBHOOK_SECRET) return res.status(400).send('Missing STRIPE_WEBHOOK_SECRET');

    // Lazy init to avoid order issues
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const twitchId = session.metadata?.twitchId || session.metadata?.twitch_user_id;
      const sku = session.metadata?.sku;

      if (twitchId && sku && firestoreOk) {
        const userRef = db.collection('billing_users').doc(String(twitchId));

        // Optional: prevent duplicates by checking ledger for stripeSessionId
        try {
          const dup = await userRef.collection('ledger').where('stripeSessionId', '==', session.id).limit(1).get();
          if (!dup.empty) return res.json({ received: true, duplicate: true });
        } catch (_) {}

        const packCredits = { credits_500: 500, credits_1250: 1250 };
        ...
        await db.runTransaction(async (tx) => {
          ...
          if (isPremium) {
            next.plan = 'premium';
          } else if (packCredits[sku]) {
            next.credits = Number(cur.credits || 0) + Number(packCredits[sku]);
          }
        ...
        try {
          await userRef.collection('ledger').add({
            type: isPremium ? 'subscription' : 'purchase',
            sku,
...
async function getBillingDoc(twitchUser){
  ...
  if(!snap.exists){
    ...
    // Ledger (optional, for audit)
    try {
      await ref.collection('ledger').add({
        type: 'grant',
        amount: 1200,
        reason: 'welcome_free_credits',
        at: admin.firestore.FieldValue.serverTimestamp()
      });
    } catch (_) {}
...
app.post('/api/billing/spend', async (req, res) => {
  try {
    ...
    if (plan === 'premium') {
      return res.json({ success: true, plan: 'premium', credits: Number(b.credits || 0), actions: Infinity });
    }

    const cost = Number(req.body?.cost || ACTION_COST_CREDITS);
    if (!Number.isFinite(cost) || cost <= 0) return res.status(400).json({ success: false, error: 'INVALID_COST' });

    if (!firestoreOk) return res.status(503).json({ success: false, error: 'FIRESTORE_DISABLED' });
    ...
    await db.runTransaction(async (tx) => {
      ...
      const cur = snap.exists ? snap.data() : { credits: 0, plan: 'free' };
      const curCredits = Number(cur.credits || 0);
      if (curCredits < cost) throw new Error('NOT_ENOUGH_CREDITS');
      nextCredits = curCredits - cost;
      tx.set(ref, {
        credits: nextCredits,
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      }, { merge: true });
    ...
    return res.json({
      ...
      plan: 'free',
      credits: nextCredits,
      actions: Math.floor(nextCredits / ACTION_COST_CREDITS)
    });
  } catch (e) {
