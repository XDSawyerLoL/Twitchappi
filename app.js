 ...
 * - Chat Hub : messages globaux liés à l’auth Twitch
 */
...
async function getTwitchToken(tokenType = 'app') {
    if (CACHE.twitchTokens[tokenType] &&
        CACHE.twitchTokens[tokenType].expiry > Date.now()) {
...
async function runGeminiAnalysis(prompt) {
    ...
    try {
        const response = await aiClient.models.generateContent({
            ...
            config: {
                systemInstruction:
                    "Tu es un expert Data Twitch. Réponds UNIQUEMENT en HTML simple (<h4>, <ul>, <li>, <p>). Sois précis, donne des chiffres et des horaires concrets."
            }
        });
...
app.get('/twitch_auth_start', (req, res) => {
    ...
    const url =
        ...
        `&response_type=code&scope=user:read:follows` +
        `&state=${state}`;
...
app.get('/twitch_auth_callback', async (req, res) => {
    ...
    try {
        ...
        if (tokenData.access_token) {
            ...
            // ✅ on stocke login + avatar pour le Chat Hub
            CACHE.twitchUser = {
...
app.get('/twitch_user_status', (req, res) => {
    if (CACHE.twitchUser && CACHE.twitchUser.expiry > Date.now()) {
        return res.json({
            ...
            login: CACHE.twitchUser.login,
            avatar_url: CACHE.twitchUser.profile_image_url
        });
    }
...
app.get('/get_latest_vod', async (req, res) => {
    try {
        ...
        const v = await twitchAPI(
            `videos?user_id=${u.data[0].id}&type=archive&first=1`
        );
...
async function refreshGlobalStreamList() {
    ...
    if (now - rot.lastFetchTime < rot.fetchCooldown &&
        rot.streams.length > 0) return;
    try {
    ...
    try {
        ...
        if (uRes.data.length) {
            ...
            const uData = {
                ...
                ai_calculated_niche_score:
                    isLive && streamInfo.viewer_count < 100 ? "4.8/5" : "3.0/5"
            };
...
app.post('/start_raid', async (req, res) => {
    ...
    try {
        const gRes = await twitchAPI(
            `search/categories?query=${encodeURIComponent(game)}&first=1`
        );
...
app.post('/analyze_schedule', async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            game_name: gameName,
            box_art: gRes.data[0].box_art_url
                .replace('{width}', '60')
                .replace('{height}', '80'),
...
app.get('/export_csv', (req, res) => {
    ...
    res.setHeader(
        'Content-Disposition',
        'attachment; filename=Twitch_Analysis.csv'
    );
    res.send(
...
app.get('/check_boost_status', async (req, res) => {
    ...
    try {
        ...
        if (!boostQuery.empty) {
            return res.json({
                is_boosted: true,
                channel: boostQuery.docs[0].data().channel,
                remaining_seconds: Math.ceil(
                    (boostQuery.docs[0].data().endTime - now) / 1000
                )
            });
        }
    } catch (e) {}
...
// =========================================================
// 7. CHAT HUB GLOBAL (Messages partagés)
// =========================================================

const CHAT_COLLECTION = 'hub_chat';

app.get('/chathub/messages', async (req, res) => {
    try {
        ...
        console.error('❌ [CHAT] Erreur fetch messages:', e.message);
        res.json({ success: false, error: 'Erreur chargement chat.' });
    }
});

app.post('/chathub/send', async (req, res) => {
    try {
        if (!CACHE.twitchUser || CACHE.twitchUser.expiry <= Date.now()) {
            return res.json({ success: false, error: 'Utilisateur non connecté.' });
        }
        ...
        console.error('❌ [CHAT] Erreur send message:', e.message);
        res.json({ success: false, error: 'Erreur envoi message.' });
    }
});

// =========================================================
// 8. ROUTE STATIC + STATS BACKGROUND
// =========================================================
...
app.listen(PORT, () =>
    console.log(`🚀 SERVER V50 (FINAL PATCH + CHAT HUB) ON PORT ${PORT}`)
);
