...
// =========================================================
// STRIPE WEBHOOK (server-side credits/plan updates)
...
app.post('/api/billing/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try{
    ...
    const userId = String(meta.twitch_user_id || obj.client_reference_id || '').trim();

    async function applyCredits(uid, credits){
      if(!firestoreOk || !uid) return;
      const ref = db.collection(BILLING_USERS).doc(uid);
      await ref.set({
        credits: admin.firestore.FieldValue.increment(Number(credits||0)),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      }, { merge:true });
    }
    async function applyPlan(uid, plan){
      if(!firestoreOk || !uid) return;
      const ref = db.collection(BILLING_USERS).doc(uid);
      await ref.set({
    ...
    if(event.type === 'checkout.session.completed'){
      ...
      const plan = meta.plan ? String(meta.plan) : '';
      if(userId && credits) await applyCredits(userId, credits);
      if(userId && plan) await applyPlan(userId, plan);
    }

    if(event.type === 'customer.subscription.deleted'){
      // if you store user id in metadata on subscription (recommended), fallback is not always available here
      const uid = String(meta.twitch_user_id || '').trim();
      if(uid) await applyPlan(uid, 'free');
    }
...
app.get('/twitch_auth_callback', async (req, res) => {
  ...
  try {
    ...
    req.session.save(() => {
      // Signed token fallback for iframe environments where cookies can be blocked
      const token = signAuthToken({
        exp: Math.floor((Date.now() + (Number(tokenData.expires_in || 0) * 1000)) / 1000),
      ...
      res.send(`
        ...
        <body style="background:#050607;color:#fff;font-family:system-ui;padding:18px">
          <div>✅ Connexion Twitch OK. Tu peux fermer cette fenêtre.</div>
          <script>
            (function(){
              try {
                if (window.opener && !window.opener.closed) {
                  window.opener.postMessage({ type:'TWITCH_AUTH', token:${'`'}${'${'}JSON.stringify(token)${'}'}${'`'} }, '*');
                }
              } catch(e){}
              window.close();
            })();
...
app.get('/twitch_user_status', (req, res) => {
  const u = getTwitchUserFromRequest(req);
...
// =========================================================
// AUTH TOKEN (fallback when 3rd-party cookies are blocked in iframes)
// - Signed HMAC token issued after Twitch OAuth
...
function signAuthToken(payload){
  const secret = String(process.env.SESSION_SECRET || 'dev_secret_change_me');
...
function verifyAuthToken(token){
  try{
    ...
    if(payload && payload.exp && Number(payload.exp) * 1000 <= Date.now()) return null;
    return payload;
  }catch(_){ return null; }
}
function getTwitchUserFromRequest(req){
  // 1) Authorization token (iframe-safe)
  const auth = req.get('authorization') || req.get('Authorization');
  if(auth && /^Bearer\s+/i.test(auth)){
    const tok = auth.replace(/^Bearer\s+/i,'').trim();
    const p = verifyAuthToken(tok);
    if(p && p.twitchUser){
      return p.twitchUser;
    }
  }
  // 2) Session cookie (normal)
  const u = req.session?.twitchUser;
