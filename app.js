...
// =========================================================
// BILLING WEBHOOK (Stripe) — MUST be registered before JSON body parsing
// Notes:
// - requires STRIPE_WEBHOOK_SECRET
// - credits / plan are stored in Firestore billing_users/{twitchUserId}
// =========================================================
app.post('/api/billing/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    ...
    if (!secret) return res.status(501).send('Missing STRIPE_WEBHOOK_SECRET');
    ...
    // Helpers by userId (no session here)
    async function addCreditsByUserId(userId, delta) {
    ...
    async function setPlanByUserId(userId, plan) {
      if (!firestoreOk) return;
      const id = String(userId || '').trim();
      if (!id) return;
      const ref = db.collection(BILLING_USERS).doc(id);
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const userId = s?.metadata?.twitch_user_id || s?.client_reference_id || '';
      const credits = Number(s?.metadata?.credits || 0);
      const plan = String(s?.metadata?.plan || '').toLowerCase();
      ...
      // (optional) store Stripe customer id for later billing portal / cancels
      if (firestoreOk && userId && s?.customer) {
        await db.collection(BILLING_USERS).doc(String(userId)).set({
    ...
    // When subscription is canceled -> downgrade plan
    if (event.type === 'customer.subscription.deleted') {
      const sub = event.data.object;
      const customerId = sub?.customer ? String(sub.customer) : null;
      if (firestoreOk && customerId) {
        const q = await db.collection(BILLING_USERS).where('stripeCustomerId', '==', customerId).limit(1).get();
        if (!q.empty) {
          const doc = q.docs[0];
          await doc.ref.set({ plan: 'free', updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
        }
      }
    }
...
// Packs catalog for the pricing UI
app.get('/api/billing/packs', (req,res)=>{
  res.json({
    success:true,
    packs:[
      { sku:'credits_500',  type:'credits', credits:500,  label:'500 crédits',  hint:'Pack crédits' },
      { sku:'credits_1250', type:'credits', credits:1250, label:'1250 crédits', hint:'Pack crédits' },
      { sku:'premium_monthly', type:'subscription', plan:'premium', label:'Premium (mensuel)', hint:'Marché + actions illimitées' }
    ]
  });
});

// Backward-compatible endpoints expected by pricing.js
...
app.post('/api/billing/subscribe-premium', async (req,res)=>{
  try{
    const tu = requireTwitchSession(req, res);
    if(!tu) return;
    req.body = Object.assign({}, req.body, { sku: 'premium_monthly' });
