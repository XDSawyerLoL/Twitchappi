...
// =========================================================
// ===== BILLING (Firestore source unique) =================
// =========================================================
const BILLING_USERS_COL = 'billing_users';
const BILLING_LEDGER_COL = 'billing_ledger';

const CREDIT_FREE_START = parseInt(process.env.FREE_START_CREDITS || '1200', 10);
const CREDITS_PER_ACTION = parseInt(process.env.CREDITS_PER_ACTION || '20', 10);

function requireTwitch(req, res){
  const u = req.session?.twitchUser;
  if (!u || (u.expiry && u.expiry <= Date.now())) {
    return res.status(401).json({ success:false, error:'Twitch non connecté' });
  }
  return u;
}

async function getOrCreateBillingUser(twitchUser){
  const uid = String(twitchUser.id);
  const ref = db.collection(BILLING_USERS_COL).doc(uid);
  const snap = await ref.get();
  ...
  const payload = {
    ...
    twitchDisplayName: twitchUser.display_name || null,
    plan: 'free',
    credits: CREDIT_FREE_START,
    createdAt: admin.firestore.Timestamp.fromMillis(Date.now()),
    updatedAt: admin.firestore.Timestamp.fromMillis(Date.now())
  };
  await ref.set(payload, { merge: true });
  await db.collection(BILLING_LEDGER_COL).add({
    twitchId: uid,
    type: 'grant',
    amount: CREDIT_FREE_START,
    reason: 'free_start',
    ts: admin.firestore.Timestamp.fromMillis(Date.now())
  });
  return { id: uid, ...payload };
}

function computeActions(credits){
  const c = Number(credits || 0);
  return Math.floor(Math.max(0, c) / CREDITS_PER_ACTION);
}

// Status billing (plan + credits)
app.get('/api/billing/me', async (req, res) => {
  const twitchUser = requireTwitch(req, res);
  if (!twitchUser) return;
  try{
    const u = await getOrCreateBillingUser(twitchUser);
    return res.json({
      success: true,
      plan: u.plan || 'free',
      credits: Number(u.credits || 0),
      actions: computeActions(u.credits),
...
app.get('/api/billing/packs', (req, res) => {
  return res.json({
    ...
    credits_per_action: CREDITS_PER_ACTION,
    packs: [
      { sku:'credits_500', credits:500, price_eur:9.99, label:'500 crédits' },
      { sku:'credits_1250', credits:1250, price_eur:19.99, label:'1250 crédits' }
    ]
  });
});

// Spend credits (1 action)
app.post('/api/billing/spend', async (req, res) => {
  const twitchUser = requireTwitch(req, res);
  if (!twitchUser) return;
  const cost = Number(req.body?.cost || CREDITS_PER_ACTION);
  const reason = String(req.body?.reason || 'action').slice(0, 64);
  if (!Number.isFinite(cost) || cost <= 0) {
    return res.status(400).json({ success:false, error:'cost invalide' });
  }
  ...
  try{
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const plan = String(cur.plan || 'free');
      const credits = Number(cur.credits || 0);
      if (plan !== 'free') {
        return { ok: true, plan, credits, actions: computeActions(credits), spent: 0 };
      }
      if (credits < cost) {
        return { ok: false, plan, credits, actions: computeActions(credits) };
      }
      const next = credits - cost;
      tx.set(ref, { credits: next, updatedAt: admin.firestore.Timestamp.fromMillis(Date.now()) }, { merge: true });
      return { ok: true, plan, credits: next, actions: computeActions(next), spent: cost };
    });

    if (!result.ok) return res.status(402).json({ success:false, error:'Crédits insuffisants', ...result });
    ...
    return res.json({ success:true, ...result, credits_per_action: CREDITS_PER_ACTION });
  }catch(e){
    return res.status(500).json({ success:false, error: e.message });
  }
});

// Create checkout session (Stripe optional)
app.post('/api/billing/create-checkout-session', async (req, res) => {
  const twitchUser = requireTwitch(req, res);
  if (!twitchUser) return;

  const sku = String(req.body?.sku || '').trim();
  if (!sku) return res.status(400).json({ success:false, error:'sku manquant' });

  const STRIPE_SECRET_KEY = process.env.STRIPE_SECRET_KEY;
  if (!STRIPE_SECRET_KEY) {
    return res.status(501).json({ success:false, error:'Stripe non configuré (STRIPE_SECRET_KEY manquante)' });
  }

  // NOTE: Ici, on attend les priceId dans l'env pour prod.
  const stripe = require('stripe')(STRIPE_SECRET_KEY);
  const priceId = process.env[`STRIPE_PRICE_${sku.toUpperCase()}`];
  if (!priceId) {
    return res.status(501).json({ success:false, error:`PriceId Stripe manquant (STRIPE_PRICE_${sku.toUpperCase()})` });
  }
  ...
  try{
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: (process.env.BILLING_SUCCESS_URL || (req.protocol + '://' + req.get('host') + '/pricing')),
      cancel_url: (process.env.BILLING_CANCEL_URL || (req.protocol + '://' + req.get('host') + '/pricing')),
      metadata: {
