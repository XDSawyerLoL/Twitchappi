...
<head>
  ...
  /* ====== TWITFLIX MODAL (Netflix-like) ====== */
    ...
    .tf-modal-overlay.active { display:flex; animation: fadeIn .25s ease; }
    ...
    .tf-brand .dot{
      ...
      background:#00f2ea;
      box-shadow: 0 0 12px rgba(0,242,234,.55);
    }
    .tf-actions{
      display:flex;
      align-items:center;
    ...
    /* Netflix-like hover: zoom + lift + video preview */
    .tf-card{
      position: relative;
      aspect-ratio: 2/3;
      border-radius: 10px;
    ...
    .tf-card:hover .tf-poster{ opacity:.35; }
    ...
    .tf-card.previewing .tf-preview{ display:block; }
...
<body class="h-screen flex flex-col p-4">
        ...
        <div id="under-overview" class="u-tab-panel active p-4 bg-[#0b0b0d]">
          ...
          <div class="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div class="bg-[#111] p-3 rounded border border-[#222]">
              ...
              <div id="alerts-box" class="text-xs text-gray-400">—</div>
            </div>
        ...
        <div id="chat-container-hub" class="flex-grow hidden flex-col relative bg-[#0b0b0d]">
          <div id="hub-messages" class="flex-grow overflow-y-auto p-2 scrollbar-thin">
            <div class="text-center text-gray-600 text-xs mt-4">
              Bienvenue sur le Hub Sécurisé — Connecté en tant que : <span id="hub-user-display" class="text-[#00f2ea] font-bold">Anonyme</span>
            </div>
          </div>
  ...
  <div id="twitflix-modal" class="tf-modal-overlay">
    <div class="tf-modal-content">
      ...
      <div class="tf-sub">
        Style Netflix: survol = “bande-annonce” (preview Twitch) • clic = lance un stream < 100 viewers.
      </div>

      <div id="twitflix-grid" class="tf-grid">
        <div id="tf-loading" class="tf-empty"><i class="fas fa-spinner fa-spin"></i> Chargement du catalogue...</div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function initPlayer(){
      ...
      document.getElementById('twitch-chat-frame').src = `https://www.twitch.tv/embed/${currentChannel}/chat?parent=${domain}`;
      changeChannel(currentChannel);
      fetchDefaultStream();
    }

    async function fetchDefaultStream(){
      try{
        ...
        if (data.success && data.channel){
          ...
          if (data.viewers != null) document.getElementById('viewer-count').innerText = `${data.viewers} viewers`;
          changeChannel(currentChannel);
        }
      }catch(e){}
    }
    ...
    function changeChannel(channel){
      ...
      container.innerHTML = `
        <iframe
          src="https://player.twitch.tv/?channel=${encodeURIComponent(channel)}&parent=${domain}&muted=false&autoplay=true"
          width="100%"
          height="100%"
          frameborder="0"
      ...
      document.getElementById('twitch-chat-frame').src = `https://www.twitch.tv/embed/${encodeURIComponent(channel)}/chat?parent=${domain}`;
      loadChannelKPIs(channel);
    }
    ...
    async function cycle(direction){
      try{
        ...
        const data = await res.json();
        if (data.success && data.channel){
          document.getElementById('player-mode-badge').innerText = 'AUTO';
    ...
    function toggleChatMode(mode){
      const t = document.getElementById('chat-container-twitch');
      const h = document.getElementById('chat-container-hub');
      const b1 = document.getElementById('btn-mode-twitch');
      const b2 = document.getElementById('btn-mode-hub');
      if (mode === 'twitch'){
        t.classList.remove('hidden'); h.classList.add('hidden');
        b1.className = 'flex-1 py-1 text-xs font-bold bg-[#6441a5] text-white rounded';
        b2.className = 'flex-1 py-1 text-xs font-bold bg-[#1a1a1a] text-gray-400 rounded hover:text-white';
      } else {
        t.classList.add('hidden'); h.classList.remove('hidden');
        b2.className = 'flex-1 py-1 text-xs font-bold bg-[#00f2ea] text-black rounded';
        b1.className = 'flex-1 py-1 text-xs font-bold bg-[#1a1a1a] text-gray-400 rounded hover:text-white';
      }
    }

    // CAROUSEL (followed streams)
      ...
      try{
        ...
        if (data.success && data.streams && data.streams.length){
          data.streams.forEach(s=>{
            ...
            card.onclick = ()=> changeChannel(s.user_login);
            const thumb = (s.thumbnail_url||'').replace('{width}','320').replace('{height}','180');
            card.innerHTML = `
              <img class="card-img" src="${thumb}" />
              <div class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black to-transparent">
                <div class="text-white text-sm font-bold truncate">${escapeHtml(s.user_name)}</div>
                <div class="text-xs text-[#00f2ea] truncate">${escapeHtml(s.game_name||'')}</div>
                <div class="text-[10px] text-gray-300">${s.viewer_count||0} viewers</div>
              </div>
            `;
            carousel.appendChild(card);
          });
        } else {
          carousel.innerHTML = `<div class="w-full text-center py-10 border border-dashed border-[#333] rounded text-gray-500">
            <i class="fas fa-satellite-dish mb-2"></i><br>Aucun suivi live.
          </div>`;
        }
      }catch(e){}
    }
    ...
    function initSocket(){
      try{
        ...
        socket.on('chat message',(msg)=>{
          ...
          box.innerHTML += `<div class="hub-msg"><strong class="text-[#00f2ea]">${escapeHtml(user)}</strong> <span class="text-gray-300">${escapeHtml(text)}</span></div>`;
    ...
    function sendHubMessage(e){
      ...
      const box = document.getElementById('hub-messages');
      box.innerHTML += `<div class="hub-msg"><strong class="text-[#ff0099]">${escapeHtml(currentUser || 'Anon')}</strong> <span class="text-gray-300">${escapeHtml(input.value)}</span></div>`;
      box.scrollTop = box.scrollHeight;
      input.value = '';
    }
    ...
    // TWITFLIX LOGIC (Netflix-like UI + search + hover preview)
    // NOTE: on reste compatible avec le backend actuel:
    //  - GET  /api/categories/top (pagination cursor)
    //  - POST /api/stream/by_category { game_id } -> { channel }

    let tfAllCategories = [];          // toutes les catégories chargées
    let tfRenderedIds = new Set();     // évite doublons
    let tfSearchQuery = '';
    const tfPreviewCache = new Map();  // gameId -> { channel, t }
    const tfPreviewInflight = new Map(); // gameId -> Promise
    ...
    async function openTwitFlix() {
      ...
      // Reset state
      tfAllCategories = [];
      tfRenderedIds = new Set();
      tfSearchQuery = '';
      currentCursor = null;

      // Reset UI
      grid.innerHTML = '<div id="tf-loading" class="tf-empty"><i class="fas fa-spinner fa-spin"></i> Chargement du catalogue...</div>';
      if (search) {
        search.value = '';
        search.oninput = debounce((e) => {
          tfSearchQuery = String(e.target.value || '').trim().toLowerCase();
          renderTwitFlixGrid();
        }, 180);
      }

      await loadMoreCategories(); // first batch
    }

    function closeTwitFlix() {
      // stop previews (si on ferme pendant un hover)
      document.querySelectorAll('.tf-card.previewing').forEach(stopTwitFlixPreview);
      document.getElementById('twitflix-modal').classList.remove('active');
    }

    function renderTwitFlixGrid() {
      const grid = document.getElementById('twitflix-grid');
      if (!grid) return;

      // si recherche active: filtre sur le déjà chargé
      if (tfSearchQuery) {
        const filtered = tfAllCategories.filter(c => (c.name || '').toLowerCase().includes(tfSearchQuery));
        grid.innerHTML = '';
        if (filtered.length === 0) {
          grid.innerHTML = '<div class="tf-empty">Aucun résultat pour cette recherche (essaie un autre titre).</div>';
          return;
        }
        filtered.forEach(cat => grid.appendChild(buildTwitFlixCard(cat)));
        return;
      }

      // pas de recherche -> grid complet (append-only)
      if (grid.children.length === 0) {
        grid.innerHTML = '<div id="tf-loading" class="tf-empty"><i class="fas fa-spinner fa-spin"></i> Chargement du catalogue...</div>';
      } else {
        // rien à faire: on append dans loadMoreCategories()
      }
    }

    function buildTwitFlixCard(cat) {
      ...
      div.innerHTML = `
        <img src="${cat.box_art_url}" class="tf-poster" loading="lazy" />
        <div class="tf-preview" aria-hidden="true"></div>
        <div class="tf-overlay">
          <div class="tf-name">${escapeHtml(cat.name)}</div>
          <div class="tf-meta">Survol = preview • Clic = lancer</div>
          <div class="flex justify-center"><span class="tf-badge"><i class="fas fa-play"></i> Preview</span></div>
        </div>
      `;
      ...
      div.addEventListener('mouseenter', () => {
        hoverTimer = setTimeout(() => startTwitFlixPreview(div), 420);
      });
      div.addEventListener('mouseleave', () => {
        if (hoverTimer) clearTimeout(hoverTimer);
        stopTwitFlixPreview(div);
      });
    ...
    async function loadMoreCategories() {
      ...
      try {
        ...
        if (data.success && data.categories) {
          ...
          // stocke dans tfAllCategories
          data.categories.forEach((cat) => {
            if (!tfRenderedIds.has(cat.id)) {
              tfAllCategories.push(cat);
              tfRenderedIds.add(cat.id);
            }
          });

          // si recherche active, re-render (sinon append)
          if (tfSearchQuery) {
            renderTwitFlixGrid();
          } else {
            if (grid && grid.children.length === 0) grid.innerHTML = '';
            data.categories.forEach((cat) => {
              if (grid && cat && cat.id && tfRenderedIds.has(cat.id)) {
                // append uniquement la batch si elle n'était pas déjà dans le DOM
                if (!grid.querySelector('.tf-card[data-game-id="' + String(cat.id) + '"]')) {
                  grid.appendChild(buildTwitFlixCard(cat));
                }
              }
            });
          }
          ...
          if (grid && grid.children.length === 0) {
            grid.innerHTML = '<div class="tf-empty" style="color:#ff6666;">Erreur chargement.</div>';
          }
        }
      } catch (e) {
        console.error(e);
        if (grid && grid.children.length === 0) {
          grid.innerHTML = '<div class="tf-empty" style="color:#ff6666;">Erreur réseau.</div>';
        }
      } finally {
    ...
    async function startTwitFlixPreview(cardEl) {
      try {
        ...
        const previewHost = cardEl.querySelector('.tf-preview');
        if (!previewHost) return;

        const channel = await getPreviewChannelForGame(gameId);
        if (!channel) return;

        // build Twitch embed iframe
        ...
        // on ignore les erreurs de preview (CORS / embed / rate limit)
      }
    }

    function stopTwitFlixPreview(cardEl) {
      try {
        if (!cardEl) return;
        const previewHost = cardEl.querySelector('.tf-preview');
    ...
    async function getPreviewChannelForGame(gameId) {
      const now = Date.now();
    ...
    async function playTwitFlixCategory(gameId, gameName) {
      ...
      document.getElementById('current-channel-display').innerText = "RECHERCHE TWITFLIX...";
      ...
      try {
        ...
        if (data.success && data.channel) {
          changeChannel(data.channel);
          document.getElementById('player-mode-badge').innerText = "TWITFLIX";
        } else {
          alert("Aucun stream < 100 viewers trouvé pour ce jeu !");
    ...
    async function loadStatsDashboard(){
      try{
        ...
        const lLabels = (dLang.languages||[]).map(x=>x.name);
        const lVals = (dLang.languages||[]).map(x=>x.percent);
        renderChart('chartLangs','doughnut',lLabels,lVals,'Langues');
      }catch(e){}
    }
    ...
    async function runScan(){
      ...
      if(!data.success){ alert('Introuvable'); return; }
      box.classList.remove('hidden');

      if(data.type==='user'){
        document.getElementById('scan-img').src = data.user_data.profile_image_url || '';
        document.getElementById('scan-name').innerText = data.user_data.display_name;
        document.getElementById('scan-game').innerText = data.user_data.game_name;
        const ai = await fetch(`${API_BASE}/critique_ia`,{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({ type:'user', query:q })
        });
        ...
        document.getElementById('scan-name').innerText = data.game_data.name;
        document.getElementById('scan-game').innerText = `${data.game_data.total_viewers} viewers`;
        const ai = await fetch(`${API_BASE}/critique_ia`,{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({ type:'niche', query:data.game_data.name })
        });
        const aiData = await ai.json();
        document.getElementById('scan-ai').innerHTML = aiData.html_response || '';
      }
    ...
    async function runRaid(){
      ...
      const maxv = parseInt(document.getElementById('raid-viewers').value||'100',10);
      if(!game) return;

      const res = await fetch(`${API_BASE}/start_raid`,{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({ game, max_viewers:maxv })
      });
      const data = await res.json();
      if(!data.success){ alert('Aucun target trouvé'); return; }
      ...
      document.getElementById('raid-info').innerHTML = `
        <div><strong>${escapeHtml(raidTarget.name)}</strong> (${escapeHtml(raidTarget.login)})</div>
        <div class="text-[#00f2ea]">${escapeHtml(raidTarget.game)}</div>
        <div>${raidTarget.viewers} viewers</div>
      `;
    }

    function goToRaid(){
      if(!raidTarget) return;
      changeChannel(raidTarget.login);
      document.getElementById('player-mode-badge').innerText = "RAID";
    }

    // CO-STREAM (optionnel: si ton backend l’a)
    async function loadCoStreamer(){
      const out = document.getElementById('costream-out');
      out.style.display = 'block';
      out.innerHTML = '<div class="text-gray-400 text-xs"><i class="fas fa-spinner fa-spin"></i> Recherche...</div>';
      try{
        const res = await fetch(`${API_BASE}/api/costream/find`);
        const data = await res.json();
        if(data && data.success){
          out.innerHTML = data.html_response || '<div class="text-gray-300 text-xs">OK</div>';
        }else{
          out.innerHTML = '<div class="text-gray-400 text-xs">Endpoint /api/costream/find absent (OK si pas activé).</div>';
        }
      }catch(e){
        out.innerHTML = '<div class="text-gray-400 text-xs">Endpoint /api/costream/find absent (OK si pas activé).</div>';
      }
    ...
    async function runBoost(){
      const ch = document.getElementById('boost-query').value.trim().toLowerCase();
      if(!ch) return;
      ...
      const data = await res.json();
      document.getElementById('boost-msg').innerHTML = data.html_response || '';
    }

    // KPIs (si ton backend les renvoie/si Firestore est configuré)
    async function loadChannelKPIs(channel){
      // placeholder safe: ton code original fait déjà ses appels ailleurs
    }

    function runSimulation(){
      const hours = parseInt(document.getElementById('sim-hours').value||'0',10);
      document.getElementById('sim-result').innerText = `${hours}h/sem → (simulation interne)`;
    }
  </script>
</body>
</html>
