/**
 * STREAMER HUB - BACKEND V28 (FINAL PRODUCTION)
 * =============================================
 * 1. Market Recorder : Sauvegarde les tendances (Jeux/Langues) dans Firebase.
 * 2. Multi-Type Market : Gère 'games' et 'languages' séparément.
 * 3. Raid Fix : IDs et logique de filtrage réparés.
 * 4. Dashboard Concis : Données formatées pour la clarté.
 */
...
if (process.env.FIREBASE_SERVICE_KEY) {
    ...
    } catch (e) { console.error("❌ Erreur Clé Firebase"); }
...
if (serviceAccount) {
    ...
    } catch (e) { console.error("❌ Erreur Init Firebase"); }
...
// --- 1. GLOBAL PULSE (MARKET DATA + FIREBASE) ---
app.post('/global_pulse', async (req, res) => {
    ...
    try {
        ...
        // Extrapolation "TwitchTracker Style"
        ...
        // SAUVEGARDE FIREBASE (Important !)
        try {
            await db.collection('market_history').add({
                type: type,
                timestamp: admin.firestore.FieldValue.serverTimestamp(),
                total_viewers: estViewers,
                data_snapshot: tableData.slice(0, 5) // On garde le top 5 pour l'historique
            });
        } catch(e) { console.log("Info: Firebase save skipped (local env?)"); }

        res.json({ success: true, overview, table: tableData });
    } catch(e) { res.status(500).json({error: e.message}); }
});

// --- 2. RAID (CORRIGÉ & SIMPLIFIÉ) ---
app.post('/start_raid', async (req, res) => {
    ...
    try {
        ...
        // Logique de tri : Trouver le plus gros stream SOUS la limite max
        ...
        return res.json({error: "Aucune cible trouvée avec ces critères."});
    } catch(e) { res.json({error: e.message}); }
});

// --- 3. DASHBOARD & SCAN (CONCIS) ---
app.post('/scan_target', async (req, res) => {
    ...
    try {
        ...
        if(u.data.length){
            ...
            if(old.exists) { diff = followers - (old.data().total_followers || 0); trend = diff > 0 ? "up" : (diff < 0 ? "down" : "stable"); }
            await ref.set({total_followers: followers, last_scan: new Date()}, {merge:true});

            // IA CONCISE
            const p = `Streamer: ${d.display_name}. Followers: ${followers}. Tendance: ${trend}. Donne 1 phrase de conseil stratégique ultra-court.`;
            const r = await ai.models.generateContent({ model: GEMINI_MODEL, contents: [{ role: "user", parts: [{ text: p }] }] });

            res.json({success:true, data: {...d, followers, days}, diff, trend, ia: r.response.text()});
        } else { res.json({success:false, error: "Pseudo introuvable"}); }
...
app.post('/analyze_schedule', async (req, res) => {
    ...
    try {
        ...
        const prompt = `Jeu: ${gRes.data[0].name}. ${sRes.data.length} streamers, ${totalV} viewers. Donne 3 meilleurs créneaux horaires (Jour + Heure) pour streamer. Réponds en liste HTML <ul>.`;
...
// --- AUTRES ROUTES (BOOST, ROTATION, AUTH) ---
app.post('/stream_boost', async(req,res)=>{
    const {channel}=req.body;
    try { 
        await db.collection('boosts').add({channel, endTime: Date.now()+900000}); 
        res.json({success:true}); 
    } catch(e){res.json({error:"Erreur DB"});}
});
...
app.post('/cycle_stream', async (req, res) => {
    ...
    // Simple rotation aléatoire pour l'exemple
...
app.listen(PORT, () => console.log(`SERVER V28 PRODUCTION ON PORT ${PORT}`));
