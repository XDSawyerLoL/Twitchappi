...
// =========================================================
// DISCOVERY USER SPACE (likes / recents / progress)
// Firestore: user_discovery/{twitchUserId}
// =========================================================
const USER_DISCOVERY = 'user_discovery';
...
async function getUserDiscoveryRef(tu){
  if(!firestoreOk) return null;
  const id = String(tu.id || tu.login || tu.display_name || 'unknown');
  return db.collection(USER_DISCOVERY).doc(id);
}

async function readUserDiscovery(tu){
  const ref = await getUserDiscoveryRef(tu);
  if(!ref) return { likes:{}, recents:[], progress:{} };
  const snap = await ref.get();
  const d = snap.exists ? (snap.data()||{}) : {};
  return {
    likes: d.anime_likes || {},
    recents: Array.isArray(d.anime_recents) ? d.anime_recents : [],
    progress: d.anime_progress || {}
  };
}

function normalizeRecentList(list){
  // keep last 80, unique by key, most recent first
  const out = [];
  const seen = new Set();
  for(const it of (Array.isArray(list)?list:[])){
    ...
    seen.add(k);
    out.push({ key:k, ts: ts||Date.now() });
    if(out.length>=80) break;
  }
  return out;
}

app.get('/api/user/discovery-state', async (req,res)=>{
  try{
    const tu = requireTwitchSession(req, res);
    if(!tu) return;
    const data = await readUserDiscovery(tu);
    res.json({ success:true, ...data, twitch_user_id: String(tu.id||'') });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
  }
});

app.post('/api/user/discovery-like', async (req,res)=>{
  try{
    const tu = requireTwitchSession(req, res);
    if(!tu) return;
...
app.post('/api/user/discovery-recent', async (req,res)=>{
  try{
    const tu = requireTwitchSession(req, res);
    if(!tu) return;
    if(!firestoreOk) return res.status(503).json({ success:false, error:'firestore_unavailable' });
    ...
    await db.runTransaction(async (tx)=>{
      ...
      const rec = [{ key, ts: Date.now() }, ...(Array.isArray(cur.anime_recents)?cur.anime_recents:[])];
      tx.set(ref, {
        anime_recents: normalizeRecentList(rec),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      }, { merge:true });
    });
    res.json({ success:true });
...
app.post('/api/user/discovery-progress', async (req,res)=>{
  try{
    const tu = requireTwitchSession(req, res);
    if(!tu) return;
    if(!firestoreOk) return res.status(503).json({ success:false, error:'firestore_unavailable' });
    ...
    const positionSec = clampNum(req.body?.positionSec||0, 0, 24*3600);
    const durationSec = clampNum(req.body?.durationSec||0, 0, 24*3600);
    if(!key || !episodeId) return res.status(400).json({ success:false, error:'missing_key_or_episode' });

    const ref = await getUserDiscoveryRef(tu);
    const patch = {};
    patch[`anime_progress.${key}`] = {
      episodeId,
      positionSec,
      durationSec,
      ts: Date.now()
