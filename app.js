/**
 * STREAMER & NICHE AI HUB - BACKEND (V70 - FINAL FIXES)
 * =======================================================
 * - Moteur IA : @google/genai (Gemini 2.5 Flash)
 * - Scan : Design et Data enrichie
 * - Raid : Cible unique précise
 * - Chat : Avatar + Pseudo via Socket.io
 */
...
if (process.env.FIREBASE_SERVICE_KEY) {
    try {
        ...
        if (rawJson.startsWith('"') && rawJson.endsWith('"')) rawJson = rawJson.slice(1, -1);
        // Nettoyage des sauts de ligne pour éviter le SyntaxError
        rawJson = rawJson.replace(/\n/g, '\\n').replace(/\r/g, '');
        serviceAccount = JSON.parse(rawJson);
    } catch (error) { console.error("❌ Erreur JSON Firebase (Mode limité):", error.message); }
} else {
...
app.get('/twitch_auth_callback', async (req, res) => {
    ...
    try {
        ...
        if (tokenData.access_token) {
            // Récupérer les infos user ET l'image de profil
            const userRes = await twitchAPI('users', tokenData.access_token);
            const user = userRes.data[0];
            CACHE.twitchUser = { 
                display_name: user.display_name, 
                id: user.id, 
                profile_image_url: user.profile_image_url, // Sauvegarde l'avatar
...
app.get('/twitch_user_status', (req, res) => {
    if (CACHE.twitchUser && CACHE.twitchUser.expiry > Date.now()) {
        return res.json({ 
            ...
            profile_image_url: CACHE.twitchUser.profile_image_url // Renvoie l'avatar au front
        });
    }
    res.json({ is_connected: false });
});

// SUIVI DE CHAINES (Correction Miniatures)
    ...
    try {
        ...
        if(uRes.data.length) {
            ...
            const uData = { 
                ...
                view_count: viewDisplay // Ajouté pour le frontend
            };
...
app.post('/critique_ia', async (req, res) => {
    ...
    const prompt = type === 'niche' ? `Audit expert de la chaîne Twitch "${query}". Format HTML court.` : `Idée de clip viral pour "${query}". Format HTML court.`;
...
// ✅ RAID FINDER (CORRIGÉ : CIBLE UNIQUE)
app.post('/start_raid', async (req, res) => {
    ...
    try {
        ...
        // Logique : Trouver le stream qui a le plus de viewers MAIS qui reste sous la limite max
        // Ex: Si max=50, on veut celui qui a 45, pas celui qui a 1.
        const target = sRes.data
            .filter(s => s.viewer_count <= parseInt(max_viewers))
            .sort((a,b) => b.viewer_count - a.viewer_count)[0]; // Trie décroissant et prend le premier
...
// Route par défaut (Pointe vers l'HTML)
app.get('/', (req,res) => {
...
// Lancement avec 'server.listen' pour Socket.io
server.listen(PORT, () => console.log(`🚀 SERVER V70 (CHAT & FIXES) ON PORT ${PORT}`));
