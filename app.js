...
// Serve UI & assets from /public (Option A)
...
// Page principale (UI)
// IMPORTANT: on sert la base UI depuis /public (Option A) pour garantir /assets/*
app.get('/', (req, res) => {
  // Priorité: UI_FILE si tu veux pointer vers un autre fichier
  if (process.env.UI_FILE) {
    const fp = path.isAbsolute(process.env.UI_FILE)
      ? process.env.UI_FILE
      : path.join(__dirname, process.env.UI_FILE);
    if (fs.existsSync(fp)) return res.sendFile(fp);
  }

  const publicUi = path.join(__dirname, 'public', 'NicheOptimizer.html');
  if (fs.existsSync(publicUi)) return res.sendFile(publicUi);

  // Fallback legacy (au cas où)
  const legacyCandidates = ['NicheOptimizer.html', 'NicheOptimizer_v56.html', 'index.html'];
  const found = legacyCandidates.find(f => fs.existsSync(path.join(__dirname, f)));
  if (!found) return res.status(500).send('UI introuvable sur le serveur.');
  return res.sendFile(path.join(__dirname, found));
});

// /pricing (extension directe de l'app)
app.get('/pricing', (req, res) => {
  const fp = path.join(__dirname, 'public', 'pricing.html');
  if (!fs.existsSync(fp)) return res.status(404).send('pricing.html introuvable');
...
// Pricing page (extension directe de l'app)
app.get("/pricing", (req, res) => {
...
const FREE_START_CREDITS = 1200;
const ACTION_COST_CREDITS = 20;

function requireTwitch(req, res){
  const u = req.session?.twitchUser;
  if (!u || (u.expiry && u.expiry <= Date.now())) {
    if (req.session) req.session.twitchUser = null;
    res.status(401).json({ error: 'TWITCH_LOGIN_REQUIRED' });
    return null;
  }
  return u;
}

async function getOrInitBillingDoc(twitchId){
  const ref = db.collection(BILLING_COLLECTION).doc(String(twitchId));
  const snap = await ref.get();
  ...
  await db.collection(BILLING_LEDGER).add({ twitchId: String(twitchId), type:'init_free', delta: FREE_START_CREDITS, ts: Date.now() });
  return { ref, data: init };
}

app.get('/api/billing/status', async (req, res) => {
  const u = requireTwitch(req, res); if (!u) return;
  try{
    const { data } = await getOrInitBillingDoc(u.id);
    const plan = data.plan || 'free';
    const credits = Number(data.credits || 0);
    const actions = (plan === 'premium' || plan === 'pro') ? Infinity : Math.floor(credits / ACTION_COST_CREDITS);
    res.json({ plan, credits, actions });
  }catch(e){
...
app.get('/api/billing/packs', (req, res) => {
  // Display packs (front uses pricing.html buttons)
  res.json({
    packs: [
      { sku:'credits_500', credits: 500, price: '9,99€' },
      { sku:'credits_1250', credits: 1250, price: '19,99€' }
    ],
    premium: { sku:'premium_monthly', price:'14,99€/mois' },
    actionCost: ACTION_COST_CREDITS,
    freeStartCredits: FREE_START_CREDITS
  });
});
...
app.post('/api/billing/spend', express.json(), async (req,res) => {
  ...
  try{
    const { ref, data } = await getOrInitBillingDoc(u.id);
    const plan = data.plan || 'free';
    if (plan === 'premium' || plan === 'pro') return res.json({ ok:true, plan, credits: data.credits ?? 0, actions: Infinity });

    const cur = Number(data.credits || 0);
    if (cur < ACTION_COST_CREDITS) return res.status(402).json({ error:'NOT_ENOUGH_CREDITS' });

    const next = cur - ACTION_COST_CREDITS;
    await ref.set({ credits: next, updatedAt: Date.now() }, { merge: true });
    await db.collection(BILLING_LEDGER).add({ twitchId: String(u.id), type:'spend', reason, delta: -ACTION_COST_CREDITS, ts: Date.now() });
    res.json({ ok:true, plan, credits: next, actions: Math.floor(next / ACTION_COST_CREDITS) });
  }catch(e){
    res.status(500).json({ error: e.message });
  }
});

// Stripe checkout (optional). Requires adding "stripe" dependency + env vars.
app.post('/api/billing/create-checkout-session', express.json(), async (req, res) => {
  const u = requireTwitch(req, res); if (!u) return;
  const sku = String(req.body?.sku || '');

  const STRIPE_SECRET_KEY = process.env.STRIPE_SECRET_KEY;
  if (!STRIPE_SECRET_KEY) return res.status(400).json({ success:false, error:'Stripe non configuré (STRIPE_SECRET_KEY manquant).' });

  let stripe;
  try { stripe = require('stripe')(STRIPE_SECRET_KEY); }
  catch(e){ return res.status(500).json({ success:false, error:'Dépendance stripe manquante. Ajoute "stripe" dans package.json.' }); }

  // Map SKUs to Stripe Price IDs
  const PRICE_CREDITS_500 = process.env.STRIPE_PRICE_CREDITS_500;
  const PRICE_CREDITS_1250 = process.env.STRIPE_PRICE_CREDITS_1250;
  const PRICE_PREMIUM = process.env.STRIPE_PRICE_PREMIUM;

  const priceId = (
    sku === 'credits_500' ? PRICE_CREDITS_500 :
    sku === 'credits_1250' ? PRICE_CREDITS_1250 :
    sku === 'premium_monthly' ? PRICE_PREMIUM :
    null
  );
  if (!priceId) return res.status(400).json({ success:false, error:'SKU inconnu ou PRICE_ID manquant côté serveur.' });

  try{
    const session = await stripe.checkout.sessions.create({
      mode: sku === 'premium_monthly' ? 'subscription' : 'payment',
      line_items: [{ price: priceId, quantity: 1 }],
      success_url: `${req.protocol}://${req.get('host')}/pricing?success=1`,
      cancel_url: `${req.protocol}://${req.get('host')}/pricing?cancel=1`,
      metadata: { twitchId: String(u.id), sku }
    });
...
// Dev-only toggles (no payment) — enable with BILLING_DEMO=true
app.post('/api/billing/dev/grant', express.json(), async (req,res)=>{
  if ((process.env.BILLING_DEMO || 'false').toLowerCase() !== 'true') return res.status(404).json({ error:'not_found' });
  const u = requireTwitch(req, res); if (!u) return;
  ...
  try{
    ...
    await ref.set({ credits: next, updatedAt: Date.now() }, { merge:true });
    await db.collection(BILLING_LEDGER).add({ twitchId:String(u.id), type:'dev_grant', delta, ts:Date.now() });
