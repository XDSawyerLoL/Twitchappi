...
app.post('/api/billing/buy-pack', express.json(), async (req, res) => {
  try{
    ...
    const nextCredits = curCredits + Number(p.credits || 0);
    const nextPlan = (data.plan && data.plan !== 'free') ? data.plan : 'credits';

    await ref.set({ credits: nextCredits, plan: nextPlan, updatedAt: Date.now() }, { merge:true });
    res.json({ ok:true, plan: nextPlan, credits: nextCredits, actions: computeActionsFromCredits(nextCredits) });
  }catch(e){
...
// Spend 1 action (20 credits) — for gating paid features
app.post('/api/billing/spend', express.json(), async (req, res) => {
  try{
    const { reason } = req.body || {};
    const { ref, data } = await getOrInitBillingState(req);
    const plan = data.plan || 'free';
    if (plan === 'premium') return res.json({ ok:true, plan:'premium', credits: Number(data.credits||0), actions:'∞' });

    const curCredits = Number(data.credits || 0);
    if (curCredits < ACTION_COST) return res.status(402).json({ error:'not_enough_credits', needed: ACTION_COST });
    ...
    // optional ledger
    try{
      await db.collection('billing_ledger').add({ key: getBillingUserKey(req), delta: -ACTION_COST, reason: String(reason||'action'), ts: Date.now() });
    }catch(_){}
