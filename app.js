...
const __jsonParser = bodyParser.json({ limit: '2mb' });
app.use((req,res,next)=>{
  if (req.originalUrl === '/api/billing/stripe/webhook') return next();
  return __jsonParser(req,res,next);
});
...
// =========================================================
// BILLING (Credits & Premium) — Firestore source of truth
// - Requires Twitch login (multi-user safe)
// - Packs add credits; Premium unlocks unlimited actions
// - Stripe Checkout optional (set STRIPE_SECRET_KEY + STRIPE_PRICE_PREMIUM)
// =========================================================
const BILLING_COLLECTION = 'billing_users';
const ACTION_COST = 20; // 1 action = 20 credits

function requireTwitchConnected(req, res){
  const u = req.session?.twitchUser;
  if (u && (!u.expiry || u.expiry > Date.now())) return true;
  res.status(401).json({ success:false, error:'Twitch non connecté. Connecte-toi pour accéder à cette fonctionnalité.' });
  return false;
}

function getBillingUserKey(req){
  const u = req.session?.twitchUser;
  if (u?.id) return `twitch:${u.id}`;
  if (u?.login) return `twitch_login:${String(u.login).toLowerCase()}`;
  return `session:${req.sessionID || 'anon'}`;
}

async function getBillingDocRef(req){
  const key = getBillingUserKey(req);
  return db.collection(BILLING_COLLECTION).doc(key);
}

function computeActionsFromCredits(credits){
  return Math.floor((Number(credits)||0) / ACTION_COST);
}

function listCreditPacks(){
  // Core pack = 500 credits (your chosen baseline)
  return [
    { id:'starter', name:'Starter', credits:200, actions:10, price_eur:4.99, price_label:'4,99 €', best:false },
    { id:'core', name:'Core', credits:500, actions:25, price_eur:9.99, price_label:'9,99 €', best:true },
    { id:'power', name:'Power', credits:1200, actions:60, price_eur:19.99, price_label:'19,99 €', best:false }
  ];
}

async function getOrInitBillingState(req){
  const ref = await getBillingDocRef(req);
  const snap = await ref.get();
  if (snap.exists) return { ref, data: snap.data() || {} };
  const init = { plan: 'free', credits: 0, updatedAt: Date.now(), createdAt: Date.now() };
  await ref.set(init, { merge: true });
  return { ref, data: init };
}

async function addCredits(req, deltaCredits, reason='pack'){
  const { ref, data } = await getOrInitBillingState(req);
  const curCredits = Number(data.credits || 0);
  const nextCredits = Math.max(0, curCredits + Number(deltaCredits || 0));
  const curPlan = String(data.plan || 'free');
  const nextPlan = (curPlan === 'premium') ? 'premium' : (nextCredits > 0 ? 'credits' : 'free');
  await ref.set({ credits: nextCredits, plan: nextPlan, updatedAt: Date.now() }, { merge:true });
  try{
    await db.collection('billing_ledger').add({ key: getBillingUserKey(req), delta: Number(deltaCredits||0), reason: String(reason||'pack'), ts: Date.now() });
  }catch(_){}
  return { plan: nextPlan, credits: nextCredits, actions: computeActionsFromCredits(nextCredits) };
}

// --- Public endpoints (UI) ---
app.get('/api/billing/status', async (req,res)=>{
  try{
    if(!requireTwitchConnected(req,res)) return;
    const { data } = await getOrInitBillingState(req);
    const plan = String(data.plan || 'free');
    const credits = Number(data.credits || 0);
    const actions = (plan === 'premium') ? '∞' : computeActionsFromCredits(credits);
    res.json({ success:true, plan, credits, actions });
  }catch(e){
    console.error('[BILLING] status error:', e);
    res.status(500).json({ success:false, error:'billing_status_failed' });
  }
});

app.get('/api/billing/packs', (req,res)=>{
  res.json({ success:true, packs: listCreditPacks(), action_cost: ACTION_COST });
});

// Demo / fallback buy (no Stripe) — increments credits directly
app.post('/api/billing/buy-pack', express.json(), async (req,res)=>{
  try{
    if(!requireTwitchConnected(req,res)) return;
    const packId = String(req.body?.packId || '');
    const p = listCreditPacks().find(x=>x.id===packId);
    if(!p) return res.status(400).json({ success:false, error:'Pack invalide' });
    const r = await addCredits(req, p.credits, `pack:${p.id}`);
    res.json({ success:true, ...r });
  }catch(e){
    console.error('[BILLING] buy-pack error:', e);
    res.status(500).json({ success:false, error:'buy_pack_failed' });
  }
});

app.post('/api/billing/subscribe-premium', express.json(), async (req,res)=>{
  try{
    if(!requireTwitchConnected(req,res)) return;
    const { ref } = await getOrInitBillingState(req);
...
app.post('/api/billing/cancel-premium', express.json(), async (req,res)=>{
  try{
    if(!requireTwitchConnected(req,res)) return;
    const { ref, data } = await getOrInitBillingState(req);
    const credits = Number(data.credits || 0);
    const nextPlan = credits > 0 ? 'credits' : 'free';
    await ref.set({ plan: nextPlan, updatedAt: Date.now() }, { merge:true });
    res.json({ success:true, plan: nextPlan });
  }catch(e){
...
// Spend 1 action (20 credits) — front calls this to unlock gated analysis
app.post('/api/billing/spend', express.json(), async (req,res)=>{
  try{
    if(!requireTwitchConnected(req,res)) return;
    const reason = String(req.body?.reason || 'action');
    const { ref, data } = await getOrInitBillingState(req);
    const plan = String(data.plan || 'free');
    if(plan === 'premium') return res.json({ success:true, plan:'premium', credits: Number(data.credits||0), actions:'∞' });

    const curCredits = Number(data.credits || 0);
    if(curCredits < ACTION_COST){
      return res.status(402).json({ success:false, error:'not_enough_credits', needed: ACTION_COST });
    }
    const nextCredits = curCredits - ACTION_COST;
    const nextPlan = nextCredits > 0 ? 'credits' : 'free';
    await ref.set({ credits: nextCredits, plan: nextPlan, updatedAt: Date.now() }, { merge:true });

    try{
      await db.collection('billing_ledger').add({ key: getBillingUserKey(req), delta: -ACTION_COST, reason, ts: Date.now() });
    }catch(_){}

    res.json({ success:true, plan: nextPlan, credits: nextCredits, actions: computeActionsFromCredits(nextCredits) });
  }catch(e){
    console.error('[BILLING] spend error:', e);
    res.status(500).json({ success:false, error:'spend_failed' });
  }
});

// =========================================================
// Stripe Checkout (optional)
// - If stripe not configured: endpoints return 501, UI falls back to demo /buy-pack
// =========================================================
app.post('/api/billing/stripe/create-checkout-session', express.json(), async (req,res)=>{
  try{
    if(!requireTwitchConnected(req,res)) return;
    if(!stripe) return res.status(501).json({ success:false, error:'stripe_not_configured' });

    const mode = String(req.body?.mode || 'payment'); // 'payment' or 'subscription'
    const packId = String(req.body?.packId || '');
    const userKey = getBillingUserKey(req);
    ...
    if(mode === 'subscription'){
      ...
      if(!priceId) return res.status(500).json({ success:false, error:'STRIPE_PRICE_PREMIUM_missing' });
      session = await stripe.checkout.sessions.create({
        mode:'subscription',
        line_items:[{ price: priceId, quantity: 1 }],
        success_url,
        cancel_url,
        metadata:{ kind:'premium', userKey }
      });
    }else{
      const p = listCreditPacks().find(x=>x.id===packId);
      if(!p) return res.status(400).json({ success:false, error:'Pack invalide' });
      session = await stripe.checkout.sessions.create({
        ...
        line_items:[{
          price_data:{
            currency:'eur',
            product_data:{ name:`StreamerHub Credits — ${p.name}` },
        ...
        success_url,
        cancel_url,
        metadata:{ kind:'credits', userKey, packId: p.id, credits: String(p.credits) }
      });
    }
    ...
    res.status(500).json({ success:false, error:'stripe_session_failed' });
  }
});
...
app.post('/api/billing/stripe/webhook',
  ...
  async (req,res)=>{
    try{
      ...
      if(!whSecret) return res.status(500).send('STRIPE_WEBHOOK_SECRET_missing');
      ...
      const applyCreditsByKey = async (userKey, deltaCredits, reason)=>{
        const ref = db.collection(BILLING_COLLECTION).doc(String(userKey));
        await db.runTransaction(async (tx)=>{
          const snap = await tx.get(ref);
          const cur = snap.exists ? (snap.data()||{}) : { plan:'free', credits:0 };
          const curCredits = Number(cur.credits||0);
          const nextCredits = Math.max(0, curCredits + Number(deltaCredits||0));
          const curPlan = String(cur.plan||'free');
          const nextPlan = (curPlan==='premium') ? 'premium' : (nextCredits>0?'credits':'free');
          tx.set(ref, { credits: nextCredits, plan: nextPlan, updatedAt: Date.now(), createdAt: cur.createdAt || Date.now() }, { merge:true });
        });
        try{
          await db.collection('billing_ledger').add({ key: String(userKey), delta: Number(deltaCredits||0), reason: String(reason||'stripe'), ts: Date.now() });
        }catch(_){}
      };
      ...
      if(type === 'checkout.session.completed'){
        ...
        const userKey = String(meta.userKey||'');
        if(kind === 'credits' && userKey){
          const credits = parseInt(meta.credits||'0', 10) || 0;
          await applyCreditsByKey(userKey, credits, `stripe:pack:${meta.packId||''}`);
        }
        if(kind === 'premium' && userKey){
          await db.collection(BILLING_COLLECTION).doc(userKey).set({ plan:'premium', updatedAt: Date.now() }, { merge:true });
        }
      }
