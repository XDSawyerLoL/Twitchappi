...
// =========================================================
// Twitch: Clips & VOD (requires Twitch user session)
// =========================================================
app.get('/api/twitch/clips', async (req, res) => {
  try{
    const u = requireTwitchSession(req, res);
    if(!u) return;

    const first = clamp(Number(req.query.first || 20), 1, 50);
    const endpoint = `clips?broadcaster_id=${encodeURIComponent(u.id)}&first=${first}`;
    const data = await twitchAPI(endpoint, u.access_token);
...
app.get('/api/twitch/videos', async (req, res) => {
  try{
    ...
    const endpoint = `videos?user_id=${encodeURIComponent(u.id)}&first=${first}&type=${encodeURIComponent(type)}`;
    const data = await twitchAPI(endpoint, u.access_token);
