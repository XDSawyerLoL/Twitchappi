...
async function runGeminiAnalysis(prompt) {
    try {
        const response = await ai.models.generateContent({
            ...
            config: {
                systemInstruction: "Tu es un stratège Twitch expert en data. Tu dois prédire les opportunités. Réponds TOUJOURS en HTML simple (<ul>, <li>, <h4>, <strong>, <p>, <span style='color:...'>) sans balises <html>/<body>."
            }
        });
...
// =========================================================
// ROUTE SPÉCIALE : ANALYSE HEURE D'OR (PREDICTION)
// =========================================================
app.post('/analyze_golden_hour', async (req, res) => {
    const { game, date } = req.body;
    
    // Prompt spécifique pour la stratégie de créneau
    const prompt = `
        Agis comme un expert data analyst Twitch. Je veux streamer le jeu "${game}" à la date suivante : "${date}".
        Analyse les habitudes des spectateurs et la concurrence pour ce jeu.
        
        1. Identifie "L'Heure d'Or" : Le créneau horaire précis (ex: 18h30 - 21h00) où il y a beaucoup de viewers potentiels mais MOINS de gros streamers connectés.
        2. Explique pourquoi ce créneau est le meilleur (Ratio Visibilité/Concurrence).
        3. Donne un conseil spécifique pour maximiser la rétention sur ce créneau.
        
        Format HTML strict (titres <h4>, listes <ul>, gras <strong>). Sois précis et stratégique.
    `;
...
app.post('/critique_ia', async (req, res) => {
    ...
    if (type === 'niche') prompt += "Donne 3 faiblesses des concurrents et 1 opportunité. Format HTML.";
...
async function refreshGlobalStreamList() {
    ...
    try {
        ...
        let suitableStreams = data.data.filter(stream => stream.viewer_count <= 100);
        if (suitableStreams.length === 0 && data.data.length > 0) suitableStreams = data.data.sort((a, b) => a.viewer_count - b.viewer_count).slice(0, 10);
...
app.get('/get_default_stream', async (req, res) => {
    ...
    const rotation = CACHE.globalStreamRotation;
    if (rotation.streams.length === 0) return res.json({ success: true, channel: 'twitch', message: "Aucun stream." });
    const currentStream = rotation.streams[rotation.currentIndex];
    return res.json({ success: true, channel: currentStream.channel, viewers: currentStream.viewers, message: `Cycle Auto: ${currentStream.channel} (${currentStream.viewers} vues)` });
});
...
app.post('/cycle_stream', async (req, res) => {
    ...
    await refreshGlobalStreamList();
    const rotation = CACHE.globalStreamRotation;
    if (rotation.streams.length === 0) return res.status(404).json({ error: "Vide." });
