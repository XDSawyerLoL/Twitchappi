...
// =========================================================
// BILLING / CREDITS (Twitflix free, Market paid)
// - Firestore source of truth
// - Demo mode by default (CREDITS_DEMO=true, BILLING_DEMO=true)
// =========================================================
const BILLING_PLANS = 'billing_plans';
const USER_CREDITS = 'user_credits';
const CREDIT_LEDGER = 'ledger';
const COST_PER_ACTION = 20;

function requireFirestore(res){
  if(!db) { res.status(500).json({ success:false, error:'Firestore indisponible.' }); return false; }
  return true;
}

function getUserKey(req){
  // Prefer Twitch user id if logged-in; fallback to session id for multi-user.
  const tu = req.session?.twitchUser;
  if (tu?.id) return `twitch:${tu.id}`;
  return req.session?.id || req.sessionID || `anon:${crypto.randomBytes(6).toString('hex')}`;
}

async function getPlan(req){
  const userKey = getUserKey(req);
  try{
    const snap = await db.collection(BILLING_PLANS).doc(userKey).get();
    const plan = snap.exists ? (snap.data()?.plan || 'FREE') : 'FREE';
    return { userKey, plan };
  }catch(_){
    return { userKey, plan:'FREE' };
  }
}

async function setPlan(userKey, plan){
  await db.collection(BILLING_PLANS).doc(userKey).set({ plan, updatedAt: Date.now() }, { merge:true });
}

async function getCredits(userKey){
  const snap = await db.collection(USER_CREDITS).doc(userKey).get();
...
app.get('/api/credits/status', async (req,res)=>{
  try{
    ...
    const actions = String(plan).toUpperCase() === 'PRO' ? Infinity : Math.floor(credits / COST_PER_ACTION);
...
// MOCK purchase: adds credits (no real payment). In prod, replace by Stripe checkout + webhook.
...
// Spend credits (for gating market actions)
app.post('/api/credits/spend', async (req,res)=>{
  try{
    ...
    if(String(plan).toUpperCase() === 'PRO') return res.json({ success:true, plan:'PRO', spent:0, remaining: Infinity });
