/**
 * STREAMER HUB V60 - BACKEND ULTIMATE (FIXED: AVATAR, VOD, RAID, PLAYER)
 */
...
// --- CHAT LOGIC (AVEC AVATAR) ---
const chatHistory = [];
io.on('connection', (socket) => {
    socket.emit('history', chatHistory);
    socket.on('send_message', (data) => {
        if (chatHistory.length > 50) chatHistory.shift();
        // On s'assure que l'avatar est présent, sinon image par défaut
...
// --- HELPERS ---
// On stocke aussi l'avatar dans le cache utilisateur
...
async function twitch(endpoint, userToken = null) {
    const t = userToken || await getToken();
...
app.get('/twitch_auth_callback', async (req, res) => {
    ...
    try {
        ...
        if (tData.access_token) {
            // On récupère le profil complet pour l'avatar
            ...
            CACHE.twitchUser = { 
                ...
                profile_image_url: user.profile_image_url, // ON SAUVEGARDE L'AVATAR
...
app.get('/twitch_user_status', (req, res) => {
    if (CACHE.twitchUser && CACHE.twitchUser.expiry > Date.now()) {
        return res.json({ 
            ...
            profile_image_url: CACHE.twitchUser.profile_image_url // ON LE RENVOIE AU FRONT
        });
    }
...
app.post('/scan_target', async (req, res) => {
    ...
    try {
        const uRes = await twitch(`users?login=${query}`);
        if(uRes.data && uRes.data.length) {
            ...
            return res.json({ 
                ...
                user_data: { 
                    ...
                    game_name: cRes.data[0]?.game_name || "N/A",
                    title: cRes.data[0]?.title || "",
                    is_live: isLive,
                    login: u.login,
...
app.get('/get_latest_vod', async (req, res) => {
    try {
        ...
        // Remplacement propre des dimensions
...
app.post('/start_raid', async (req, res) => {
    ...
    try {
        ...
        // Trouve le streamer le plus proche du max_viewers sans dépasser
        const target = sRes.data
            .filter(s => s.viewer_count <= parseInt(max_viewers))
            .sort((a,b) => b.viewer_count - a.viewer_count)[0]; // Prend le plus gros des petits

        if(target) {
            const thumb = target.thumbnail_url.replace('{width}','320').replace('{height}','180');
            return res.json({ success: true, target: { name: target.user_name, login: target.user_login, thumbnail_url: thumb, viewers: target.viewer_count, game: target.game_name } });
        }
        res.json({ success: false, error: "Aucune cible trouvée" });
    } catch(e) { res.status(500).json({error:e.message}); }
});

// SUIVI (Correction Miniatures)
...
app.get('/get_default_stream', async (req, res) => {
    ...
    if (now - CACHE.rotation.last > 180000 || CACHE.rotation.list.length === 0) {
        try {
            ...
            if(d.data) { CACHE.rotation.list = d.data.filter(s => s.viewer_count < 100).map(s => s.user_login); CACHE.rotation.last = now; }
        } catch(e) {}
    }
...
app.get('/api/stats/global', async (req, res) => {
    ...
    res.json({ success: true, total_viewers: v * 4, total_channels: "15k+", top_game_name: d.data[0]?.game_name, history: { live: { labels: ['-4h','-1h','Now'], values: [v*0.8, v*0.9, v] } } });
});
...
server.listen(PORT, () => console.log(`🚀 V60 (FIXED) ON PORT ${PORT}`));
