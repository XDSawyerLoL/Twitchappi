...
app.get("/", (req, res) => {
  const candidates = [
    process.env.UI_FILE,
    "public/NicheOptimizer.html",
    "public/NicheOptimizer_v56.html",
    "public/index.html",
    "NicheOptimizer.html",
    "index.html"
  ].filter(Boolean);
...
const DEFAULT_FREE_CREDITS = 1200;
const ACTION_COST = 20;

const BILLING_PACKS = [
  { id: "pack_200", title: "Pack Découverte", credits: 200, priceLabel: "4,99€" },
  { id: "pack_500", title: "Pack Standard", credits: 500, priceLabel: "9,99€" },
  { id: "pack_1200", title: "Pack Booster", credits: 1200, priceLabel: "19,99€" }
];

function getTwitchUser(req){ return req.session?.twitchUser || null; }
function getUserKey(req){ const u=getTwitchUser(req); return u ? ("twitch:"+String(u.id)) : null; }

async function getBillingDoc(userKey){
  if(firestoreOk){
    const ref = db.collection(BILLING_USERS).doc(userKey);
    const snap = await ref.get();
    return { ref, exists: snap.exists, data: snap.exists ? snap.data() : null };
  }
  global.__billingMem = global.__billingMem || new Map();
  const data = global.__billingMem.get(userKey) || null;
  return { ref: null, exists: !!data, data };
}

async function setBillingDoc(userKey, data){
  if(firestoreOk){
    await db.collection(BILLING_USERS).doc(userKey).set(data, { merge: true });
    return;
  }
  global.__billingMem = global.__billingMem || new Map();
  global.__billingMem.set(userKey, { ...(global.__billingMem.get(userKey)||{}), ...data });
}

async function appendLedger(entry){
  if(!firestoreOk) return;
  try{ await db.collection(BILLING_LEDGER).add({ ...entry, ts: Date.now() }); }catch(e){}
}

async function ensureBillingUser(userKey){
  const { exists, data } = await getBillingDoc(userKey);
  if(exists && data) return data;
  const fresh = { plan: "free", credits: DEFAULT_FREE_CREDITS, createdAt: Date.now() };
  await setBillingDoc(userKey, fresh);
  await appendLedger({ userKey, type:"grant", amount: DEFAULT_FREE_CREDITS, reason:"welcome" });
  return fresh;
}

app.get("/api/billing/packs", (req,res)=>{
  res.json({ packs: BILLING_PACKS, actionCost: ACTION_COST, defaultFreeCredits: DEFAULT_FREE_CREDITS });
});
...
app.get("/api/billing/status", async (req,res)=>{
  try{
    ...
    if(!u){
      return res.json({ isConnected:false, login:null, plan:"free", credits:0, actionCost: ACTION_COST });
    }
    const userKey = getUserKey(req);
    const bill = await ensureBillingUser(userKey);
    return res.json({
      isConnected:true,
      login: u.login || u.display_name || u.name || null,
      plan: bill.plan || "free",
      credits: Number(bill.credits || 0),
...
app.post("/api/billing/buy-pack", async (req,res)=>{
  try{
    const userKey = getUserKey(req);
    if(!userKey) return res.status(401).json({ error:"Twitch non connecté" });
    const { packId } = req.body || {};
    const pack = BILLING_PACKS.find(p=>p.id===packId);
    if(!pack) return res.status(400).json({ error:"Pack invalide" });

    const bill = await ensureBillingUser(userKey);
    const nextCredits = Number(bill.credits||0) + Number(pack.credits);
    await setBillingDoc(userKey, { credits: nextCredits });
    await appendLedger({ userKey, type:"buy_pack", packId: pack.id, amount: pack.credits });

    res.json({ success:true, added: pack.credits, credits: nextCredits });
  }catch(e){
...
app.post("/api/billing/spend", async (req,res)=>{
  try{
    ...
    const cost = Number(amount || ACTION_COST);

    const bill = await ensureBillingUser(userKey);
    if((bill.plan||"free") === "premium"){
      return res.json({ success:true, plan:"premium", credits: bill.credits||0 });
    }
    const cur = Number(bill.credits||0);
    if(cur < cost) return res.status(402).json({ error:"Crédits insuffisants" });
    const next = cur - cost;
    await setBillingDoc(userKey, { credits: next });
    await appendLedger({ userKey, type:"spend", amount: -cost });
    res.json({ success:true, spent: cost, credits: next });
  }catch(e){
    res.status(500).json({ error: e.message });
  }
});
...
app.post("/api/billing/subscribe-premium", async (req,res)=>{
  try{
    const userKey = getUserKey(req);
    if(!userKey) return res.status(401).json({ error:"Twitch non connecté" });
    await ensureBillingUser(userKey);
    await setBillingDoc(userKey, { plan:"premium" });
    await appendLedger({ userKey, type:"plan", plan:"premium" });
