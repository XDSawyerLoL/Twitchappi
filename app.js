...
// Page pricing (séparée) — n'altère pas l'apparence de l'app principale
app.get(['/pricing', '/pricing.html'], (req, res) => {
  const p = path.join(__dirname, 'public', 'pricing.html');
  if (!fs.existsSync(p)) return res.status(404).send('Pricing introuvable.');
...
// =========================================================
//  SaaS: Plans (FREE/PRO) + Crédits (usage) — SANS IMPACT VISUEL
//  Objectif: Twitflix gratuit, le reste peut être débloqué via crédits ou PRO.
//  NOTE: paiements réels (Stripe etc.) à brancher plus tard. Ici: endpoints "mock" sûrs.
// =========================================================

const BILLING_PLANS = 'billing_plans';      // doc id = userKey
const USER_CREDITS = 'user_credits';        // doc id = userKey
const CREDIT_LEDGER = 'ledger';            // subcollection

const COST_PER_ACTION = 20; // 1 action = 20 crédits

function getUserKeyFromSession(req){
  const tu = req.session?.twitchUser;
  if(tu?.id) return `twitch:${String(tu.id)}`;
  // sessionID is stable per browser session => multi-user OK
  const sid = req.sessionID || req.session?.id;
  if(sid) return `sess:${String(sid)}`;
  return `anon:${crypto.randomBytes(6).toString('hex')}`;
}

async function getPlan(req){
  if(!requireFirestore(req.res || { status:()=>({ json:()=>{} }) })){}
  const userKey = getUserKeyFromSession(req);
  const doc = await db.collection(BILLING_PLANS).doc(userKey).get();
  const plan = (doc.exists ? String(doc.data()?.plan || 'FREE') : 'FREE').toUpperCase();
  return { userKey, plan };
}

async function getCredits(userKey){
  const doc = await db.collection(USER_CREDITS).doc(userKey).get();
  const credits = doc.exists ? Number(doc.data()?.credits || 0) : 0;
  return { credits };
}

async function addCredits(userKey, delta, meta={}){
  const ref = db.collection(USER_CREDITS).doc(userKey);
  await db.runTransaction(async (tx)=>{
    const snap = await tx.get(ref);
    const cur = snap.exists ? Number(snap.data()?.credits || 0) : 0;
    const next = Math.max(0, cur + Number(delta||0));
    tx.set(ref, { credits: next, updatedAt: Date.now() }, { merge:true });
    const ledRef = ref.collection(CREDIT_LEDGER).doc(Date.now().toString(36) + '-' + crypto.randomBytes(4).toString('hex'));
    tx.set(ledRef, { delta: Number(delta||0), at: Date.now(), ...meta });
  });
}

function packsDefinition(){
  // Pack “LoL-like”: tu achètes des crédits, tu consommes des actions.
  return [
    { id:'pack_200',  name:'Starter',   credits:200,  actions:Math.floor(200 / COST_PER_ACTION),  price:'4,99€' },
    { id:'pack_500',  name:'Standard',  credits:500,  actions:Math.floor(500 / COST_PER_ACTION),  price:'9,99€', best:true },
    { id:'pack_1200', name:'Creator+',  credits:1200, actions:Math.floor(1200 / COST_PER_ACTION), price:'19,99€' }
  ];
}

// Status used by /pricing
app.get('/api/credits/status', async (req,res)=>{
  try{
    if(!requireFirestore(res)) return;
    const { userKey, plan } = await getPlan(req);
    const { credits } = await getCredits(userKey);
    const actions = plan === 'PRO' ? Infinity : Math.floor(credits / COST_PER_ACTION);
    res.json({ success:true, plan, credits, actions, costPerAction: COST_PER_ACTION });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
  }
});
...
app.get('/api/credits/packs', async (_req,res)=>{
  res.json({ success:true, packs: packsDefinition(), costPerAction: COST_PER_ACTION });
});

// MOCK purchase: adds credits (no real payment). In prod, replace by Stripe webhook.
app.post(['/api/credits/buy-pack','/api/credits/buy_pack'], async (req,res)=>{
  try{
    if(!requireFirestore(res)) return;
    const { userKey } = await getPlan(req);
    const packId = String(req.body?.packId || '').trim();
    const pack = packsDefinition().find(p=>p.id === packId) || packsDefinition().find(p=>p.id === 'pack_500');
    // Safety: allow only in demo unless explicitly enabled
    const demoOk = (process.env.CREDITS_DEMO || 'true').toLowerCase() === 'true';
    if(!demoOk) return res.status(501).json({ success:false, error:'Paiement non branché. Active CREDITS_DEMO=true pour tester.' });

    await addCredits(userKey, pack.credits, { reason:'buy-pack', packId: pack.id, price: pack.price });
    res.json({ success:true, added: pack.credits, packId: pack.id });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
  }
});

// Spend credits (optional integration for future gating)
app.post('/api/credits/spend', async (req,res)=>{
  try{
    if(!requireFirestore(res)) return;
    const { userKey, plan } = await getPlan(req);
    const amount = clamp(Number(req.body?.amount || COST_PER_ACTION), 1, 100000);
    const reason = sanitizeText(req.body?.reason || 'action', 80);
    if(plan === 'PRO') return res.json({ success:true, plan:'PRO', spent:0, remaining: Infinity });
    ...
    await db.runTransaction(async (tx)=>{
      ...
      tx.set(ref, { credits: remaining, updatedAt: Date.now() }, { merge:true });
      const ledRef = ref.collection(CREDIT_LEDGER).doc(Date.now().toString(36) + '-' + crypto.randomBytes(4).toString('hex'));
      tx.set(ledRef, { delta: -amount, at: Date.now(), reason });
    });
    res.json({ success:true, spent: amount, remaining });
  }catch(e){
    res.status(400).json({ success:false, error:e.message });
  }
});

// PRO subscribe (mock). In prod: Stripe subscription.
app.post('/api/billing/subscribe_pro', async (req,res)=>{
  try{
    if(!requireFirestore(res)) return;
    const allowDemo = (process.env.BILLING_DEMO || 'true').toLowerCase() === 'true';
    if(!allowDemo) return res.status(501).json({ success:false, error:'Abonnement non branché. Active BILLING_DEMO=true pour tester.' });
    const { userKey } = await getPlan(req);
    await db.collection(BILLING_PLANS).doc(userKey).set({ plan:'PRO', updatedAt: Date.now() }, { merge:true });
