...
app.get('/', (req, res) => {
  ...
  const found = candidates.find(f => fs.existsSync(path.join(__dirname,'public', f)));
...
// ------------------ STREAMER MARKET PORTFOLIO (file fallback) ------------------
const STREAMER_MARKET_DIR = path.join(__dirname, 'data', 'streamer_market');
const STREAMER_MARKET_USERS_FILE = path.join(STREAMER_MARKET_DIR, 'users.json');
const STREAMER_MARKET_MARKET_FILE = path.join(STREAMER_MARKET_DIR, 'market.json');
...
function writeJsonFile(file, data){
  try{
    ...
    console.warn('⚠️ [MARKET] file write failed:', e.message);
  }
}
...
function getUserKeyFromReq(req){
  ...
  // fallback: stable per-session
  if (req.sessionID) return `sess:${String(req.sessionID)}`;
  return 'anon';
}
function getUserDisplayFromReq(req){
  const tu = req.session?.twitchUser;
  return sanitizeName(tu?.display_name || tu?.login || 'Anon', 50);
}

let __fileMarketCache = null; // { login: sharesOutstanding }
let __fileUsersCache = null;  // { userKey: { userKey, displayName, cash, holdings } }

function loadFileCaches(){
  if (__fileMarketCache && __fileUsersCache) return;
  __fileMarketCache = readJsonFile(STREAMER_MARKET_MARKET_FILE, {});
  __fileUsersCache = readJsonFile(STREAMER_MARKET_USERS_FILE, {});
}
function persistFileCaches(){
  if (!__fileMarketCache || !__fileUsersCache) return;
  writeJsonFile(STREAMER_MARKET_MARKET_FILE, __fileMarketCache);
  writeJsonFile(STREAMER_MARKET_USERS_FILE, __fileUsersCache);
}
...
async function getMarket(login){
  ...
  if(firestoreOk){
    ...
    loadFileCaches();
    sharesOutstanding = Number(__fileMarketCache[key] || 0);
  }
...
async function bumpShares(login, deltaShares){
  ...
  if(firestoreOk){
    ...
    loadFileCaches();
    const cur = Number(__fileMarketCache[key] || 0);
    __fileMarketCache[key] = Math.max(0, cur + deltaShares);
    persistFileCaches();
  }
}

async function getUserWallet(userKey, displayName){
  const key = sanitizeText(userKey || 'anon', 120) || 'anon';
  const name = sanitizeName(displayName || 'Anon', 50) || 'Anon';

  if(!firestoreOk){
    loadFileCaches();
    if(!__fileUsersCache[key]){
      __fileUsersCache[key] = { userKey: key, displayName: name, cash: 10000, holdings: {}, updatedAt: Date.now() };
      persistFileCaches();
    } else if (name && __fileUsersCache[key].displayName !== name){
      __fileUsersCache[key].displayName = name;
      __fileUsersCache[key].updatedAt = Date.now();
      persistFileCaches();
    }
    return __fileUsersCache[key];
  }

  const ref = db.collection(FANTASY_USERS).doc(key.toLowerCase());
  const doc = await ref.get();
  if(!doc.exists){
    const init = { userKey: key, displayName: name, cash: 10000, holdings: {}, updatedAt: Date.now() };
    await ref.set(init);
    return init;
  }
  ...
  if (name && data.displayName !== name){
    await ref.set({ displayName: name, updatedAt: Date.now() }, { merge:true });
...
async function saveUserWallet(wallet){
  ...
  if(!firestoreOk){
    loadFileCaches();
    __fileUsersCache[wallet.userKey] = wallet;
    persistFileCaches();
    return;
  }
  const ref = db.collection(FANTASY_USERS).doc(String(wallet.userKey).toLowerCase());
...
app.get('/api/fantasy/profile', async (req,res)=>{
  try{
    ...
    const displayName = getUserDisplayFromReq(req);
    const w = await getUserWallet(userKey, displayName);
    ...
    res.json({ success:true, user: (w.displayName||w.userKey), cash: Number(w.cash||0), holdings: enriched });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
  }
});
...
app.get('/api/fantasy/leaderboard', async (req,res)=>{
  try{
    ...
    if(firestoreOk){
      ...
      loadFileCaches();
      users = Object.values(__fileUsersCache || {});
    }
    ...
    for(const u of users){
      ...
      items.push({ user: (u.displayName||u.userKey), netWorth: worth });
    }
