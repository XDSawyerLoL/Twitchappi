...
async function runGeminiAnalysis(prompt) {
  ...
  try {
    const response = await aiClient.models.generateContent({
      ...
      config: {
        systemInstruction:
          'Tu es un expert Data Twitch. Réponds UNIQUEMENT en HTML simple (<p>, <ul>, <li>). Sois précis, donne des chiffres et des horaires concrets.',
      },
    });
...
app.get('/get_latest_vod', async (req, res) => {
  try {
    const u = await twitchAPI(
      `users?login=${encodeURIComponent(req.query.channel)}`
    );
...
app.post('/scan_target', async (req, res) => {
  ...
  try {
    // 1. Récupération User
    const uRes = await twitchAPI(
      `users?login=${encodeURIComponent(query)}`
    );
    ...
    if (uRes.data.length) {
      ...
      let channelInfo = {};
      try {
        const cRes = await twitchAPI(
          `channels?broadcaster_id=${u.id}`
        );
      ...
      // View count (souvent 0 sur API récente)
      ...
      const uData = {
        ...
        tags: channelInfo.tags
          ? channelInfo.tags.slice(0, 3).join(', ')
          : 'Aucun',
    ...
    if (gRes.data.length) {
      ...
      const total = sRes.data.reduce(
        (a, b) => a + b.viewer_count,
        0
      );
...
app.post('/analyze_schedule', async (req, res) => {
  ...
  try {
    ...
    const prompt = `Analyse le jeu Twitch : "${gameName}".
...
app.get('/export_csv', (req, res) => {
  ...
  res.setHeader(
    'Content-Disposition',
    'attachment; filename="TwitchAnalysis.csv"'
  );
...
app.listen(PORT, () => {
  console.log(`SERVER V50 FINAL PATCH ON PORT ${PORT}`);
});
