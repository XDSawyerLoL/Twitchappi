...
app.get('/', (req, res) => {
  ...
  const found = candidates.find(f => fs.existsSync(path.join(uiDir, f)));
  if (!found) return res.status(500).send('UI introuvable sur le serveur.');
  return res.sendFile(path.join(uiDir, found));
});
...
// =========================================================
// 9. STREAMER MARKET (Portefeuille Marché du Streamer) — FIRESTORE ONLY
// =========================================================
// NOTE: Firestore est la source unique de vérité.
// Si Firestore n'est pas disponible, on renvoie 503 (pas de fallback en mémoire).

const MARKET_USERS = 'streamer_market_users';
const MARKET_STATE = 'streamer_market';
const MARKET_HISTORY = 'history';

const MARKET_ALPHA = 0.35;      // impact of outstanding shares
const MARKET_SCALE = 180;       // soft scale
const MARKET_MAX_HOLDINGS = 12; // max distinct streamers per portfolio
const MARKET_START_CASH = 10000;

function requireFirestoreOr503(res){
  if (!firestoreOk){
    res.status(503).json({ success:false, error:'Firestore indisponible. Vérifie la config Firebase Admin.' });
    return false;
  }
  return true;
}

function getUserKeyFromSession(req){
  const u = req.session?.twitchUser;
  if (u?.id) return `twitch:${u.id}`;
  // sessionID is stable per browser session (with cookie). good for anon.
  return `sess:${req.sessionID || 'unknown'}`;
}

function getUserLabelFromSession(req){
  const u = req.session?.twitchUser;
...
function applyImpact(basePrice, sharesOutstanding){
  ...
  const mult = 1 + MARKET_ALPHA * Math.log1p(so / MARKET_SCALE);
...
async function getMarket(login){
  ...
  const ref = db.collection(MARKET_STATE).doc(key);
  const doc = await ref.get();
  let sharesOutstanding = 0;
  if(doc.exists) sharesOutstanding = Number(doc.data()?.sharesOutstanding || 0);
  else {
    await ref.set({ login:key, sharesOutstanding:0, createdAt: Date.now(), updatedAt: Date.now() }, { merge:true });
  }
...
async function bumpShares(login, deltaShares){
  ...
  // record history point (append-only)
  const m = await getMarket(key);
  await db.collection(MARKET_STATE).doc(key).collection(MARKET_HISTORY).add({
    ts: Date.now(),
    price: m.price,
...
async function getUserWalletByKey(userKey, label){
  const ref = db.collection(MARKET_USERS).doc(userKey);
  const doc = await ref.get();
  if(!doc.exists){
    const init = { userKey, label: label || 'Anon', cash: MARKET_START_CASH, holdings: {}, createdAt: Date.now(), updatedAt: Date.now() };
    await ref.set(init, { merge:true });
    return init;
  }
  const data = doc.data();
  // keep label fresh (if user logs in)
  if(label && data.label !== label){
    await ref.set({ label, updatedAt: Date.now() }, { merge:true });
    data.label = label;
  }
  return data;
}

async function saveUserWalletByKey(userKey, wallet){
  const ref = db.collection(MARKET_USERS).doc(userKey);
...
// ---- Compatibility layer: keep old /api/fantasy/* routes but rename UI label on frontend.
// ---- New canonical routes are /api/streamer-market/*

async function marketHandler(req, res){
  try{
    if(!requireFirestoreOr503(res)) return;
    ...
    const snap = await db.collection(MARKET_STATE).doc(login).collection(MARKET_HISTORY)
...
async function profileHandler(req, res){
  try{
    if(!requireFirestoreOr503(res)) return;
    const userKey = getUserKeyFromSession(req);
    const label = getUserLabelFromSession(req);

    const w = await getUserWalletByKey(userKey, label);
    ...
    for(const it of holdingsArr){
      ...
      const value = it.shares * m.price;
      holdingsValue += value;
      enriched.push({ login: it.login, shares: it.shares, price: m.price, value });
    }

    const cash = Number(w.cash||0);
    res.json({
      success:true,
      user: { key: userKey, label },
      cash,
      holdings: enriched,
      holdingsValue,
...
async function investHandler(req, res){
  try{
    if(!requireFirestoreOr503(res)) return;
    const userKey = getUserKeyFromSession(req);
    const label = getUserLabelFromSession(req);
    ...
    if(isNew && distinct >= MARKET_MAX_HOLDINGS){
      return res.status(400).json({ success:false, error:`Limite: ${MARKET_MAX_HOLDINGS} streamers max.` });
    }
    ...
    await saveUserWalletByKey(userKey, w);
...
async function sellHandler(req, res){
  try{
    if(!requireFirestoreOr503(res)) return;
    const userKey = getUserKeyFromSession(req);
    const label = getUserLabelFromSession(req);
...
async function leaderboardHandler(req, res){
  try{
    if(!requireFirestoreOr503(res)) return;
    const snap = await db.collection(MARKET_USERS).limit(60).get();
    ...
    for(const u of users){
      ...
      items.push({ user: u.label || u.userKey, netWorth: worth });
    }
    items.sort((a,b)=>b.netWorth-a.netWorth);
    res.json({ success:true, items: items.slice(0,20), leaderboard: items.slice(0,20) });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
  }
}
...
// Old routes (compat)
app.get('/api/fantasy/market', marketHandler);
app.get('/api/fantasy/profile', profileHandler);
app.post('/api/fantasy/invest', investHandler);
app.post('/api/fantasy/sell', sellHandler);
app.get('/api/fantasy/leaderboard', leaderboardHandler);

// New canonical routes
app.get('/api/streamer-market/market', marketHandler);
app.get('/api/streamer-market/profile', profileHandler);
app.post('/api/streamer-market/invest', investHandler);
app.post('/api/streamer-market/sell', sellHandler);
app.get('/api/streamer-market/leaderboard', leaderboardHandler);


// =========================================================
// 10. SaaS Plans + Prescriptive Analytics
// =========================================================
const BILLING_COLLECTION = 'billing_plans'; // doc id = userKey

async function getPlanForUser(req){
  const userKey = getUserKeyFromSession(req);
  const ref = db.collection(BILLING_COLLECTION).doc(userKey);
  const doc = await ref.get();
  if (!doc.exists) return { plan: 'FREE', userKey };
  const d = doc.data() || {};
  return { plan: (d.plan || 'FREE').toUpperCase(), userKey, ...d };
}

async function requirePro(req, res){
  if(!requireFirestoreOr503(res)) return false;
  const p = await getPlanForUser(req);
  if (p.plan !== 'PRO'){
    res.status(402).json({ success:false, error:'Fonction PRO. Active un abonnement PRO.', plan: p.plan });
    return false;
  }
  return true;
}
...
app.get('/api/billing/plan', async (req,res)=>{
  try{
    if(!requireFirestoreOr503(res)) return;
    const p = await getPlanForUser(req);
    res.json({ success:true, plan: p.plan });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
  }
});

// Simple activation endpoint (for you/admin). In prod, replace by Stripe.
// POST /api/billing/activate_pro  { adminKey: "...", userKey?: "twitch:123" }
app.post('/api/billing/activate_pro', async (req,res)=>{
  try{
    if(!requireFirestoreOr503(res)) return;
    const adminKey = String(req.body.adminKey||'');
    if(!process.env.ADMIN_KEY || adminKey !== process.env.ADMIN_KEY){
      return res.status(403).json({ success:false, error:'Admin key invalide.' });
    }
    const userKey = sanitizeText(req.body.userKey || getUserKeyFromSession(req), 80);
    await db.collection(BILLING_COLLECTION).doc(userKey).set({ plan:'PRO', updatedAt: Date.now() }, { merge:true });
    res.json({ success:true, userKey, plan:'PRO' });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
  }
});
...
const CREDITS_COLLECTION = 'user_credits'; // doc id = userKey
const ACTION_COST = 20; // 20 credits = 1 action

const CREDIT_PACKS = [
  { id:'pack_500', name:'500 crédits', credits:500, actions:25, price:'4,99 €', best:false },
  { id:'pack_1250', name:'1250 crédits', credits:1250, actions:62, price:'9,99 €', best:true },
  { id:'pack_3000', name:'3000 crédits', credits:3000, actions:150, price:'19,99 €', best:false }
];

async function getCreditsForUser(req){
  const userKey = getUserKeyFromSession(req);
  const ref = db.collection(CREDITS_COLLECTION).doc(userKey);
  const doc = await ref.get();
  if(!doc.exists){
    const init = { credits: 0, updatedAt: Date.now() };
    await ref.set(init);
    return { userKey, ...init };
  }
  const d = doc.data() || {};
  return { userKey, credits: Number(d.credits||0), updatedAt: d.updatedAt||0 };
}

async function addCredits(userKey, delta){
  const ref = db.collection(CREDITS_COLLECTION).doc(userKey);
  await db.runTransaction(async (tx)=>{
    const doc = await tx.get(ref);
    const cur = doc.exists ? Number((doc.data()||{}).credits||0) : 0;
    tx.set(ref, { credits: Math.max(0, cur + delta), updatedAt: Date.now() }, { merge:true });
  });
}
...
app.get('/api/credits/status', async (req,res)=>{
  try{
    if(!requireFirestoreOr503(res)) return;
    const p = await getPlanForUser(req);
    const c = await getCreditsForUser(req);
    const actions = Math.floor((c.credits||0) / ACTION_COST);
    res.json({ success:true, plan: p.plan, credits: c.credits, actions, actionCost: ACTION_COST });
  }catch(e){ res.status(500).json({ success:false, error:e.message }); }
});

app.get('/api/credits/packs', (req,res)=>{
  res.json({ success:true, packs: CREDIT_PACKS, actionCost: ACTION_COST });
});

// DEMO purchase: enabled only if BILLING_DEMO_MODE=1 (no real payment).
app.post('/api/credits/buy-pack', async (req,res)=>{
  try{
    if(!requireFirestoreOr503(res)) return;
    if(String(process.env.BILLING_DEMO_MODE||'0') !== '1'){
      return res.status(501).json({ success:false, error:'Paiement non configuré. Active BILLING_DEMO_MODE=1 pour tester.' });
    }
    const packId = sanitizeText(req.body.packId||'', 40);
    const pack = CREDIT_PACKS.find(p=>p.id===packId);
    if(!pack) return res.status(400).json({ success:false, error:'Pack invalide.' });

    const userKey = getUserKeyFromSession(req);
    await addCredits(userKey, pack.credits);
    const c = await getCreditsForUser(req);
    res.json({ success:true, credits: c.credits });
  }catch(e){ res.status(500).json({ success:false, error:e.message }); }
});

// DEMO subscribe: enabled only if BILLING_DEMO_MODE=1 (no Stripe).
app.post('/api/billing/subscribe_pro', async (req,res)=>{
  try{
    if(!requireFirestoreOr503(res)) return;
    if(String(process.env.BILLING_DEMO_MODE||'0') !== '1'){
      return res.status(501).json({ success:false, error:'Abonnement non configuré. Active BILLING_DEMO_MODE=1 pour tester.' });
    }
    const userKey = getUserKeyFromSession(req);
    await db.collection(BILLING_COLLECTION).doc(userKey).set({ plan:'PRO', updatedAt: Date.now() }, { merge:true });
...
// =========================================================
// 10A. Recommendations (Prescriptive) — PRO
// =========================================================
...
app.get('/api/recommendations', async (req,res)=>{
  try{
    ...
    const gameName = sanitizeText(req.query.game_name || '', 80);
    const days = clamp(parseInt(req.query.days || '7',10), 3, 21);
    if(!gameName) return res.status(400).json({ success:false, error:'game_name requis' });

    // Find game id from Twitch
    const g = await twitchAPI(`games?name=${encodeURIComponent(gameName)}`);
    const game = g?.data?.[0];
    if(!game?.id) return res.json({ success:true, recommendations: [], reason:'Jeu introuvable.' });

    const since = Date.now() - days*24*60*60*1000;
    // Pull up to 1000 recent stats points (MVP) for this game
    const snap = await db.collection('games').doc(String(game.id)).collection('hourly_stats')
      .where('timestamp','>=', since).orderBy('timestamp','desc').limit(1000).get();

    // aggregate by hour-of-week: 0..167
    const buckets = new Map(); // how -> {sumViewers, countChannels, samples}
    snap.forEach(doc=>{
      const d = doc.data() || {};
      const ts = Number(d.timestamp || 0);
      if(!ts) return;
      const dt = new Date(ts);
      const how = (dt.getUTCDay()*24) + dt.getUTCHours(); // UTC to be stable; frontend will display local suggestion
      const b = buckets.get(how) || { sumViewers:0, countChannels:0, samples:0 };
      b.sumViewers += Number(d.viewers||0);
      b.countChannels += 1;
      b.samples += 1;
      buckets.set(how, b);
    });

    // score = viewers/channel with mild penalty for overcrowding
    const scored = [];
    for(const [how, b] of buckets.entries()){
      const avgViewersPerChannel = b.sumViewers / Math.max(1, b.countChannels);
      const compPenalty = Math.log1p(b.countChannels) * 2.2;
      const score = avgViewersPerChannel - compPenalty;
      scored.push({
        how,
        avgViewersPerChannel: Math.round(avgViewersPerChannel),
        competition: b.countChannels,
        score: Math.round(score)
      });
    }
    scored.sort((a,b)=>b.score-a.score);

    // pick top 6 windows in next 7 days (convert to next occurrence timestamps)
    const now = new Date();
    const out = [];
    for(const s of scored.slice(0, 18)){
      const target = nextOccurrenceUtc(now, s.how);
      out.push({
        startsAtUtc: target.getTime(),
        hourOfWeekUtc: s.how,
        score: s.score,
        avgViewersPerChannel: s.avgViewersPerChannel,
        competition: s.competition,
        label: s.score >= 30 ? 'OPPORTUNITÉ' : (s.score >= 10 ? 'BON' : 'RISQUÉ')
      });
    ...
    res.json({
      success:true,
      game: { id: game.id, name: game.name, box_art_url: game.box_art_url },
      recommendations: out
    });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
...
function nextOccurrenceUtc(nowLocal, hourOfWeekUtc){
  // hourOfWeekUtc 0..167 (Sun 00:00 UTC)
  const now = new Date(nowLocal.getTime());
  const nowUtcDay = now.getUTCDay();
  const nowUtcHour = now.getUTCHours();
  const nowHow = nowUtcDay*24 + nowUtcHour;

  let deltaHours = hourOfWeekUtc - nowHow;
  if(deltaHours < 0) deltaHours += 168;
  const t = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), nowUtcHour, 0, 0, 0));
  t.setUTCHours(t.getUTCHours() + deltaHours);
  return t;
}


// =========================================================
// 10B. Alerts (Prescriptive) — PRO
// =========================================================
const ALERTS_COLLECTION = 'user_alerts'; // doc id = userKey

app.get('/api/alerts', async (req,res)=>{
  try{
    if(!await requirePro(req,res)) return;
    const userKey = getUserKeyFromSession(req);
    const doc = await db.collection(ALERTS_COLLECTION).doc(userKey).get();
    res.json({ success:true, alerts: doc.exists ? doc.data() : { enabled:false } });
  }catch(e){ res.status(500).json({ success:false, error:e.message }); }
});
...
app.post('/api/alerts', async (req,res)=>{
  try{
    ...
    const payload = {
      ...
      game_name: sanitizeText(req.body.game_name || '', 80),
      minScore: clamp(Number(req.body.minScore||20), 0, 200),
      updatedAt: Date.now()
    };
    await db.collection(ALERTS_COLLECTION).doc(userKey).set(payload, { merge:true });
    res.json({ success:true, alerts: payload });
  }catch(e){ res.status(500).json({ success:false, error:e.message }); }
});

// Lightweight check: if enabled and current window score >= threshold -> alert
app.get('/api/alerts/check', async (req,res)=>{
  try{
    if(!await requirePro(req,res)) return;
    const userKey = getUserKeyFromSession(req);
    const doc = await db.collection(ALERTS_COLLECTION).doc(userKey).get();
    if(!doc.exists) return res.json({ success:true, triggered:false });

    const a = doc.data() || {};
    if(!a.enabled || !a.game_name) return res.json({ success:true, triggered:false });

    // Use recommendations; take best window in next 6 hours
    const r = await (await fetch(`http://localhost:${PORT}/api/recommendations?game_name=${encodeURIComponent(a.game_name)}&days=7`, {
      headers: { cookie: req.headers.cookie || '' } // keep session
    })).json();

    const now = Date.now();
    const soon = (r.recommendations || []).filter(x => x.startsAtUtc >= now && x.startsAtUtc <= now + 6*60*60*1000);
    const best = soon.sort((x,y)=>y.score-x.score)[0];
    if(best && best.score >= Number(a.minScore||20)){
      return res.json({ success:true, triggered:true, alert:{
        game_name: a.game_name,
        score: best.score,
        startsAtUtc: best.startsAtUtc,
        label: best.label
      }});
    }
...
// =========================================================
// 10C. Benchmark (Peers) — PRO
// =========================================================
app.get('/api/benchmark/me', async (req,res)=>{
  try{
    if(!await requirePro(req,res)) return;

    const u = req.session?.twitchUser;
    if(!u?.id) return res.status(400).json({ success:false, error:'Connecte Twitch pour le benchmark.' });

    const days = clamp(parseInt(req.query.days || '14',10), 7, 60);
    const sinceDay = yyyy_mm_dd_from_ms(Date.now() - days*24*60*60*1000);

    // Collect my daily stats
    const mySnap = await db.collection('channels').doc(String(u.id)).collection('daily_stats')
      .where('day','>=', sinceDay).orderBy('day','asc').get();

    const myRows = mySnap.docs.map(d=>d.data());
    const myAvg = Math.round(myRows.reduce((s,r)=>s+Number(r.avg_viewers||0),0)/Math.max(1,myRows.length));
    const myHours = Math.round(myRows.reduce((s,r)=>s+Number(r.minutes_live_est||0),0)/60);

    // Determine my "main game" over window
    const gameCounts = {};
    for(const r of myRows){
      const g = (r.top_game_name||'').toLowerCase();
      if(!g) continue;
      gameCounts[g] = (gameCounts[g]||0)+1;
    }
    const mainGame = Object.entries(gameCounts).sort((a,b)=>b[1]-a[1])[0]?.[0] || null;

    // Peers: sample channels that recently played same game + language fr
    let peers = [];
    if(mainGame){
      const peersSnap = await db.collection('channels').where('language','==','fr').where('current_game_name','==', mainGame).limit(60).get();
      peers = peersSnap.docs.map(d=>d.data()).filter(x=>String(x.login||'').toLowerCase() !== String(u.login||'').toLowerCase());
    } else {
      const peersSnap = await db.collection('channels').where('language','==','fr').limit(60).get();
      peers = peersSnap.docs.map(d=>d.data());
    }

    const peerStats = [];
    for(const p of peers.slice(0, 25)){
      const pid = String(p.id || p.user_id || '');
      // our channels docs are keyed by user_id, but data might not include it; best effort:
      // try fetch by login-> not possible here; so skip if missing
      const chId = pid;
      if(!chId) continue;
      const snap = await db.collection('channels').doc(chId).collection('daily_stats').where('day','>=', sinceDay).orderBy('day','asc').limit(30).get();
      const rows = snap.docs.map(d=>d.data());
      if(!rows.length) continue;
      const avg = Math.round(rows.reduce((s,r)=>s+Number(r.avg_viewers||0),0)/rows.length);
      const hours = Math.round(rows.reduce((s,r)=>s+Number(r.minutes_live_est||0),0)/60);
      peerStats.push({ avgViewers: avg, hoursPerPeriod: hours });
    }

    const combined = peerStats.map(x=>x.avgViewers).sort((a,b)=>a-b);
    const percentile = combined.length ? Math.round(100 * (combined.filter(v=>v<=myAvg).length / combined.length)) : null;

    res.json({
      success:true,
      me: { avgViewers: myAvg, hours: myHours, mainGame },
      peers: { n: combined.length, avgViewersMedian: combined.length ? combined[Math.floor(combined.length/2)] : null },
      percentile
    });
  }catch(e){
    res.status(500).json({ success:false, error:e.message });
