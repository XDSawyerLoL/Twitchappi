...
try {
  ...
  openid = {
    RelyingParty: function(){
      return {
        authenticate: function(_provider, _immediate, cb){ cb(new Error('OpenID dependency disabled')); },
        verifyAssertion: function(_req, cb){ cb(new Error('OpenID dependency disabled'), { authenticated:false }); }
      };
    }
  };
}
...
function parseYouTubePlaylistFromHTML(html) {
  ...
  try {
    ...
    for (const c of contents) {
      const shelf = c?.itemSectionRenderer?.contents?.[0]?.playlistVideoListRenderer?.contents;
...
// =========================================================
// 0. FIREBASE SAFE INIT — robust Render / Firestore support
// =========================================================
// Oryon accounts must not live only in Render's ephemeral filesystem.
// This initializer accepts several common env names and formats:
// - FIREBASE_SERVICE_KEY / FIREBASE_SERVICE_ACCOUNT / FIREBASE_CONFIG_JSON / GOOGLE_SERVICE_ACCOUNT_JSON: raw JSON
// - FIREBASE_SERVICE_KEY_BASE64 / FIREBASE_SERVICE_ACCOUNT_BASE64: base64 JSON
...
function readFirebaseServiceAccountFromEnv(){
  const rawNames = ['FIREBASE_SERVICE_KEY','FIREBASE_SERVICE_ACCOUNT','FIREBASE_CONFIG_JSON','GOOGLE_SERVICE_ACCOUNT_JSON'];
  for(const name of rawNames){
    ...
    if(raw.startsWith('"') && raw.endsWith('"')) raw = raw.slice(1,-1);
    raw = raw.replace(/\\r\\n/g, '\n').replace(/\\n/g, '\n').replace(/\\r/g, '\n');
    try { return { account: JSON.parse(raw), source:name }; } catch(e){ console.warn(`[FIREBASE] ${name} JSON invalide:`, e.message); }
  }
  const b64Names = ['FIREBASE_SERVICE_KEY_BASE64','FIREBASE_SERVICE_ACCOUNT_BASE64','GOOGLE_SERVICE_ACCOUNT_BASE64'];
  for(const name of b64Names){
    const raw = process.env[name];
    if(!raw) continue;
    try { return { account: JSON.parse(Buffer.from(String(raw).trim(), 'base64').toString('utf8')), source:name }; } catch(e){ console.warn(`[FIREBASE] ${name} base64 invalide:`, e.message); }
  }
  const credPath = process.env.GOOGLE_APPLICATION_CREDENTIALS;
  if(credPath){
    try { return { account: JSON.parse(fs.readFileSync(credPath, 'utf8')), source:'GOOGLE_APPLICATION_CREDENTIALS' }; } catch(e){ console.warn('[FIREBASE] GOOGLE_APPLICATION_CREDENTIALS illisible:', e.message); }
  }
  return null;
}
...
let firestoreSource = null;
if (admin) {
  try {
    const found = readFirebaseServiceAccountFromEnv();
    if(found?.account){
      serviceAccount = found.account;
      firestoreSource = found.source;
      if(!serviceAccount.project_id || !serviceAccount.client_email || !serviceAccount.private_key){
        throw new Error('service account incomplet: project_id/client_email/private_key requis');
      }
      ...
      firestoreOk = true;
      console.log(`✅ [FIREBASE] Firestore connecté (${firestoreSource}, project=${serviceAccount.project_id}).`);
    } else {
      console.warn('⚠️ [FIREBASE] aucune variable service account détectée: comptes Oryon non persistants hors disque Render.');
    }
  } catch (error) {
    firestoreOk = false;
    db = makeFirestoreStub();
    console.warn('⚠️ [FIREBASE] désactivé, fallback local:', error.message);
  }
} else {
  console.warn('⚠️ [FIREBASE] firebase-admin indisponible: fallback local.');
}
...
app.use(helmet({
  ...
  frameguard: false,
  // Reduce noisy browser console warnings for embedded players
  permissionsPolicy: {
    directives: {
      accelerometer: ["self"],
      gyroscope: ["self"],
...
const generatedSessionSecret = configuredSessionSecret ? '' : crypto.randomBytes(48).toString('hex');
if (!configuredSessionSecret) {
  console.warn('⚠️ SESSION_SECRET manquant: secret aléatoire temporaire utilisé. Ajoute SESSION_SECRET sur Render pour conserver les sessions après redémarrage.');
}
...
const sessionMiddleware = session({
  ...
  secret: configuredSessionSecret || generatedSessionSecret || DEV_SESSION_SECRET,
  resave: false,
...
// Static assets: never expose the repository root in production.
// Only the browser files required by the UI are public.
const staticOptions = {
  dotfiles: 'deny',
  index: false,
  fallthrough: false,
...
function sendPublicFile(res, file, type){
...
app.get('/api/twitch/vods/by-game-small', heavyLimiter, async (req, res) => {
  try{
    ...
    const key = `vodsmall:v2:${gameId}:${limit}:${days}:${lang}:${minViewers}:${maxViewers}:${perChannel}:${maxViews}`;
    // Longer cache: this endpoint is expensive and "good enough" if it refreshes every few minutes.
    const cached = __oryonCacheGet(__oryonVodSearchCache, key, 5 * 60_000);
    if(cached) return res.json({ success:true, items:cached, cached:true });
    ...
    // ---------------------------------------------------------
    // FAST PATH (3 calls max): query VODs directly by game_id.
    // Much faster and more reliable than "seed channels then fetch per-user videos".
    // ---------------------------------------------------------
    ...
    // Prefer FR, fallback any language
    await fetchByGame(lang || '');
    if (lang && out.length < Math.min(10, limit)) {
      await fetchByGame('');
    }

    // ---------------------------------------------------------
    // SLOW PATH (limited): If too few items, try "small streamers band" and fetch a couple VODs per channel.
    // Concurrency-limited to keep latency down.
    // ---------------------------------------------------------
    if (out.length < Math.min(10, limit)) {
      const streams = [];
      const fetchStreams = async (useLang) => {
        ...
        for(let page=0; page<2; page++){
          ...
          if(!after) break;
          if(streams.length >= 120) break;
        }
      };

      await fetchStreams(lang || '');
      if(lang && streams.length < 15){
        streams.length = 0;
        await fetchStreams('');
      }
      ...
      for(const s of streams){
        ...
        if(!uid || seenCh.has(uid)) continue;
        seenCh.add(uid);
        channelIds.push(uid);
        if(channelIds.length >= 20) break; // hard cap
      }

      const fetchVideosForChannel = async (uid) => {
      ...
      // Concurrency = 4
      const concurrency = 4;
      for (let i = 0; i < channelIds.length && out.length < limit; i += concurrency) {
        const batch = channelIds.slice(i, i + concurrency);
        const results = await Promise.all(batch.map(uid => fetchVideosForChannel(uid).catch(()=>[])));
    ...
    return res.json({ success:true, items: out, optimized:true, band:{ minViewers, maxViewers } });
  }catch(e){
...
// Swapp streamer/channel hardening. No visual change: this only normalizes persistent
// channel data so every account has a stable public channel and reliable live status.
...
function ensureOryonUserShape(user, options = {}){
  if(!user || typeof user !== 'object') return user;
  const now = Date.now();
  const login = normalizeOryonLogin(user.login || user.username || user.name || '');
  if(login) user.login = login;
  if(!user.id) user.id = crypto.randomBytes(12).toString('hex');
  if(!user.createdAt) user.createdAt = now;
  if(!user.display_name) user.display_name = user.login || 'Streamer';
  user.display_name = String(user.display_name || user.login || 'Streamer').trim().slice(0,40) || user.login;
  user.channel_id = String(user.channel_id || user.id);
  user.channel_schema_version = SWAPP_CHANNEL_SCHEMA_VERSION;
  user.channel_public = user.channel_public !== false;
  user.channel_createdAt = Number(user.channel_createdAt || user.createdAt || now);
  user.channel_updatedAt = Number(user.channel_updatedAt || user.updatedAt || user.createdAt || now);
  user.public_path = swappChannelPathForLogin(user.login);
  user.public_url = user.public_path;
  user.language = String(user.language || 'fr').trim().slice(0,16) || 'fr';
  user.content_rating = String(user.content_rating || 'general').trim().slice(0,30) || 'general';
  user.tags = safeTags(user.tags || []);
  user.following = normalizeArrayUnique(user.following, normalizeOryonLogin, 500);
  user.liked_channels = normalizeArrayUnique(user.liked_channels, normalizeOryonLogin, 500);
  user.swapp_likes = normalizeArrayUnique(user.swapp_likes, x => String(x || '').trim().slice(0,180), 1000);
  user.followers_count = Math.max(0, Number(user.followers_count || 0));
  user.likes_count = Math.max(0, Number(user.likes_count || 0));
  user.channel_badges = Array.isArray(user.channel_badges) ? user.channel_badges.slice(0,8) : [];
  user.channel_panels = Array.isArray(user.channel_panels) ? user.channel_panels.slice(0,8) : [];
  user.channel_vignettes = Array.isArray(user.channel_vignettes) ? user.channel_vignettes.slice(0,8) : [];
  user.channel_links = Array.isArray(user.channel_links) ? user.channel_links.slice(0,8) : [];
  if(options.ensureStreamKey && !user.stream_key) user.stream_key = makeOryonStreamKey();
  const localFresh = user.local_agent_live && Number(user.local_agent_last_seen || 0) && (now - Number(user.local_agent_last_seen || 0) <= oryonLiveSignalTimeoutMs());
  const nativeLive = String(user.live_status || '').toLowerCase() === 'live' && Number(user.live_started_at || 0) && !Number(user.last_live_ended_at || 0);
  if(localFresh || nativeLive) user.live_status = 'live';
  else if(user.live_status === 'live' && !localFresh && !nativeLive) user.live_status = 'offline';
  if(!user.live_status) user.live_status = user.local_agent_live ? 'live' : 'offline';
  if(user.live_status !== 'live') user.local_agent_live = !!localFresh;
  return user;
}
function normalizeOryonUsersPayload(data, options = { ensureStreamKey:true }){
  ...
  for(const raw of rawUsers){
    const u = ensureOryonUserShape(raw, options);
    if(!u?.login) continue;
    const idKey = String(u.id || '');
    if(idKey && seenIds.has(idKey)) continue;
    if(seenLogins.has(u.login)){
  ...
  return { users, login_conflicts: loginConflicts };
}
function oryonLoginAvailable(data, login, exceptUserId = ''){
  const target = normalizeOryonLogin(login);
  if(!target) return false;
  return !(Array.isArray(data?.users) ? data.users : []).some(u => normalizeOryonLogin(u.login) === target && String(u.id || '') !== String(exceptUserId || ''));
}
function markOryonLiveFields(user, active, meta = {}){
  if(!user) return user;
  const now = Date.now();
  ensureOryonUserShape(user, { ensureStreamKey: true });
  user.local_agent_last_seen = now;
  user.channel_last_seen = now;
  if(active){
    if(user.live_status !== 'live' || !user.live_started_at) user.live_started_at = now;
    user.live_status = 'live';
    user.local_agent_live = meta.localAgent === false ? !!user.local_agent_live : true;
    user.last_live_ended_at = null;
    user.current_live_title = String(meta.title || user.current_live_title || `Live de ${user.display_name || user.login}`).trim().slice(0,120);
    user.current_live_category = String(meta.category || user.current_live_category || '').trim().slice(0,80);
    user.current_live_tags = safeTags(meta.tags || user.current_live_tags || []);
    user.external_live_platform = String(meta.platform || user.external_live_platform || 'oryon-live').slice(0,40);
    user.has_launched_live = true;
  }else{
    user.live_status = 'offline';
    user.local_agent_live = false;
    user.last_live_ended_at = now;
    user.current_live_viewers = 0;
  }
  user.updatedAt = now;
  user.channel_updatedAt = now;
  return user;
}
async function persistOryonLiveStateByLogin(login, active, meta = {}){
  const data = readOryonUsers();
  const target = normalizeOryonLogin(login);
  const user = (data.users || []).find(u => normalizeOryonLogin(u.login) === target);
  if(!user) return { success:false, error:'channel_not_found' };
  markOryonLiveFields(user, active, meta);
  const persistence = await writeOryonUsersAndWait(data);
  return { success:true, user, persistence };
}
async function sweepStaleOryonLiveSignals(){
  const data = readOryonUsers();
  ...
  for(const u of users){
    ...
    if(staleLocal || staleGeneric){
      ...
      u.last_live_ended_at = u.last_live_ended_at || (last ? last + oryonLiveSignalTimeoutMs() : now);
      u.updatedAt = now;
      u.channel_updatedAt = now;
      changed++;
    }
  }
  if(changed) await writeOryonUsersAndWait(data);
...
function readOryonUsers(){
  ...
  try{
    if(fs.existsSync(ORYON_USERS_FILE)){
      ...
      __oryonUsersCache = normalizeOryonUsersPayload({ users: Array.isArray(data.users) ? data.users : [] });
...
async function loadOryonUsersPersistent(){
  ...
  if(firestoreOk && db && typeof db.collection === 'function'){
    try{
      ...
      if(!snapUsers.empty){
        __oryonUsersCache = normalizeOryonUsersPayload({ users: snapUsers.docs.map(d => ({ id:d.id, ...d.data() })).filter(Boolean) });
        __oryonUsersPersistence = `firestore:oryon_users:${firestoreSource||'service-account'}`;
        console.log(`[ORYON] ${__oryonUsersCache.users.length} comptes chargés depuis Firestore collection oryon_users.`);
        return;
      }
    }catch(e){ console.warn('[ORYON] Firestore collection users unavailable:', e.message); }
    try{
      const snap = await db.collection('oryon_state').doc('users').get();
      if(snap.exists && Array.isArray(snap.data()?.users)){
        __oryonUsersCache = normalizeOryonUsersPayload({ users: snap.data().users });
        __oryonUsersPersistence = `firestore:oryon_state:${firestoreSource||'service-account'}`;
        console.log(`[ORYON] ${__oryonUsersCache.users.length} comptes chargés depuis Firestore state.`);
        return;
      }
    }catch(e){ console.warn('[ORYON] Firestore state users unavailable:', e.message); }
  }

  // 2) Render disk / local JSON fallback. This is not enough for redeploy unless ORYON_DATA_DIR is a mounted disk.
  try{
    ...
    if(fs.existsSync(ORYON_USERS_FILE)){
      ...
      __oryonUsersCache = normalizeOryonUsersPayload({ users: Array.isArray(data.users) ? data.users : [] });
      __oryonUsersPersistence = process.env.ORYON_DATA_DIR ? 'render-disk-json' : 'ephemeral-json-local';
      console.warn(`[ORYON] ${__oryonUsersCache.users.length} comptes chargés depuis ${__oryonUsersPersistence}.`);
      return;
    }
...
async function persistOryonUsers(data){
  ...
  try{
    if(!fs.existsSync(ORYON_DATA_DIR)) fs.mkdirSync(ORYON_DATA_DIR,{recursive:true});
    fs.writeFileSync(ORYON_USERS_FILE, JSON.stringify(payload, null, 2));
  }catch(e){ console.warn('[ORYON] write json users failed:', e.message); }

  if(!(firestoreOk && db && typeof db.collection === 'function')){
    __oryonUsersPersistence = process.env.ORYON_DATA_DIR ? 'render-disk-json' : 'ephemeral-json-local';
    return { firestore:false, persistence:__oryonUsersPersistence };
  }
  ...
  try{
    ...
    batch.set(stateRef, { users: payload.users, updatedAt: now, persistence:'firestore', source: firestoreSource || null }, { merge:true });
    for(const u of payload.users){
      if(!u?.id) continue;
      const clean = { ...u, updatedAt: u.updatedAt || now };
      batch.set(db.collection('oryon_users').doc(String(u.id)), clean, { merge:true });
      if(u.login) batch.set(db.collection('oryon_user_logins').doc(String(u.login).toLowerCase()), { userId:u.id, login:u.login, updatedAt:now }, { merge:true });
      if(u.email) batch.set(db.collection('oryon_user_emails').doc(String(u.email).toLowerCase()), { userId:u.id, email:u.email, updatedAt:now }, { merge:true });
    }
    await batch.commit();
    __oryonUsersPersistence = `firestore:write-ok:${firestoreSource||'service-account'}`;
    return { firestore:true, persistence:__oryonUsersPersistence };
  }catch(e){
    console.warn('[ORYON] Firestore users persist failed:', e.message);
    __oryonUsersPersistence = process.env.ORYON_DATA_DIR ? 'render-disk-json-after-firestore-fail' : 'ephemeral-json-after-firestore-fail';
    return { firestore:false, error:e.message, persistence:__oryonUsersPersistence };
  }
}

function writeOryonUsers(data){
  __oryonUsersCache = { users: Array.isArray(data?.users) ? data.users : [] };
  persistOryonUsers(__oryonUsersCache).catch(e=>console.warn('[ORYON] async persist failed:', e.message));
}
async function writeOryonUsersAndWait(data){
...
app.get('/api/oryon/persistence-status', async (_req, res) => {
  ...
  res.json({
    ...
    firestore_source: firestoreSource || null,
...
app.post('/api/oryon/register', async (req, res) => {
  try{
    ...
    const user = ensureOryonUserShape({ id: crypto.randomBytes(12).toString('hex'), login, display_name: display, email, email_verified: true, email_verify_token: null, password_hash: hashOryonPassword(password), createdAt: Date.now(), channel_createdAt: Date.now() }, { ensureStreamKey:true });
    data.users.push(user);
    const remember_token = issueOryonRememberToken(user, data);
...
app.post('/api/oryon/login', async (req, res) => {
  try{
    try{ await loadOryonUsersPersistent(); }catch(_){}
    const login = normalizeOryonLogin(req.body?.login);
    const password = String(req.body?.password || '');
...
app.get('/api/oryon/profile/:login', async (req, res) => {
  try{
    await ORYON_USERS_READY.catch(()=>{});
    await sweepStaleOryonLiveSignals().catch(()=>null);
    const login = normalizeOryonLogin(req.params.login);
    const data = readOryonUsers();
    const user = (data.users || []).find(u => u.login === login);
    if(!user) return res.status(404).json({ success:false, error:'Chaîne introuvable.', public:true });
    ensureOryonUserShape(user, { ensureStreamKey:true });
    ...
    pub.channel_exists = true;
    pub.public_url = swappChannelPathForLogin(login);
    pub.public_abs_url = (swappPublicBase(req)||'').replace(/\/$/,'') + pub.public_url;
    pub.is_live = !!room || localLiveFresh;
    pub.live_status = pub.is_live ? 'live' : 'offline';
    pub.oryon_local_player_url = localLiveFresh ? String(user.oryon_local_player_url || '') : '';
    pub.oryon_local_status_url = localLiveFresh ? String(user.oryon_local_status_url || '') : '';
    pub.current_live_title = room?.title || user.current_live_title || '';
    pub.current_live_category = room?.category || user.current_live_category || '';
    res.setHeader('Cache-Control','no-store');
    res.json({ success:true, public:true, requires_auth:false, user: pub });
  }catch(e){ res.status(500).json({ success:false, error:e.message, public:true }); }
});
...
app.post('/api/oryon/profile', mutationLimiter, async (req, res) => {
  try{
    ...
    user.channel_updatedAt = user.updatedAt;
    const persistResult = await writeOryonUsersAndWait(data);
    req.session.oryonUser = sessionOryonUser(user);
...
function swappRejectBadLocalAgentUrl(raw){
  try{
    const u = new URL(String(raw || ''));
    const h = String(u.hostname || '').toLowerCase();
    if(h === 'localhost' || h === '127.0.0.1') return 'URL locale refusée : Swapp doit recevoir une URL publique.';
    if(h.endsWith('loca.lt') || h.includes('localtunnel')) return 'localtunnel / loca.lt refusé : ce service affiche une page de sécurité et casse le player intégré.';
    return '';
  }catch(_){ return 'URL publique invalide.'; }
}
...
app.post('/api/oryon/local-agent/register-public-url', mutationLimiter, async (req, res) => {
  try {
    ...
    const urlRefusal = swappRejectBadLocalAgentUrl(playerUrl) || (statusUrl ? swappRejectBadLocalAgentUrl(statusUrl) : '') || (publicBaseUrl ? swappRejectBadLocalAgentUrl(publicBaseUrl) : '');
    if(urlRefusal) return res.status(400).json({ success:false, error:urlRefusal });
    ...
    await writeOryonUsersAndWait(data);
    res.json({ success:true, login:user.login, public_url:swappChannelPathForLogin(user.login), public_abs_url:(swappPublicBase(req)||'').replace(/\/$/,'') + swappChannelPathForLogin(user.login), live:true, user:publicOryonUser(user), message:'Live Swapp Local publié sur la page publique.' });
  } catch(e) { res.status(500).json({ success:false, error:e.message }); }
});
...
app.post('/api/oryon/local-agent/heartbeat', mutationLimiter, async (req, res) => {
  try {
    ...
    if(liveActive){
      const heartbeatUrlRefusal = playerUrl ? swappRejectBadLocalAgentUrl(playerUrl) : '';
      const heartbeatStatusRefusal = statusUrl ? swappRejectBadLocalAgentUrl(statusUrl) : '';
      if(heartbeatUrlRefusal || heartbeatStatusRefusal) return res.status(400).json({ success:false, error:heartbeatUrlRefusal || heartbeatStatusRefusal });
    ...
    await writeOryonUsersAndWait(data);
    res.json({ success:true, live:user.local_agent_live, login:user.login, public_url:swappChannelPathForLogin(user.login), public_abs_url:(swappPublicBase(req)||'').replace(/\/$/,'') + swappChannelPathForLogin(user.login), user:publicOryonUser(user) });
  } catch(e) { res.status(500).json({ success:false, error:e.message }); }
});
...
app.post('/api/oryon/stream-key/regenerate', mutationLimiter, async (req, res) => {
  try{
    ...
    const user = data.users.find(u => u.id === cur.id);
    if(!user) return res.status(404).json({ success:false, error:'Utilisateur introuvable.' });
    ensureOryonUserShape(user, { ensureStreamKey:true }); user.stream_key = makeOryonStreamKey(); user.updatedAt = Date.now(); user.channel_updatedAt = user.updatedAt; await writeOryonUsersAndWait(data);
...
const oryonSupportFile = path.join(__dirname, '.oryon-first-supports.json');
...
function getFirstSupportPayload(login, viewerLogin=''){
  ...
  const channel = data.channels[login] || {first_supporters:[]};
  const first = Array.isArray(channel.first_supporters) ? channel.first_supporters.slice(0,12) : [];
  return {success:true, login, first_supporters:first, count:first.length, already:!!(viewerLogin && first.some(s=>s.login===viewerLogin)), is_self:!!(viewerLogin && viewerLogin===login), full:first.length>=12};
}
app.get('/api/oryon/supporters/:login', (req,res)=>{
  const login=normalizeOryonLogin(req.params.login);
  if(!login) return res.status(400).json({success:false,error:'Pseudo invalide.',first_supporters:[]});
  res.json(getFirstSupportPayload(login, normalizeOryonLogin(req.session?.oryonUser?.login||'')));
});
app.post('/api/oryon/support/first/:login', (req,res)=>{
  const cur=requireOryon(req,res); if(!cur) return;
  const login=normalizeOryonLogin(req.params.login);
  if(!login) return res.status(400).json({success:false,error:'Pseudo invalide.'});
  if(login===normalizeOryonLogin(cur.login)) return res.status(400).json({success:false,error:'Tu ne peux pas devenir premier soutien de ta propre chaîne.'});
  const users=readOryonUsers().users||[];
  const target=users.find(u=>u.login===login);
  if(!target) return res.status(404).json({success:false,error:'Chaîne Oryon introuvable.'});
  const data=readJsonSafe(oryonSupportFile,{channels:{}}); data.channels=data.channels||{};
  const channel=data.channels[login] || {first_supporters:[]}; channel.first_supporters=Array.isArray(channel.first_supporters)?channel.first_supporters:[];
  if(channel.first_supporters.some(s=>s.login===cur.login)){
    data.channels[login]=channel; writeJsonSafe(oryonSupportFile,data);
    return res.json({...getFirstSupportPayload(login,cur.login), message:'Tu es déjà premier soutien de cette chaîne.'});
  }
  if(channel.first_supporters.length>=12) return res.status(409).json({success:false,error:'Les badges premiers soutiens sont déjà complets.',...getFirstSupportPayload(login,cur.login)});
  channel.first_supporters.push({login:cur.login,display_name:cur.display_name||cur.login,avatar_url:cur.avatar_url||'',since:Date.now()});
  channel.updatedAt=Date.now(); data.channels[login]=channel; writeJsonSafe(oryonSupportFile,data);
  pushOryonEvent('first_support',cur.login,{channel:login});
  res.json({...getFirstSupportPayload(login,cur.login), message:'Premier soutien enregistré.'});
});
...
io.on('connection', async (socket) => {
  ...
  socket.on('native:create', (payload) => {
    ...
    try{ const ud=readOryonUsers(); const uu=ud.users.find(x=>x.id===user.id); if(uu){ markOryonLiveFields(uu, true, { localAgent:false, platform:'browser-webrtc', title, category: payload?.category, tags: payload?.tags }); uu.best_chat_messages=Math.max(Number(uu.best_chat_messages||0),0); writeOryonUsers(ud); } pushOryonEvent('live_started', user.login, {room,title}); }catch(_e){}
...
// Unlock Market access with credits (Pass Marché)
app.post('/api/billing/unlock-market', async (req,res)=>{
  try{
    const tu = requireTwitchSession(req, res);
    if(!tu) return;
    if(!firestoreOk) return res.status(503).json({ success:false, error:'firestore_unavailable' });
    ...
    if(String(e.message||'') === 'credits_insufficient'){
      return res.status(400).json({ success:false, error:'credits_insufficient' });
    }
    res.status(500).json({ success:false, error:e.message });
  }
});
...
// Server-side Market access decision (single source of truth)
app.get('/api/market/access', async (req,res)=>{
  try{
    const tu = req.session?.twitchUser;
    if(!tu){
      return res.json({ success:true, connected:false, allowed:false, reason:'TWITCH_AUTH_REQUIRED' });
    }
...
// Public discovery for home: never uses followed Twitch. It returns Oryon native + Twitch public streams.
app.get('/api/discovery/home-lives', heavyLimiter, async (req,res)=>{
  try{
    const lang = String(req.query.lang || 'fr').trim().toLowerCase();
    ...
    function push(x, source){
      ...
      const key=(source||x.platform||'live')+':'+login+':'+String(x.id||'');
      if(!login || seen.has(key)) return;
      seen.add(key);
      const viewers=Number(x.viewer_count ?? x.viewers ?? 0) || 0;
      out.push({
        ...x,
        platform: source || x.platform || 'twitch',
        login: x.login || x.user_login || x.host_login || x.room || '',
        display_name: x.display_name || x.user_name || x.host_name || x.login || x.room || 'Live',
        game_name: x.game_name || x.category || x.game || 'Live',
        viewer_count: viewers,
        thumbnail_url: String(x.thumbnail_url || x.img || '').replace('{width}','1280').replace('{height}','720')
      });
    }

    // Oryon native lives first if present.
    try{ for(const x of discoverNativeItemsFor('', 100000, 'both')) push(x, 'oryon'); }catch(_){ }

    // Twitch public, progressive broadening. No followed fallback here.
    if(TWITCH_CLIENT_ID && TWITCH_CLIENT_SECRET){
      const endpoints = [
        `streams?first=100&language=${encodeURIComponent(lang)}`,
        'streams?first=100'
      ];
      for(const ep of endpoints){
        try{
          const tr = await twitchAPI(ep);
          for(const s of (tr.data||[])) push(s, 'twitch');
          if(out.length >= limit) break;
        }catch(e){ console.warn('[HOME DISCOVERY] Twitch public failed:', e.message); }
      }
    }

    out.sort((a,b)=>{
      const av=Number(a.viewer_count||0), bv=Number(b.viewer_count||0);
      const as=(av>=30&&av<=100?1000:0)+(av>=15&&av<=300?300:0)-Math.abs(av-60);
      const bs=(bv>=30&&bv<=100?1000:0)+(bv>=15&&bv<=300?300:0)-Math.abs(bv-60);
      return bs-as;
    });
...
app.get('/api/oryon/channel/:login/status', async (req, res) => {
  try{
    await ORYON_USERS_READY.catch(()=>{});
    await sweepStaleOryonLiveSignals().catch(()=>null);
    const login = normalizeOryonLogin(req.params.login);
    ...
    res.json({
      ...
      live:{
        is_live:!!room || localLiveFresh,
        source:room ? 'browser-webrtc' : (localLiveFresh ? 'local-agent' : 'offline'),
        status:(!!room || localLiveFresh) ? 'live' : 'offline',
...
function swappCountLiveUsers(users){
  const now = Date.now();
  const timeout = Number(process.env.ORYON_LIVE_SIGNAL_TIMEOUT_MS || 120000);
...
async function buildSwappDiagnostic(req){
  ...
  try{ await sweepStaleOryonLiveSignals(); }catch(_e){}
  const users = (readOryonUsers().users || []);
  const teams = (oryonRead(ORYON_TEAMS_FILE,{teams:[]}).teams || []);
...
app.post('/api/oryon/client-recover', async (req, res) => {
  try{
    ...
    if(user){
      if(!token || !verifyOryonRememberToken(user, token)) return res.status(401).json({success:false,error:'Ce pseudo existe déjà. Reconnecte-toi avec le mot de passe ou un jeton valide.'});
      ...
      if(!oryonLoginAvailable(data, backup.login)) return res.status(409).json({success:false,error:'Ce pseudo est déjà réservé.'});
      user = ensureOryonUserShape({
        id: crypto.randomBytes(12).toString('hex'),
        ...backup,
        email_verified:true,
