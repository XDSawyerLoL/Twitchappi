...
(function(){
  ...
  async function __ensureTwitchConnected(){
    // 1) already exposed
    if(window.currentUser && (window.currentUser.is_connected || window.currentUser.display_name || window.currentUser===true)) return true;

    // 2) DOM heuristic (user area visible)
    try{
      const ua = document.getElementById('user-area');
      if(ua && !ua.classList.contains('hidden')){
        const name = (document.getElementById('user-name')?.textContent || document.getElementById('hub-user-display')?.textContent || '').trim();
        window.currentUser = { display_name: name || 'User', is_connected:true };
        return true;
      }
    }catch(_){}
    ...
    // 3) server truth
    try{
      const r = await fetch('/twitch_user_status', { credentials:'include' });
      const d = await r.json().catch(()=>null);
      if(d && d.is_connected){
        window.currentUser = { display_name: d.display_name || d.login || 'User', login: d.login || null, id: d.user_id || null, profile_image_url: d.profile_image_url || null, is_connected:true };
        return true;
      }
    }catch(_){}
    return false;
  ...
  window.openMarketOverlay = function(){
    (async ()=>{
      const ok = await __ensureTwitchConnected();
      if(!ok){
      ...
      // If base open handler exists, call it. Otherwise, open overlay directly.
