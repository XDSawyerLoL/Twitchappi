...
async function ensurePublicTunnelReady(options = {}){
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      log('error', 'Tunnel invalide, relance automatique', { attempt:i+1, error:e.message });
      await stopPublicTunnel();
      await delay(1800);
    }
  }
  throw new Error('Tunnel non publié après relances automatiques. ' + errors.slice(-3).join(' | '));
}
...
async function refreshDiag(){try{const r=await fetch('/api/setup/check?key='+encodeURIComponent(currentKey())+'&auto=1&t='+Date.now());const d=await r.json();const st=d.stream||{};const obs=!!d.obs_active,hls=!!d.hls,tun=!!d.tunnel_published;let label='En attente d’OBS.';if(obs&&!hls)label='⏳ OBS reçu. Préparation du flux…';if(obs&&hls&&!tun)label='⏳ Flux prêt. Publication du tunnel…';if(obs&&hls&&tun)label='✅ Flux prêt. Tunnel publié.';document.getElementById('simpleStatus').innerHTML=label;const diag=document.getElementById('diag');if(diag)diag.innerHTML='OBS actif: '+obs+'\nFFmpeg: '+!!d.ffmpeg_running+'\nHLS: '+hls+'\nTunnel: '+(tun?'publié':'non publié')+(d.tunnel_error?'\nErreur tunnel: '+d.tunnel_error:'');const pub=document.getElementById('publicUrl');if(pub&&tun&&d.player_url)pub.textContent='Tunnel publié: '+d.player_url;if(publishedLiveKey){await sendHeartbeat(obs&&hls&&tun)}}catch(e){document.getElementById('simpleStatus').textContent='Diagnostic indisponible.'}}
...
app.post('/api/go-live', async (req, res) => {
  ...
  try {
    ...
    await ensurePublicTunnelReady({ key, attempts: 4 });
...
app.get('/api/setup/check', async (req, res) => {
  ...
  if (stream.active && stream.hls_exists && String(req.query.auto || '1') !== '0') {
    try { tunnel = await ensurePublicTunnelReady({ key, attempts: 2 }); tunnelOk = true; }
  ...
  res.json({
    success:true,
    ready:Boolean(stream.active && stream.ffmpeg_running && stream.hls_exists && tunnelOk),
    obs_active:Boolean(stream.active),
    ffmpeg:true,
    ffmpeg_info:ffmpegInfo(),
    ffmpeg_running:Boolean(stream.ffmpeg_running),
    hls:Boolean(stream.hls_exists),
    ...
    message: stream.active && stream.hls_exists ? (tunnelOk ? 'Flux prêt et tunnel publié.' : 'Flux prêt, tunnel en cours ou bloqué.') : 'En attente du flux OBS.'
  });
});
