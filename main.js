...
const DEFAULT_ORYON_SITE_URL = process.env.ORYON_SITE_URL || 'https://swapp.tv';
...
const TRANSCODE_MODE = String(process.env.ORYON_LOCAL_TRANSCODE || 'transcode').toLowerCase();
...
function hlsSegmentFiles(key = currentConfiguredKey()) {
  return listFiles(hlsDir(key)).filter(f => /\.ts$/i.test(f));
}
function hlsReady(key = currentConfiguredKey()) {
  const k = safeKey(key);
  return fs.existsSync(hlsPath(k)) && hlsSegmentFiles(k).length > 0;
}
function streamStatus(key = currentConfiguredKey()) {
  const k = safeKey(key);
  const files = listFiles(hlsDir(k));
  const segments = files.filter(f => /\.ts$/i.test(f));
  return {
    key: k,
    active: Boolean(active[k]),
    ffmpeg_running: Boolean(ffmpegJobs[k]),
    hls_exists: fs.existsSync(hlsPath(k)),
    hls_ready: fs.existsSync(hlsPath(k)) && segments.length > 0,
    hls_url: hlsUrl(k),
...
function startHlsTransmux(key) {
  ...
  if (TRANSCODE_MODE === 'copy') {
    ...
    args.push('-c:v', 'libx264', '-preset', 'veryfast', '-tune', 'zerolatency', '-pix_fmt', 'yuv420p', '-profile:v', 'baseline', '-level', '3.1', '-g', '60', '-sc_threshold', '0', '-c:a', 'aac', '-ar', '44100', '-b:a', '128k');
  }
  args.push('-f', 'hls', '-hls_time', '2', '-hls_list_size', '6', '-hls_flags', 'delete_segments+append_list+omit_endlist+program_date_time', '-hls_segment_type', 'mpegts', '-hls_segment_filename', segmentPattern, output);
...
async function startLocalTunnelProvider() {
  throw new Error('localtunnel/loca.lt est désactivé : ce service injecte une page de sécurité et casse le lecteur Swapp. Utilise Cloudflare Tunnel ou un vrai serveur RTMP Swapp.');
}
async function startPublicTunnel(provider = 'auto') {
  if (Date.now() < tunnelCooldownUntil) {
    const left = Math.ceil((tunnelCooldownUntil - Date.now()) / 1000);
    throw new Error(`Cloudflare limite les quick tunnels. Attends encore ${left}s avant de retenter.`);
  }
  if (publicTunnelInfo?.url && cloudflaredProcess) return publicTunnelInfo;
  const requested = String(provider || 'auto').toLowerCase();
  if (requested === 'manual') return publicTunnelInfo;
  try {
    return await startCloudflaredProvider();
  } catch (e) {
    const msg = String(e && e.message || e || '');
    if (/429|too many requests|1015/i.test(msg + ' ' + cloudflaredLastLog)) {
      tunnelCooldownUntil = Date.now() + 10 * 60 * 1000;
      throw new Error('Cloudflare a bloqué temporairement les quick tunnels (429). Le flux local reste prêt, mais le live public ne peut pas être publié avant le cooldown.');
    }
    throw e;
  }
}
...
function pageHtml(key){
  ...
  return String.raw`<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Swapp Live</title><style>
...
async function refreshDiag(){try{const r=await fetch('/api/status?key='+encodeURIComponent(currentKey())+'&t='+Date.now());const d=await r.json();const st=d.stream||{};const ok=st.active&&st.ffmpeg_running&&st.hls_ready;document.getElementById('simpleStatus').innerHTML=ok?'✅ Flux prêt. Tu peux démarrer sur Swapp.':(st.active?'⏳ Flux reçu. Préparation…':'En attente d’OBS.'); const diag=document.getElementById('diag'); if(diag) diag.innerHTML=ok?'Flux prêt.':(st.active?'OBS reçu, conversion en cours.':'Aucun flux OBS détecté.'); if(publishedLiveKey){ await sendHeartbeat(ok); }}catch(e){document.getElementById('simpleStatus').textContent='Diagnostic indisponible.'}}
...
app.post('/api/go-live', async (req, res) => {
  ...
  try {
    ...
    if (!st.hls_ready) return res.status(400).json({ success:false, error:'OBS est détecté, mais le HLS n’a pas encore de segment vidéo lisible. Attends 5 à 15 secondes puis réessaie.' });
...
app.get('/hls/:key/:file', (req, res) => {
  ...
  if (!/^(index\.m3u8|seg_\d+\.ts|[a-zA-Z0-9_.-]+\.ts)$/i.test(file)) return res.status(404).send('Not found');
  const dir = hlsDir(key);
  const target = path.join(dir, file);
  if (!target.startsWith(dir) || !fs.existsSync(target)) return res.status(404).send('Not found');
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Cache-Control', file.endsWith('.m3u8') ? 'no-store, no-cache, must-revalidate' : 'public, max-age=30');
...
app.get('/api/setup/check', (req, res) => {
  ...
  res.json({
    success:true,
    ready: !!(st.active && st.ffmpeg_running && st.hls_ready),
    obs_active: !!st.active,
    ffmpeg: !!st.ffmpeg_running,
    hls: !!st.hls_ready,
    tunnel_published: !!publicTunnelInfo?.url,
    tunnel: publicTunnelInfo,
    tunnel_cooldown_until: tunnelCooldownUntil || null,
    stream: st,
    health_url: `${LOCAL_BASE_URL}/health`,
    player_url: playerUrl(key),
    hls_url: hlsUrl(key),
...
app.get('/player/:key', (req, res) => {
  ...
  const status = `${LOCAL_BASE_URL}/api/setup/check?key=${encodeURIComponent(key)}`;
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Swapp Local Player</title><style>html,body{margin:0;width:100%;height:100%;background:#030508;color:white;font-family:system-ui;overflow:hidden}video{width:100%;height:100%;object-fit:contain;background:#030508}.state{position:absolute;inset:0;display:grid;place-items:center;text-align:center;color:#c8d2e7;padding:20px;background:rgba(3,5,8,.35)}.small{font-size:13px;color:#94a3b8;margin-top:8px}</style></head><body><video id="v" autoplay muted playsinline controls></video><div id="s" class="state"><div>Connexion au flux…<div class="small" id="d"></div></div></div><script src="https://cdn.jsdelivr.net/npm/hls.js@latest"></script><script>const src=${JSON.stringify(hls)},statusUrl=${JSON.stringify(status)},v=document.getElementById('v'),s=document.getElementById('s'),d=document.getElementById('d');let h=null,booted=false;function ok(){s.style.display='none'}function msg(t,detail){s.style.display='grid';s.firstElementChild.childNodes[0].nodeValue=t;d.textContent=detail||''}async function check(){try{const r=await fetch(statusUrl+'&t='+Date.now(),{cache:'no-store'});return await r.json()}catch(e){return {success:false,error:e.message}}}async function ready(){const st=await check();if(!st.hls){msg('Connexion au flux…', st.obs_active?'OBS reçu, génération HLS…':'En attente d’OBS');return false}try{const r=await fetch(src+'?t='+Date.now(),{cache:'no-store'});if(!r.ok){msg('Playlist HLS introuvable','HTTP '+r.status);return false}return true}catch(e){msg('Playlist HLS introuvable',e.message);return false}}async function boot(){if(booted)return;if(!await ready()){setTimeout(boot,1500);return}booted=true;if(v.canPlayType('application/vnd.apple.mpegurl')){v.src=src+'?t='+Date.now();v.addEventListener('loadedmetadata',ok,{once:true});v.addEventListener('playing',ok,{once:true});v.play().catch(()=>{});return}if(window.Hls&&Hls.isSupported()){h=new Hls({lowLatencyMode:true,liveSyncDurationCount:2,backBufferLength:30});h.loadSource(src+'?t='+Date.now());h.attachMedia(v);h.on(Hls.Events.MANIFEST_PARSED,()=>{ok();v.play().catch(()=>{})});h.on(Hls.Events.FRAG_LOADED,ok);h.on(Hls.Events.ERROR,(_e,data)=>{if(data&&data.fatal){booted=false;try{h.destroy()}catch(e){}h=null;msg('Reconnexion au flux…',data.type||'');setTimeout(boot,1500)}});return}msg('Lecteur HLS indisponible','Connexion internet requise pour charger hls.js, ou utilise Chrome récent.')}boot();</script></body></html>`;
