...
app.get('/twitch_auth_callback', async (req, res) => {
  ...
  try {
    ...
    // Also generate a signed token for iframe / cross-site cases (when cookies are blocked).
    const authToken = signAuthToken({ twitchUser: req.session.twitchUser });
    const tokenJson = JSON.stringify(authToken);

    // Persist session then notify opener with token (postMessage) + close popup
    req.session.save(() => {
          ...
          (function(){
            try{
              if(window.opener && !window.opener.closed){
                window.opener.postMessage({ type:'twitch_auth', token: ${tokenJson} }, '*');
    ...
    return res.status(500).send('Erreur Callback: ' + e.message);
  }
});
...
// =========================================================
// AUTH TOKEN FALLBACK (for iframe / third-party cookie restrictions)
// - When embedded on another domain (ex: justplayer.fr iframe), browsers may block session cookies.
// - We provide a signed short-lived token that the front can store (localStorage) and send via header.
// Header supported: X-Auth-Token or Authorization: Bearer <token>
// =========================================================
const AUTH_TOKEN_TTL_MS = Number(process.env.AUTH_TOKEN_TTL_MS || (7 * 24 * 60 * 60 * 1000)); // 7 days default
const AUTH_TOKEN_SECRET = process.env.AUTH_TOKEN_SECRET || process.env.SESSION_SECRET || process.env.TWITCH_CLIENT_SECRET || 'dev_auth_token_secret_change_me';
...
function signAuthToken(payloadObj){
  const header = { alg:'HS256', typ:'JWT' };
  const iat = Date.now();
  const exp = iat + AUTH_TOKEN_TTL_MS;
  const payload = { ...payloadObj, iat, exp };
  const h = b64urlEncode(JSON.stringify(header));
  const p = b64urlEncode(JSON.stringify(payload));
  const data = `${h}.${p}`;
  const sig = crypto.createHmac('sha256', AUTH_TOKEN_SECRET).update(data).digest('base64')
    .replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
  return `${data}.${sig}`;
}
function verifyAuthToken(token){
  try{
    ...
    const [h,p,sig] = parts;
    const data = `${h}.${p}`;
    const expected = crypto.createHmac('sha256', AUTH_TOKEN_SECRET).update(data).digest('base64')
      .replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
...
function getAuthTokenFromReq(req){
  const h = req.headers || {};
  const direct = h['x-auth-token'] || h['X-Auth-Token'];
...
app.post('/api/billing/webhook', async (req, res) => {
  try{
    if(!stripe) return res.status(501).send('Stripe non configuré.');
    ...
    // Helper: build a minimal twitchUser object from metadata
    function tuFromMeta(meta){
      const id = String(meta?.twitch_user_id || meta?.twitchUserId || meta?.user_id || '').trim();
      const login = String(meta?.twitch_login || meta?.twitchLogin || meta?.login || '').trim();
      return { id: id || login || 'unknown', login: login || null, display_name: login || null };
    }

    if(event.type === 'checkout.session.completed'){
      const s = event.data.object;
      const meta = s.metadata || {};
      const tu = tuFromMeta(meta);

      // One-time payments -> credits
      const credits = Number(meta.credits || 0);
      if(credits > 0){
        await updateBillingCredits(tu, credits);
      }

      // Subscriptions -> plan
      const plan = String(meta.plan || '').trim();
      if(plan){
        await setBillingPlan(tu, plan);
      }

      // Save stripe ids for support/debug
      if(firestoreOk){
        const id = String(tu.id || 'unknown');
        await db.collection(BILLING_USERS).doc(id).set({
          stripe_customer_id: s.customer || null,
          stripe_subscription_id: s.subscription || null,
          last_checkout_session: s.id || null,
          updatedAt: admin.firestore.FieldValue.serverTimestamp()
        }, { merge:true });
      }
    }

    // Subscription cancellations -> back to free
    if(event.type === 'customer.subscription.deleted'){
      const sub = event.data.object;
      const customerId = String(sub.customer || '');
      if(customerId && firestoreOk){
        const q = await db.collection(BILLING_USERS).where('stripe_customer_id','==', customerId).limit(1).get();
        if(!q.empty){
          const doc = q.docs[0];
          await doc.ref.set({
            plan: 'free',
