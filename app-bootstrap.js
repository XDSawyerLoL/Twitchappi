...
function buildPortal(el){
  ...
  const isAnalytics = (kind === 'analytics') || (kind === 'dashboard') || el.id === 'under-analytics' || el.getAttribute('data-paywall-scope') === 'analytics';
  const title = isAnalytics ? 'Dashboard Premium — Statistiques avancées' : (el.getAttribute('data-paywall-title') || 'Module Premium');
  const desc  = isAnalytics ? "Dashboard Premium (3 onglets). KPIs avancés, tendances, alertes et recommandations pour optimiser ta chaîne." : (el.getAttribute('data-paywall-desc') || 'Débloque ce module avec Premium/Pro ou des crédits.');
  const bullets = isAnalytics ? `
      <ul style="margin:10px 0 0; padding-left:18px; font-size:12.5px; line-height:1.35; color:rgba(255,255,255,.78)">
        <li>Graphes & historique (pics, patterns)</li>
        <li>Best Time to Stream + reco IA</li>
        <li>Alertes & analyse Niche actionnables</li>
      </ul>` : '';
  const foot = isAnalytics ? 'Offre Premium / Pro' : 'Plan + crédits + outils IA';
...
function applyPaywallUI(){
  ...
  // Lock rule: FREE with 0 credits => lock premium areas.
  const locked = (plan === 'FREE' && credits <= 0);

  const els = Array.from(document.querySelectorAll('[data-paywall]'));

  // Dashboard Premium root = container that holds the 3 tabs (OVERVIEW / ANALYTICS PRO / NICHE)
  let dashboardRoot = null;
  const nav = document.getElementById('under-tabs-nav');
  if (nav) dashboardRoot = nav.closest('.glass-panel');

  // Fallback: if layout changes, try closest common wrapper
  if (!dashboardRoot){
  ...
  if (!locked){
    ...
    if (dashboardRoot) unlockPaywallElement(dashboardRoot);
  } else {
    // SINGLE paywall for the whole dashboard premium area
    if (dashboardRoot){
      dashboardRoot.setAttribute('data-paywall-kind','dashboard');
      if (!dashboardRoot.hasAttribute('data-paywall')) dashboardRoot.setAttribute('data-paywall','1');
      lockPaywallElement(dashboardRoot);
    }

    els.forEach(el=>{
      // prevent duplicates inside dashboard scope
      if (dashboardRoot && el !== dashboardRoot && dashboardRoot.contains(el)){
        unlockPaywallElement(el);
        return;
      }
      // If element is the dashboardRoot we already handled it
      if (dashboardRoot && el === dashboardRoot) return;
