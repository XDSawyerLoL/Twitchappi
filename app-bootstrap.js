...
let tfHeaderTrailerTimer = null;
let tfHeaderTrailerIndex = 0;

// Curated OFFICIAL YouTube trailers (IDs). Replace/extend freely.
const OFFICIAL_TRAILERS = [
  // Example set (replace with your official IDs)
  "wS_qbDztgVY", // GTA VI Trailer 1 (Rockstar)
  "QdBZY2fkU-0", // Cyberpunk 2077 Official Trailer
  "h0P4pKvy7Mw", // The Witcher 4 (placeholder example)
];
...
// Header: rotating official YouTube trailers (no rail)
function tfInitHeaderTrailers(){
  const host = document.getElementById('tf-header-trailers');
  if (!host) return;
  if (host.dataset.bound) return;
  host.dataset.bound = '1';

  // If the list is empty, keep the header hidden.
  if (!Array.isArray(OFFICIAL_TRAILERS) || OFFICIAL_TRAILERS.length === 0){
  ...
  host.style.display = '';
  host.innerHTML = `
    <div class="tf-header-trailers-inner">
      <div class="tf-header-trailers-label">TRAILERS OFFICIELS</div>
      <div class="tf-header-trailers-frame">
        <iframe id="tf-header-trailer-iframe"
          src=""
  ...
  tfHeaderTrailerIndex = 0;
  tfHeaderTrailerTick(true);

  if (tfHeaderTrailerTimer) clearInterval(tfHeaderTrailerTimer);
  tfHeaderTrailerTimer = setInterval(()=>tfHeaderTrailerTick(false), 22000);
}

function tfStopHeaderTrailers(){
  if (tfHeaderTrailerTimer){
    clearInterval(tfHeaderTrailerTimer);
    tfHeaderTrailerTimer = null;
  }
}

function tfHeaderTrailerTick(force){
  const host = document.getElementById('tf-header-trailers');
  if (!host) return;
  if (!tfModalOpen) return;
  if (!Array.isArray(OFFICIAL_TRAILERS) || OFFICIAL_TRAILERS.length === 0) return;

  const iframe = document.getElementById('tf-header-trailer-iframe');
  if (!iframe) return;

  // Keep trailer header visible only on LIVE/VOD
  const isAnime = (tfPrimaryMode === 'anime');
  host.style.display = isAnime ? 'none' : '';
  if (isAnime) return;

  const vid = OFFICIAL_TRAILERS[tfHeaderTrailerIndex % OFFICIAL_TRAILERS.length];
  tfHeaderTrailerIndex = (tfHeaderTrailerIndex + 1) % OFFICIAL_TRAILERS.length;
  ...
  const src = `https://www.youtube-nocookie.com/embed/${encodeURIComponent(vid)}?${qs}`;
  if (force || iframe.src !== src){
    iframe.src = src;
  }
}
...
const modal = document.getElementById('twitflix-modal');
      ...
      try{ if(!tfPrimaryMode) tfPrimaryMode = 'live'; tfInitPrimaryTabs(); }catch(_){ }
      try{ tfInitHeaderTrailers(); }catch(_){ }
...
async function tfRenderVodHome(){
  ...
  try{ tfStopHeaderTrailers(); }catch(_){ }
