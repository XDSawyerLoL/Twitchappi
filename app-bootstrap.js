...
nav.querySelectorAll('.u-tab-btn').forEach(b=>b.classList.remove('active'));
    ...
    async function checkAuth() {
      ...
      if (data.is_connected) {
        ...
        // expose for other modules (Market, tools, etc.)
        window.currentUser = {
        ...
        try{ document.getElementById('hub-user-display').innerText = 'INVITÉ'; }catch(_){ }
        try{ document.getElementById('btn-auth').classList.remove('hidden'); }catch(_){ }
        try{ document.getElementById('user-area').classList.add('hidden'); }catch(_){ }
      }
    }
