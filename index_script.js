/* Swapp Product Clarity v2 — navigation viewer + menu créateur à droite */
...
function liveCard(x){const platform=x.platform||((x.embed_url||x.watch_url)?'peertube':(x.login||x.user_login?'twitch':'oryon')); const isT=platform==='twitch'; const isP=platform==='peertube'; const login=x.login||x.user_login||x.host_login||x.room; const name=x.display_name||x.user_name||x.host_name||login||'Live'; const viewers=x.viewer_count??x.viewers??0; const game=x.game_name||x.category||platform; const title=x.title||`Live de ${name}`; const img=x.thumbnail_url?thumbT(x):''; const action=isP?`openPeerTube('${esc(x.embed_url||'')}','${esc(x.watch_url||'')}','${esc(name)}')`:(isT?`openTwitch('${esc(login)}')`:`openOryon('${esc(login)}')`); return `<article class="liveCard"><div class="thumb">${img?`<img src="${esc(img)}" alt="">`:`<div style="height:100%;display:grid;place-items:center;color:#64748b">${isP?'PEERTUBE':isT?'TWITCH':'LIVE SWAPP'}</div>`}</div><div class="liveBody"><div class="row"><span class="pill">${isP?'PeerTube':isT?'Twitch':'Swapp'}</span><span class="pill">${viewers} viewers</span></div><div class="title">${esc(title)}</div><div class="desc">${esc(name)} · ${esc(game)}</div><div class="row" style="margin-top:10px"><button class="btn" onclick="${action}">Regarder</button></div></div></article>`}


/* Swapp Discovery Experience — ambiance, zapping, viewer impact, mini-player */
...
function platformLabel(p){return p==='twitch'?'Twitch':p==='oryon'?'Swapp':'Swapp'}
...
function discoverReasonFor(x){const id=liveIdentity(x); const reasons=[]; if(id.platform==='oryon')reasons.push('Natif Swapp'); if(id.platform==='twitch')reasons.push('Twitch filtré'); if(id.viewers<=20)reasons.push('Très petit live'); else if(id.viewers<=50)reasons.push('Chat lisible'); else reasons.push('Audience raisonnable'); if(id.game)reasons.push(id.game); reasons.push('Entrée sans pression'); return reasons.slice(0,4)}
...
function spotlightMedia(x){
 if(!x)return `<div class="spotlightEmpty"><div><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><h2>Choisis une ambiance</h2></div></div>`;
...
function renderZap(){
 ...
 box.innerHTML=`<div class="spotlightFusion"><div>${renderZapCard(cur)}</div><aside class="spotlightSide"><div class="spotlightPanel compact"><div class="row" style="justify-content:space-between"><div><h2 style="margin:0">Signal</h2><span class="small">${cur?esc(platformLabel(id.platform)):'Swapp + Twitch'}</span></div><div class="comfortRing" style="--score:${score}%">${cur?score:'—'}</div></div>${cur?`<div class="vibeBar section">${vibeMetrics(cur).map(v=>`<div class="vibeLine"><span>${esc(v.k)}</span><i style="--w:${v.v}%"></i><b>${v.v}</b></div>`).join('')}</div>`:'<div class="empty section">Lance une proposition.</div>'}</div><div class="spotlightPanel compact"><h2>Pourquoi</h2><div class="signalGrid">${cur?discoverReasonFor(cur).map(r=>`<div class="signalChip"><b>${esc(r)}</b><span>✓</span></div>`).join(''):'<div class="empty">Aucun live sélectionné.</div>'}</div></div><div class="spotlightPanel spotlightChat"><div class="chatHeader"><span>${cur?`Tchat · ${esc(id.name)}`:'Tchat'}</span></div>${spotlightChat(cur)}</div></aside></div>${items.length?`<div class="section"><div class="spotlightQueue">${spotlightQueue(items)}</div></div>`:''}`;
 renderViewerImpact();
}
...
async function likeOryon(login){if(!state.session.local){toast('Compte Swapp requis');return setView('settings')} const r=await api('/api/oryon/like/'+encodeURIComponent(login),{method:'POST'}); toast(r.success?'Aimé':r.error); const b=$('#likeBtn'); if(b)b.classList.add('likePulse'); await refreshEmoteShelf(login);}
function chatQuick(kind){const input=$('#chatInput'); if(!input)return; const presets={question:'C’est quoi le contexte ?',new:'Je découvre via Swapp 👋',react:'Bonne vibe ✨'}; input.value=presets[kind]||''; input.focus()}
...
async function renderHome(){
 ...
 <div class="section"><div class="pageHead"><div><h1>En direct maintenant</h1><p>Swapp d’abord. Si personne n’est live, petits lives Twitch FR 10–50 viewers.</p></div></div><div id="homeLives" class="rail"><div class="empty">Chargement…</div></div></div>
...
function featuredCard(x,i){const cls=i===0?'main':i===1?'left':i===2?'right':'far';const login=x.login||x.user_login||x.host_login||x.room;const name=x.display_name||x.user_name||x.host_name||login;const viewers=x.viewer_count??x.viewers??0;const img=x.thumbnail_url?thumbT(x):'';const fn=(x.platform==='twitch'||x.login||x.user_login)?`openTwitch('${esc(login)}')`:`openOryon('${esc(login)}')`;return `<article class="featuredCard ${cls}" onclick="${fn}">${img?`<img src="${esc(img)}" alt="">`:`<div class="thumb"><div style="height:100%;display:grid;place-items:center;color:#64748b">LIVE SWAPP</div></div>`}<div class="liveBody"><div class="row"><span class="pill">${viewers} viewers</span><span class="pill">${esc(x.game_name||x.category||'Live')}</span></div><div class="title">${esc(name)}</div></div></article>`}
...
async function findLive(){
 ...
 $('#discoverResults').innerHTML='<div class="empty">Recherche…</div>'; $('#zapResult').innerHTML='<div class="empty">Swapp cherche une proposition lisible…</div>';
...
function openPeerTube(embed,watch,name='PeerTube'){setView('discover').then(()=>{const target=normalizePeerTubeEmbed(embed,watch); const link=watch||target||''; const box=$('#twitchPlayerArea'); if(!box)return; box.innerHTML=`<div class="pageHead"><div><h2>${esc(name)}</h2><p class="muted">Live public affiché dans Swapp.</p></div>${link?`<a class="btn secondary" href="${esc(link)}" target="_blank" rel="noopener">Ouvrir la source</a>`:''}</div><div class="watchShell twitchWatch"><div class="player premiumPlayer peertubePlayer"><iframe allowfullscreen sandbox="allow-same-origin allow-scripts allow-popups allow-forms" src="${esc(target)}"></iframe></div><aside class="chatPanel nativeFixedChat"><div class="chatHeader">Tchat Swapp</div><div class="chatLog"><div class="empty">PeerTube public est affiché ici. Le tchat Swapp reste séparé pour ne pas mélanger les plateformes.</div></div></aside></div>`; box.scrollIntoView({behavior:'smooth',block:'start'});});}
...
async function loadTeams(){const r=await api('/api/oryon/teams'); $('#teamsList').innerHTML=(r.items||[]).map(t=>`<article class="card"><div class="row"><img class="teamLogo" src="${esc(t.logo_url||'')}" alt=""><div><h2>${esc(t.name)}</h2><p class="muted">${esc(t.description||'Équipe Swapp')}</p></div></div><p><span class="pill">${t.members?.length||0} membres</span> <span class="pill">${t.points||0} points</span></p><div class="row"><button class="btn" onclick="joinTeam('${esc(t.slug)}')">Rejoindre</button><button class="btn secondary" onclick="viewTeam('${esc(t.slug)}')">Voir</button></div></article>`).join('')||'<div class="empty">Aucune équipe.</div>'}
async function createTeam(e){e.preventDefault(); await loadSession(); if(!state.session.local){toast('Compte Swapp requis');return setView('settings')} const r=await api('/api/oryon/teams',{method:'POST',body:JSON.stringify({name:$('#teamName').value,description:$('#teamDesc').value,tags:$('#teamTags').value,logo_url:$('#teamLogo')?.value||''})}); toast(r.success?'Équipe créée':r.error); if(r.success)loadTeams()}
async function joinTeam(slug){await loadSession(); if(!state.session.local){toast('Compte Swapp requis');return setView('settings')} const r=await api('/api/oryon/teams/'+slug+'/join',{method:'POST'}); toast(r.success?'Équipe rejointe':r.error); loadTeams()}
...
async function renderChannel(){
 ...
 const liveMode=p.oryon_local_player_url?'Swapp Live / OBS actif':(isLive?'Live navigateur actif':'Hors live');
...
async function renderManager(){
 ...
 $('#manager').innerHTML=creatorShell('manager',`<div class="pageHead"><div><h1>Gestionnaire de stream</h1><p>Deux méthodes utiles : live navigateur rapide ou OBS avec Swapp Live.</p></div><span class="pill">Compte : ${esc(u.login)}</span></div>
 <div class="streamModeGrid section"><div class="modeCard active"><span class="pill">Navigateur</span><h2>Écran / caméra</h2><p class="muted">Sans installation. Idéal pour tester rapidement.</p></div><div class="modeCard active"><span class="pill">OBS</span><h2>Swapp Live</h2><p class="muted">L’app sur ton PC reçoit OBS et publie le live sur ta chaîne Swapp.</p></div></div>
 <div class="managerHero section"><div class="panel"><h2>Lancer un live navigateur</h2><p class="muted">Capture écran/caméra en 1080p idéal. Le live apparaît sur l’accueil et sur ta chaîne.</p><div class="three"><input id="liveTitle" placeholder="Titre du live" value="Live de ${esc(u.display_name||u.login)}"><input id="liveCategory" placeholder="Catégorie"><input id="liveTags" placeholder="tags : RP, chill, FR"></div><div class="three section"><select id="liveQuality"><option value="1080" selected>1080p recommandé</option><option value="720">720p léger</option></select><select id="liveSource"><option value="screen" selected>Partage écran</option><option value="screen-camera">Écran + caméra incrustée</option><option value="camera">Caméra</option><option value="obs">OBS avec Swapp Live</option></select><button id="startLiveBtn" class="btn good" onclick="if($('#liveSource')?.value==='obs'){document.getElementById('oryonLivePanel')?.scrollIntoView({behavior:'smooth'});toast('Lance Swapp Live puis OBS.')}else startLive()">Lancer en direct</button><button id="stopLiveBtn" class="btn bad hidden" onclick="stopLive()">Arrêter le live</button></div><div class="row section"><span id="streamStateBadge" class="pill">Hors ligne</span><button class="btn ghost" onclick="state.watchRoom=null;setView('channel')">Voir ma chaîne</button><button class="btn ghost" onclick="setView('settings')">Image hors live</button></div><div class="managerPreview section"><h2>Prévisualisation</h2><div class="player premiumPlayer"><video id="localVideo" autoplay muted playsinline></video><div id="managerPreviewEmpty" class="offlineOverlay"><div><h2>Hors ligne</h2><p class="muted">Choisis Navigateur pour prévisualiser ici. Choisis OBS avec Swapp Live pour utiliser l’app.</p></div></div></div></div></div>
 <aside class="obsBox" id="oryonLivePanel"><h2>OBS avec Swapp Live</h2><p class="muted">Télécharge l’app, connecte ton compte Swapp, puis clique Démarrer sur Swapp dans l’app.</p><div class="summaryItem downloadAppCard"><b>Application Swapp Live</b><p class="small">Ton PC reçoit OBS et publie automatiquement le player sur ta chaîne.</p><a class="btn good" href="${esc(local.download_url||'/downloads/oryon-local-app.zip')}" download>Télécharger Swapp Live</a></div><div class="summaryItem section"><b>Réglage OBS</b><div class="codeBox" id="obsLocalConfig">Serveur : ${esc(local.local_rtmp_url||'rtmp://127.0.0.1:1935/live')}<br>Clé : ${esc(cfg.stream_key||local.stream_key||'ta_clé_oryon')}</div><button class="btn secondary section" onclick="copyText('#obsLocalConfig')">Copier config OBS</button><p class="small">Dans OBS : Service personnalisé. Désactive Vidéo multipiste.</p></div></aside></div>`);
...
/* Swapp clean live polish */


/* Swapp Flow fusion — banner + lecteur + chat in one spotlight */
...
function ensureSpotlightItem(platform, login){
 ...
 const seed = platform==='twitch' ? {platform:'twitch', login, display_name:login, user_name:login, title:'Live Twitch', viewer_count:0, game_name:'Twitch'} : {platform:'oryon', host_login:login, host_name:login, title:'Live Swapp', viewers:0, category:'Swapp'};
...
function renderSpotlightPreview(x){
 if(!x)return `<div class="flowLive fused"><div class="zapThumb"></div><div class="flowOverlay fused"><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><h2>Choisis une ambiance.</h2><p class="muted">Puis Zapper pour recevoir une proposition claire.</p></div></div>`;
...
async function findLive(){
 ...
 const zap=$('#zapResult'); if(zap) zap.innerHTML='<div class="empty">Swapp cherche une proposition lisible…</div>';
...
/* Swapp Premium Immersion v3 — calm cinematic discovery */
...
function signalCardsHtml(cur){if(!cur)return '<div class="empty">Aucun signal.</div>'; const score=comfortScore(cur), id=liveIdentity(cur); const small=id.viewers<=20?'Très petite':id.viewers<=50?'Petite':'Moyenne'; const type=id.platform==='twitch'?'Twitch':'Swapp'; return `<div class="signalCards"><div class="signalMini"><span>Confort</span><b>${score}%</b></div><div class="signalMini"><span>Taille</span><b>${small}</b></div><div class="signalMini"><span>Source</span><b>${esc(type)}</b></div><div class="signalMini"><span>Entrée</span><b>Facile</b></div></div>`}
...
function renderSpotlightPreview(x){
 if(!x)return `<div class="flowLive fused premium"><div class="zapThumb"></div><div class="flowOverlay fused premium"><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><div class="premiumHeadline"><h2>Choisis une ambiance.</h2><p class="muted">Une scène, un signal, une entrée simple.</p></div><div class="premiumActions"><button class="btn good" onclick="findLive()">Zapper</button></div></div></div>`;
 const id=liveIdentity(x), reasons=discoverReasonFor(x), score=comfortScore(x);
...
/* Swapp Room Depth v4 — immersive room, soft quests, duo discovery, living emotes */
...
function sendChat(){
 if(!state.room)return toast('Aucun salon Swapp actif'); if(!state.socket)setupSocket(); if(!state.session.local)return toast('Connecte-toi à Swapp pour écrire dans le tchat natif.');
...
function renderRoomShell(cur){
 ...
 const media=cur?(playing?`<iframe allowfullscreen src="https://player.twitch.tv/?channel=${encodeURIComponent(id.login)}&parent=${encodeURIComponent(parent)}"></iframe>`:(id.img?`<img src="${esc(id.img)}" alt="">`:`<div class="spotlightEmpty">LIVE ${esc(id.platform||'Swapp').toUpperCase()}</div>`)):`<div class="spotlightEmpty">Choisis une ambiance</div>`;
...
/* Swapp UX Declutter v5 — fixes overlapping controls and removes the bazaar effect */
...
async function renderChannel(){
 ...
 const liveMode=p.oryon_local_player_url?'Swapp Live / OBS':(isLive?'Live navigateur':'Hors live');
 ...
 $('#channel').innerHTML=`<div class="channelPage publicChannel cleanChannel"><div class="pageHead"><div><h1>${isOwner?'Ma chaîne':esc(p.display_name||p.login)}</h1><p>${isOwner?'Page publique.':'Page publique du streamer.'}</p></div>${ownerActions}</div><div class="channelBanner">${p.banner_url?`<img src="${esc(p.banner_url)}" alt="">`:`<div class="bannerFallback"></div>`}</div><div class="channelMeta"><img class="avatar" src="${esc(p.avatar_url||'')}" alt=""><div><h1>${esc(p.display_name||p.login)}</h1><p class="muted">${esc(p.bio||'Chaîne Swapp')}</p><div class="row"><span id="channelLiveBadge" class="pill">${isLive?'🔴 En direct':'Hors ligne'}</span><span class="pill">${esc(p.login)}</span><span class="pill">${Number(p.followers_count||0)} followers</span>${support?.count?`<span class="pill">${support.count} premiers soutiens</span>`:''}</div></div>${isOwner?`<button id="channelLaunchBtn" class="btn good" onclick="setView('manager')">${isLive?'Gérer':'Préparer'}</button>`:''}</div><div class="channelProofStrip">${channelBadges.map(b=>`<div class="channelBadgeBig"><strong>${esc(b.icon)}</strong><b>${esc(b.label)}</b><span class="small">${esc(b.note)}</span></div>`).join('')}${supporters.length?supporters.map(s=>`<span class="supportChip">⭐ ${esc(s.display_name||s.login)}</span>`).join(''):`<span class="small">Aucun premier soutien.</span>`}</div><div class="watchShell channelWatch channelLiveFocus"><div class="watchMain"><div class="channelInfoBar"><div><h2>${isLive?'En direct':'Salon hors ligne'}</h2><span class="small">${esc(liveMode)}</span></div><div class="reasonChips"><span class="reasonChip">chat lisible</span><span class="reasonChip">nouveaux bienvenus</span><span class="reasonChip">réactions rapides</span></div></div><div class="player premiumPlayer oryonMainPlayer">${playerHtml}</div><div class="tabs"><button class="tabBtn active" onclick="chanTab(this,'about')">À propos</button><button class="tabBtn" onclick="chanTab(this,'planning')">Planning</button><button class="tabBtn" onclick="chanTab(this,'clips')">Clips</button></div><div id="channelTab" class="panel"></div></div><aside class="chatPanel nativeFixedChat channelChatCompact" data-chat="oryon"><div class="chatHeader"><span>Tchat Swapp · ${esc(p.display_name||p.login)}</span><button class="btn ghost" onclick="reportRoom()">Signaler</button></div><div id="nativeChatLog" class="chatLog"></div><div id="customEmoteShelf" class="emotePanel hidden"></div><div id="gifGrid" class="gifGrid hidden"></div><div class="chatAssist"><button onclick="chatQuick('question')">Question</button><button onclick="chatQuick('new')">Nouveau ici</button><button onclick="chatQuick('react')">Réagir</button></div><div class="chatForm"><input id="chatInput" placeholder="Écrire sur Swapp…"><button class="btn secondary" onclick="toggleEmotes()">Emotes</button><button class="btn" onclick="sendChat()">Envoyer</button></div></aside></div></div>`;
 if(isLive)setMiniLive({type:'oryon',login:targetLogin,title:'Swapp · '+(p.display_name||p.login)});
 chanTab(null,'about'); setupSocket(); state.room=targetLogin; state.socket.emit('native:chat:history',{room:state.room});
 if(isOwner && state.stream){ attachCurrentStream(); }
...
async function renderChannel(){
 ...
 $('#channel').innerHTML=`<div class="channelPage publicChannel"><div class="pageHead"><div><h1>${isOwner?'Ma chaîne':esc(p.display_name||p.login)}</h1><p>${isOwner?'Ta page publique persistante.':'Page publique du streamer.'}</p></div>${ownerActions}</div><div class="channelBanner">${p.banner_url?`<img src="${esc(p.banner_url)}" alt="">`:`<div class="bannerFallback"></div>`}</div><div class="channelMeta"><img class="avatar" src="${esc(p.avatar_url||'')}" alt=""><div><h1>${esc(p.display_name||p.login)}</h1><p class="muted">${esc(p.bio||'Chaîne Swapp')}</p><div class="row"><span id="channelLiveBadge" class="pill">${isLive?'🔴 En direct':'Hors ligne'}</span><span class="pill">${esc(p.login)}</span><span class="pill">${Number(p.followers_count||0)} followers</span>${support?.count?`<span class="pill">${support.count} premiers soutiens</span>`:''}</div>${intro}</div>${isOwner?`<button id="channelLaunchBtn" class="btn good" onclick="setView('manager')">${isLive?'Gérer le live':'Préparer / lancer'}</button>`:''}</div>${compactSupportBar(supporters,support?.count,channelBadges)}<div class="watchShell channelWatch clean section"><div class="watchMain"><div class="player premiumPlayer oryonMainPlayer">${(p.oryon_local_player_url) ? `<iframe allowfullscreen sandbox="allow-same-origin allow-scripts allow-popups allow-forms" src="${esc(p.oryon_local_player_url)}"></iframe>` : ((p.peertube_embed_url||p.peertube_watch_url) && !isLive ? `<iframe allowfullscreen sandbox="allow-same-origin allow-scripts allow-popups allow-forms" src="${esc(normalizePeerTubeEmbed(p.peertube_embed_url,p.peertube_watch_url))}"></iframe>` : `<video id="localVideo" autoplay muted playsinline class="${isOwner&&state.stream?'':'hidden'}"></video><video id="remoteVideo" autoplay playsinline class="hidden"></video><div id="offlinePanel" class="emptyStatePlayer" style="display:${(isOwner&&state.stream)?'none':'grid'}">${offlineImg?`<img class="offlinePoster" src="${esc(offlineImg)}" alt="">`:''}<div class="offlineOverlay"><div><h2>${isLive?'Connexion au live…':'Chaîne hors ligne'}</h2><p class="muted">${isLive?'Si la vidéo tarde, recharge la page ou relance le live.':'Image hors live ou bannière configurée.'}</p>${isOwner?`<button class="btn" onclick="setView('manager')">Ouvrir le gestionnaire</button>`:`<button class="btn" onclick="quickGem()">Trouver une autre pépite</button>`}</div></div></div>`)}</div><div class="tabs"><button class="tabBtn active" onclick="chanTab(this,'about')">À propos</button><button class="tabBtn" onclick="chanTab(this,'planning')">Planning</button><button class="tabBtn" onclick="chanTab(this,'clips')">Clips</button></div><div id="channelTab" class="panel"></div></div><aside class="channelSideClean"><div class="chatPanel nativeFixedChat compact" data-chat="oryon"><div class="chatHeader"><span>Tchat Swapp · ${esc(p.display_name||p.login)}</span><button class="btn ghost" onclick="reportRoom()">Signaler</button></div><div id="nativeChatLog" class="chatLog"></div><div id="customEmoteShelf" class="emotePanel hidden"></div><div id="gifGrid" class="gifGrid hidden"></div><div class="chatAssist"><button onclick="chatQuick('question')">Question</button><button onclick="chatQuick('new')">Nouveau ici</button><button onclick="chatQuick('react')">Réagir</button></div><div class="chatForm"><input id="chatInput" placeholder="Écrire sur Swapp…"><button class="btn secondary" onclick="toggleEmotes()">Emotes</button><button class="btn secondary" onclick="toggleGifs()">GIF</button><button class="btn" onclick="sendChat()">Envoyer</button></div></div><div class="channelSideCard"><h3>Badges</h3><div class="channelBadgeRail tight">${channelBadges.map(b=>`<div class="channelBadgeBig"><strong>${esc(b.icon)}</strong><b>${esc(b.label)}</b><span class="small">${esc(b.note)}</span></div>`).join('')}</div></div></aside></div></div>`;
 if(isLive)setMiniLive({type:'oryon',login:targetLogin,title:'Swapp · '+(p.display_name||p.login)});
 chanTab(null,'about'); setupSocket(); state.room=targetLogin; state.socket.emit('native:chat:history',{room:state.room});
 if(isOwner && state.stream){ attachCurrentStream(); }
 else if(isLive){ state.socket.emit('native:join',{room:targetLogin}); if(!p.oryon_local_player_url){ setTimeout(()=>requestOffer(),500); setTimeout(()=>{ if($('#remoteVideo') && !$('#remoteVideo').srcObject) toast('Connexion vidéo en attente. Clique sur Relancer si besoin.'); },3500); } }
...
function renderSpotlightPreview(x){
  if(!x) return `<article class="proCard"><div class="proMedia"><div class="proEmptyMedia">SWAPP FLOW</div></div><div class="proOverlay"><h2>Trouve ton live.</h2><p class="muted">Choisis une ambiance, puis lance une proposition.</p><div class="proActions"><button class="btn good" onclick="findLive()">Zapper</button></div></div></article>`;
...
async function renderDiscover(){
  ...
  el.innerHTML=`<div class="proDiscover">
    <section class="proHero"><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><h1>Un live, pas un bazar.</h1><p>Choisis une ambiance. Swapp propose. Tu regardes, tu sauves ou tu passes au suivant.</p></section>
...
function renderSpotlightPreview(x){
  if(!x) return `<article class="proStartCard"><div><span class="eyebrow"><i class="dot"></i>Prêt à découvrir</span><h2>Choisis une ambiance, ou lance direct.</h2><p>Sur mobile comme sur PC, Swapp affiche d’abord une vraie scène de découverte. Pas de page vide, pas de bouton mort.</p><div class="proActions"><button class="btn good" onclick="findLive()">Zapper</button><button class="btn secondary" onclick="setDiscoverMood('discussion')">Discussion</button><button class="btn secondary" onclick="setDiscoverMood('chill')">Chill</button></div></div></article>`;
  const id=liveIdentity(x), reasons=discoverReasonFor(x).slice(0,3), score=comfortScore(x);
...
async function renderDiscover(){
  ...
  el.innerHTML=`<div class="proDiscover">
    ...
    <section class="proSearchPanel"><div class="proMoodRow">${AMBIANCES.map(([id,label,desc,icon])=>`<button class="proMoodBtn" data-mood="${esc(id)}" onclick="setDiscoverMood('${esc(id)}')"><i>${icon}</i><b>${esc(label)}</b><span>${esc(desc)}</span></button>`).join('')}</div>
      <div class="proSearchLine"><input id="dQuery" placeholder="jeu, pseudo, ambiance" onkeydown="if(event.key==='Enter')findLive()"><select id="dSource"><option value="both" selected>Swapp + Twitch</option><option value="oryon">Swapp</option><option value="twitch">Twitch</option></select><select id="dMood" onchange="setDiscoverMood(this.value)"><option value="">Ambiance</option>${AMBIANCES.map(([id,label])=>`<option value="${esc(id)}">${esc(label)}</option>`).join('')}</select><select id="dMax"><option value="20">≤20</option><option value="50" selected>≤50</option><option value="200">≤200</option><option value="300">≤300</option></select><select id="dLang"><option value="fr">FR</option><option value="en">EN</option></select><button class="btn good" onclick="findLive()">Zapper</button><button class="btn secondary" onclick="zapNext()">Suivant</button></div></section>
    <section id="zapResult"></section>
...
function setDiscoverMood(id){
  ...
  const zap=$('#zapResult');
  if(zap) zap.innerHTML=`<div class="moodFirstStart"><div><span class="moodFirstSelected">${esc(moodFirstLabel(normalized))}</span><h2>Je cherche le bon live…</h2><p>Swapp filtre pour toi. Ensuite tu swipes.</p></div></div>`;
  closeMini?.();
  state.discoverPlayer=null;
  requestAnimationFrame(()=>document.getElementById('zapResult')?.scrollIntoView({block:'start',behavior:'smooth'}));
  return findLive();
...
function renderSpotlightPreview(x){
  if(!x){
    return `<article class="moodFirstStart"><div><span class="eyebrow"><i class="dot"></i>Découvrir</span><h2>Choisis ton mood. Swapp choisit le live.</h2><p>Pas de formulaire obligatoire. Un tap, une proposition, puis swipe droite ou gauche.</p><div class="moodFirstMiniGrid">${AMBIANCES.slice(0,4).map(([id,label])=>`<button onclick="setDiscoverMood('${esc(id)}')">${esc(label)}</button>`).join('')}</div></div></article>`;
  }
  const id=liveIdentity(x), reasons=discoverReasonFor(x).slice(0,3), score=comfortScore(x);
  return `<article class="proCard" data-swipe-card="1"><div class="swipeStamp like">J'aime</div><div class="swipeStamp nope">Pas ouf</div><div class="proMedia">${id.img?`<img src="${esc(id.img)}" alt="" loading="eager">`:`<div class="proEmptyMedia">LIVE ${esc(platformLabel(id.platform)).toUpperCase()}</div>`}</div>
    <div class="proBadgeTop"><span class="proPill">${esc(platformLabel(id.platform))} · ${id.viewers} viewers</span><span class="proPill">${score}% confort</span></div>
...
async function renderDiscover(){
  ...
  el.innerHTML=`<div class="moodFirst">
    <section class="moodFirstHero">
      <div><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><h1>Choisis ton mood.</h1><p>Swapp te propose un live en deux secondes. Ensuite tu swipes : droite si ça te parle, gauche si ce n’est pas ta vibe.</p></div>
    ...
    <section class="moodFirstPanel">
      ...
      <details class="moodFirstAdvanced">
        ...
        <div class="proSearchLine"><input id="dQuery" placeholder="jeu, pseudo, ambiance" onkeydown="if(event.key==='Enter')findLive()"><select id="dSource"><option value="both" selected>Swapp + Twitch</option><option value="oryon">Swapp</option><option value="twitch">Twitch</option></select><select id="dMood"><option value="${esc(current)}">${esc(moodFirstLabel(current))}</option>${AMBIANCES.map(([id,label])=>`<option value="${esc(id)}">${esc(label)}</option>`).join('')}</select><select id="dMax"><option value="20">≤20</option><option value="50" selected>≤50</option><option value="200">≤200</option><option value="300">≤300</option></select><select id="dLang"><option value="fr">FR</option><option value="en">EN</option></select><button class="btn secondary" onclick="findLive()">Relancer</button></div>
      </details>
    </section>
    <section id="zapResult"></section>
...
function liveStableTags(x){
  const id=liveIdentity(x||{});
  const tags=[];
  tags.push(id.platform==='twitch'?'Twitch':'Swapp');
...
function signalTagsFor(cur){
  ...
  const tags=[];
  tags.push(id.platform==='twitch'?'Twitch intégré':'Swapp natif');
...
function renderSpotlightPreview(x){
  if(!x){
    return `<article class="moodFirstStart"><div><span class="eyebrow"><i class="dot"></i>Découvrir</span><h2>Choisis ton mood. Swapp choisit le live.</h2><p>Propose-moi un live lance automatiquement une recommandation. Choisir une ambiance te laisse décider la vibe.</p><div class="moodFirstMiniGrid">${AMBIANCES.slice(0,4).map(([id,label])=>`<button onclick="setDiscoverMood('${esc(id)}')">${esc(label)}</button>`).join('')}</div></div></article>`;
  }
...
async function pickCat(name,img=''){
  ...
  target.innerHTML=`<section class="categoryPickHero">${safeImg?`<img class="catBg" src="${esc(safeImg)}" alt="">`:''}<div class="categoryPickContent"><span class="eyebrow"><i class="dot"></i>Jeu sélectionné</span><h2>${esc(safeName)}</h2><p>Swapp cherche un petit live dans cette catégorie, pas un flux saturé.</p><div class="homeTagCloud"><span class="homeTag">petit live</span><span class="homeTag">chat lisible</span><span class="homeTag">${esc(safeName)}</span></div><div class="categoryPickLive"><div class="proNotice">Recherche d’une pépite…</div></div></div></section>`;
  try{
...
(function injectHomeOryonVitrinePass(){
  ...
  .homeSlideCard.oryonPromo:before{content:"SWAPP LIVE";position:absolute;right:22px;top:18px;z-index:2;border:1px solid rgba(255,255,255,.18);border-radius:999px;padding:8px 11px;background:rgba(2,6,23,.48);font-weight:1000;font-size:12px;letter-spacing:.04em}
...
function streamTargetLabel(){ return state.session.local ? 'Streamer sur Swapp' : 'Créer ma chaîne Swapp'; }
function homeStreamPromoCard(size='large'){
  return `<article class="homeSlideCard oryonPromo" onclick="setView('${streamTargetView()}')"><div class="homeSlideBody"><div class="homeTagCloud"><span class="homeTag">Swapp Live</span><span class="homeTag">OBS ou navigateur</span><span class="homeTag">Twitch friendly</span></div><h2>À toi de live.</h2><p>Lance ton stream sur Swapp, garde Twitch à côté, et deviens découvrable sans être noyé.</p><button class="btn streamBtn" onclick="event.stopPropagation();setView('${streamTargetView()}')">${esc(streamTargetLabel())}</button></div></article>`;
}
function oryonMiniLiveCard(x){
  const id=liveIdentity(x||{});
  const img=id.img || x?.offline_image_url || x?.banner_url || '';
  return `<button class="oryonMiniLive" onclick="openOryon('${esc(id.login||x?.room||x?.host_login||'')}')">${img?`<img src="${esc(img)}" alt="" loading="lazy">`:''}<div class="oryonMiniBody"><b>${esc(id.name||'Live Swapp')}</b><span>${esc(id.title||id.game||'En direct sur Swapp')}</span></div></button>`;
}
function oryonCreateMiniCard(){
  return `<button class="oryonMiniLive create" onclick="setView('${streamTargetView()}')"><div><b>À toi de live</b><span>Crée ton live Swapp en navigateur ou avec OBS.</span></div></button>`;
}
function homeRecommendationCard(x,i){
  const id=liveIdentity(x||{}), tags=liveStableTags(x), action=liveOpenAction(x);
...
async function findLive(){
  ...
  const results=$('#discoverResults'); if(results) results.innerHTML='';
  const zap=$('#zapResult');
  if(zap) zap.innerHTML=`<div class="moodFirstStart"><div><span class="moodFirstSelected">${esc(moodFirstLabel(mood))}</span><h2>Recherche en cours…</h2><p>Swapp élargit si la catégorie est trop vide.</p></div></div>`;
...
async function findLive(){
  ...
  const lang=$('#dLang')?.value || 'fr';
  const zap=$('#zapResult');
  if(zap) zap.innerHTML=`<div class="moodFirstStart"><div><span class="moodFirstSelected">${esc(moodFirstLabel(mood))}</span><h2>Recherche en cours…</h2><p>Swapp trie selon ton mood, puis retire ce que tu as déjà swipé.</p></div></div>`;
  state.currentTwitch=null; state.discoverPlayer=null; closeMini?.();
...
function renderSpotlightPreview(x){
  if(!x){return `<article class="moodFirstStart"><div><span class="eyebrow"><i class="dot"></i>Découvrir</span><h2>Choisis ton mood. Swapp choisit le live.</h2><p>Un tap lance la recherche. Ensuite : droite j’aime, gauche pas ouf.</p><div class="moodFirstMiniGrid">${AMBIANCES.slice(0,4).map(([id,label])=>`<button onclick="setDiscoverMood('${esc(id)}')">${esc(label)}</button>`).join('')}</div></div></article>`;}
...
async function loadHomeRecommendations(){
  ...
  try{
    ...
    box.innerHTML=items.length?items.map(homeRecommendationCard).join(''):`<article class="homeSlideCard finalLive" onclick="autoProposeLive()"><div class="homeSlideBody"><div class="homeTagCloud"><span class="homeTag">Recherche directe</span><span class="homeTag">Live vitrine</span></div><h2>Pas de live vitrine maintenant.</h2><p>Swapp peut quand même te proposer un live.</p></div></article>`;
  }catch(e){box.innerHTML='<div class="followEmpty">Impossible de charger la vitrine.</div>';}
}
async function renderHome(){
  const el=$('#home'); if(!el) return;
  el.innerHTML=`<section class="homeShowcase section"><div class="homeShowcaseInner finalReco"><div class="homeShowcaseCopy final"><div><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><p>Un live vitrine entre 30 et 100 viewers, puis une découverte plus fine selon ton mood.</p><div class="homeMoodStrip">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive()">${esc(label)}</button>`).join('')}</div></div><div class="homeShowcaseActions finalReco"><button class="btn" onclick="autoProposeLive()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Choisir mon ambiance</button></div></div><div id="homeShowcaseLives" class="homeLiveCarousel finalReco"><div class="followEmpty">Chargement…</div></div></div></section><div class="homeTwoGrid section"><section>${viewerProfileCard()}</section><section class="homeFollowPanel"><div class="proTwitchHead"><div><h2 style="margin:0">Tes suivis Twitch en ligne</h2><p class="small" style="margin:6px 0 0">Uniquement les chaînes live, avec miniature et logo.</p></div><div>${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div id="followedWrapCompact"></div></section></div>`;
...
async function loadHomeRecommendations(){
  ...
  try{
    ...
    box.innerHTML=items.length?items.map(homeRecommendationCard).join(''):`<article class="homeSlideCard full homeFallbackLive" onclick="autoProposeLive()"><div class="homeSlideBody"><div class="homeTagCloud"><span class="homeTag">Recherche directe</span><span class="homeTag">Swapp Flow</span></div><h2>Lance la vitrine.</h2><p>Swapp va chercher un live maintenant, même hors créneau vitrine.</p></div></article>`;
  }catch(e){box.innerHTML='<div class="followEmpty">Impossible de charger la vitrine.</div>';}
}
async function renderHome(){
  const el=$('#home'); if(!el) return;
  el.innerHTML=`<section class="homeShowcase fullBleed"><div class="homeFullInner"><div class="homeFullCopy"><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><p>Un grand live vitrine entre 30 et 100 viewers, puis une découverte plus fine selon ton mood.</p><div class="homeMoodStrip full">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive()">${esc(label)}</button>`).join('')}</div><div class="homeFullActions"><button class="btn" onclick="autoProposeLive()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Choisir mon ambiance</button></div></div><div id="homeShowcaseLives" class="homeVitrineStage"><div class="followEmpty">Chargement…</div></div></div></section><div class="homePlatformGrid"><section>${viewerProfileCard()}</section><section class="homeFollowPanel"><div class="proTwitchHead"><div><h2 style="margin:0">Tes suivis Twitch en ligne</h2><p class="small" style="margin:6px 0 0">Uniquement les chaînes live, avec miniature et logo.</p></div><div>${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div id="followedWrapCompact"></div></section></div>`;
  await loadHomeRecommendations(); await renderCompactFollowed(); closeMini?.();
}
...
/* =========================================================
   SWAPP HARD FIX — full-width platform + working live pools
...
function hfTags(x){ const id=hfLiveId(x); const base=[]; base.push(id.platform==='oryon'?'Swapp Live':'Twitch'); if(id.viewers) base.push(`${id.viewers} viewers`); if(id.game) base.push(id.game); const t=signalTagsFor?.(x)||[]; return [...new Set([...t,...base].filter(Boolean))].slice(0,5); }
...
async function loadHomeRecommendations(){
  const box=$('#homeShowcaseLives'); if(!box) return;
  box.innerHTML=`<div class="hfEmptyLive"><div><h2>Chargement de la vitrine…</h2><p>Swapp cherche un live entre 30 et 100 viewers.</p></div></div>`;
...
async function renderHome(){
  const el=$('#home'); if(!el) return;
  el.innerHTML=`<div class="hfHome"><section class="hfHero"><div class="hfHeroCopy"><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><p>Un live vitrine entre 30 et 100 viewers. Si le créneau est vide, Swapp élargit sans laisser l’accueil mort.</p><div class="hfMoodStrip">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive()">${esc(label)}</button>`).join('')}</div><div class="hfActions"><button class="btn" onclick="autoProposeLive()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Choisir mon ambiance</button></div></div><div id="homeShowcaseLives" class="hfLiveStage">${hfEmptyLiveHtml()}</div></section><div class="hfBelow"><section class="hfPanel">${viewerProfileCard?.()||'<h2>Profil Viewer</h2>'}</section><section class="hfPanel"><div class="proTwitchHead"><div><h2>Tes suivis Twitch en ligne</h2><p>Uniquement les chaînes live, en bandeau avec miniature.</p></div><div>${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div id="followedWrapCompact" class="hfRail"></div></section></div></div>`;
  await loadHomeRecommendations();
  await renderCompactFollowed?.();
  closeMini?.();
}
...
/* =====================================================================
   Swapp emergency platform pass — real desktop full-width, no centered app
   ===================================================================== */
...
async function loadHomeRecommendations(){
  const box=$('#homeShowcaseLives'); if(!box) return;
  box.innerHTML=`<div class="hfEmptyLive"><div><h2>Recherche d’un live vitrine…</h2><p>Swapp privilégie un live entre 30 et 100 viewers.</p></div></div>`;
  try{
    ...
    box.innerHTML=items.length
      ...
      : `<div class="hfEmptyLive"><div><h2>Aucun live récupéré.</h2><p>Ajoute TWITCH_CLIENT_ID et TWITCH_CLIENT_SECRET sur Render, ou lance un live Swapp natif.</p><button class="btn" onclick="autoProposeLive()">Relancer</button></div></div>`;
  }catch(e){
    console.error(e);
...
async function renderHome(){
  const el=$('#home'); if(!el) return;
  el.innerHTML=`<div class="owHomeFull"><section class="owHeroTheater"><div class="owHeroCopy"><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><p>Un grand live vitrine entre 30 et 100 viewers. Swapp élargit si besoin, sans laisser l’accueil vide.</p><div class="owMoodStrip">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive()">${esc(label)}</button>`).join('')}</div><div class="owActions"><button class="btn" onclick="autoProposeLive()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Choisir mon ambiance</button></div></div><div id="homeShowcaseLives" class="owLiveTheater"></div></section><div class="owBelow"><section class="owPanel">${viewerProfileCard?.()||'<h2>Profil Viewer</h2>'}</section><section class="owPanel"><div class="proTwitchHead"><div><h2>Tes suivis Twitch en ligne</h2><p>Uniquement les chaînes live, en bandeau avec miniature.</p></div><div>${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div id="followedWrapCompact" class="hfRail"></div></section></div></div>`;
  await loadHomeRecommendations();
  await renderCompactFollowed?.();
  closeMini?.();
}
...
/* =====================================================================
   Swapp user-request final pass — home 3-up, viewer memory, viewer/streamer mode,
...
async function renderSettings(){
  ...
  <div class="two section"><div class="panel">${mode==='viewer'?viewerProfileCard():(u?profileSettings(u):authSettings())}</div><div class="panel">${mode==='streamer' ? (u ? `<h2>Menu stream</h2><div class="summaryList"><div class="summaryItem"><b>Ma chaîne</b><p class="small">Page publique, lecteur et chat.</p><button class="btn" onclick="setView('channel')">Ouvrir ma chaîne</button></div><div class="summaryItem"><b>Gestionnaire de stream</b><p class="small">Préparer, lancer et configurer ton live Swapp.</p><button class="btn" onclick="setView('manager')">Ouvrir le gestionnaire</button></div><div class="summaryItem"><b>Outils créateur</b><p class="small">Studio, overlays et outils de chaîne.</p><button class="btn" onclick="setView('studio')">Ouvrir les outils</button></div></div>` : authSettings()) : `<h2>Connexions</h2><div class="summaryList"><div class="summaryItem"><b>Twitch</b><p class="small">${state.session.twitch?'Connecté : '+esc(state.session.twitch.display_name||state.session.twitch.login):'Non connecté. Connecte Twitch pour retrouver tes suivis et améliorer les recommandations.'}</p><div class="row">${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div class="summaryItem"><b>Mémoire viewer</b><p class="small">Conserve tes swipes, pépites vues et chaînes aimées même après navigation ou reconnexion.</p><button class="btn secondary" onclick="toast('La mémoire viewer est locale et persistante dans ce navigateur.')">Compris</button></div></div>`}</div></div>`;
  bindSettingsForms?.();
}
...
async function loadHomeRecommendations(){
  ...
  try{
    ...
    if(!items.length) items=await hfFetchLivePool({mood:'petite-commu',min:15,max:180,lang:'fr'});
    if(!items.length) items=await hfFetchLivePool({mood:'discussion',min:1,max:350,lang:'fr'});
    box.innerHTML=items.length ? items.slice(0,3).map((x,i)=>owLiveCardHtml(x,i)).join('') : `<div class="hfEmptyLive"><div><h2>Aucun live récupéré.</h2><p>Vérifie les variables Twitch sur Render, ou lance un live Swapp natif.</p><button class="btn" onclick="autoProposeLive()">Relancer</button></div></div>`;
  }catch(e){
    console.error(e);
...
async function renderHome(){
  const el=$('#home'); if(!el) return;
  el.innerHTML=`<div class="owHomeFull"><section class="owHeroTheater userFixHero"><div class="owHeroCopy userFixCopy"><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><div class="owMoodStrip">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive()">${esc(label)}</button>`).join('')}</div><div class="owActions"><button class="btn" onclick="autoProposeLive()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Choisir mon ambiance</button></div></div><div id="homeShowcaseLives" class="owLiveTheater userFix3"></div></section><div class="owBelow"><section class="owPanel">${viewerProfileCard?.()||'<h2>Profil Viewer</h2>'}</section><section class="owPanel"><div class="proTwitchHead"><div><h2>Tes suivis Twitch en ligne</h2><p>Fil de chaînes suivies en miniature.</p></div><div>${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div id="followedWrapCompact" class="hfFollowMiniRail"></div></section></div></div>`;
  await loadHomeRecommendations();
  await renderCompactFollowed();
  closeMini?.();
}
...
/* =========================================================
   Swapp polish pass — settings/profile/channel layout fixes
   ========================================================= */
...
function authSettings(){
  return `<div class="authCenterWrap"><div class="authCenterGrid"><form id="loginForm" class="authCard"><h2>Connexion Swapp</h2><p>Connecte-toi pour retrouver ta chaîne, ton profil viewer et ton menu stream.</p><input id="loginName" placeholder="pseudo"><input id="loginPass" type="password" placeholder="mot de passe"><button class="btn">Se connecter</button></form><form id="registerForm" class="authCard"><h2>Créer un compte</h2><p>Crée ton compte Swapp dans un espace propre et centré.</p><input id="regName" placeholder="pseudo"><input id="regEmail" type="email" placeholder="email"><input id="regPass" type="password" placeholder="mot de passe"><button class="btn">Créer</button></form></div></div>`;
}

function viewerProfilePanel(){
  return `<section class="panel viewerProfileCompact"><h2>Profil viewer</h2><p class="small">Visible aussi depuis ton profil Swapp quand tu es connecté.</p>${viewerProfileCard()}</section>`;
}
...
async function renderSettings(){
  ...
  const mode=getAppMode();
  if(!u){
    root.innerHTML=`<div class="settingsShell"><div class="settingsHero"><div><h1>${mode==='streamer'?'Page streamer':'Page viewer'}</h1><p>${mode==='streamer'?'Crée ou connecte ton compte pour débloquer le menu stream et ta chaîne Swapp.':'Crée ou connecte ton compte pour conserver ton profil viewer, tes préférences et ta mémoire.'}</p></div><div class="modeSwitch"><button class="btn ${mode==='viewer'?'':'secondary'}" onclick="setAppMode('viewer')">Page viewer</button><button class="btn ${mode==='streamer'?'':'secondary'}" onclick="setAppMode('streamer')">Page streamer</button></div></div>${authSettings()}<section class="panel"><h2>Connexions externes</h2><div class="summaryList"><div class="summaryItem"><b>Twitch</b><p class="small">${state.session.twitch?'Connecté : '+esc(state.session.twitch.display_name||state.session.twitch.login):'Non connecté. Connecte Twitch pour remonter tes suivis et nourrir les recommandations.'}</p><div class="row">${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div></div></section></div>`;
    bindSettingsForms?.();
    return;
  }
  root.innerHTML=`<div class="settingsShell"><div class="settingsHero"><div><h1>${mode==='streamer'?'Page streamer':'Page viewer'}</h1><p>${mode==='streamer'?'Ton espace créateur, ton menu stream et aussi ton profil viewer dans la même page de profil.':'Ton profil viewer complet, ta mémoire et tes connexions Swapp / Twitch.'}</p></div><div class="modeSwitch"><button class="btn ${mode==='viewer'?'':'secondary'}" onclick="setAppMode('viewer')">Page viewer</button><button class="btn ${mode==='streamer'?'':'secondary'}" onclick="setAppMode('streamer')">Page streamer</button></div></div>${mode==='viewer'?`<div class="settingsStack">${viewerProfilePanel()}<section class="panel"><h2>Connexions</h2><div class="summaryList"><div class="summaryItem"><b>Twitch</b><p class="small">${state.session.twitch?'Connecté : '+esc(state.session.twitch.display_name||state.session.twitch.login):'Non connecté. Connecte Twitch pour retrouver tes suivis et améliorer les recommandations.'}</p><div class="row">${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div class="summaryItem"><b>Mémoire viewer</b><p class="small">Conserve tes swipes, pépites vues, chaînes aimées et sauvegardes dans ce navigateur.</p><button class="btn secondary" onclick="toast('La mémoire viewer est bien conservée pour ton compte Swapp dans ce navigateur.')">Compris</button></div><div class="summaryItem"><b>Compte Swapp</b><p class="small">Connecté en tant que ${esc(u.display_name||u.login)}.</p><button class="btn secondary" onclick="logoutOryon()">Déconnecter Swapp</button></div></div></section></div>`:`<div class="settingsGrid"><div class="settingsStack"><section class="panel">${profileSettings(u)}</section>${viewerProfilePanel()}</div><div class="settingsStack"><section class="panel"><h2>Menu stream</h2><div class="summaryList"><div class="summaryItem"><b>Ma chaîne</b><p class="small">Page publique, lecteur géant et tchat adaptatif.</p><button class="btn" onclick="setView('channel')">Ouvrir ma chaîne</button></div><div class="summaryItem"><b>Gestionnaire</b><p class="small">Préparer, lancer et configurer ton live Swapp.</p><button class="btn" onclick="setView('manager')">Ouvrir le gestionnaire</button></div><div class="summaryItem"><b>Outils créateur</b><p class="small">Studio, overlays et outils de chaîne.</p><button class="btn" onclick="setView('studio')">Ouvrir les outils</button></div><div class="summaryItem"><b>Twitch</b><p class="small">${state.session.twitch?'Connecté : '+esc(state.session.twitch.display_name||state.session.twitch.login):'Non connecté. Connecte Twitch pour relier tes suivis et ton profil viewer.'}</p><div class="row">${state.session.twitch?`<button class="btn secondary" onclick="logoutTwitch()">Déconnecter Twitch</button>`:`<button class="btn" onclick="connectTwitch()">Connecter Twitch</button>`}</div></div><div class="summaryItem"><b>Compte</b><p class="small">Déconnecte-toi si besoin.</p><button class="btn secondary" onclick="logoutOryon()">Déconnecter Swapp</button></div></div></section><section id="planningSettings" class="panel"></section></div></div>`}</div>`;
  bindSettingsForms?.();
  loadFoundation?.();
...
/* =========================================================
   Swapp final requested fixes — discovery home, global theme, channel chat fit
   ========================================================= */
...
async function loadHomeRecommendations(){
  ...
  try{
    ...
    box.innerHTML=items.length ? items.slice(0,3).map((x,i)=>owLiveCardHtml(x,i)).join('') : `<div class="hfEmptyLive"><div><h2>Aucun live public récupéré.</h2><p>Connecte les variables Twitch serveur ou lance un live Swapp natif. La home ne pioche pas dans tes suivis.</p><button class="btn" onclick="autoProposeLive()">Proposer un live</button></div></div>`;
...
async function renderHome(){
  const el=$('#home'); if(!el) return;
  el.innerHTML=`<div class="owHomeFull"><section class="owHeroTheater userFixHero"><div class="owHeroCopy userFixCopy"><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><div class="owMoodStrip">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive()">${esc(label)}</button>`).join('')}</div><div class="owActions"><button class="btn" onclick="autoProposeLive()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Choisir mon ambiance</button></div></div><div id="homeShowcaseLives" class="owLiveTheater userFix3"></div></section><div class="owBelow"><section class="owPanel">${viewerProfileCard?.()||'<h2>Profil Viewer</h2>'}</section><section class="owPanel homeDiscoveryPanel"><h2>Découverte, pas suivis.</h2><p>L’accueil sert à découvrir de nouveaux lives Swapp ou Twitch. Tes suivis Twitch restent dans Accès Twitch, pas dans la vitrine.</p><div class="homeDiscoveryGrid"><div class="homeDiscoveryMini"><b>Mood</b><span>Choisis une ambiance et Swapp cherche pour toi.</span></div><div class="homeDiscoveryMini"><b>Swipe</b><span>J’aime / pas ouf remplit ton profil viewer.</span></div><div class="homeDiscoveryMini"><b>Streamer</b><span>Le bouton streamer garde Swapp visible comme plateforme native.</span></div></div></section></div></div>`;
  await loadHomeRecommendations();
  applyViewerThemeColor?.();
  closeMini?.();
}
...
async function loadHomeRecommendations(){
  const box=$('#homeShowcaseLives'); if(!box) return;
  box.innerHTML=`<div class="hfEmptyLive"><div><h2>Recherche de lives publics…</h2><p>Swapp cherche des lives à découvrir, pas tes suivis.</p></div></div>`;
  try{
    ...
    box.innerHTML=items.length ? items.slice(0,3).map((x,i)=>owLiveCardHtml(x,i)).join('') : `<div class="hfEmptyLive"><div><h2>Aucun live public récupéré.</h2><p>La vitrine ne peut afficher Twitch que si TWITCH_CLIENT_ID et TWITCH_CLIENT_SECRET sont actifs côté Render, ou si un live Swapp natif est lancé.</p><button class="btn" onclick="loadHomeRecommendations()">Réessayer</button></div></div>`;
  }catch(e){ console.error(e); box.innerHTML=`<div class="hfEmptyLive"><div><h2>Recherche indisponible.</h2><button class="btn" onclick="loadHomeRecommendations()">Réessayer</button></div></div>`; }
}

async function renderHome(){
  const el=$('#home'); if(!el) return;
  el.innerHTML=`<div class="owHomeFull homeClean"><section class="owHeroTheater homeCleanHero"><div class="owHeroCopy homeCleanCopy"><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><div class="owMoodStrip">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive()">${esc(label)}</button>`).join('')}</div><div class="owActions"><button class="btn" onclick="autoProposeLive()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Choisir mon ambiance</button></div></div><div id="homeShowcaseLives" class="owLiveTheater homeCleanLives"></div></section><div class="homeOnlyMain"><section class="homeStreamerStrip"><div><h2>Streamer sur Swapp</h2><p>La plateforme met aussi en avant les créateurs Swapp natifs. Lance ton live depuis le gestionnaire.</p></div><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button></section></div></div>`;
  await loadHomeRecommendations();
  applyViewerThemeColor?.();
  closeMini?.();
}
...
async function saveChannelVignettes(images){
  ...
  if(!login){toast('Compte Swapp requis'); return}
...
renderChannel = async function(){
  ...
  applyViewerThemeColor?.(); if(isLive)setMiniLive?.({type:'oryon',login:targetLogin,title:'Swapp · '+(p.display_name||p.login)});
  setupSocket?.(); state.room=targetLogin; state.socket?.emit('native:chat:history',{room:state.room});
  if(isOwner && state.stream){attachCurrentStream?.();}
  else if(isLive){state.socket?.emit('native:join',{room:targetLogin}); if(!p.oryon_local_player_url){setTimeout(()=>requestOffer?.(),500);}}
  updateLiveUi?.(isLive); refreshEmoteShelf?.(targetLogin);
...
createTeam = async function(e){
  e.preventDefault(); await loadSession(); if(!state.session.local){toast('Compte Swapp requis');return setView('settings')}
...
joinTeam = async function(slug){
  await loadSession(); if(!state.session.local){toast('Compte Swapp requis');return setView('settings')}
...
/* =========================================================
   SWAPP — Pulse Live + mobile player/chat polish + creator About links
   ========================================================= */
(function(){
...
/* =========================================================
   SWAPP EXPERIENCE PASS — AfterLive, DeckLurker, mobile live/chat sizing, soft entry
   ========================================================= */
...
/* =========================================================
   SWAPP MOBILE SANITY PASS — sane live/chat proportions + Pulse placement
   ========================================================= */
...
(function finalOryonGlobalAudit(){
  ...
  *,*::before,*::after{box-sizing:border-box;}
  ...
  window.renderHome = async function renderHomeFinal(){
    const el=$('#home'); if(!el) return;
    el.innerHTML=`<div class="owHomeFull homeClean"><section class="owHeroTheater homeCleanHero"><div class="owHeroCopy homeCleanCopy"><span class="eyebrow"><i class="dot"></i>Vitrine Swapp</span><h1>Des lives à taille humaine.</h1><p>Accueil = vitrine. Pas de salon de lurk ici. Découvre des lives, ou lance ton propre live Swapp.</p><div class="owMoodStrip">${AMBIANCES.slice(0,6).map(([id,label])=>`<button onclick="state.moodFirstMood='${esc(id)}';autoProposeLive?.()">${esc(label)}</button>`).join('')}</div><div class="owActions"><button class="btn" onclick="autoProposeLive?.()">Propose-moi un live</button><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button><button class="btn secondary" onclick="setView('discover')">Swap ton mood</button></div></div><div id="homeShowcaseLives" class="owLiveTheater homeCleanLives"></div></section><div class="homeOnlyMain"><section class="homeStreamerStrip"><div><h2>Créer sur Swapp</h2><p>Page chaîne, chat, vignettes, emotes, planning et résumé AfterLive sont dans les outils créateur.</p></div><button class="btn streamBtn" onclick="setView('${streamTargetView?.()||'manager'}')">${esc(streamTargetLabel?.()||'Streamer sur Swapp')}</button></section></div></div>`;
    await loadHomeRecommendations?.();
    applyViewerThemeColor?.();
    closeMini?.();
    finalClean();
  ...
  window.renderDiscover = async function renderDiscoverFinal(){
    const el=$('#discover'); if(!el) return;
    const current=state.moodFirstMood||'petite-commu';
    el.innerHTML=`<div class="hfDiscover"><section class="hfDiscoverHero"><div><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><h1>Swap ton mood.</h1><p>Choisis une ambiance. Swapp propose un live. Tu swipes, tu gardes, ou tu passes.</p></div><div class="hfDiscoverHint"><span>mood</span><span>live</span><span>swipe</span></div></section><section class="hfMoodPanel"><div class="hfMoodHead"><h2>Ambiance</h2><span class="moodFirstSelected">${esc(moodFirstLabel?.(current)||current)}</span></div><div class="hfMoodGrid">${AMBIANCES.map(([id,label,desc,icon])=>`<button class="hfMoodCard ${id===current?'active':''}" onclick="setDiscoverMood('${esc(id)}')"><i>${icon}</i><b>${esc(label)}</b><span>${esc(desc)}</span></button>`).join('')}</div><details class="hfAdvanced"><summary>Options avancées</summary><div class="hfAdvancedBody"><input id="dQuery" placeholder="jeu, pseudo, ambiance" onkeydown="if(event.key==='Enter')findLive?.()"><select id="dMax"><option value="80">≤80</option><option value="150" selected>≤150</option><option value="300">≤300</option><option value="500">≤500</option></select><select id="dLang"><option value="fr">FR</option><option value="en">EN</option></select><button class="btn secondary" onclick="localStorage.removeItem(hfSeenKey?.()||'');findLive?.()">Réinitialiser</button><button class="btn" onclick="findLive?.()">Relancer</button></div></details></section><section id="zapResult"></section></div>`;
    applyViewerThemeColor?.();
    finalClean();
  ...
  window.studioSec=function studioSecFinal(btn,name){
    ...
    const blocks={
      ...
      'Badges de chaîne':`<h2>Badges de chaîne</h2><div class="summaryList"><div class="summaryItem"><b>Badges visibles</b><p>Créateur, Swapp, Ambiance, Accueil et futurs premiers soutiens.</p><button class="btn" onclick="setView('settings')">Modifier depuis le profil</button></div><div class="summaryItem"><b>Objectif</b><p>Rendre l’identité de chaîne visible sans polluer le lecteur.</p></div></div>`,
      ...
      'Portefeuille':`<h2>Portefeuille</h2><div class="summaryList"><div class="summaryItem"><b>Préparation monétisation</b><p>Zone prête pour pubs, dons, soutiens et revenus Swapp.</p></div><div class="summaryItem"><b>Important</b><p>À connecter uniquement après persistance compte fiable.</p></div></div>`
    };
...
/* =========================================================
   Swapp hard repair — Discover restore + mobile discipline
   ========================================================= */
(function(){
  const st=document.createElement('style');
  ...
  window.renderDiscover = async function renderDiscoverRestored(){
    ...
    el.innerHTML=`<div class="hfDiscover"><section class="hfDiscoverHero"><div><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><h1>Swap ton mood.</h1><p>Un tap lance une vraie proposition. Ensuite tu swipes, tu gardes, ou tu passes.</p></div><div class="hfDiscoverHint"><span>mood</span><span>live</span><span>swipe</span></div></section><section class="hfMoodPanel"><div class="hfMoodHead"><h2>Ambiance</h2><span class="moodFirstSelected">${esc(typeof moodFirstLabel==='function'?moodFirstLabel(current):current)}</span></div><div class="hfMoodGrid">${amb.map(([id,label,desc,icon])=>`<button class="hfMoodCard ${id===current?'active':''}" onclick="setDiscoverMood('${esc(id)}')"><i>${icon||''}</i><b>${esc(label)}</b><span>${esc(desc||'')}</span></button>`).join('')}</div><details class="hfAdvanced"><summary>Options avancées</summary><div class="hfAdvancedBody"><input id="dQuery" placeholder="jeu, pseudo, ambiance" onkeydown="if(event.key==='Enter')findLive?.()"><select id="dMax"><option value="80">≤80</option><option value="150" selected>≤150</option><option value="300">≤300</option><option value="500">≤500</option></select><select id="dLang"><option value="fr">FR</option><option value="en">EN</option></select><button class="btn secondary" onclick="localStorage.removeItem(hfSeenKey?.()||'oryon_seen');findLive?.()">Réinitialiser</button><button class="btn" onclick="findLive?.()">Relancer</button></div></details></section><section id="zapResult"><div class="discoverFallbackCard"><div><h2>Prêt à swiper.</h2><p>Choisis une ambiance ou lance une proposition. Swapp cherche un live qui correspond, pas une liste froide.</p><div class="row"><button class="btn" onclick="findLive?.()">Propose-moi un live</button><button class="btn secondary" onclick="setView('home')">Retour accueil</button></div></div></div></section><div class="discoverMobileActionBar"><button class="btn secondary" onclick="addCurrentToDeck?.()">+ Deck</button><button class="btn secondary" onclick="toggleDiscoverChat?.()">Chat</button><button class="btn" onclick="findLive?.()">Suivant</button></div></div>`;
    applyViewerThemeColor?.();
    closeMini?.();
    removeBazar();
...
/* =========================================================
   SWAPP UNIFIED DISCOVER MODULE REPAIR — no competing modules
   - one stable Discover state machine
   - one action bar
   - one DeckLurker renderer
   - desktop + mobile safe
   ========================================================= */
(function installOryonUnifiedDiscoverRepair(){
  if(window.__ORYON_UNIFIED_DISCOVER_REPAIR_V1__) return;
  window.__ORYON_UNIFIED_DISCOVER_REPAIR_V1__ = true;

  const STYLE_ID='oryonUnifiedDiscoverRepairStyle';
  document.getElementById(STYLE_ID)?.remove();
  const st=document.createElement('style');
  st.id=STYLE_ID;
  st.textContent=`
    #discover{overflow-x:hidden!important;padding-bottom:calc(96px + env(safe-area-inset-bottom,0px))!important;}
    #discover .discoverMobileActionBar,#discover .hfDiscoverActions,#discover .proActions,#discover .discoverActionBarOld{display:none!important;}
    #discover .hfDiscover{width:min(1240px,100%)!important;margin:0 auto!important;display:grid!important;gap:18px!important;}
    #discover #zapResult{width:100%!important;max-width:100%!important;overflow:visible!important;display:grid!important;gap:12px!important;}

    #oryonDiscoverActionBar{
      width:100%!important;box-sizing:border-box!important;display:grid!important;gap:12px!important;
      grid-template-columns:repeat(3,minmax(0,1fr));margin:12px 0 18px!important;padding:10px!important;
      border:1px solid rgba(148,163,184,.18)!important;border-radius:24px!important;
      background:linear-gradient(180deg,rgba(15,23,42,.94),rgba(8,13,24,.98))!important;
      box-shadow:0 22px 64px rgba(0,0,0,.24)!important;position:relative!important;z-index:5!important;
    }
    #oryonDiscoverActionBar[data-mode="watch"]{grid-template-columns:repeat(4,minmax(0,1fr));}
    #oryonDiscoverActionBar button{
      min-height:54px!important;border-radius:18px!important;font-size:16px!important;font-weight:1000!important;
      text-transform:none!important;letter-spacing:0!important;line-height:1!important;white-space:nowrap!important;
      animation:none!important;transition:transform .12s ease,opacity .12s ease,background .12s ease!important;box-sizing:border-box!important;
    }
    #oryonDiscoverActionBar button:hover{transform:translateY(-1px)!important;}
    #oryonDiscoverActionBar button:active{transform:scale(.985)!important;}
    #oryonDiscoverActionBar .primaryAction{background:linear-gradient(135deg,#8b5cf6,#d946ef)!important;color:#fff!important;border-color:rgba(255,255,255,.16)!important;box-shadow:0 16px 38px rgba(168,85,247,.25)!important;}

    #discover .hfLiveCard{position:relative!important;will-change:transform;touch-action:pan-y!important;overflow:hidden!important;}
    #discover .hfLiveCard .hfTagRow{display:flex!important;flex-wrap:wrap!important;gap:7px!important;align-items:flex-start!important;margin-bottom:8px!important;max-width:96%!important;}
    #discover .hfLiveCard .hfTag{font-size:12px!important;line-height:1!important;padding:7px 9px!important;border-radius:999px!important;background:rgba(4,8,18,.74)!important;border:1px solid rgba(255,255,255,.12)!important;backdrop-filter:blur(10px)!important;}
    #discover .hfLiveCard .hfLiveBody h2{font-size:clamp(26px,4.4vw,52px)!important;line-height:1.02!important;letter-spacing:-.045em!important;max-width:95%!important;text-wrap:balance!important;}
    #discover .hfSwipeStamp{position:absolute!important;z-index:10!important;left:50%!important;top:50%!important;transform:translate(-50%,-50%) scale(.92)!important;opacity:0!important;pointer-events:none!important;padding:14px 20px!important;border-radius:999px!important;font-size:clamp(22px,5vw,42px)!important;font-weight:1000!important;text-transform:uppercase!important;color:#fff!important;background:rgba(5,10,22,.72)!important;border:1px solid rgba(255,255,255,.22)!important;box-shadow:0 18px 50px rgba(0,0,0,.35)!important;}
    #discover .hfLiveCard.swipe-like .hfSwipeStamp.like{opacity:1!important;transform:translate(-50%,-50%) scale(1)!important;background:rgba(34,197,94,.9)!important;}
    #discover .hfLiveCard.swipe-nope .hfSwipeStamp.nope{opacity:1!important;transform:translate(-50%,-50%) scale(1)!important;background:rgba(244,63,94,.9)!important;}

    #discover .oryonWatchBox{display:grid!important;grid-template-columns:minmax(0,1fr)!important;gap:12px!important;width:100%!important;}
    #discover .oryonWatchPlayer{width:100%!important;aspect-ratio:16/9!important;border-radius:24px!important;overflow:hidden!important;background:#000!important;border:1px solid rgba(34,211,238,.24)!important;box-shadow:0 22px 80px rgba(0,0,0,.36)!important;}
    #discover .oryonWatchPlayer iframe{display:block!important;width:100%!important;height:100%!important;border:0!important;}
    #discover .oryonWatchChat{display:none!important;width:100%!important;height:min(48vh,460px)!important;min-height:300px!important;border-radius:22px!important;overflow:hidden!important;background:#111318!important;border:1px solid rgba(148,163,184,.22)!important;}
    #discover .oryonWatchChat iframe{display:block!important;width:100%!important;height:100%!important;border:0!important;}
    #discover.oryonDiscoverChatOpen .oryonWatchChat{display:block!important;}

    #oryonDeckLurkerMount{width:100%!important;max-width:100%!important;box-sizing:border-box!important;}
    #discover .oryonDeck{margin:0 0 96px!important;padding:16px!important;border:1px solid rgba(34,211,238,.22)!important;border-radius:28px!important;background:linear-gradient(180deg,rgba(15,23,42,.92),rgba(8,13,24,.96))!important;box-shadow:0 22px 70px rgba(0,0,0,.22)!important;display:grid!important;gap:14px!important;}
    #discover .oryonDeckHead{display:flex!important;justify-content:space-between!important;gap:14px!important;align-items:flex-start!important;}
    #discover .oryonDeckHead h2{font-size:clamp(26px,3vw,42px)!important;margin:6px 0 4px!important;letter-spacing:-.04em!important;}
    #discover .oryonDeckBadge{display:inline-flex;align-items:center;gap:8px;padding:7px 12px;border-radius:999px;border:1px solid rgba(34,211,238,.35);background:rgba(8,145,178,.14);font-weight:1000;color:#dffbff;}
    #discover .oryonDeckStatus{display:flex;gap:8px;flex-wrap:wrap;}
    #discover .oryonDeckStatus button{border:1px solid rgba(148,163,184,.22);background:rgba(255,255,255,.06);color:#fff;border-radius:999px;padding:10px 14px;font-weight:950;}
    #discover .oryonDeckStatus button.active{background:linear-gradient(135deg,#22c55e,#22d3ee);color:#04111a;}
    #discover .oryonDeckActive{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(280px,.36fr)!important;gap:12px!important;align-items:stretch!important;}
    #discover .oryonDeckPreview{position:relative;min-height:210px;border-radius:22px;overflow:hidden;border:1px solid rgba(34,211,238,.25);background:linear-gradient(135deg,rgba(139,92,246,.18),rgba(34,211,238,.08));}
    #discover .oryonDeckPreview img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.74;}
    #discover .oryonDeckPreview:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.04),rgba(2,6,23,.88));}
    #discover .oryonDeckPreviewBody{position:absolute;z-index:2;left:16px;right:16px;bottom:16px;color:#fff;}
    #discover .oryonDeckPreviewBody h3{margin:0;font-size:clamp(24px,2.2vw,38px);line-height:.98;letter-spacing:-.04em;}
    #discover .oryonDeckPreviewBody p{margin:6px 0 0;color:#d7e3f7;}
    #discover .oryonDeckInfo{border:1px solid rgba(148,163,184,.16);border-radius:22px;background:rgba(255,255,255,.04);padding:16px;display:grid;align-content:center;gap:10px;}
    #discover .oryonDeckActions{display:flex;gap:8px;flex-wrap:wrap;}
    #discover .oryonDeckActions button{min-height:40px;border-radius:13px;font-weight:950;}
    #discover .oryonDeckSlots{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;}
    #discover .oryonDeckSlot{position:relative;min-height:86px;border:1px dashed rgba(148,163,184,.18);border-radius:18px;background:rgba(255,255,255,.035);overflow:hidden;color:#dbeafe;text-align:left;}
    #discover .oryonDeckSlot.active{border-color:rgba(34,211,238,.62);box-shadow:0 0 0 1px rgba(34,211,238,.18) inset;}
    #discover .oryonDeckSlot img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.55;}
    #discover .oryonDeckSlot:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.12),rgba(2,6,23,.9));}
    #discover .oryonDeckSlotBody{position:absolute;z-index:2;left:10px;right:10px;bottom:9px;}
    #discover .oryonDeckSlotBody b,#discover .oryonDeckSlotBody span{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
    #discover .oryonDeckSlotBody span{font-size:12px;color:#cbd5e1;}
    #discover .oryonDeckSlotActions{display:flex;gap:6px;margin-top:5px;}
    #discover .oryonDeckSlotActions button{font-size:11px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.09);color:#fff;border-radius:9px;padding:5px 7px;font-weight:900;}
    #discover .oryonFollowQuick{border:1px solid rgba(148,163,184,.16);border-radius:20px;background:rgba(255,255,255,.035);padding:12px;display:grid;gap:10px;}
    #discover .oryonFollowHead{display:flex;justify-content:space-between;gap:10px;align-items:center;}
    #discover .oryonFollowRail{display:flex;gap:10px;overflow:auto;padding-bottom:4px;}
    #discover .oryonFollowMini{position:relative;flex:0 0 190px;min-height:92px;border:1px solid rgba(148,163,184,.18);border-radius:16px;overflow:hidden;background:#0b1220;text-align:left;color:#fff;}
    #discover .oryonFollowMini img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.65;}
    #discover .oryonFollowMini:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.05),rgba(2,6,23,.9));}
    #discover .oryonFollowMini div{position:absolute;z-index:2;left:10px;right:10px;bottom:10px;}
    #discover .oryonFollowMini b,#discover .oryonFollowMini span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
    #discover .oryonFollowMini span{font-size:12px;color:#dbeafe;}

    @media(min-width:761px){
      #discover .oryonWatchBox{grid-template-columns:minmax(0,1fr) minmax(320px,.34fr)!important;align-items:stretch!important;}
      #discover.oryonDiscoverChatOpen .oryonWatchChat{display:block!important;height:auto!important;min-height:360px!important;}
    }
    @media(max-width:760px){
      #discover .hfDiscover{gap:14px!important;}
      #oryonDiscoverActionBar{gap:8px!important;padding:8px!important;border-radius:22px!important;position:relative!important;bottom:auto!important;}
      #oryonDiscoverActionBar button{min-height:50px!important;font-size:14px!important;border-radius:16px!important;padding:0 7px!important;}
      ...
      #discover .oryonWatchChat{height:44vh!important;min-height:320px!important;}
      #discover .oryonDeck{padding:14px!important;border-radius:24px!important;margin-bottom:104px!important;}
      #discover .oryonDeckHead{display:grid!important;}
      #discover .oryonDeckActive{grid-template-columns:1fr!important;}
      #discover .oryonDeckPreview{min-height:130px!important;}
      #discover .oryonDeckSlots{grid-template-columns:repeat(2,minmax(0,1fr));}
      #discover .oryonDeckSlot{min-height:92px;}
      #discover .oryonDeckActions button{flex:1;}
      #discover .oryonFollowMini{flex-basis:150px;min-height:82px;}
    }
  `;
  document.head.appendChild(st);

  const safe = (s)=>{ try { return typeof esc==='function' ? esc(s) : String(s ?? '').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c])); } catch { return String(s ?? ''); } };
  const parentHost=()=>encodeURIComponent(location.hostname||'localhost');
  const isDiscover=()=> (state?.view||'')==='discover' && !!document.querySelector('#discover');
  const setMode=(mode)=>{ state.oryonDiscoverMode = mode==='watch' ? 'watch' : 'preview'; window.__oryonDiscoverMode=state.oryonDiscoverMode; };
  const mode=()=> state.oryonDiscoverMode || 'preview';
  const currentRaw=()=> (state?.zap?.items||[])[state?.zap?.index||0] || state?.zap?.last || null;
  const normalizeLive=(x)=>{
    x=x||{};
    const login=String(x.login||x.user_login||x.broadcaster_login||x.host_login||x.room||x.name||x.display_name||state?.discoverPlayer?.login||'').toLowerCase();
    let img=x.thumbnail_url||x.img||x.preview_url||x.box_art_url||'';
    if(typeof img==='string') img=img.replace('{width}','960').replace('{height}','540');
    return {
      platform:x.platform||x.source||'twitch', login,
      name:x.name||x.user_name||x.display_name||x.broadcaster_name||login||'Live',
      title:x.title||x.game_name||x.category||x.game||'Live en cours',
      game:x.game||x.game_name||x.category||'',
      viewers:Number(x.viewers||x.viewer_count||0), img
    };
  };
  const currentLive=()=>normalizeLive(currentRaw()||{login:state?.discoverPlayer?.login,platform:state?.discoverPlayer?.type||'twitch'});

  function cleanupDiscover(){
    const root=document.querySelector('#discover'); if(!root) return;
    root.querySelectorAll('.discoverMobileActionBar,.hfDiscoverActions,.proActions,#discoverActionBarFinal,.discoverActionBarOld').forEach(n=>n.remove());
    root.querySelectorAll('#oryonDiscoverActionBar').forEach((n,i)=>{ if(i>0) n.remove(); });
    root.querySelectorAll('#oryonDeckLurkerMount').forEach((n,i)=>{ if(i>0) n.remove(); });
  }
  ...
  function ensureActionBar(){
    ...
    const zap=document.querySelector('#discover #zapResult')||root;
    let bar=document.getElementById('oryonDiscoverActionBar');
    if(!bar){ bar=document.createElement('div'); bar.id='oryonDiscoverActionBar'; }
    const watch=mode()==='watch';
    bar.dataset.mode=watch?'watch':'preview';
    bar.innerHTML = watch
      ? `<button class="btn secondary" type="button" onclick="oryonAddCurrentToDeck()">LURK</button><button class="btn secondary" type="button" onclick="oryonToggleDiscoverChat()">CHAT</button><button class="btn primaryAction" type="button" onclick="zapNext()">SUIVANT</button>`
      : `<button class="btn secondary" type="button" onclick="hfSwipeLeft()">Pas ouf</button><button class="btn primaryAction" type="button" onclick="hfWatchCurrent()">Voir</button><button class="btn secondary" type="button" onclick="hfSwipeRight()">J’aime</button>`;
    zap.insertAdjacentElement('afterend',bar);
    ensureDeckMount();
  }

  function ensureSwipeLabels(){
    const card=document.querySelector('#discover .hfLiveCard');
    if(!card) return;
    if(!card.querySelector('.hfSwipeStamp.like')) card.insertAdjacentHTML('afterbegin','<div class="hfSwipeStamp like">J’AIME</div>');
    if(!card.querySelector('.hfSwipeStamp.nope')) card.insertAdjacentHTML('afterbegin','<div class="hfSwipeStamp nope">PAS OUF</div>');
  }

  function bindSwipe(){
    const card=document.querySelector('#discover .hfLiveCard');
    if(!card || card.__oryonUnifiedSwipe) return;
    card.__oryonUnifiedSwipe=true;
    let sx=0, sy=0, dx=0, dy=0, drag=false;
    const start=(e)=>{ if(mode()==='watch') return; if(e.target.closest('button,a,input,select,textarea')) return; const p=e.touches?e.touches[0]:e; sx=p.clientX; sy=p.clientY; dx=dy=0; drag=true; card.style.transition='none'; };
    const move=(e)=>{ if(!drag) return; const p=e.touches?e.touches[0]:e; dx=p.clientX-sx; dy=p.clientY-sy; if(Math.abs(dx)>Math.abs(dy)*1.05 && e.cancelable) e.preventDefault(); const rot=Math.max(-18,Math.min(18,dx/12)); const ty=Math.max(-10,Math.min(10,dy/8)); card.style.setProperty('transform',`translate3d(${dx}px,${ty}px,0) rotate(${rot}deg)`,'important'); card.classList.toggle('swipe-like',dx>34); card.classList.toggle('swipe-nope',dx<-34); };
    const end=()=>{ if(!drag) return; drag=false; const th=Math.min(150,Math.max(82,innerWidth*.18)); card.style.transition='transform .2s cubic-bezier(.18,.85,.18,1), opacity .18s ease'; if(dx>th){ card.style.transform='translate3d(118vw,0,0) rotate(18deg)'; card.style.opacity='.18'; return setTimeout(()=>window.hfSwipeRight(),120); } if(dx<-th){ card.style.transform='translate3d(-118vw,0,0) rotate(-18deg)'; card.style.opacity='.18'; return setTimeout(()=>window.hfSwipeLeft(),120); } card.style.removeProperty('transform'); card.style.removeProperty('opacity'); card.classList.remove('swipe-like','swipe-nope'); };
    card.addEventListener('touchstart',start,{passive:true});
    card.addEventListener('touchmove',move,{passive:false});
    card.addEventListener('touchend',end,{passive:true});
    card.addEventListener('pointerdown',start);
  ...
  window.renderZap=function(){
    if(!isDiscover()) return oldRenderZap?.apply(this,arguments);
    if(mode()==='watch') return renderWatch(currentLive());
    const r=oldRenderZap?.apply(this,arguments);
    setTimeout(()=>{ setMode('preview'); cleanupDiscover(); ensureSwipeLabels(); bindSwipe(); ensureActionBar(); },20);
    return r;
  };

  function renderWatch(x){
    if(!isDiscover()) return;
    const id=normalizeLive(x);
    if(!id.login){ toast?.('Live introuvable'); setMode('preview'); ensureActionBar(); return; }
    state.discoverPlayer={type:id.platform||'twitch',login:id.login};
    setMode('watch');
    const zap=document.querySelector('#discover #zapResult'); if(!zap) return;
    if(id.platform==='oryon'){
      zap.innerHTML=`<section class="oryonWatchBox"><div class="oryonWatchPlayer"><div class="discoverFallbackCard"><div><h2>${safe(id.name)}</h2><p>Live Swapp natif.</p><button class="btn" onclick="openOryon('${safe(id.login)}')">Ouvrir la chaîne</button></div></div></div></section>`;
    } else {
      ...
      zap.innerHTML=`<section class="oryonWatchBox"><div class="oryonWatchPlayer"><iframe allow="autoplay; fullscreen; picture-in-picture" allowfullscreen playsinline src="${src}"></iframe></div><aside class="oryonWatchChat"><iframe src="${chat}"></iframe></aside></section>`;
    }
    document.querySelector('#discover')?.classList.remove('oryonDiscoverChatOpen');
    ensureActionBar();
  }

  window.hfWatchCurrent=function(){ renderWatch(currentRaw()); };
  window.zapOpenCurrent=window.hfWatchCurrent;
  window.oryonToggleDiscoverChat=function(){ if(mode()!=='watch') return; document.querySelector('#discover')?.classList.toggle('oryonDiscoverChatOpen'); };
  window.toggleDiscoverChatStable=window.oryonToggleDiscoverChat;
  window.toggleDiscoverChat=window.oryonToggleDiscoverChat;
  window.oryonOpenCurrentUrl=function(){ const id=currentLive(); if(!id.login) return toast?.('Aucun live à ouvrir'); if(id.platform==='oryon' && typeof openOryon==='function') return openOryon(id.login); window.open('https://www.twitch.tv/'+encodeURIComponent(id.login),'_blank','noopener'); };
  window.openCurrentDiscoverUrl=window.oryonOpenCurrentUrl;
  ...
  window.zapNext=function(){
    ...
    document.querySelector('#discover')?.classList.remove('oryonDiscoverChatOpen');
    state.discoverPlayer=null;
    const r=oldNext?.apply(this,arguments);
    setTimeout(()=>{ setMode('preview'); window.renderZap?.(); ensureActionBar(); },80);
    return r;
  };

  window.hfSwipeLeft=function(){ const x=currentRaw(); try{ if(x && typeof hfMark==='function') hfMark(x,'nope'); }catch{} toast?.('Pas ouf — on passe'); setTimeout(()=>window.zapNext(),80); };
  window.hfSwipeRight=function(){ const x=currentRaw(); try{ if(x && typeof hfMark==='function') hfMark(x,'like'); }catch{} toast?.('J’aime — signal enregistré'); setTimeout(()=>window.zapNext(),80); };
  window.proSwipeLeft=window.hfSwipeLeft; window.proSwipeRight=window.hfSwipeRight;

  function readDeck(){
    let d=null; try{ d=typeof loadDeckLurker==='function'?loadDeckLurker():null; }catch{}
    if(!d || !Array.isArray(d.slots)){ try{ d=JSON.parse(localStorage.getItem('oryon_deck_lurker')||localStorage.getItem('oryon_deck_fallback')||'null'); }catch{} }
    if(!d || !Array.isArray(d.slots)) d={active:0,status:'lurk',slots:Array(8).fill(null)};
    d.slots=d.slots.slice(0,8); while(d.slots.length<8) d.slots.push(null); d.active=Number(d.active||0); d.status=d.status||'lurk'; return d;
  }
  function writeDeck(d){ try{ if(typeof saveDeckLurker==='function') saveDeckLurker(d); }catch{} try{ localStorage.setItem('oryon_deck_lurker',JSON.stringify(d)); localStorage.setItem('oryon_deck_fallback',JSON.stringify(d)); }catch{} }
  function deckMeta(x){ const id=normalizeLive(x); return {platform:id.platform,source:id.platform,login:id.login,name:id.name,title:id.title,game:id.game,viewers:id.viewers,img:id.img,thumbnail_url:id.img,addedAt:Date.now()}; }
  window.oryonAddCurrentToDeck=function(){ const m=deckMeta(currentRaw()||currentLive()); if(!m.login) return toast?.('Aucun live à ajouter'); const d=readDeck(); const same=s=>s && String(s.login||'').toLowerCase()===m.login; let idx=d.slots.findIndex(same); if(idx<0) idx=d.slots.findIndex(s=>!s); if(idx<0) idx=0; d.slots[idx]=m; d.active=idx; writeDeck(d); renderDeck(); toast?.(idx>=0?'Ajouté au DeckLurker':'Deck mis à jour'); };
  window.addCurrentToDiscoverDeckFinal=window.oryonAddCurrentToDeck;
  window.addCurrentToDeck=window.oryonAddCurrentToDeck;

  function deckSlot(s,i,active){
    if(!s) return `<article class="oryonDeckSlot"><div class="oryonDeckSlotBody"><b>Case ${i+1}</b><span>Ajoute un live</span></div></article>`;
    const img=s.thumbnail_url||s.img||'';
    return `<article class="oryonDeckSlot ${active?'active':''}">${img?`<img src="${safe(img)}" alt="">`:''}<div class="oryonDeckSlotBody"><b>${safe(s.name||s.login)}</b><span>${safe(s.game||s.title||'Live')}</span><div class="oryonDeckSlotActions"><button type="button" onclick="event.stopPropagation();oryonOpenDeckSlot(${i})">Voir</button><button type="button" onclick="event.stopPropagation();oryonClearDeckSlot(${i})">Vider</button></div></div></article>`;
  }

  function ensureDeckMount(){
    if(!isDiscover()) return;
    const root=document.querySelector('#discover .hfDiscover')||document.querySelector('#discover'); if(!root) return;
    let mount=document.getElementById('oryonDeckLurkerMount');
    if(!mount){ mount=document.createElement('section'); mount.id='oryonDeckLurkerMount'; }
    const bar=document.getElementById('oryonDiscoverActionBar');
    if(bar) bar.insertAdjacentElement('afterend',mount); else root.appendChild(mount);
    renderDeck();
  }

  function renderDeck(){
    const mount=document.getElementById('oryonDeckLurkerMount'); if(!mount) return;
    const d=readDeck(); const slots=d.slots||[]; const active=slots[d.active]||slots.find(Boolean); const activeIndex=active?slots.indexOf(active):-1; const count=slots.filter(Boolean).length; const img=active?.thumbnail_url||active?.img||'';
    mount.innerHTML=`<section class="oryonDeck"><div class="oryonDeckHead"><div><span class="oryonDeckBadge">👀 DeckLurker · ${count} live${count>1?'s':''}</span><h2>Ton salon de lurk</h2><p>Dans Découvrir : garde plusieurs lives, soutiens en silence et switch en un clic.</p></div><div class="oryonDeckStatus"><button class="${d.status==='watch'?'active':''}" onclick="oryonSetDeckStatus('watch')">Je regarde</button><button class="${d.status==='lurk'?'active':''}" onclick="oryonSetDeckStatus('lurk')">Je lurk</button><button class="${d.status==='work'?'active':''}" onclick="oryonSetDeckStatus('work')">Au calme</button></div></div>${active?`<div class="oryonDeckActive"><div class="oryonDeckPreview">${img?`<img src="${safe(img)}" alt="">`:''}<div class="oryonDeckPreviewBody"><h3>${safe(active.name||active.login)}</h3><p>${safe(active.game||active.title||'Live gardé')}</p></div></div><aside class="oryonDeckInfo"><h3>${safe(active.name||active.login)}</h3><p>${safe(active.title||active.game||'Live gardé')}</p><div class="oryonDeckActions"><button class="btn" onclick="oryonOpenDeckSlot(${activeIndex})">Voir ici</button><button class="btn secondary" onclick="oryonNextDeckSlot()">Suivant deck</button><button class="btn secondary" onclick="oryonClearDeckSlot(${activeIndex})">Vider</button></div></aside></div>`:`<div class="oryonDeckInfo"><h3>Aucun live dans ton deck</h3><p>Ajoute un live avec LURK depuis une proposition.</p></div>`}<div class="oryonDeckSlots">${slots.map((s,i)=>deckSlot(s,i,i===activeIndex)).join('')}</div><section class="oryonFollowQuick"><div class="oryonFollowHead"><div><b>Suivis Twitch en ligne</b><p>Accès rapide séparé des recommandations.</p></div>${state?.session?.twitch?`<button class="btn secondary" onclick="oryonLoadFollowedQuick()">Actualiser</button>`:`<button class="btn" onclick="connectTwitch?.()">Connecter Twitch</button>`}</div><div id="oryonFollowQuickRail" class="oryonFollowRail"><div class="followEmpty">${state?.session?.twitch?'Chargement…':'Connecte Twitch pour voir tes suivis.'}</div></div></section></section>`;
    setTimeout(window.oryonLoadFollowedQuick,20);
  }

  window.oryonSetDeckStatus=function(status){ const d=readDeck(); d.status=status; writeDeck(d); renderDeck(); };
  window.oryonClearDeckSlot=function(i){ const d=readDeck(); d.slots[i]=null; if(d.active===i){ const n=d.slots.findIndex(Boolean); d.active=n>=0?n:0; } writeDeck(d); renderDeck(); };
  window.oryonOpenDeckSlot=function(i){ const d=readDeck(); const s=d.slots[i]; if(!s) return; d.active=i; writeDeck(d); state.zap=state.zap||{}; state.zap.last=s; renderDeck(); renderWatch(s); };
  window.oryonDeckUrl=function(i){ const s=readDeck().slots[i]; if(!s?.login) return; if((s.platform||s.source)==='oryon' && typeof openOryon==='function') return openOryon(s.login); window.open('https://www.twitch.tv/'+encodeURIComponent(s.login),'_blank','noopener'); };
  window.oryonNextDeckSlot=function(){ const d=readDeck(); const filled=d.slots.map((s,i)=>s?i:-1).filter(i=>i>=0); if(!filled.length) return toast?.('Deck vide'); const cur=filled.indexOf(d.active); const ni=filled[(cur+1+filled.length)%filled.length]; window.oryonOpenDeckSlot(ni); };

  window.oryonLoadFollowedQuick=async function(){
    const rail=document.getElementById('oryonFollowQuickRail'); if(!rail) return;
    ...
    try{
      ...
      rail.innerHTML=items.slice(0,18).map(x=>{ const id=normalizeLive({...x,platform:'twitch'}); const img=id.img||x.profile_image_url||x.avatar_url||''; return `<button class="oryonFollowMini" type="button" onclick="openTwitch('${safe(id.login)}')">${img?`<img src="${safe(img)}" alt="">`:''}<div><b>${safe(id.name)}</b><span>${safe(id.game||'Live')} · ${Number(id.viewers||0)}</span></div></button>`; }).join('');
    }catch(e){ rail.innerHTML='<div class="followEmpty">Impossible de charger les suivis Twitch.</div>'; }
  };

  const oldFind=window.findLive;
  if(oldFind) window.findLive=async function(){ setMode('preview'); document.querySelector('#discover')?.classList.remove('oryonDiscoverChatOpen'); state.discoverPlayer=null; const r=await oldFind.apply(this,arguments); setTimeout(()=>{ setMode('preview'); ensureSwipeLabels(); bindSwipe(); ensureActionBar(); },60); return r; };

  const oldDiscover=window.renderDiscover;
  window.renderDiscover=async function(){
    setMode('preview'); state.discoverPlayer=null;
    const r=await oldDiscover?.apply(this,arguments);
    setTimeout(()=>{ if(!isDiscover()) return; setMode('preview'); cleanupDiscover(); ensureSwipeLabels(); bindSwipe(); ensureActionBar(); },90);
    return r;
  };

  // Public route remains available, but normal app tabs are hash routes after navigation.
  const oldSetView=window.setView;
  if(typeof oldSetView==='function' && !oldSetView.__oryonUnifiedRouteWrapped){
    const wrapped=function(view){
      if(view && view!=='channel'){
        try{ history.replaceState(null,'', location.pathname.replace(/\/c\/[^/?#]+$/,'/') + '#'+view); }catch{}
      }
      return oldSetView.apply(this,arguments);
    };
    wrapped.__oryonUnifiedRouteWrapped=true; window.setView=wrapped;
  }

  window.oryonVerifyDiscoverModules=function(){
    if(!isDiscover()) return {view:state?.view, ok:false};
    cleanupDiscover(); ensureSwipeLabels(); bindSwipe(); ensureActionBar();
    return {view:'discover', mode:mode(), actionBars:document.querySelectorAll('#oryonDiscoverActionBar').length, decks:document.querySelectorAll('#oryonDeckLurkerMount').length, chatOpen:document.querySelector('#discover')?.classList.contains('oryonDiscoverChatOpen')};
  };
})();

/* =========================================================
   SWAPP DISCOVER STABLE V2 — final deterministic repair
   Fixes: broken preview/watch buttons, mobile/desktop layout,
   DeckLurker without black screen, chat toggle, Twitch follows access.
   ========================================================= */
(function installOryonDiscoverStableV2(){
  window.__ORYON_DISCOVER_STABLE_V2__ = true;
  const STYLE_ID='oryonDiscoverStableV2Style';
  document.getElementById(STYLE_ID)?.remove();
  const st=document.createElement('style');
  st.id=STYLE_ID;
  st.textContent=`
    #discover{overflow-x:hidden!important;padding-bottom:calc(96px + env(safe-area-inset-bottom,0px))!important;}
    #discover .hfDiscover{width:min(1180px,100%)!important;margin:0 auto!important;padding:clamp(14px,2vw,24px)!important;display:grid!important;gap:16px!important;overflow:visible!important;}
    #discover .hfDiscoverHero{min-height:0!important;height:auto!important;padding:clamp(18px,3vw,34px)!important;border-radius:26px!important;display:flex!important;align-items:flex-end!important;justify-content:space-between!important;gap:12px!important;overflow:hidden!important;}
    #discover .hfDiscoverHero h1{font-size:clamp(38px,6vw,72px)!important;line-height:.9!important;letter-spacing:-.075em!important;margin:6px 0!important;}
    #discover .hfDiscoverHero p{font-size:clamp(13px,1.8vw,16px)!important;line-height:1.35!important;margin:0!important;max-width:760px!important;}
    #discover .hfDiscoverHint{display:flex!important;gap:8px!important;flex-wrap:wrap!important;}
    #discover .hfMoodPanel{padding:14px!important;border-radius:24px!important;overflow:hidden!important;min-height:0!important;}
    #discover .hfMoodHead{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;margin-bottom:10px!important;}
    #discover .hfMoodHead h2{font-size:clamp(24px,3vw,34px)!important;margin:0!important;}
    #discover .hfMoodGrid{display:grid!important;grid-template-columns:repeat(6,minmax(0,1fr))!important;gap:10px!important;}
    #discover .hfMoodCard{min-height:94px!important;height:auto!important;padding:12px!important;border-radius:18px!important;align-content:end!important;overflow:hidden!important;}
    #discover .hfMoodCard i{font-size:23px!important;line-height:1!important;}
    #discover .hfMoodCard b{font-size:14px!important;line-height:1.05!important;}
    #discover .hfMoodCard span{font-size:11px!important;line-height:1.18!important;display:block!important;}
    #discover .hfAdvanced{margin-top:10px!important;border-radius:16px!important;overflow:hidden!important;}
    #discover .hfAdvanced summary{min-height:42px!important;padding:12px 14px!important;font-size:14px!important;display:flex!important;align-items:center!important;}
    #discover .hfAdvancedBody{display:grid!important;grid-template-columns:minmax(220px,1fr) 120px 100px auto auto!important;gap:8px!important;padding:12px!important;}

    #discover #zapResult{display:block!important;width:100%!important;max-width:100%!important;min-height:0!important;margin:0!important;padding:0!important;overflow:visible!important;}
    #discover .hfZap{display:block!important;min-height:0!important;}
    #discover .hfLiveCard:not(.oryonStableLiveCard){display:none!important;}
    #discover .discoverMobileActionBar,#discover .hfDiscoverActions,#discover .proActions,#discoverActionBarFinal,#discover .discoverActionBarOld,#discover #oryonDiscoverActionBar{display:none!important;}

    .oryonStableCardWrap{display:grid!important;gap:12px!important;width:100%!important;max-width:100%!important;}
    .oryonStableLiveCard{position:relative!important;width:100%!important;max-width:100%!important;min-width:0!important;aspect-ratio:16/9!important;border-radius:24px!important;overflow:hidden!important;border:1px solid rgba(34,211,238,.34)!important;background:#020617!important;box-shadow:0 24px 78px rgba(0,0,0,.32)!important;touch-action:pan-y!important;will-change:transform!important;cursor:grab!important;}
    .oryonStableLiveCard img{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:cover!important;display:block!important;filter:saturate(1.06) contrast(1.04)!important;}
    .oryonStableLiveCard:after{content:"";position:absolute;inset:0;z-index:1;background:linear-gradient(180deg,rgba(2,6,23,.05) 0%,rgba(2,6,23,.20) 38%,rgba(2,6,23,.90) 100%);} 
    .oryonStableBody{position:absolute!important;z-index:2!important;left:18px!important;right:18px!important;bottom:16px!important;display:grid!important;gap:8px!important;color:#fff!important;}
    .oryonStableTags{display:flex!important;flex-wrap:wrap!important;gap:7px!important;max-width:100%!important;}
    .oryonStableTag{display:inline-flex!important;align-items:center!important;max-width:46%!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;border:1px solid rgba(255,255,255,.14)!important;background:rgba(3,7,18,.62)!important;backdrop-filter:blur(12px)!important;border-radius:999px!important;padding:6px 9px!important;font-size:11px!important;line-height:1!important;font-weight:950!important;}
    .oryonStableTitle{font-size:clamp(24px,3.8vw,48px)!important;line-height:1.02!important;letter-spacing:-.045em!important;margin:0!important;max-width:96%!important;display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;overflow:hidden!important;text-shadow:0 10px 26px rgba(0,0,0,.46)!important;}
    .oryonStableMeta{font-size:clamp(13px,1.7vw,17px)!important;line-height:1.2!important;font-weight:950!important;color:#f3f7ff!important;display:-webkit-box!important;-webkit-line-clamp:1!important;-webkit-box-orient:vertical!important;overflow:hidden!important;}
    .oryonStableStamp{position:absolute!important;z-index:8!important;left:50%!important;top:50%!important;transform:translate(-50%,-50%) scale(.92)!important;opacity:0!important;pointer-events:none!important;border-radius:999px!important;padding:13px 20px!important;font-size:clamp(24px,6vw,54px)!important;font-weight:1000!important;color:#fff!important;background:rgba(3,7,18,.75)!important;border:1px solid rgba(255,255,255,.20)!important;box-shadow:0 20px 50px rgba(0,0,0,.36)!important;}
    .oryonStableLiveCard.swipe-like .oryonStableStamp.like{opacity:1!important;background:rgba(16,185,129,.92)!important;}
    ...
    .oryonStableWatch{display:grid!important;gap:12px!important;width:100%!important;}
    .oryonStablePlayer{width:100%!important;aspect-ratio:16/9!important;border-radius:24px!important;overflow:hidden!important;background:#000!important;border:1px solid rgba(34,211,238,.28)!important;box-shadow:0 24px 78px rgba(0,0,0,.32)!important;}
    .oryonStablePlayer iframe{display:block!important;width:100%!important;height:100%!important;border:0!important;}
    .oryonStableChat{display:none!important;width:100%!important;height:min(48vh,520px)!important;min-height:330px!important;border-radius:22px!important;overflow:hidden!important;border:1px solid rgba(148,163,184,.22)!important;background:#111318!important;}
    .oryonStableChat iframe{display:block!important;width:100%!important;height:100%!important;border:0!important;}
    #discover.oryonStableChatOpen .oryonStableChat{display:block!important;}

    #oryonDeckLurkerMount{display:block!important;width:100%!important;max-width:100%!important;margin-top:8px!important;}
    #oryonDeckLurkerMount .oryonDeck{border:1px solid rgba(34,211,238,.26)!important;background:linear-gradient(180deg,rgba(15,23,42,.86),rgba(7,12,24,.93))!important;border-radius:24px!important;padding:16px!important;display:grid!important;gap:12px!important;box-shadow:0 18px 54px rgba(0,0,0,.22)!important;}
    .oryonDeckHead{display:flex!important;align-items:flex-start!important;justify-content:space-between!important;gap:12px!important;}
    .oryonDeckHead h2{font-size:clamp(22px,2.5vw,32px)!important;margin:8px 0 4px!important;line-height:1!important;}
    .oryonDeckHead p{margin:0!important;color:#aab8d0!important;font-size:13px!important;line-height:1.35!important;}
    .oryonDeckBadge{display:inline-flex!important;border:1px solid rgba(34,211,238,.42)!important;background:rgba(34,211,238,.10)!important;color:#dff9ff!important;border-radius:999px!important;padding:7px 10px!important;font-size:12px!important;font-weight:1000!important;}
    .oryonDeckStatus{display:flex!important;gap:8px!important;flex-wrap:wrap!important;justify-content:flex-end!important;}
    .oryonDeckStatus button{min-height:34px!important;border-radius:999px!important;padding:0 12px!important;font-size:12px!important;font-weight:1000!important;}
    .oryonDeckStatus button.active{background:linear-gradient(135deg,#22c55e,#22d3ee)!important;color:#04111a!important;}
    .oryonDeckActive{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(220px,300px)!important;gap:12px!important;align-items:stretch!important;}
    .oryonDeckPreview{position:relative!important;aspect-ratio:16/9!important;border-radius:18px!important;overflow:hidden!important;background:#020617!important;border:1px solid rgba(148,163,184,.16)!important;min-height:0!important;}
    .oryonDeckPreview img{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:cover!important;}
    .oryonDeckPreview:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.88));}
    .oryonDeckPreviewBody{position:absolute!important;z-index:2!important;left:12px!important;right:12px!important;bottom:12px!important;}
    .oryonDeckPreviewBody h3,.oryonDeckInfo h3{margin:0 0 6px!important;font-size:18px!important;}
    .oryonDeckPreviewBody p,.oryonDeckInfo p{margin:0!important;color:#aab8d0!important;font-size:12px!important;line-height:1.3!important;}
    .oryonDeckInfo{border:1px solid rgba(148,163,184,.16)!important;border-radius:18px!important;background:rgba(255,255,255,.035)!important;padding:14px!important;display:grid!important;align-content:center!important;gap:10px!important;}
    .oryonDeckActions{display:flex!important;gap:8px!important;flex-wrap:wrap!important;}
    .oryonDeckActions button{min-height:34px!important;border-radius:12px!important;padding:0 10px!important;font-size:12px!important;}
    .oryonDeckSlots{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:10px!important;}
    .oryonDeckSlot{position:relative!important;min-height:82px!important;border:1px dashed rgba(148,163,184,.22)!important;border-radius:16px!important;background:rgba(255,255,255,.025)!important;overflow:hidden!important;}
    .oryonDeckSlot.active{border-color:rgba(34,211,238,.74)!important;background:rgba(34,211,238,.07)!important;}
    .oryonDeckSlot img{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:cover!important;opacity:.52!important;}
    .oryonDeckSlotBody{position:relative!important;z-index:2!important;padding:10px!important;height:100%!important;display:grid!important;align-content:center!important;gap:3px!important;}
    .oryonDeckSlotBody b{font-size:13px!important;}.oryonDeckSlotBody span{font-size:11px!important;color:#aab8d0!important;}
    .oryonDeckSlotActions{display:flex!important;gap:6px!important;margin-top:5px!important;}.oryonDeckSlotActions button{font-size:11px!important;border-radius:9px!important;padding:5px 7px!important;}
    .oryonFollowQuick{border-top:1px solid rgba(148,163,184,.13)!important;padding-top:12px!important;}
    .oryonFollowHead{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:10px!important;margin-bottom:8px!important;}
    .oryonFollowHead b{font-size:16px!important;}.oryonFollowHead p{margin:3px 0 0!important;color:#aab8d0!important;font-size:12px!important;}
    .oryonFollowRail{display:flex!important;gap:10px!important;overflow-x:auto!important;padding-bottom:6px!important;}
    .oryonFollowMini{flex:0 0 190px!important;min-height:86px!important;border:1px solid rgba(148,163,184,.16)!important;background:rgba(255,255,255,.035)!important;color:white!important;border-radius:16px!important;display:flex!important;gap:9px!important;align-items:center!important;padding:9px!important;text-align:left!important;}
    .oryonFollowMini img{width:48px!important;height:48px!important;border-radius:13px!important;object-fit:cover!important;}.oryonFollowMini b{display:block!important;font-size:13px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;}.oryonFollowMini span{display:block!important;font-size:11px!important;color:#aab8d0!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;}.followEmpty{border:1px dashed rgba(148,163,184,.22)!important;border-radius:14px!important;padding:12px!important;color:#aab8d0!important;font-size:13px!important;}

    @media(max-width:900px){
      #discover .hfMoodGrid{display:flex!important;overflow-x:auto!important;gap:10px!important;scroll-snap-type:x proximity!important;padding-bottom:6px!important;}
      #discover .hfMoodCard{flex:0 0 145px!important;min-height:96px!important;scroll-snap-align:start!important;}
      .oryonDeckActive{grid-template-columns:1fr!important;}.oryonDeckSlots{grid-template-columns:repeat(2,minmax(0,1fr))!important;}
    }
    @media(max-width:760px){
      #discover .hfDiscover{padding:12px 14px 110px!important;gap:12px!important;}
      #discover .hfDiscoverHero{padding:16px!important;border-radius:22px!important;}
      #discover .hfDiscoverHero h1{font-size:38px!important;}
      #discover .hfDiscoverHero p{font-size:13px!important;}
      #discover .hfDiscoverHint{display:none!important;}
      #discover .hfMoodPanel{padding:12px!important;border-radius:20px!important;}
      #discover .hfMoodHead h2{font-size:24px!important;}
      #discover .hfMoodCard{flex-basis:132px!important;min-height:90px!important;padding:10px!important;}
      #discover .hfMoodCard i{font-size:21px!important;}#discover .hfMoodCard b{font-size:12px!important;}#discover .hfMoodCard span{font-size:10px!important;}
      #discover .hfAdvancedBody{grid-template-columns:1fr!important;}
      .oryonStableLiveCard{border-radius:22px!important;}
      .oryonStableBody{left:12px!important;right:12px!important;bottom:12px!important;gap:6px!important;}
      .oryonStableTag{font-size:10px!important;padding:5px 7px!important;max-width:49%!important;}
      ...
      #oryonDeckLurkerMount .oryonDeck{padding:12px!important;border-radius:20px!important;}
      .oryonDeckHead{display:grid!important;gap:9px!important;}.oryonDeckStatus{justify-content:flex-start!important;}.oryonDeckStatus button{font-size:12px!important;}
      .oryonDeckSlots{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important;}.oryonDeckSlot{min-height:74px!important;}
      .oryonFollowMini{flex-basis:160px!important;}
    }
  `;
  document.head.appendChild(st);
  ...
  function currentMood(){ return state.moodFirstMood || 'petite-commu'; }
  function ambiances(){ return Array.isArray(window.AMBIANCES)?window.AMBIANCES:(typeof AMBIANCES!=='undefined'?AMBIANCES:[]); }
  function liveId(x){
    x=x||{};
    const login=String(x.login||x.user_login||x.broadcaster_login||x.host_login||x.room||x.name||x.display_name||'').toLowerCase();
    let img=x.thumbnail_url||x.img||x.preview_url||x.box_art_url||'';
    if(typeof img==='string') img=img.replace('{width}','960').replace('{height}','540');
    return {platform:x.platform||x.source||(x.host_login||x.room?'oryon':'twitch'),login,name:x.name||x.user_name||x.display_name||x.broadcaster_name||login||'Live',title:x.title||'Live en cours',game:x.game||x.game_name||x.category||'',viewers:Number(x.viewers||x.viewer_count||0)||0,img};
  }
  function liveKey(x){const id=liveId(x); return `${id.platform}:${id.login}`;}
  function currentRaw(){ return (state.zap?.items||[])[state.zap?.index||0] || state.zap?.last || null; }
  function currentLive(){ return liveId(currentRaw()||{}); }
  function moodQuery(m){return ({chill:'chill',discussion:'Just Chatting','nuit-calme':'chill',rp:'roleplay','decouverte-jeu':'first play','petite-commu':''}[m]||'');}
  function moodLabel(id){const a=ambiances().find(x=>x[0]===id); return a?a[1]:id;}
  function tagsFor(x){ const id=liveId(x); const t=[]; t.push(id.platform==='oryon'?'Swapp':'Twitch intégré'); if(id.viewers)t.push(id.viewers<80?'petit live':'audience active'); t.push('entrée normale'); if(id.game)t.push(id.game); return [...new Set(t)].slice(0,4); }
  function scoreLive(x,m){ const id=liveId(x); const text=[id.title,id.game,id.name].join(' ').toLowerCase(); let s=0; if(m==='petite-commu'){ if(id.viewers>=3&&id.viewers<=80)s+=80; if(id.viewers>180)s-=100; } if(m==='discussion'&&/just chatting|discussion|talk|irl|chat/.test(text))s+=90; if(m==='chill'&&/chill|calme|cozy|lofi|musique/.test(text))s+=70; if(m==='rp'&&/rp|roleplay|jdr|dnd|gta/.test(text))s+=80; if(m==='decouverte-jeu'&&/first|blind|découverte|decouverte|exploration|new/.test(text))s+=60; s+=Math.max(0,40-Math.abs((id.viewers||30)-45)/2); return s; }
  async function fetchLivePool(){
    const mood=currentMood(); const q=document.getElementById('dQuery')?.value || moodQuery(mood); const max=Number(document.getElementById('dMax')?.value||180); const lang=document.getElementById('dLang')?.value||'fr';
    const calls=[
      api('/api/oryon/discover/find-live?'+qs({q,mood,max,lang,source:'both',discover:'1'})).catch(()=>({items:[]})),
      api(`/api/twitch/streams/small?lang=${enc(lang)}&min=1&max=${enc(Math.max(max,220))}&first=100&discover=1`).catch(()=>({items:[]})),
      api('/api/native/lives').catch(()=>({items:[]}))
    ];
    const results=await Promise.all(calls); const seen=new Set(); const items=[];
    for(const r of results){ for(const it of (r.items||r.streams||[])){ const x={...it,platform:it.platform||(it.host_login||it.room?'oryon':'twitch')}; const id=liveId(x); if(!id.login)continue; const k=liveKey(x); if(seen.has(k))continue; seen.add(k); items.push(x); } }
    return items.sort((a,b)=>scoreLive(b,mood)-scoreLive(a,mood));
  }
  ...
  function renderZapStable(){ const box=document.getElementById('zapResult'); if(!box)return; const x=currentRaw(); if(!x){ box.innerHTML=`<div class="discoverFallbackCard"><div><h2>Prêt à swiper.</h2><p>Choisis une ambiance. Swapp propose un live, sans liste froide.</p><button class="btn" onclick="findLive()">Propose-moi un live</button></div></div>`; renderDeck(); return; } box.innerHTML=mode()==='watch'?watchHtml(x):cardHtml(x); bindSwipe(); ensureDeckMount(); }
  window.renderZap=renderZapStable;
  window.findLive=async function(){ setMode('preview'); const box=document.getElementById('zapResult'); if(box) box.innerHTML=`<div class="discoverFallbackCard"><div><h2>Recherche…</h2><p>Swapp cherche un live qui colle au mood ${safe(moodLabel(currentMood()))}.</p></div></div>`; try{ const items=await fetchLivePool(); state.zap=state.zap||{}; state.zap.items=items; state.zap.index=0; state.zap.last=items[0]||null; if(!items.length && box) box.innerHTML=`<div class="discoverFallbackCard"><div><h2>Aucun live trouvé.</h2><p>Change de mood ou relance la recherche.</p><button class="btn" onclick="findLive()">Relancer</button></div></div>`; else renderZapStable(); }catch(e){ console.error(e); if(box) box.innerHTML=`<div class="discoverFallbackCard"><div><h2>Recherche indisponible.</h2><p>Le serveur n’a pas répondu correctement.</p><button class="btn" onclick="findLive()">Réessayer</button></div></div>`; } };
  window.renderDiscover=async function(){ const el=document.getElementById('discover'); if(!el)return; setMode('preview'); const cur=currentMood(); const amb=ambiances(); el.innerHTML=`<div class="hfDiscover"><section class="hfDiscoverHero"><div><span class="eyebrow"><i class="dot"></i>Swapp Flow</span><h1>Swap ton mood.</h1><p>Choisis une ambiance. Tu swipes à gauche si ce n’est pas ta vibe, à droite si tu aimes, ou tu ouvres le live.</p></div><div class="hfDiscoverHint"><span>mood</span><span>live</span><span>swipe</span></div></section><section class="hfMoodPanel"><div class="hfMoodHead"><h2>Ambiance</h2><span class="moodFirstSelected">${safe(moodLabel(cur))}</span></div><div class="hfMoodGrid">${amb.map(([id,label,desc,icon])=>`<button class="hfMoodCard ${id===cur?'active':''}" onclick="setDiscoverMood('${safe(id)}')"><i>${icon||''}</i><b>${safe(label)}</b><span>${safe(desc||'')}</span></button>`).join('')}</div><details class="hfAdvanced"><summary>Options avancées</summary><div class="hfAdvancedBody"><input id="dQuery" placeholder="jeu, pseudo, ambiance" onkeydown="if(event.key==='Enter')findLive()"><select id="dMax"><option value="80">≤80</option><option value="150" selected>≤150</option><option value="300">≤300</option><option value="500">≤500</option></select><select id="dLang"><option value="fr">FR</option><option value="en">EN</option></select><button class="btn secondary" onclick="localStorage.removeItem('oryon_seen_soft_v4:'+String(state.session?.local?.login||state.session?.twitch?.login||'guest').toLowerCase());findLive()">Réinitialiser</button><button class="btn" onclick="findLive()">Relancer</button></div></details></section><section id="zapResult"></section><section id="oryonDeckLurkerMount"></section></div>`; closeMini?.(); setTimeout(findLive,60); };
  window.setDiscoverMood=function(id){ state.moodFirstMood=id; setMode('preview'); renderDiscover(); };
  window.hfWatchCurrent=function(){ const x=currentRaw(); if(!x)return; setMode('watch'); renderZapStable(); };
  window.zapOpenCurrent=window.hfWatchCurrent;
  window.zapNext=function(){ const z=state.zap=state.zap||{items:[],index:0}; setMode('preview'); if(!z.items?.length) return findLive(); z.index=(Number(z.index||0)+1)%z.items.length; z.last=z.items[z.index]; renderZapStable(); };
  window.hfSwipeLeft=function(){ const card=document.querySelector('.oryonStableLiveCard'); if(card){card.classList.add('swipe-nope');card.style.transform='translateX(-120vw) rotate(-14deg)';card.style.opacity='.18';} setTimeout(()=>zapNext(),150); };
  window.hfSwipeRight=function(){ const card=document.querySelector('.oryonStableLiveCard'); if(card){card.classList.add('swipe-like');card.style.transform='translateX(120vw) rotate(14deg)';card.style.opacity='.18';} try{ const x=currentRaw(); if(x&&typeof hfMark==='function')hfMark(x,'like'); }catch{} setTimeout(()=>zapNext(),150); };
  window.proSwipeLeft=window.hfSwipeLeft; window.proSwipeRight=window.hfSwipeRight;
  function bindSwipe(){ const card=document.querySelector('.oryonStableLiveCard'); if(!card||card.__swipeV2)return; card.__swipeV2=true; let sx=0,sy=0,dx=0,dy=0,drag=false; const start=e=>{if(e.target.closest('button,a,input,select,textarea'))return; const p=e.touches?e.touches[0]:e; sx=p.clientX;sy=p.clientY;dx=dy=0;drag=true;card.style.transition='none';}; const move=e=>{if(!drag)return; const p=e.touches?e.touches[0]:e; dx=p.clientX-sx;dy=p.clientY-sy;if(Math.abs(dx)>Math.abs(dy)*1.05&&e.cancelable)e.preventDefault(); const rot=Math.max(-16,Math.min(16,dx/12)); card.style.setProperty('transform',`translate3d(${dx}px,${Math.max(-8,Math.min(8,dy/10))}px,0) rotate(${rot}deg)`,'important'); card.classList.toggle('swipe-like',dx>32);card.classList.toggle('swipe-nope',dx<-32);}; const end=()=>{if(!drag)return;drag=false; const th=Math.min(140,Math.max(72,innerWidth*.16)); card.style.transition='transform .18s ease, opacity .18s ease'; if(dx>th)return hfSwipeRight(); if(dx<-th)return hfSwipeLeft(); card.style.transform='';card.classList.remove('swipe-like','swipe-nope');}; card.addEventListener('touchstart',start,{passive:true});card.addEventListener('touchmove',move,{passive:false});card.addEventListener('touchend',end,{passive:true});card.addEventListener('pointerdown',start);window.addEventListener('pointermove',move,{passive:false});window.addEventListener('pointerup',end); }
  window.oryonToggleDiscoverChatV2=function(){ if(mode()!=='watch')return; document.getElementById('discover')?.classList.toggle('oryonStableChatOpen'); };
  window.oryonOpenCurrentUrlV2=function(){ const id=currentLive(); if(!id.login)return; if(id.platform==='oryon'&&typeof openOryon==='function')return openOryon(id.login); window.open('https://www.twitch.tv/'+enc(id.login),'_blank','noopener'); };
  function deckKey(){return 'oryon_deck_lurker_v2:'+String(state.session?.local?.login||state.session?.twitch?.login||'guest').toLowerCase();}
  function readDeck(){let d=null;try{d=JSON.parse(localStorage.getItem(deckKey())||'null')}catch{} if(!d||!Array.isArray(d.slots))d={active:0,status:'lurk',slots:Array(8).fill(null)}; d.slots=d.slots.slice(0,8);while(d.slots.length<8)d.slots.push(null);return d;}
  function writeDeck(d){try{localStorage.setItem(deckKey(),JSON.stringify(d));localStorage.setItem('oryon_deck_lurker',JSON.stringify(d));}catch{}}
  window.oryonAddCurrentToDeckV2=function(){ const x=currentRaw(); if(!x)return; const m={...liveId(x),thumbnail_url:liveId(x).img,img:liveId(x).img,addedAt:Date.now()}; const d=readDeck(); const same=s=>s&&String(s.login).toLowerCase()===m.login; let i=d.slots.findIndex(same); if(i<0)i=d.slots.findIndex(s=>!s); if(i<0)i=0; d.slots[i]=m; d.active=i; writeDeck(d); renderDeck(); toast?.('Ajouté au DeckLurker'); };
  window.oryonAddCurrentToDeck=window.oryonAddCurrentToDeckV2;
  function ensureDeckMount(){ let m=document.getElementById('oryonDeckLurkerMount'); if(!m){ const root=document.querySelector('#discover .hfDiscover')||document.querySelector('#discover'); if(!root)return; m=document.createElement('section');m.id='oryonDeckLurkerMount';root.appendChild(m);} renderDeck(); }
  function slotHtml(s,i,active){ if(!s)return `<article class="oryonDeckSlot"><div class="oryonDeckSlotBody"><b>Case ${i+1}</b><span>Ajoute un live</span></div></article>`; return `<article class="oryonDeckSlot ${active?'active':''}">${(s.img||s.thumbnail_url)?`<img src="${safe(s.img||s.thumbnail_url)}" alt="">`:''}<div class="oryonDeckSlotBody"><b>${safe(s.name||s.login)}</b><span>${safe(s.game||s.title||'Live')}</span><div class="oryonDeckSlotActions"><button onclick="event.stopPropagation();oryonOpenDeckSlotV2(${i})">Voir</button><button onclick="event.stopPropagation();oryonClearDeckSlotV2(${i})">Vider</button></div></div></article>`;}
  function renderDeck(){ const m=document.getElementById('oryonDeckLurkerMount'); if(!m)return; const d=readDeck(); const slots=d.slots; const active=slots[d.active]||slots.find(Boolean); const ai=active?slots.indexOf(active):-1; const count=slots.filter(Boolean).length; m.innerHTML=`<section class="oryonDeck"><div class="oryonDeckHead"><div><span class="oryonDeckBadge">👀 DeckLurker · ${count} live${count>1?'s':''}</span><h2>Ton salon de lurk</h2><p>Garde plusieurs lives, soutiens en silence et switch en un clic.</p></div><div class="oryonDeckStatus"><button class="${d.status==='watch'?'active':''}" onclick="oryonSetDeckStatusV2('watch')">Je regarde</button><button class="${d.status==='lurk'?'active':''}" onclick="oryonSetDeckStatusV2('lurk')">Je lurk</button><button class="${d.status==='work'?'active':''}" onclick="oryonSetDeckStatusV2('work')">Au calme</button></div></div>${active?`<div class="oryonDeckActive"><div class="oryonDeckPreview">${(active.img||active.thumbnail_url)?`<img src="${safe(active.img||active.thumbnail_url)}" alt="">`:''}<div class="oryonDeckPreviewBody"><h3>${safe(active.name||active.login)}</h3><p>${safe(active.game||active.title||'Live gardé')}</p></div></div><aside class="oryonDeckInfo"><h3>${safe(active.name||active.login)}</h3><p>${safe(active.title||active.game||'Live gardé')}</p><div class="oryonDeckActions"><button class="btn" onclick="oryonOpenDeckSlotV2(${ai})">Voir ici</button><button class="btn secondary" onclick="oryonNextDeckSlotV2()">Suivant deck</button><button class="btn secondary" onclick="oryonClearDeckSlotV2(${ai})">Vider</button></div></aside></div>`:`<div class="oryonDeckInfo"><h3>Aucun live dans ton deck</h3><p>Ajoute un live avec LURK après avoir cliqué sur Voir.</p></div>`}<div class="oryonDeckSlots">${slots.map((s,i)=>slotHtml(s,i,i===ai)).join('')}</div><section class="oryonFollowQuick"><div class="oryonFollowHead"><div><b>Suivis Twitch en ligne</b><p>Accès rapide, séparé des recommandations.</p></div>${state.session?.twitch?`<button class="btn secondary" onclick="oryonLoadFollowedQuickV2()">Actualiser</button>`:`<button class="btn" onclick="connectTwitch?.()">Connecter Twitch</button>`}</div><div id="oryonFollowQuickRail" class="oryonFollowRail"><div class="followEmpty">${state.session?.twitch?'Chargement…':'Connecte Twitch pour voir tes suivis.'}</div></div></section></section>`; setTimeout(window.oryonLoadFollowedQuickV2,20); }
  window.oryonSetDeckStatusV2=function(s){const d=readDeck();d.status=s;writeDeck(d);renderDeck();};
  window.oryonClearDeckSlotV2=function(i){const d=readDeck();d.slots[i]=null;if(d.active===i){const n=d.slots.findIndex(Boolean);d.active=n>=0?n:0;}writeDeck(d);renderDeck();};
  window.oryonOpenDeckSlotV2=function(i){const d=readDeck();const s=d.slots[i];if(!s)return;d.active=i;writeDeck(d);state.zap=state.zap||{};state.zap.items=[s];state.zap.index=0;state.zap.last=s;setMode('watch');renderZapStable();};
  window.oryonDeckUrlV2=function(i){const s=readDeck().slots[i];if(!s?.login)return;window.open('https://www.twitch.tv/'+enc(s.login),'_blank','noopener');};
...
/* =========================================================
   SWAPP SAFE DISCOVER GUARD — fullwidth, no URL, no black screen
   ========================================================= */
(function installOryonSafeDiscoverGuard(){
  const STYLE_ID='oryonSafeDiscoverGuardStyle';
  document.getElementById(STYLE_ID)?.remove();
  const st=document.createElement('style');
  st.id=STYLE_ID;
  st.textContent=`
    body.oryon-discover-active .app,
    body:has(#discover.view.active) .app{width:100%!important;max-width:none!important;margin:0!important;padding-left:0!important;padding-right:0!important;overflow-x:hidden!important;}
    ...
    #discover .hfDiscover{width:100%!important;max-width:none!important;margin:0!important;padding:clamp(18px,2.2vw,34px) clamp(18px,3vw,54px) 100px!important;}
    #discover .hfDiscoverHero,#discover .hfMoodPanel,#discover #zapResult,#discover #oryonDeckLurkerMount{width:100%!important;max-width:none!important;}
    #discover .oryonStableActions.watch{grid-template-columns:repeat(3,minmax(0,1fr))!important;}
    #discover .oryonDeckActions{grid-template-columns:repeat(3,minmax(0,1fr))!important;}
    #discover button[data-oryon-url],
    #discover .oryonUrlBtn,
    #discover .oryonDeckUrlButton,
    #discover button[onclick*="oryonOpenCurrentUrl"],
    #discover button[onclick*="oryonDeckUrl"],
    #discover button[onclick*="window.open"]{display:none!important;}
    #discover .oryonStableChat{background:#050814!important;border:1px solid rgba(148,163,184,.22)!important;border-radius:22px!important;overflow:hidden!important;}
    #discover.oryonStableChatOpen .oryonStableChat{display:block!important;visibility:visible!important;opacity:1!important;min-height:420px!important;}
    #discover .discoverFallbackCard,#discover .hfPanel,#discover .oryonDeckInfo{color:#e5eefc!important;}
    @media(max-width:760px){
      #discover .hfDiscover{padding:12px 12px 110px!important;}
      #discover.oryonStableChatOpen .oryonStableChat{min-height:390px!important;height:440px!important;}
    }
  `;
  document.head.appendChild(st);
  function clean(){
    const root=document.getElementById('discover');
    document.body.classList.toggle('oryon-discover-active', window.state?.view==='discover');
    if(!root) return;
    root.querySelectorAll('button').forEach(btn=>{
      const txt=(btn.textContent||'').trim().toLowerCase();
      const on=(btn.getAttribute('onclick')||'').toLowerCase();
      if(txt==='url' || on.includes('oryonopencurrenturl') || on.includes('oryondeckurl') || on.includes('window.open')) btn.remove();
    });
    root.querySelectorAll('.oryonStableActions.watch').forEach(bar=>{
      const buttons=[...bar.querySelectorAll('button')];
      if(buttons.length>3 || buttons.some(b=>(b.textContent||'').trim().toLowerCase()==='url')){
        bar.innerHTML='<button class="btn secondary" type="button" onclick="oryonAddCurrentToDeckV2?.();oryonAddCurrentToDeck?.();">LURK</button><button class="btn secondary" type="button" onclick="oryonToggleDiscoverChatV2?.();oryonToggleDiscoverChat?.();">CHAT</button><button class="btn primaryAction" type="button" onclick="zapNext?.();">SUIVANT</button>';
      }
    });
  }
  const oldSetView=window.setView;
  if(typeof oldSetView==='function' && !oldSetView.__oryonSafeDiscoverGuard){
    const wrapped=function(){const out=oldSetView.apply(this,arguments); setTimeout(clean,30); setTimeout(clean,160); return out;};
    wrapped.__oryonSafeDiscoverGuard=true;
    window.setView=wrapped;
  }
  const oldToggle=window.oryonToggleDiscoverChatV2;
  window.oryonToggleDiscoverChatV2=function(){
    const root=document.getElementById('discover');
    if(!root) return;
    root.classList.toggle('oryonStableChatOpen');
    const chat=root.querySelector('.oryonStableChat');
    if(chat){chat.style.display=root.classList.contains('oryonStableChatOpen')?'block':''; setTimeout(()=>chat.scrollIntoView({behavior:'smooth',block:'nearest'}),40);}
    if(typeof oldToggle==='function' && oldToggle!==window.oryonToggleDiscoverChatV2){try{oldToggle.apply(this,arguments)}catch(_){}}
    clean();
  };
  window.oryonOpenCurrentUrlV2=()=>false;
  window.oryonOpenCurrentUrl=()=>false;
  window.oryonDeckUrlV2=()=>false;
  window.oryonDeckUrl=()=>false;
...
/* =========================================================
   SWAPP CLEAN DISCOVER PAGE — stable full page, no URL, real chat panel
...
(function installOryonCleanDiscoverPage(){
  ...
  const tags=x=>{const id=liveId(x);const arr=[];arr.push(id.platform==='oryon'?'Swapp':'Twitch'); if(id.viewers)arr.push(`${id.viewers} viewers`); if(id.game)arr.push(id.game); try{arr.push(...(signalTagsFor?.(x)||[]));}catch{} return [...new Set(arr.filter(Boolean))].slice(0,6);};
  ...
  function renderMainLoading(){const m=document.getElementById('ocMain'); if(m)m.innerHTML=`<div class="ocEmpty"><div><h2>Recherche…</h2><p>Swapp cherche un live ${safe(moodLabel(cleanState.mood))}.</p></div></div>`;}
  ...
  function nativePlayer(id){return `<div class="ocEmpty"><div><h2>${safe(id.name)}</h2><p>Live Swapp sélectionné. Ouvre la chaîne pour le lecteur natif complet.</p><button class="btn" onclick="openOryon?.('${safe(id.login)}')">Ouvrir la chaîne</button></div></div>`;}
  function nativeChat(){return `<div class="ocNativeChat"><div class="ocNativeLog"><p>Chat Swapp disponible sur la page chaîne.</p></div><div class="ocNativeForm"><input placeholder="Écrire sur Swapp…"><button class="btn secondary" type="button">Envoyer</button></div></div>`;}
  ...
  function renderSide(){
    ...
    side.innerHTML=`<section class="ocPanel"><h2>Pourquoi ce live ?</h2><div class="ocWhyList">${x?`<span><b>${safe(moodLabel(cleanState.mood))}</b><em>mood choisi</em></span><span><b>${id.viewers||'—'} viewers</b><em>taille humaine</em></span><span><b>${safe(id.game)}</b><em>catégorie</em></span><span><b>${safe(id.platform==='oryon'?'Swapp Live':'Twitch')}</b><em>source</em></span>`:'<span><b>En attente</b><em>relance une recherche</em></span>'}</div></section><section class="ocPanel"><h2>Actions</h2><p class="small">Avant lecture : Pas ouf / Voir / J’aime. Après Voir : Lurk / Chat / Suivant.</p><button class="btn secondary" type="button" onclick="ocFindLive()">Nouvelle recherche</button></section>`;
  }
  function renderDeck(){
...
/* =========================================================
   SWAPP DISCOVER UX REPAIR — not stretched, clearer desktop/mobile, category launches the chosen game
   ========================================================= */
(function installOryonDiscoverUxRepair(){
  ...
  function applyDiscoverCopy(){
    ...
    if(p) p.textContent='Choisis une ambiance, Swapp propose un live. Tu swipes, tu ouvres, tu lurk ou tu discutes.';
...
/* =========================================================
   SWAPP DISCOVER — FINAL REBUILD FROM ZERO
   Full width, grand mais non étiré, logique stable, catégories -> live
   ========================================================= */
(function installOryonDiscoverFinalRebuild(){
  const STYLE_ID='oryonDiscoverFinalRebuildStyle';
  document.getElementById(STYLE_ID)?.remove();
  const st=document.createElement('style');
  st.id=STYLE_ID;
  st.textContent=`
    body.oryonDiscoverFinalActive{overflow-x:hidden!important;background:#05070d!important;}
    body.oryonDiscoverFinalActive .app{width:100%!important;max-width:none!important;margin:0!important;padding:0!important;}
    body.oryonDiscoverFinalActive #discover.view.active{display:block!important;width:100%!important;max-width:none!important;margin:0!important;padding:0!important;overflow-x:hidden!important;background:#05070d!important;}
    #discover .sdPage{width:100%;min-height:calc(100vh - 56px);padding:clamp(18px,2vw,32px) clamp(18px,2.8vw,54px) 118px;background:radial-gradient(circle at 8% 0%,rgba(139,92,246,.25),transparent 34%),radial-gradient(circle at 100% 0%,rgba(34,211,238,.18),transparent 36%),linear-gradient(180deg,#06111a 0%,#05070d 54%,#05070d 100%);}
    #discover .sdShell{width:100%;max-width:1780px;margin:0 auto;display:grid;gap:22px;}
    #discover .sdHero{display:grid;grid-template-columns:minmax(360px,.78fr) minmax(520px,1.22fr);gap:22px;align-items:end;}
    #discover .sdHeroCopy{min-width:0;}
    #discover .sdEyebrow{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(139,92,246,.62);background:rgba(139,92,246,.17);border-radius:999px;padding:7px 11px;font-weight:950;font-size:12px;color:#fff;box-shadow:0 0 24px rgba(139,92,246,.12)}
    #discover .sdEyebrow:before{content:"";width:8px;height:8px;border-radius:999px;background:#22c55e;box-shadow:0 0 12px #22c55e;}
    #discover .sdHero h1{font-size:clamp(58px,6.2vw,122px);line-height:.82;letter-spacing:-.08em;margin:13px 0 12px;font-weight:1000;}
    #discover .sdHero p{max-width:760px;color:#c7d2e5;font-size:clamp(15px,1.12vw,20px);line-height:1.4;margin:0;}
    #discover .sdControls{border:1px solid rgba(148,163,184,.22);border-radius:26px;background:linear-gradient(180deg,rgba(15,23,42,.82),rgba(15,23,42,.55));box-shadow:0 24px 80px rgba(0,0,0,.28);padding:14px;display:grid;gap:10px;min-width:0;}
    #discover .sdMoodRail{display:flex;gap:9px;overflow:auto;scrollbar-width:thin;padding-bottom:3px;}
    #discover .sdMoodBtn{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.065);color:#fff;border-radius:999px;padding:9px 12px;font-weight:950;white-space:nowrap;cursor:pointer;transition:.16s ease;}
    #discover .sdMoodBtn.active,#discover .sdMoodBtn:hover{border-color:rgba(34,211,238,.85);background:linear-gradient(135deg,rgba(139,92,246,.36),rgba(34,211,238,.18));box-shadow:0 0 0 1px rgba(34,211,238,.12) inset;}
    #discover .sdSearch{display:grid;grid-template-columns:minmax(260px,1fr) 155px 105px auto;gap:9px;}
    #discover .sdSearch input,#discover .sdSearch select{min-width:0;border:1px solid rgba(148,163,184,.20);background:#070b13;color:#fff;border-radius:14px;padding:13px 14px;outline:none;}
    #discover .sdBtn{border:1px solid rgba(255,255,255,.14);background:linear-gradient(180deg,rgba(255,255,255,.10),rgba(255,255,255,.05));color:#fff;border-radius:16px;padding:13px 16px;font-weight:1000;cursor:pointer;transition:transform .14s ease,filter .14s ease,border-color .14s ease;}
    #discover .sdBtn:hover{transform:translateY(-1px);border-color:rgba(139,92,246,.72);}
    #discover .sdBtn.primary{background:linear-gradient(135deg,#8b5cf6,#d946ef);border-color:rgba(255,255,255,.10);box-shadow:0 18px 60px rgba(168,85,247,.24);}
    #discover .sdStageWrap{width:100%;display:grid;gap:14px;}
    #discover .sdStage{width:100%;max-width:1500px;margin:0 auto;position:relative;}
    #discover .sdCard{position:relative;width:100%;aspect-ratio:16/9;border:1px solid rgba(34,211,238,.38);border-radius:30px;overflow:hidden;background:#02040a;box-shadow:0 28px 95px rgba(0,0,0,.36),0 0 0 1px rgba(139,92,246,.08) inset;touch-action:pan-y;transform-origin:center bottom;transition:transform .18s ease,opacity .18s ease;}
    #discover .sdCard img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;filter:saturate(1.06) contrast(1.04);}
    #discover .sdCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.10),rgba(2,6,23,.20) 38%,rgba(2,6,23,.88));pointer-events:none;}
    #discover .sdTags{position:absolute;z-index:2;top:18px;left:18px;right:18px;display:flex;gap:8px;flex-wrap:wrap;align-items:flex-start;}
    #discover .sdTag{display:inline-flex;max-width:210px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;border:1px solid rgba(255,255,255,.16);background:rgba(3,7,18,.66);backdrop-filter:blur(12px);border-radius:999px;padding:7px 10px;color:#fff;font-weight:1000;font-size:12px;line-height:1;}
    #discover .sdCardText{position:absolute;z-index:2;left:24px;right:24px;bottom:24px;display:grid;gap:10px;}
    #discover .sdTitle{font-size:clamp(34px,4vw,72px);line-height:.92;letter-spacing:-.06em;margin:0;max-width:1250px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-shadow:0 14px 34px rgba(0,0,0,.48);}
    #discover .sdMeta{display:flex;gap:8px;align-items:center;flex-wrap:wrap;color:#e5edff;font-size:clamp(15px,1.3vw,22px);font-weight:900;text-shadow:0 8px 24px rgba(0,0,0,.45);}
    #discover .sdSwipeText{position:absolute;z-index:4;left:50%;top:48%;transform:translate(-50%,-50%);font-size:clamp(40px,7vw,104px);font-weight:1000;letter-spacing:-.06em;text-shadow:0 18px 50px rgba(0,0,0,.55);opacity:0;pointer-events:none;transition:opacity .08s ease,transform .08s ease;}
    #discover .sdSwipeText.like{color:#34d399}.sdCard.like .sdSwipeText.like{opacity:1;transform:translate(-50%,-50%) rotate(-3deg) scale(1.03)}
    #discover .sdSwipeText.nope{color:#fb7185}.sdCard.nope .sdSwipeText.nope{opacity:1;transform:translate(-50%,-50%) rotate(3deg) scale(1.03)}
    #discover .sdActions{width:100%;max-width:840px;margin:14px auto 0;display:grid;grid-template-columns:1fr 1.22fr 1fr;gap:12px;border:1px solid rgba(148,163,184,.18);border-radius:24px;background:rgba(15,23,42,.62);padding:12px;box-shadow:0 18px 60px rgba(0,0,0,.24);}
    #discover .sdActions .sdBtn{min-height:58px;font-size:18px;border-radius:18px;}
    #discover .sdActions .sdBtn.primary{font-size:20px;}
    #discover .sdWatchGrid{display:grid;grid-template-columns:minmax(0,1fr) minmax(360px,440px);gap:16px;align-items:stretch;}
    #discover .sdWatchGrid.noChat{grid-template-columns:1fr;}
    #discover .sdPlayerShell{border:1px solid rgba(34,211,238,.30);border-radius:28px;background:#02040a;overflow:hidden;box-shadow:0 28px 95px rgba(0,0,0,.36);}
    #discover .sdPlayerHead{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;border-bottom:1px solid rgba(148,163,184,.14);background:rgba(15,23,42,.70);font-weight:950;}
    #discover .sdPlayerBox{aspect-ratio:16/9;background:#000;}
    #discover .sdPlayerBox iframe{width:100%;height:100%;border:0;display:block;background:#000;}
    #discover .sdChatShell{border:1px solid rgba(148,163,184,.22);border-radius:28px;background:#111318;overflow:hidden;min-height:100%;display:flex;flex-direction:column;box-shadow:0 28px 95px rgba(0,0,0,.25);}
    #discover .sdChatHead{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,.12);font-weight:950;background:rgba(255,255,255,.035);}
    #discover .sdChatFrame{flex:1;min-height:580px;background:#18181b;}
    #discover .sdChatFrame iframe{width:100%;height:100%;border:0;display:block;background:#18181b;}
    #discover .sdDeck{width:100%;max-width:1500px;margin:4px auto 0;border:1px solid rgba(34,211,238,.24);border-radius:24px;background:linear-gradient(180deg,rgba(15,23,42,.74),rgba(15,23,42,.38));padding:16px;box-shadow:0 22px 70px rgba(0,0,0,.22);}
    #discover .sdDeckHead{display:flex;justify-content:space-between;align-items:start;gap:12px;margin-bottom:12px;}
    #discover .sdDeckHead h2{margin:0;font-size:24px;}.sdDeckHead p{margin:4px 0 0;color:#aab6cc;}
    #discover .sdDeckGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;}
    #discover .sdDeckSlot{position:relative;min-height:118px;border:1px solid rgba(148,163,184,.18);border-radius:16px;background:rgba(0,0,0,.22);overflow:hidden;padding:12px;display:flex;flex-direction:column;justify-content:flex-end;}
    #discover .sdDeckSlot img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.58;}
    #discover .sdDeckSlot:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.88));}
    #discover .sdDeckSlot>*{position:relative;z-index:1}.sdDeckSlot b{display:block}.sdDeckSlot span{display:block;color:#b7c3d7;font-size:12px;margin-top:2px}.sdDeckSlot.empty{align-items:center;justify-content:center;text-align:center;color:#6d7890}.sdDeckMini{display:flex;gap:6px;margin-top:8px}.sdDeckMini button{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:#fff;border-radius:10px;padding:6px 8px;font-weight:900;cursor:pointer;}
    #discover .sdFollow{width:100%;max-width:1500px;margin:0 auto;border:1px solid rgba(148,163,184,.16);border-radius:22px;background:rgba(15,23,42,.46);padding:14px;}
    #discover .sdFollowHead{display:flex;align-items:center;justify-content:space-between;gap:10px}.sdFollowHead h2{margin:0}.sdFollowRail{display:flex;gap:10px;overflow:auto;margin-top:12px;padding-bottom:4px}.sdFollowCard{min-width:260px;height:132px;border:1px solid rgba(255,255,255,.14);border-radius:18px;background:#080d18;overflow:hidden;position:relative;text-align:left;color:#fff;cursor:pointer}.sdFollowCard img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.7}.sdFollowCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.85))}.sdFollowCard div{position:absolute;left:12px;right:12px;bottom:11px;z-index:1}.sdFollowCard b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.sdFollowCard span{font-size:12px;color:#d7deee}
    #discover .sdEmpty{min-height:460px;border:1px dashed rgba(148,163,184,.26);border-radius:28px;display:grid;place-items:center;text-align:center;color:#b7c3d7;background:rgba(15,23,42,.42);padding:26px}.sdEmpty h2{font-size:clamp(32px,5vw,64px);margin:0 0 8px;color:#fff;letter-spacing:-.055em}.sdEmpty p{max-width:600px;margin:0 auto 16px}.sdSmall{font-size:12px;color:#9aa7bd}.sdPill{display:inline-flex;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);border-radius:999px;padding:6px 9px;font-size:12px;font-weight:950;color:#fff;}
    @media(max-width:1150px){#discover .sdHero{grid-template-columns:1fr}#discover .sdSearch{grid-template-columns:1fr 1fr 1fr}#discover .sdWatchGrid{grid-template-columns:1fr!important}#discover .sdChatFrame{min-height:470px}#discover .sdDeckGrid{grid-template-columns:repeat(2,minmax(0,1fr));}}
    @media(max-width:700px){#discover .sdPage{padding:14px 14px 98px}#discover .sdShell{gap:16px}#discover .sdHero h1{font-size:clamp(42px,13vw,64px)}#discover .sdHero p{font-size:13px}.sdHero .sdControls{border-radius:22px;padding:11px}#discover .sdMoodRail{gap:8px}#discover .sdMoodBtn{font-size:12px;padding:8px 10px}#discover .sdSearch{grid-template-columns:1fr}#discover .sdCard{aspect-ratio:auto;min-height:470px;border-radius:24px}#discover .sdTags{left:12px;right:12px;top:12px;gap:6px}#discover .sdTag{font-size:10px;padding:6px 8px;max-width:132px}#discover .sdCardText{left:14px;right:14px;bottom:14px}#discover .sdTitle{font-size:clamp(27px,8vw,40px)}#discover .sdMeta{font-size:13px}#discover .sdActions{grid-template-columns:1fr 1fr 1fr;padding:8px;gap:8px;border-radius:20px;margin-top:10px}#discover .sdActions .sdBtn{min-height:50px;font-size:13px;padding:9px 7px;border-radius:15px}#discover .sdActions .sdBtn.primary{font-size:14px}#discover .sdPlayerShell{border-radius:22px}#discover .sdPlayerHead{font-size:13px}#discover .sdChatShell{border-radius:22px}.sdChatFrame{min-height:430px!important}#discover .sdDeck{padding:12px;border-radius:22px}#discover .sdDeckHead{display:block}#discover .sdDeckGrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.sdDeckSlot{min-height:92px!important;padding:9px!important}#discover .sdFollow{border-radius:20px}.sdFollowHead{align-items:flex-start!important}.sdFollowCard{min-width:78vw;height:128px}}
  `;
  document.head.appendChild(st);

  const moods=[['petite-commu','Petite commu'],['chill','Chill'],['discussion','Discussion'],['nuit','Nuit calme'],['rp','RP'],['decouverte-jeu','Découverte jeu']];
  const sd=window.__oryonDiscoverFinalState=window.__oryonDiscoverFinalState||{mood:'petite-commu',mode:'preview',chat:false,loading:false,drag:null,category:null};
  const safe=s=>typeof esc==='function'?esc(s):String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  const enc=s=>encodeURIComponent(String(s??''));
  const host=()=>location.hostname||'localhost';
  const cur=()=>{const z=state.zap||{};return (z.items||[])[Number(z.index||0)]||null};
  function idOf(x={}){let id={};try{id=typeof liveIdentity==='function'?liveIdentity(x):{};}catch{}const login=String(id.login||x.login||x.user_login||x.broadcaster_login||x.host_login||x.room||'').toLowerCase();const platform=(id.platform||x.platform||x.source||(x.host_login||x.room?'oryon':'twitch')).toLowerCase();return {platform,login,name:id.name||x.display_name||x.user_name||x.broadcaster_name||x.host_name||login||'Live',title:id.title||x.title||`Live de ${login||'créateur'}`,game:id.game||x.game_name||x.game||x.category||'Live',viewers:Number(id.viewers??x.viewer_count??x.viewers??0)||0,img:id.img||thumbOf(x)};}
  function thumbOf(x={}){let img=x.img||x.image_url||x.thumbnail_url||''; if(img&&img.includes('{width}'))img=img.replace('{width}','1280').replace('{height}','720'); return img||'https://static-cdn.jtvnw.net/ttv-static/404_preview-1280x720.jpg';}
  function tagList(x){const id=idOf(x);return [...new Set([id.platform==='oryon'?'Swapp':'Twitch intégré',id.viewers?`${id.viewers} viewers`:'taille humaine',id.game,'chat lisible'].filter(Boolean))].slice(0,5);}
  function deckKey(){return 'oryon_final_deck_'+String(state.session?.local?.login||state.session?.twitch?.login||'guest').toLowerCase();}
  function readDeck(){try{const d=JSON.parse(localStorage.getItem(deckKey())||'[]');return Array.isArray(d)?d.slice(0,8):[]}catch{return []}}
  function writeDeck(d){try{localStorage.setItem(deckKey(),JSON.stringify((d||[]).slice(0,8)))}catch{} }
  function normalizeForDeck(x){const id=idOf(x);return {platform:id.platform,login:id.login,name:id.name,title:id.title,game:id.game,viewers:id.viewers,img:id.img,addedAt:Date.now()};}
  function setItems(items){const seen=new Set(), out=[];for(const raw of (items||[])){const item={...raw};const id=idOf(item);if(!id.login)continue;item.platform=id.platform;item.login=item.login||item.user_login||id.login;item.display_name=item.display_name||item.user_name||id.name;item.game_name=item.game_name||id.game;item.viewer_count=item.viewer_count??id.viewers;item.thumbnail_url=item.thumbnail_url||id.img;const key=id.platform+':'+id.login;if(seen.has(key))continue;seen.add(key);out.push(item);}state.zap=state.zap||{};state.zap.items=out;state.zap.index=0;state.zap.last=out[0]||null;}
  async function fetchLivePool({game='',autoWatch=false}={}){sd.loading=true;renderSdMainOnly();const mood=sd.mood||'petite-commu';const q=String(game||document.getElementById('sdQuery')?.value||'').trim();const max=Number(document.getElementById('sdMax')?.value||150);const lang=document.getElementById('sdLang')?.value||'fr';let items=[];try{if(game){const r=await api('/api/twitch/random-small-live?'+qs({game,max,language:lang})).catch(()=>null);if(r?.success&&r.target){items=[{...r.target,platform:'twitch',login:r.target.login||r.target.user_login,display_name:r.target.name||r.target.display_name,user_name:r.target.name,viewer_count:r.target.viewers,game_name:r.target.game||game,thumbnail_url:r.target.thumbnail_url||r.target.img,title:r.target.title}];}}
      if(!items.length){const r=await api('/api/oryon/discover/find-live?'+qs({q,mood,max,lang,source:'both'})).catch(()=>null);items=r?.items||r?.results||(r?.target?[r.target]:[]);} 
      if(!items.length){const r=await api('/api/twitch/streams/small?'+qs({lang,min:0,max})).catch(()=>null);items=r?.items||[];}
      if(!items.length){const r=await api('/api/native/lives').catch(()=>null);items=(r?.items||[]).map(x=>({...x,platform:'oryon'}));}
    }catch(e){console.warn('discover final fetch',e)}
    setItems(items.slice(0,30));sd.loading=false;sd.mode=autoWatch&&cur()?'watch':'preview';sd.chat=false;renderSdAll();}

  function playerHtml(x){const id=idOf(x);if(id.platform==='twitch')return `<iframe allowfullscreen allow="autoplay; fullscreen" src="https://player.twitch.tv/?channel=${enc(id.login)}&parent=${enc(host())}&autoplay=true&muted=false"></iframe>`;return `<div class="sdEmpty"><div><h2>${safe(id.name)}</h2><p>Live Swapp sélectionné.</p><button class="sdBtn primary" onclick="openOryon?.('${safe(id.login)}')">Ouvrir la chaîne</button></div></div>`;}
  function chatHtml(x){const id=idOf(x);if(id.platform==='twitch')return `<iframe src="https://www.twitch.tv/embed/${enc(id.login)}/chat?parent=${enc(host())}&darkpopout" title="Chat Twitch ${safe(id.login)}"></iframe>`;return `<div class="sdEmpty"><div><h2>Chat Swapp</h2><p>Le chat Swapp complet est disponible sur la page chaîne.</p></div></div>`;}
  function previewHtml(){const x=cur();if(sd.loading)return `<div class="sdEmpty"><div><h2>Recherche…</h2><p>Swapp cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="sdEmpty"><div><h2>Aucun live trouvé</h2><p>Change de mood ou relance la recherche.</p><button class="sdBtn primary" onclick="sdFindLive()">Chercher un live</button></div></div>`;const id=idOf(x);return `<article id="sdSwipeCard" class="sdCard"><img src="${safe(id.img)}" alt=""><div class="sdTags">${tagList(x).map(t=>`<span class="sdTag">${safe(t)}</span>`).join('')}</div><div class="sdSwipeText nope">PAS OUF</div><div class="sdSwipeText like">J’AIME</div><div class="sdCardText"><h2 class="sdTitle">${safe(id.title)}</h2><div class="sdMeta"><span>${safe(id.name)}</span><span>·</span><span>${safe(id.game)}</span><span>·</span><span>${id.viewers} viewers</span></div></div></article><div class="sdActions"><button class="sdBtn" onclick="sdNope()">Pas ouf</button><button class="sdBtn primary" onclick="sdWatch()">Voir</button><button class="sdBtn" onclick="sdLike()">J’aime</button></div>`;}
  function watchHtml(){const x=cur();if(!x)return previewHtml();const id=idOf(x);return `<div class="sdWatchGrid ${sd.chat?'':'noChat'}"><main class="sdPlayerShell"><div class="sdPlayerHead"><div><b>${safe(id.name)}</b><div class="sdSmall">${safe(id.game)} · ${id.viewers} viewers</div></div><span class="sdPill">${safe(id.platform==='twitch'?'Twitch':'Swapp')}</span></div><div class="sdPlayerBox">${playerHtml(x)}</div><div class="sdActions"><button class="sdBtn" onclick="sdLurk()">LURK</button><button class="sdBtn" onclick="sdToggleChat()">CHAT</button><button class="sdBtn primary" onclick="sdNext()">SUIVANT</button></div></main>${sd.chat?`<aside class="sdChatShell"><div class="sdChatHead"><span>Chat du stream</span><button class="sdBtn" onclick="sdToggleChat()">Fermer</button></div><div class="sdChatFrame">${chatHtml(x)}</div></aside>`:''}</div>`;}
  function deckHtml(){const d=readDeck();return `<section class="sdDeck"><div class="sdDeckHead"><div><h2>DeckLurker</h2><p>Garde tes lives sous la main et switch sans ouvrir dix onglets.</p></div><span class="sdPill">${d.length}/8</span></div><div class="sdDeckGrid">${Array.from({length:8},(_,i)=>{const s=d[i];if(!s)return `<article class="sdDeckSlot empty"><b>Case ${i+1}</b><span>Vide</span></article>`;return `<article class="sdDeckSlot">${s.img?`<img src="${safe(s.img)}" alt="">`:''}<b>${safe(s.name||s.login)}</b><span>${safe(s.game||s.title||'Live')}</span><div class="sdDeckMini"><button onclick="sdOpenDeck(${i})">Voir</button><button onclick="sdClearDeck(${i})">Vider</button></div></article>`;}).join('')}</div></section>`;}
  function followedHtml(){return `<section class="sdFollow" id="sdFollow"><div class="sdFollowHead"><div><h2>Suivis Twitch en ligne</h2><p class="sdSmall">Séparé des recommandations.</p></div>${state.session?.twitch?'<span class="sdPill">Connecté</span>':'<button class="sdBtn primary" onclick="connectTwitch?.()">Connecter Twitch</button>'}</div><div class="sdFollowRail" id="sdFollowRail">${state.session?.twitch?'<div class="sdSmall">Chargement…</div>':'<div class="sdSmall">Connexion Twitch nécessaire.</div>'}</div></section>`;}
  async function loadFollowed(){const rail=document.getElementById('sdFollowRail');if(!rail||!state.session?.twitch)return;let items=[];try{let r=await api('/api/twitch/followed/live').catch(()=>null);items=r?.items||r?.streams||[];if(!items.length){r=await api('/followed_streams').catch(()=>null);items=r?.items||r?.streams||[];}}catch{}rail.innerHTML=items.length?items.slice(0,18).map((x,i)=>{const id=idOf({...x,platform:'twitch'});return `<button class="sdFollowCard" onclick="sdUseFollow(${i})">${id.img?`<img src="${safe(id.img)}" alt="">`:''}<div><b>${safe(id.name)}</b><span>${safe(id.game)} · ${id.viewers}</span></div></button>`;}).join(''):'<div class="sdSmall">Aucun suivi en ligne.</div>';window.__sdFollowed=items;}
  function renderSdAll(){const el=document.getElementById('discover');if(!el)return;document.body.classList.add('oryonDiscoverFinalActive');const currentMood=sd.mood||'petite-commu';el.innerHTML=`<div class="sdPage"><div class="sdShell"><header class="sdHero"><div class="sdHeroCopy"><span class="sdEyebrow">Swap ton mood</span><h1>Swap ton mood.</h1><p>Choisis une ambiance, Swapp propose un live. Tu swipes, tu ouvres, tu lurk ou tu discutes.</p></div><div class="sdControls"><div class="sdMoodRail">${moods.map(([m,l])=>`<button class="sdMoodBtn ${m===currentMood?'active':''}" onclick="sdSetMood('${m}')">${safe(l)}</button>`).join('')}</div><div class="sdSearch"><input id="sdQuery" value="${safe(sd.category||'')}" placeholder="jeu, créateur, ambiance"><select id="sdMax"><option value="50">≤50 viewers</option><option value="150" selected>≤150 viewers</option><option value="300">≤300 viewers</option></select><select id="sdLang"><option value="fr" selected>FR</option><option value="en">EN</option></select><button class="sdBtn primary" onclick="sdFindLive()">Chercher un live</button></div></div></header><section class="sdStageWrap"><div class="sdStage" id="sdMain">${sd.mode==='watch'?watchHtml():previewHtml()}</div></section><div id="sdDeckMount">${deckHtml()}</div><div id="sdFollowMount">${followedHtml()}</div></div></div>`;bindSwipe();loadFollowed();cleanupOldDiscoverArtifacts();}
  function renderSdMainOnly(){const m=document.getElementById('sdMain');if(m)m.innerHTML=sd.mode==='watch'?watchHtml():previewHtml();}
  function cleanupOldDiscoverArtifacts(){const root=document.getElementById('discover');if(!root)return;[...root.querySelectorAll('button')].forEach(b=>{if(/^URL$/i.test((b.textContent||'').trim()))b.remove();});}
  function bindSwipe(){const card=document.getElementById('sdSwipeCard');if(!card)return;let startX=0,startY=0,dx=0,active=false;const move=x=>{dx=x-startX;const rot=Math.max(-12,Math.min(12,dx/16));card.style.transform=`translateX(${dx*.20}px) rotate(${rot}deg)`;card.classList.toggle('like',dx>38);card.classList.toggle('nope',dx<-38);};card.addEventListener('pointerdown',e=>{active=true;startX=e.clientX;startY=e.clientY;dx=0;card.setPointerCapture?.(e.pointerId);});card.addEventListener('pointermove',e=>{if(!active)return;if(Math.abs(e.clientY-startY)>70&&Math.abs(dx)<35)return;move(e.clientX);});card.addEventListener('pointerup',()=>{if(!active)return;active=false;const finalDx=dx;card.style.transform='';card.classList.remove('like','nope');if(finalDx>90)sdLike();else if(finalDx<-90)sdNope();});card.addEventListener('pointercancel',()=>{active=false;card.style.transform='';card.classList.remove('like','nope');});}
  window.sdSetMood=function(m){sd.mood=m;sd.category='';sd.mode='preview';sd.chat=false;fetchLivePool();};
  window.sdFindLive=function(){sd.category='';sd.mode='preview';sd.chat=false;return fetchLivePool();};
  window.sdWatch=function(){if(!cur())return;sd.mode='watch';sd.chat=false;renderSdAll();};
  window.sdToggleChat=function(){if(sd.mode!=='watch')sd.mode='watch';sd.chat=!sd.chat;renderSdAll();};
  window.sdNext=function(){const items=state.zap?.items||[];if(items.length>1){state.zap.index=(Number(state.zap.index||0)+1)%items.length;state.zap.last=cur();}sd.mode='preview';sd.chat=false;renderSdAll();};
  window.sdNope=function(){sdNext();};
  window.sdLike=function(){try{const id=idOf(cur()||{});const key='oryon_discover_likes';const arr=JSON.parse(localStorage.getItem(key)||'[]');if(id.login&&!arr.some(x=>x.platform===id.platform&&x.login===id.login)){arr.unshift({platform:id.platform,login:id.login,name:id.name,game:id.game,ts:Date.now()});localStorage.setItem(key,JSON.stringify(arr.slice(0,100)));}}catch{}sdNext();};
  window.sdLurk=function(){const x=cur();if(!x)return;const d=readDeck();const meta=normalizeForDeck(x);if(!meta.login)return toast?.('Aucun live à ajouter');const key=meta.platform+':'+meta.login;if(!d.some(s=>s&&s.platform+':'+s.login===key))d.unshift(meta);writeDeck(d.slice(0,8));toast?.('Ajouté au DeckLurker');renderSdAll();};
  window.sdOpenDeck=function(i){const s=readDeck()[i];if(!s)return;setItems([s,...(state.zap?.items||[])]);sd.mode='watch';sd.chat=false;renderSdAll();};
  window.sdClearDeck=function(i){const d=readDeck();d.splice(i,1);writeDeck(d);renderSdAll();};
  window.sdUseFollow=function(i){const x=(window.__sdFollowed||[])[i];if(!x)return;setItems([{...x,platform:'twitch'},...(state.zap?.items||[])]);sd.mode='watch';sd.chat=false;renderSdAll();};
  window.renderDiscover=renderDiscover=async function renderDiscoverFinal(){document.body.classList.add('oryonDiscoverFinalActive');renderSdAll();if(!(state.zap?.items||[]).length&&!sd.loading)await fetchLivePool();};
  window.pickCat=async function pickCatFinal(name,img=''){sd.category=String(name||'').trim();sd.mood='decouverte-jeu';await setView('discover');await fetchLivePool({game:sd.category,autoWatch:true});};
  window.pickCatEncoded=function pickCatEncodedFinal(name,img=''){return window.pickCat(decodeURIComponent(name||''),decodeURIComponent(img||''));};
  const oldSetView=window.setView;
  if(typeof oldSetView==='function'&&!oldSetView.__sdFinalWrapped){const wrapped=function(v){document.body.classList.toggle('oryonDiscoverFinalActive',v==='discover');return oldSetView.apply(this,arguments);};wrapped.__sdFinalWrapped=true;window.setView=wrapped;}
})();

/* =========================================================
   SWAPP DISCOVER — PRODUCT GRADE OVERRIDE
   Rebuilds only the Discover page: full width, large card/player, stable chat/deck.
   ========================================================= */
(function installOryonDiscoverProductGrade(){
  const STYLE_ID='oryonDiscoverProductGradeStyle';
  document.getElementById(STYLE_ID)?.remove();
  const css=document.createElement('style');
  css.id=STYLE_ID;
  css.textContent=`
    body.oryonDiscoverProduct{background:#05070d!important;overflow-x:hidden!important}
    body.oryonDiscoverProduct .app{width:100%!important;max-width:none!important;margin:0!important;padding:0!important}
    body.oryonDiscoverProduct #discover.view.active{display:block!important;width:100%!important;max-width:none!important;margin:0!important;padding:0!important;background:#05070d!important;overflow-x:hidden!important}
    #discover .mxPage{min-height:calc(100vh - 50px);padding:clamp(18px,2.3vw,40px) clamp(18px,3vw,60px) 120px;background:radial-gradient(circle at 0% 0%,rgba(139,92,246,.32),transparent 33%),radial-gradient(circle at 100% 0%,rgba(34,211,238,.22),transparent 34%),linear-gradient(180deg,#06111a 0%,#05070d 62%)}
    #discover .mxShell{max-width:1840px;margin:0 auto;display:grid;gap:22px}
    #discover .mxHero{display:grid;grid-template-columns:minmax(300px,.72fr) minmax(560px,1.28fr);gap:22px;align-items:end}
    #discover .mxHero h1{font-size:clamp(56px,5.4vw,104px);line-height:.86;letter-spacing:-.075em;margin:12px 0 10px;font-weight:1000}
    #discover .mxHero p{max-width:780px;color:#c6d2e5;font-size:clamp(15px,1.05vw,20px);line-height:1.4;margin:0}
    #discover .mxEyebrow{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(139,92,246,.64);background:rgba(139,92,246,.18);border-radius:999px;padding:7px 11px;font-size:12px;font-weight:1000}.mxEyebrow:before{content:"";width:8px;height:8px;border-radius:50%;background:#22c55e;box-shadow:0 0 14px #22c55e}
    #discover .mxControls{border:1px solid rgba(148,163,184,.20);border-radius:28px;background:linear-gradient(180deg,rgba(15,23,42,.86),rgba(15,23,42,.56));box-shadow:0 24px 80px rgba(0,0,0,.28);padding:14px;display:grid;gap:10px;min-width:0}
    #discover .mxMoodRail{display:flex;gap:9px;overflow:auto;padding-bottom:4px;scrollbar-width:thin}
    #discover .mxMoodBtn{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.065);color:#fff;border-radius:999px;padding:10px 13px;font-weight:1000;white-space:nowrap;cursor:pointer}.mxMoodBtn.active,#discover .mxMoodBtn:hover{border-color:rgba(34,211,238,.85);background:linear-gradient(135deg,rgba(139,92,246,.40),rgba(34,211,238,.18))}
    #discover .mxSearch{display:grid;grid-template-columns:minmax(240px,1fr) 160px 108px auto;gap:9px}.mxSearch input,.mxSearch select{min-width:0;border:1px solid rgba(148,163,184,.22);background:#070b13;color:#fff;border-radius:15px;padding:13px 14px;outline:none;font-weight:800}
    #discover .mxBtn{border:1px solid rgba(255,255,255,.14);background:linear-gradient(180deg,rgba(255,255,255,.10),rgba(255,255,255,.05));color:#fff;border-radius:16px;padding:13px 16px;font-weight:1000;cursor:pointer;white-space:nowrap}.mxBtn:hover{border-color:rgba(139,92,246,.75);transform:translateY(-1px)}.mxBtn.primary{background:linear-gradient(135deg,#8b5cf6,#d946ef);box-shadow:0 18px 60px rgba(168,85,247,.25)}
    #discover .mxGrid{display:grid;grid-template-columns:1fr;gap:18px;align-items:start}.mxGrid.watch{grid-template-columns:minmax(0,1fr) minmax(350px,450px)}.mxGrid.watch.noChat{grid-template-columns:1fr minmax(310px,380px)}
    #discover .mxMain{min-width:0}.mxSide{min-width:0;display:grid;gap:14px;align-content:start}.mxPanel{border:1px solid rgba(148,163,184,.20);border-radius:24px;background:linear-gradient(180deg,rgba(15,23,42,.72),rgba(15,23,42,.42));padding:16px;box-shadow:0 22px 70px rgba(0,0,0,.22)}.mxPanel h2{margin:0 0 10px;font-size:22px}.mxPanel p{margin:0;color:#aebad0;line-height:1.4}.mxSignals{display:grid;gap:8px;margin-top:12px}.mxSignal{display:flex;justify-content:space-between;gap:12px;border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.045);border-radius:14px;padding:10px 11px;font-weight:950}.mxSignal span{color:#9aa8be;font-size:12px}
    #discover .mxCard{position:relative;width:100%;aspect-ratio:16/9;min-height:clamp(520px,58vh,760px);border:1px solid rgba(34,211,238,.40);border-radius:32px;overflow:hidden;background:#02040a;box-shadow:0 30px 105px rgba(0,0,0,.38),0 0 0 1px rgba(139,92,246,.08) inset;touch-action:pan-y;transform-origin:center bottom;transition:transform .18s ease}.mxCard img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;filter:saturate(1.06) contrast(1.04)}.mxCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.10),rgba(2,6,23,.22) 36%,rgba(2,6,23,.90))}
    #discover .mxTags{position:absolute;z-index:2;top:18px;left:18px;right:18px;display:flex;gap:8px;flex-wrap:wrap}.mxTag{display:inline-flex;max-width:215px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;border:1px solid rgba(255,255,255,.16);background:rgba(3,7,18,.68);backdrop-filter:blur(12px);border-radius:999px;padding:7px 10px;color:#fff;font-weight:1000;font-size:12px;line-height:1}
    #discover .mxCardText{position:absolute;z-index:2;left:26px;right:26px;bottom:24px;display:grid;gap:10px}.mxTitle{font-size:clamp(38px,4.1vw,76px);line-height:.92;letter-spacing:-.065em;margin:0;max-width:1260px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-shadow:0 14px 34px rgba(0,0,0,.50)}.mxMeta{display:flex;gap:8px;flex-wrap:wrap;color:#e5edff;font-size:clamp(15px,1.25vw,22px);font-weight:950;text-shadow:0 8px 24px rgba(0,0,0,.45)}
    #discover .mxSwipe{position:absolute;z-index:4;left:50%;top:48%;transform:translate(-50%,-50%);font-size:clamp(42px,7vw,108px);font-weight:1000;letter-spacing:-.06em;text-shadow:0 18px 50px rgba(0,0,0,.55);opacity:0;pointer-events:none}.mxSwipe.like{color:#34d399}.mxCard.like .mxSwipe.like{opacity:1}.mxSwipe.nope{color:#fb7185}.mxCard.nope .mxSwipe.nope{opacity:1}
    #discover .mxActions{width:100%;max-width:860px;margin:14px auto 0;display:grid;grid-template-columns:1fr 1.22fr 1fr;gap:12px;border:1px solid rgba(148,163,184,.18);border-radius:24px;background:rgba(15,23,42,.68);padding:12px;box-shadow:0 18px 60px rgba(0,0,0,.24)}.mxActions .mxBtn{min-height:58px;font-size:18px;border-radius:18px}.mxActions .mxBtn.primary{font-size:20px}
    #discover .mxPlayer{border:1px solid rgba(34,211,238,.32);border-radius:30px;background:#02040a;overflow:hidden;box-shadow:0 30px 105px rgba(0,0,0,.38)}.mxPlayerHead{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;border-bottom:1px solid rgba(148,163,184,.14);background:rgba(15,23,42,.74);font-weight:1000}.mxSmall{color:#9fafc6;font-size:12px}.mxPill{display:inline-flex;border:1px solid rgba(34,211,238,.36);background:rgba(34,211,238,.10);border-radius:999px;padding:7px 10px;font-weight:1000;font-size:12px}.mxPlayerBox{aspect-ratio:16/9;background:#000}.mxPlayerBox iframe{width:100%;height:100%;border:0;display:block;background:#000}
    #discover .mxChat{border:1px solid rgba(148,163,184,.22);border-radius:30px;background:#111318;overflow:hidden;min-height:100%;display:flex;flex-direction:column;box-shadow:0 30px 95px rgba(0,0,0,.25)}.mxChatHead{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,.12);font-weight:1000}.mxChatFrame{flex:1;min-height:620px;background:#18181b}.mxChatFrame iframe{width:100%;height:100%;border:0;display:block;background:#18181b}
    #discover .mxEmpty{min-height:520px;border:1px dashed rgba(148,163,184,.24);border-radius:30px;background:linear-gradient(135deg,rgba(139,92,246,.11),rgba(34,211,238,.05));display:grid;place-items:center;text-align:center;padding:24px}.mxEmpty h2{font-size:clamp(34px,4.5vw,80px);line-height:.92;margin:0 0 10px;letter-spacing:-.06em}
    #discover .mxDeck,#discover .mxFollow{border:1px solid rgba(34,211,238,.24);border-radius:26px;background:linear-gradient(180deg,rgba(15,23,42,.76),rgba(15,23,42,.40));padding:16px;box-shadow:0 22px 70px rgba(0,0,0,.22)}.mxDeckHead,.mxFollowHead{display:flex;justify-content:space-between;align-items:start;gap:12px;margin-bottom:12px}.mxDeckHead h2,.mxFollowHead h2{margin:0;font-size:24px}.mxDeckHead p,.mxFollowHead p{margin:4px 0 0;color:#aab6cc}.mxDeckGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.mxDeckSlot{position:relative;min-height:120px;border:1px solid rgba(148,163,184,.18);border-radius:17px;background:rgba(0,0,0,.22);overflow:hidden;padding:12px;display:flex;flex-direction:column;justify-content:flex-end}.mxDeckSlot img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.58}.mxDeckSlot:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.90))}.mxDeckSlot>*{position:relative;z-index:1}.mxDeckSlot.empty{align-items:center;justify-content:center;text-align:center;color:#6d7890}.mxDeckMini{display:flex;gap:6px;margin-top:8px}.mxDeckMini button{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:white;border-radius:10px;padding:6px 8px;font-weight:900;cursor:pointer}
    .mxFollowRail{display:flex;gap:10px;overflow:auto}.mxFollowCard{min-width:210px;height:120px;position:relative;border:1px solid rgba(148,163,184,.18);border-radius:18px;overflow:hidden;background:#0b1220;color:#fff;text-align:left;padding:12px;display:flex;align-items:flex-end;cursor:pointer}.mxFollowCard img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.58}.mxFollowCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.90))}.mxFollowCard div{position:relative;z-index:1}.mxFollowCard span{font-size:12px;color:#c8d3e7;display:block}
    @media(max-width:1250px){#discover .mxHero{grid-template-columns:1fr}.mxSearch{grid-template-columns:1fr 1fr 1fr auto!important}#discover .mxGrid.watch,#discover .mxGrid.watch.noChat{grid-template-columns:1fr}.mxSide{grid-template-columns:1fr 1fr}.mxCard{min-height:460px!important}.mxChatFrame{min-height:500px!important}.mxDeckGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:760px){#discover .mxPage{padding:14px 14px 98px}.mxHero h1{font-size:clamp(42px,13vw,64px)!important}.mxHero p{font-size:13px!important}.mxControls{border-radius:22px!important;padding:11px!important}.mxSearch{grid-template-columns:1fr!important}.mxSide{display:none!important}.mxCard{aspect-ratio:auto!important;min-height:470px!important;border-radius:24px!important}.mxTags{left:12px!important;right:12px!important;top:12px!important;gap:6px!important}.mxTag{font-size:10px!important;padding:6px 8px!important;max-width:132px!important}.mxCardText{left:14px!important;right:14px!important;bottom:14px!important}.mxTitle{font-size:clamp(27px,8vw,40px)!important}.mxMeta{font-size:13px!important}.mxActions{grid-template-columns:1fr 1fr 1fr!important;padding:8px!important;gap:8px!important;border-radius:20px!important}.mxActions .mxBtn{min-height:50px!important;font-size:13px!important;padding:9px 7px!important;border-radius:15px!important}.mxPlayer,.mxChat{border-radius:22px!important}.mxChatFrame{min-height:430px!important}.mxDeck{padding:12px!important;border-radius:22px!important}.mxDeckGrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.mxDeckSlot{min-height:92px!important}.mxFollowCard{min-width:78vw;height:128px}}
  `;
  document.head.appendChild(css);

  const moods=[['petite-commu','Petite commu'],['chill','Chill'],['discussion','Discussion'],['nuit-calme','Nuit calme'],['rp','RP'],['decouverte-jeu','Découverte jeu']];
  const mx={mood:localStorage.getItem('oryon_mx_mood')||'petite-commu',mode:'preview',chat:false,loading:false,category:''};
  const H=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  const E=encodeURIComponent, host=()=>location.hostname||'localhost', img=u=>String(u||'').replace('{width}','1280').replace('{height}','720');
  function id(x={}){const platform=(x.platform||x.source||'twitch').toLowerCase().includes('oryon')?'oryon':'twitch';const login=String(x.login||x.user_login||x.broadcaster_login||x.channel_login||x.name||x.user_name||x.display_name||'');const name=String(x.display_name||x.user_name||x.name||login||'Live');return{platform,login,name,game:String(x.game_name||x.game||x.category||'Just Chatting'),viewers:Number(x.viewer_count??x.viewers??0)||0,title:String(x.title||x.status||`Live de ${name}`),img:img(x.thumbnail_url||x.img||x.image||'')};}
  function cur(){const z=state.zap||{};return (z.items||[])[Number(z.index||0)]||z.last||null}
  function setItems(items){const seen=new Set(),out=[];(items||[]).forEach(raw=>{const o={...raw}, n=id(o);if(!n.login)return;o.platform=n.platform;o.login=o.login||o.user_login||n.login;o.display_name=o.display_name||o.user_name||n.name;o.game_name=o.game_name||n.game;o.viewer_count=o.viewer_count??n.viewers;o.thumbnail_url=o.thumbnail_url||n.img;const k=n.platform+':'+n.login.toLowerCase();if(seen.has(k))return;seen.add(k);out.push(o)});state.zap=state.zap||{};state.zap.items=out;state.zap.index=0;state.zap.last=out[0]||null}
  function deckKey(){return 'oryon_deck_lurker_v4:'+String(state.session?.local?.login||state.session?.twitch?.login||'guest').toLowerCase()}
  function readDeck(){try{const d=JSON.parse(localStorage.getItem(deckKey())||'[]');return Array.isArray(d)?d.slice(0,8):[]}catch{return []}}
  function writeDeck(d){try{localStorage.setItem(deckKey(),JSON.stringify((d||[]).slice(0,8)))}catch{}}
  function tags(x){const n=id(x);return [n.platform==='twitch'?'Twitch intégré':'Swapp natif',n.viewers?`${n.viewers} viewers`:'petit live',n.game,'entrée douce'].filter(Boolean).slice(0,5)}
  async function fetchPool({game='',autoWatch=false}={}){mx.loading=true;renderMain();const q=String(game||mx.category||document.getElementById('mxQuery')?.value||'').trim();const max=Number(document.getElementById('mxMax')?.value||150);const lang=document.getElementById('mxLang')?.value||'fr';let items=[];try{if(q){const r=await api('/api/twitch/random-small-live?'+qs({game:q,max,language:lang})).catch(()=>null);if(r?.success&&r.target)items=[{...r.target,platform:'twitch',login:r.target.login||r.target.user_login,display_name:r.target.name||r.target.display_name,user_name:r.target.name,viewer_count:r.target.viewers,game_name:r.target.game||q,thumbnail_url:r.target.thumbnail_url||r.target.img,title:r.target.title}]}
    if(!items.length){const r=await api('/api/oryon/discover/find-live?'+qs({q,mood:mx.mood,max,lang,source:'both'})).catch(()=>null);items=r?.items||r?.results||(r?.target?[r.target]:[])}
    if(!items.length){const r=await api('/api/twitch/streams/small?'+qs({lang,min:0,max})).catch(()=>null);items=r?.items||r?.streams||[]}
    if(!items.length){const r=await api('/api/native/lives').catch(()=>null);items=(r?.items||[]).map(x=>({...x,platform:'oryon'}))}
  }catch(e){console.warn('mx discover fetch',e)}setItems(items.slice(0,32));mx.loading=false;mx.mode=autoWatch&&cur()?'watch':'preview';mx.chat=false;renderAll()}
  function player(x){const n=id(x);return n.platform==='twitch'?`<iframe allowfullscreen allow="autoplay; fullscreen" src="https://player.twitch.tv/?channel=${E(n.login)}&parent=${E(host())}&autoplay=true&muted=false"></iframe>`:`<div class="mxEmpty"><div><h2>${H(n.name)}</h2><p>Live Swapp sélectionné.</p><button class="mxBtn primary" onclick="openOryon?.('${H(n.login)}')">Ouvrir la chaîne</button></div></div>`}
  function chat(x){const n=id(x);return n.platform==='twitch'?`<iframe title="Chat Twitch" src="https://www.twitch.tv/embed/${E(n.login)}/chat?parent=${E(host())}&darkpopout"></iframe>`:`<div class="mxEmpty"><div><h2>Chat Swapp</h2><p>Disponible sur la page chaîne.</p></div></div>`}
  function preview(){const x=cur();if(mx.loading)return `<div class="mxEmpty"><div><h2>Recherche…</h2><p>Swapp cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="mxEmpty"><div><h2>Aucun live trouvé.</h2><p>Change de mood ou relance une recherche.</p><button class="mxBtn primary" onclick="mxFindLive()">Chercher un live</button></div></div>`;const n=id(x);return `<article id="mxSwipeCard" class="mxCard">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div class="mxTags">${tags(x).map(t=>`<span class="mxTag">${H(t)}</span>`).join('')}</div><div class="mxSwipe nope">PAS OUF</div><div class="mxSwipe like">J’AIME</div><div class="mxCardText"><h2 class="mxTitle">${H(n.title)}</h2><div class="mxMeta"><span>${H(n.name)}</span><span>·</span><span>${H(n.game)}</span><span>·</span><span>${n.viewers} viewers</span></div></div></article><div class="mxActions"><button class="mxBtn" onclick="mxNope()">Pas ouf</button><button class="mxBtn primary" onclick="mxWatch()">Voir</button><button class="mxBtn" onclick="mxLike()">J’aime</button></div>`}
  function side(){const x=cur(), n=x?id(x):null;return `<aside class="mxSide"><section class="mxPanel"><h2>Pourquoi ce live ?</h2>${n?`<div class="mxSignals"><div class="mxSignal"><b>${H(moods.find(m=>m[0]===mx.mood)?.[1]||'Ambiance')}</b><span>mood</span></div><div class="mxSignal"><b>${n.viewers} viewers</b><span>taille humaine</span></div><div class="mxSignal"><b>${H(n.game)}</b><span>catégorie</span></div><div class="mxSignal"><b>${n.platform==='twitch'?'Twitch':'Swapp'}</b><span>source</span></div></div>`:'<p>Choisis un mood et lance une proposition.</p>'}</section><section class="mxPanel"><h2>Swap clair</h2><p>Avant lecture : Pas ouf / Voir / J’aime. Après Voir : Lurk / Chat / Suivant.</p></section></aside>`}
  function watch(){const x=cur();if(!x)return preview();const n=id(x);return `<main class="mxPlayer"><div class="mxPlayerHead"><div><b>${H(n.name)}</b><div class="mxSmall">${H(n.game)} · ${n.viewers} viewers</div></div><span class="mxPill">${n.platform==='twitch'?'Twitch':'Swapp'}</span></div><div class="mxPlayerBox">${player(x)}</div><div class="mxActions"><button class="mxBtn" onclick="mxLurk()">LURK</button><button class="mxBtn" onclick="mxToggleChat()">CHAT</button><button class="mxBtn primary" onclick="mxNext()">SUIVANT</button></div></main>`}
  function deck(){const d=readDeck();return `<section class="mxDeck"><div class="mxDeckHead"><div><h2>DeckLurker</h2><p>Garde tes lives sous la main et switch sans ouvrir dix onglets.</p></div><span class="mxPill">${d.length}/8</span></div><div class="mxDeckGrid">${Array.from({length:8},(_,i)=>{const s=d[i];if(!s)return `<article class="mxDeckSlot empty"><b>Case ${i+1}</b><span>Vide</span></article>`;return `<article class="mxDeckSlot">${s.img?`<img src="${H(s.img)}" alt="">`:''}<b>${H(s.name||s.login)}</b><span>${H(s.game||s.title||'Live')}</span><div class="mxDeckMini"><button onclick="mxOpenDeck(${i})">Voir</button><button onclick="mxClearDeck(${i})">Vider</button></div></article>`}).join('')}</div></section>`}
  function followed(){return `<section class="mxFollow"><div class="mxFollowHead"><div><h2>Suivis Twitch en ligne</h2><p class="mxSmall">Séparé des recommandations.</p></div>${state.session?.twitch?'<span class="mxPill">Connecté</span>':'<button class="mxBtn primary" onclick="connectTwitch?.()">Connecter Twitch</button>'}</div><div class="mxFollowRail" id="mxFollowRail">${state.session?.twitch?'<div class="mxSmall">Chargement…</div>':'<div class="mxSmall">Connexion Twitch nécessaire.</div>'}</div></section>`}
  async function loadFollowed(){const rail=document.getElementById('mxFollowRail');if(!rail||!state.session?.twitch)return;let items=[];try{let r=await api('/api/twitch/followed/live').catch(()=>null);items=r?.items||r?.streams||[];if(!items.length){r=await api('/followed_streams').catch(()=>null);items=r?.items||r?.streams||[]}}catch{}window.__mxFollowed=items;rail.innerHTML=items.length?items.slice(0,18).map((x,i)=>{const n=id({...x,platform:'twitch'});return `<button class="mxFollowCard" onclick="mxUseFollow(${i})">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div><b>${H(n.name)}</b><span>${H(n.game)} · ${n.viewers}</span></div></button>`}).join(''):'<div class="mxSmall">Aucun suivi en ligne.</div>'}
  function renderMain(){const m=document.getElementById('mxMain');if(!m)return;m.innerHTML=mx.mode==='watch'?watch():preview();bindSwipe()}
  function renderAll(){const el=document.getElementById('discover');if(!el)return;document.body.classList.add('oryonDiscoverProduct');document.body.classList.remove('oryonDiscoverFinalActive','oryonCleanDiscoverActive');const current=mx.mood||'petite-commu';el.innerHTML=`<div class="mxPage"><div class="mxShell"><header class="mxHero"><div><span class="mxEyebrow">Swap ton mood</span><h1>Swap ton mood.</h1><p>Choisis une ambiance, Swapp propose un live. Tu swipes, tu ouvres, tu lurk ou tu discutes.</p></div><div class="mxControls"><div class="mxMoodRail">${moods.map(([m,l])=>`<button class="mxMoodBtn ${m===current?'active':''}" onclick="mxSetMood('${m}')">${H(l)}</button>`).join('')}</div><div class="mxSearch"><input id="mxQuery" value="${H(mx.category||'')}" placeholder="jeu, créateur, ambiance" onkeydown="if(event.key==='Enter')mxFindLive()"><select id="mxMax"><option value="50">≤50 viewers</option><option value="150" selected>≤150 viewers</option><option value="300">≤300 viewers</option></select><select id="mxLang"><option value="fr" selected>FR</option><option value="en">EN</option></select><button class="mxBtn primary" onclick="mxFindLive()">Chercher un live</button></div></div></header><section class="mxGrid ${mx.mode==='watch'?'watch':'preview'} ${mx.mode==='watch'&&!mx.chat?'noChat':''}"><div class="mxMain" id="mxMain">${mx.mode==='watch'?watch():preview()}</div>${mx.mode==='watch'&&mx.chat?`<aside class="mxChat"><div class="mxChatHead"><span>Chat du stream</span><button class="mxBtn" onclick="mxToggleChat()">Fermer</button></div><div class="mxChatFrame">${chat(cur())}</div></aside>`:side()}</section>${deck()}${followed()}</div></div>`;bindSwipe();loadFollowed();document.querySelectorAll('#discover button').forEach(b=>{if(/^URL$/i.test((b.textContent||'').trim()))b.remove()})}
  function bindSwipe(){const card=document.getElementById('mxSwipeCard');if(!card)return;let startX=0,startY=0,dx=0,active=false;const move=x=>{dx=x-startX;const rot=Math.max(-14,Math.min(14,dx/14));card.style.transform=`translateX(${dx*.24}px) rotate(${rot}deg)`;card.classList.toggle('like',dx>38);card.classList.toggle('nope',dx<-38)};card.addEventListener('pointerdown',e=>{active=true;startX=e.clientX;startY=e.clientY;dx=0;card.setPointerCapture?.(e.pointerId)});card.addEventListener('pointermove',e=>{if(!active)return;if(Math.abs(e.clientY-startY)>80&&Math.abs(dx)<35)return;move(e.clientX)});card.addEventListener('pointerup',()=>{if(!active)return;active=false;const f=dx;card.style.transform='';card.classList.remove('like','nope');if(f>90)mxLike();else if(f<-90)mxNope()});card.addEventListener('pointercancel',()=>{active=false;card.style.transform='';card.classList.remove('like','nope')})}
  window.mxSetMood=m=>{mx.mood=m;localStorage.setItem('oryon_mx_mood',m);mx.category='';mx.mode='preview';mx.chat=false;fetchPool()};
  window.mxFindLive=()=>{mx.category=String(document.getElementById('mxQuery')?.value||'').trim();mx.mode='preview';mx.chat=false;return fetchPool({game:mx.category})};
  window.mxWatch=()=>{if(!cur())return;mx.mode='watch';mx.chat=false;renderAll()};
  window.mxToggleChat=()=>{if(mx.mode!=='watch')mx.mode='watch';mx.chat=!mx.chat;renderAll()};
  window.mxNext=()=>{const items=state.zap?.items||[];if(items.length>1){state.zap.index=(Number(state.zap.index||0)+1)%items.length;state.zap.last=cur();mx.mode='preview';mx.chat=false;renderAll()}else fetchPool()};
  window.mxNope=()=>mxNext();
  window.mxLike=()=>{try{const n=id(cur()||{});const arr=JSON.parse(localStorage.getItem('oryon_discover_likes')||'[]');if(n.login&&!arr.some(x=>x.platform===n.platform&&x.login===n.login)){arr.unshift({platform:n.platform,login:n.login,name:n.name,game:n.game,ts:Date.now()});localStorage.setItem('oryon_discover_likes',JSON.stringify(arr.slice(0,100)))}}catch{}mxNext()};
  window.mxLurk=()=>{const x=cur();if(!x)return;const d=readDeck();const n=id(x);const meta={platform:n.platform,login:n.login,name:n.name,title:n.title,game:n.game,viewers:n.viewers,img:n.img,addedAt:Date.now()};const k=meta.platform+':'+meta.login.toLowerCase();if(!d.some(s=>s&&(s.platform+':'+String(s.login).toLowerCase())===k))d.unshift(meta);writeDeck(d);toast?.('Ajouté au DeckLurker');renderAll()};
  window.mxOpenDeck=i=>{const s=readDeck()[i];if(!s)return;setItems([s,...(state.zap?.items||[])]);mx.mode='watch';mx.chat=false;renderAll()};
  window.mxClearDeck=i=>{const d=readDeck();d.splice(i,1);writeDeck(d);renderAll()};
  window.mxUseFollow=i=>{const x=(window.__mxFollowed||[])[i];if(!x)return;setItems([{...x,platform:'twitch'},...(state.zap?.items||[])]);mx.mode='watch';mx.chat=false;renderAll()};
  window.renderDiscover=renderDiscover=async function renderDiscoverProduct(){document.body.classList.add('oryonDiscoverProduct');renderAll();if(!(state.zap?.items||[]).length&&!mx.loading)await fetchPool()};
  window.findLive=window.mxFindLive; window.renderZap=renderMain; window.zapNext=window.mxNext; window.hfWatchCurrent=window.mxWatch; window.zapOpenCurrent=window.mxWatch; window.oryonAddCurrentToDeck=window.mxLurk; window.oryonToggleDiscoverChat=window.mxToggleChat;
  window.pickCat=async function(name,img=''){mx.category=String(name||'').trim();mx.mood='decouverte-jeu';localStorage.setItem('oryon_mx_mood',mx.mood);await setView('discover');await fetchPool({game:mx.category,autoWatch:true})};
  window.pickCatEncoded=(name,img='')=>window.pickCat(decodeURIComponent(name||''),decodeURIComponent(img||''));
  const oldSetView=window.setView;if(typeof oldSetView==='function'&&!oldSetView.__mxProductWrapped){const wrapped=function(v){document.body.classList.toggle('oryonDiscoverProduct',v==='discover');if(v!=='discover')document.body.classList.remove('oryonDiscoverProduct');return oldSetView.apply(this,arguments)};wrapped.__mxProductWrapped=true;window.setView=wrapped}
})();

/* =========================================================
   SWAPP DISCOVER — DESKTOP CHAT RIGHT / PRODUCT LAYOUT V3
   Single final renderer for Discover. No URL button. PC chat stays right.
   ========================================================= */
(function installOryonDiscoverV3(){
  const STYLE_ID='oryonDiscoverV3Style';
  document.getElementById(STYLE_ID)?.remove();
  const style=document.createElement('style');
  style.id=STYLE_ID;
  style.textContent=`
    body.oryonDiscoverV3{background:#05070d!important;overflow-x:hidden!important;}
    body.oryonDiscoverV3 .app{width:100%!important;max-width:none!important;margin:0!important;padding:0!important;}
    body.oryonDiscoverV3 #discover.view.active{display:block!important;width:100%!important;max-width:none!important;margin:0!important;padding:0!important;background:#05070d!important;overflow-x:hidden!important;}
    #discover .dxPage{min-height:calc(100vh - 52px);padding:clamp(18px,2.2vw,36px) clamp(18px,3vw,58px) 120px;background:radial-gradient(circle at 2% 0%,rgba(139,92,246,.30),transparent 34%),radial-gradient(circle at 100% 0%,rgba(34,211,238,.20),transparent 34%),linear-gradient(180deg,#07131d 0%,#05070d 62%);}
    #discover .dxShell{width:100%;max-width:1880px;margin:0 auto;display:grid;gap:22px;}
    #discover .dxTop{display:grid;grid-template-columns:minmax(280px,.54fr) minmax(520px,1fr);gap:24px;align-items:end;}
    #discover .dxKicker{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(139,92,246,.65);background:rgba(139,92,246,.18);border-radius:999px;padding:7px 11px;font-size:12px;font-weight:1000;color:#fff;}
    #discover .dxKicker:before{content:"";width:8px;height:8px;border-radius:999px;background:#22c55e;box-shadow:0 0 14px #22c55e;}
    #discover .dxTitle{font-size:clamp(52px,5.2vw,96px);line-height:.86;letter-spacing:-.078em;margin:12px 0 8px;font-weight:1000;color:#fff;}
    #discover .dxLead{max-width:650px;color:#c5d1e4;font-size:clamp(14px,1vw,18px);line-height:1.42;margin:0;}
    #discover .dxControls{border:1px solid rgba(148,163,184,.20);border-radius:26px;background:linear-gradient(180deg,rgba(15,23,42,.88),rgba(15,23,42,.56));box-shadow:0 24px 80px rgba(0,0,0,.28);padding:14px;display:grid;gap:10px;min-width:0;}
    #discover .dxMoodRail{display:flex;gap:9px;overflow:auto;padding-bottom:4px;scrollbar-width:thin;}
    #discover .dxMood{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.065);color:#fff;border-radius:999px;padding:10px 13px;font-weight:1000;white-space:nowrap;cursor:pointer;}
    #discover .dxMood.active,#discover .dxMood:hover{border-color:rgba(34,211,238,.85);background:linear-gradient(135deg,rgba(139,92,246,.42),rgba(34,211,238,.18));}
    #discover .dxSearch{display:grid;grid-template-columns:minmax(260px,1fr) 150px 100px auto;gap:9px;}
    #discover .dxSearch input,#discover .dxSearch select{min-width:0;border:1px solid rgba(148,163,184,.22);background:#070b13;color:#fff;border-radius:15px;padding:13px 14px;outline:none;font-weight:850;}
    #discover .dxBtn{border:1px solid rgba(255,255,255,.14);background:linear-gradient(180deg,rgba(255,255,255,.10),rgba(255,255,255,.05));color:#fff;border-radius:16px;padding:13px 16px;font-weight:1000;cursor:pointer;white-space:nowrap;transition:transform .15s ease,border-color .15s ease,background .15s ease;}
    #discover .dxBtn:hover{border-color:rgba(139,92,246,.75);transform:translateY(-1px);}
    #discover .dxBtn.primary{background:linear-gradient(135deg,#8b5cf6,#d946ef);box-shadow:0 18px 60px rgba(168,85,247,.25);}
    #discover .dxStage{display:grid;grid-template-columns:minmax(0,1fr) minmax(360px,430px);gap:18px;align-items:start;}
    #discover .dxStage.noSide{grid-template-columns:minmax(0,1fr) minmax(320px,390px);}
    #discover .dxMain{min-width:0;}
    #discover .dxCard{position:relative;width:100%;aspect-ratio:16/9;border:1px solid rgba(34,211,238,.40);border-radius:32px;overflow:hidden;background:#02040a;box-shadow:0 30px 105px rgba(0,0,0,.38),0 0 0 1px rgba(139,92,246,.08) inset;touch-action:pan-y;transform-origin:center bottom;transition:transform .18s ease;}
    #discover .dxCard img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;filter:saturate(1.06) contrast(1.04);}
    #discover .dxCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.10),rgba(2,6,23,.18) 34%,rgba(2,6,23,.90));}
    #discover .dxTags{position:absolute;z-index:2;top:18px;left:18px;right:18px;display:flex;gap:8px;flex-wrap:wrap;}
    #discover .dxTag{display:inline-flex;max-width:210px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;border:1px solid rgba(255,255,255,.16);background:rgba(3,7,18,.68);backdrop-filter:blur(12px);border-radius:999px;padding:7px 10px;color:#fff;font-weight:1000;font-size:12px;line-height:1;}
    #discover .dxCardText{position:absolute;z-index:2;left:26px;right:26px;bottom:24px;display:grid;gap:10px;}
    #discover .dxLiveTitle{font-size:clamp(34px,3.4vw,68px);line-height:.94;letter-spacing:-.062em;margin:0;max-width:1260px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-shadow:0 14px 34px rgba(0,0,0,.50);}
    #discover .dxMeta{display:flex;gap:8px;flex-wrap:wrap;color:#e5edff;font-size:clamp(15px,1.1vw,20px);font-weight:950;text-shadow:0 8px 24px rgba(0,0,0,.45);}
    #discover .dxSwipe{position:absolute;z-index:4;left:50%;top:48%;transform:translate(-50%,-50%);font-size:clamp(42px,7vw,108px);font-weight:1000;letter-spacing:-.06em;text-shadow:0 18px 50px rgba(0,0,0,.55);opacity:0;pointer-events:none;}
    #discover .dxSwipe.like{color:#34d399;}#discover .dxCard.like .dxSwipe.like{opacity:1;}#discover .dxSwipe.nope{color:#fb7185;}#discover .dxCard.nope .dxSwipe.nope{opacity:1;}
    #discover .dxActions{width:100%;max-width:880px;margin:14px auto 0;display:grid;grid-template-columns:1fr 1.22fr 1fr;gap:12px;border:1px solid rgba(148,163,184,.18);border-radius:24px;background:rgba(15,23,42,.68);padding:12px;box-shadow:0 18px 60px rgba(0,0,0,.24);}
    #discover .dxActions .dxBtn{min-height:58px;font-size:18px;border-radius:18px;}#discover .dxActions .dxBtn.primary{font-size:20px;}
    #discover .dxPlayer{border:1px solid rgba(34,211,238,.32);border-radius:30px;background:#02040a;overflow:hidden;box-shadow:0 30px 105px rgba(0,0,0,.38);}
    #discover .dxPlayerHead{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;border-bottom:1px solid rgba(148,163,184,.14);background:rgba(15,23,42,.74);font-weight:1000;}
    #discover .dxSmall{color:#9fafc6;font-size:12px;}#discover .dxPill{display:inline-flex;border:1px solid rgba(34,211,238,.36);background:rgba(34,211,238,.10);border-radius:999px;padding:7px 10px;font-weight:1000;font-size:12px;}
    #discover .dxPlayerBox{aspect-ratio:16/9;background:#000;}#discover .dxPlayerBox iframe{width:100%;height:100%;border:0;display:block;background:#000;}
    #discover .dxPanel{border:1px solid rgba(148,163,184,.20);border-radius:24px;background:linear-gradient(180deg,rgba(15,23,42,.72),rgba(15,23,42,.42));padding:16px;box-shadow:0 22px 70px rgba(0,0,0,.22);}#discover .dxPanel h2{margin:0 0 10px;font-size:22px;}#discover .dxPanel p{margin:0;color:#aebad0;line-height:1.4;}
    #discover .dxSide{min-width:0;display:grid;gap:14px;align-content:start;}
    #discover .dxSignals{display:grid;gap:8px;margin-top:12px;}#discover .dxSignal{display:flex;justify-content:space-between;gap:12px;border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.045);border-radius:14px;padding:10px 11px;font-weight:950;}#discover .dxSignal span{color:#9aa8be;font-size:12px;}
    #discover .dxChat{border:1px solid rgba(148,163,184,.22);border-radius:30px;background:#111318;overflow:hidden;display:flex;flex-direction:column;min-height:100%;height:100%;box-shadow:0 30px 95px rgba(0,0,0,.25);}
    #discover .dxChatHead{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,.12);font-weight:1000;}
    #discover .dxChatFrame{flex:1;min-height:0;height:100%;background:#18181b;}#discover .dxChatFrame iframe{width:100%;height:100%;min-height:620px;border:0;display:block;background:#18181b;}
    #discover .dxEmpty{min-height:clamp(420px,50vh,680px);border:1px dashed rgba(148,163,184,.24);border-radius:30px;background:linear-gradient(135deg,rgba(139,92,246,.11),rgba(34,211,238,.05));display:grid;place-items:center;text-align:center;padding:24px;}#discover .dxEmpty h2{font-size:clamp(34px,4.5vw,80px);line-height:.92;margin:0 0 10px;letter-spacing:-.06em;}
    #discover .dxDeck,#discover .dxFollow{border:1px solid rgba(34,211,238,.24);border-radius:26px;background:linear-gradient(180deg,rgba(15,23,42,.76),rgba(15,23,42,.40));padding:16px;box-shadow:0 22px 70px rgba(0,0,0,.22);}
    #discover .dxDeckHead,#discover .dxFollowHead{display:flex;justify-content:space-between;align-items:start;gap:12px;margin-bottom:12px;}#discover .dxDeckHead h2,#discover .dxFollowHead h2{margin:0;font-size:24px;}#discover .dxDeckHead p,#discover .dxFollowHead p{margin:4px 0 0;color:#aab6cc;}
    #discover .dxDeckGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;}#discover .dxDeckSlot{position:relative;min-height:112px;border:1px solid rgba(148,163,184,.18);border-radius:17px;background:rgba(0,0,0,.22);overflow:hidden;padding:12px;display:flex;flex-direction:column;justify-content:flex-end;}#discover .dxDeckSlot img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.58;}#discover .dxDeckSlot:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.90));}#discover .dxDeckSlot>*{position:relative;z-index:1;}#discover .dxDeckSlot.empty{align-items:center;justify-content:center;text-align:center;color:#6d7890;}#discover .dxDeckMini{display:flex;gap:6px;margin-top:8px;}#discover .dxDeckMini button{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:white;border-radius:10px;padding:6px 8px;font-weight:900;cursor:pointer;}
    #discover .dxFollowRail{display:flex;gap:10px;overflow:auto;}#discover .dxFollowCard{min-width:210px;height:120px;position:relative;border:1px solid rgba(148,163,184,.18);border-radius:18px;overflow:hidden;background:#0b1220;color:#fff;text-align:left;padding:12px;display:flex;align-items:flex-end;cursor:pointer;}#discover .dxFollowCard img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.58;}#discover .dxFollowCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.90));}#discover .dxFollowCard div{position:relative;z-index:1;}#discover .dxFollowCard span{font-size:12px;color:#c8d3e7;display:block;}
    @media(max-width:980px){#discover .dxTop{grid-template-columns:1fr;}#discover .dxSearch{grid-template-columns:1fr 1fr 1fr;}#discover .dxStage,#discover .dxStage.noSide{grid-template-columns:1fr;}#discover .dxSide{grid-template-columns:1fr 1fr;}#discover .dxChatFrame iframe{min-height:460px;}#discover .dxDeckGrid{grid-template-columns:repeat(2,minmax(0,1fr));}}
    @media(max-width:760px){#discover .dxPage{padding:14px 14px 98px;}#discover .dxTitle{font-size:clamp(42px,13vw,64px)!important;}#discover .dxLead{font-size:13px!important;}#discover .dxControls{border-radius:22px!important;padding:11px!important;}#discover .dxSearch{grid-template-columns:1fr!important;}#discover .dxSide{display:none!important;}#discover .dxCard{aspect-ratio:auto!important;min-height:470px!important;border-radius:24px!important;}#discover .dxTags{left:12px!important;right:12px!important;top:12px!important;gap:6px!important;}#discover .dxTag{font-size:10px!important;padding:6px 8px!important;max-width:132px!important;}#discover .dxCardText{left:14px!important;right:14px!important;bottom:14px!important;}#discover .dxLiveTitle{font-size:clamp(27px,8vw,40px)!important;}#discover .dxMeta{font-size:13px!important;}#discover .dxActions{grid-template-columns:1fr 1fr 1fr!important;padding:8px!important;gap:8px!important;border-radius:20px!important;}#discover .dxActions .dxBtn{min-height:50px!important;font-size:13px!important;padding:9px 7px!important;border-radius:15px!important;}#discover .dxPlayer,#discover .dxChat{border-radius:22px!important;}#discover .dxChatFrame iframe{min-height:430px!important;}#discover .dxDeck{padding:12px!important;border-radius:22px!important;}#discover .dxDeckSlot{min-height:92px!important;}#discover .dxFollowCard{min-width:78vw;height:128px;}}
  `;
  document.head.appendChild(style);

  const moods=[['petite-commu','Petite commu'],['chill','Chill'],['discussion','Discussion'],['nuit-calme','Nuit calme'],['rp','RP'],['decouverte-jeu','Découverte jeu']];
  const dx={mood:localStorage.getItem('oryon_dx_mood')||localStorage.getItem('oryon_mx_mood')||'petite-commu',mode:'preview',chat:false,loading:false,category:''};
  const H=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  const E=encodeURIComponent, host=()=>location.hostname||'localhost', img=u=>String(u||'').replace('{width}','1280').replace('{height}','720');
  function id(x={}){const platform=(x.platform||x.source||'twitch').toLowerCase().includes('oryon')?'oryon':'twitch';const login=String(x.login||x.user_login||x.broadcaster_login||x.channel_login||x.name||x.user_name||x.display_name||'');const name=String(x.display_name||x.user_name||x.name||login||'Live');return{platform,login,name,game:String(x.game_name||x.game||x.category||'Just Chatting'),viewers:Number(x.viewer_count??x.viewers??0)||0,title:String(x.title||x.status||`Live de ${name}`),img:img(x.thumbnail_url||x.img||x.image||'')};}
  function cur(){const z=state.zap||{};return (z.items||[])[Number(z.index||0)]||z.last||null;}
  ...
  function writeDeck(d){try{localStorage.setItem(deckKey(),JSON.stringify((d||[]).slice(0,8)));}catch{}}
  function tags(x){const n=id(x);return [n.platform==='twitch'?'Twitch intégré':'Swapp natif',n.viewers?`${n.viewers} viewers`:'petit live',n.game,'entrée douce'].filter(Boolean).slice(0,5);}
  async function fetchPool({game='',autoWatch=false}={}){dx.loading=true;renderMain();const q=String(game||dx.category||document.getElementById('dxQuery')?.value||'').trim();const max=Number(document.getElementById('dxMax')?.value||150);const lang=document.getElementById('dxLang')?.value||'fr';let items=[];try{if(q){const r=await api('/api/twitch/random-small-live?'+qs({game:q,max,language:lang})).catch(()=>null);if(r?.success&&r.target)items=[{...r.target,platform:'twitch',login:r.target.login||r.target.user_login,display_name:r.target.name||r.target.display_name,user_name:r.target.name,viewer_count:r.target.viewers,game_name:r.target.game||q,thumbnail_url:r.target.thumbnail_url||r.target.img,title:r.target.title}];}
    if(!items.length){const r=await api('/api/oryon/discover/find-live?'+qs({q,mood:dx.mood,max,lang,source:'both'})).catch(()=>null);items=r?.items||r?.results||(r?.target?[r.target]:[]);}
    if(!items.length){const r=await api('/api/twitch/streams/small?'+qs({lang,min:0,max})).catch(()=>null);items=r?.items||r?.streams||[];}
    if(!items.length){const r=await api('/api/native/lives').catch(()=>null);items=(r?.items||[]).map(x=>({...x,platform:'oryon'}));}
  }catch(e){console.warn('dx discover fetch',e);}setItems(items.slice(0,32));dx.loading=false;dx.mode=autoWatch&&cur()?'watch':'preview';dx.chat=false;renderAll();}
  function player(x){const n=id(x);return n.platform==='twitch'?`<iframe allowfullscreen allow="autoplay; fullscreen" src="https://player.twitch.tv/?channel=${E(n.login)}&parent=${E(host())}&autoplay=true&muted=false"></iframe>`:`<div class="dxEmpty"><div><h2>${H(n.name)}</h2><p>Live Swapp sélectionné.</p></div></div>`;}
  function chat(x){const n=id(x);return n.platform==='twitch'?`<iframe title="Chat Twitch" src="https://www.twitch.tv/embed/${E(n.login)}/chat?parent=${E(host())}&darkpopout"></iframe>`:`<div class="dxEmpty"><div><h2>Chat Swapp</h2><p>Disponible sur la page chaîne.</p></div></div>`;}
  function preview(){const x=cur();if(dx.loading)return `<div class="dxEmpty"><div><h2>Recherche…</h2><p>Swapp cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="dxEmpty"><div><h2>Aucun live trouvé.</h2><p>Change de mood ou relance une recherche.</p><button class="dxBtn primary" onclick="dxFindLive()">Chercher un live</button></div></div>`;const n=id(x);return `<article id="dxSwipeCard" class="dxCard">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div class="dxTags">${tags(x).map(t=>`<span class="dxTag">${H(t)}</span>`).join('')}</div><div class="dxSwipe nope">PAS OUF</div><div class="dxSwipe like">J’AIME</div><div class="dxCardText"><h2 class="dxLiveTitle">${H(n.title)}</h2><div class="dxMeta"><span>${H(n.name)}</span><span>·</span><span>${H(n.game)}</span><span>·</span><span>${n.viewers} viewers</span></div></div></article><div class="dxActions"><button class="dxBtn" onclick="dxNope()">Pas ouf</button><button class="dxBtn primary" onclick="dxWatch()">Voir</button><button class="dxBtn" onclick="dxLike()">J’aime</button></div>`;}
  function side(){const x=cur(),n=x?id(x):null;return `<aside class="dxSide"><section class="dxPanel"><h2>Pourquoi ce live ?</h2>${n?`<div class="dxSignals"><div class="dxSignal"><b>${H(moods.find(m=>m[0]===dx.mood)?.[1]||'Ambiance')}</b><span>mood</span></div><div class="dxSignal"><b>${n.viewers} viewers</b><span>taille humaine</span></div><div class="dxSignal"><b>${H(n.game)}</b><span>catégorie</span></div><div class="dxSignal"><b>${n.platform==='twitch'?'Twitch':'Swapp'}</b><span>source</span></div></div>`:'<p>Choisis un mood et lance une proposition.</p>'}</section><section class="dxPanel"><h2>Lecture propre</h2><p>Voir ouvre le live. Chat apparaît à droite sur PC. Lurk garde le live dans DeckLurker.</p></section></aside>`;}
  function watch(){const x=cur();if(!x)return preview();const n=id(x);return `<main class="dxPlayer"><div class="dxPlayerHead"><div><b>${H(n.name)}</b><div class="dxSmall">${H(n.game)} · ${n.viewers} viewers</div></div><span class="dxPill">${n.platform==='twitch'?'Twitch':'Swapp'}</span></div><div class="dxPlayerBox">${player(x)}</div><div class="dxActions"><button class="dxBtn" onclick="dxLurk()">LURK</button><button class="dxBtn" onclick="dxToggleChat()">CHAT</button><button class="dxBtn primary" onclick="dxNext()">SUIVANT</button></div></main>`;}
  function deck(){const d=readDeck();return `<section class="dxDeck"><div class="dxDeckHead"><div><h2>DeckLurker</h2><p>Garde tes lives sous la main et switch sans ouvrir dix onglets.</p></div><span class="dxPill">${d.length}/8</span></div><div class="dxDeckGrid">${Array.from({length:8},(_,i)=>{const s=d[i];if(!s)return `<article class="dxDeckSlot empty"><b>Case ${i+1}</b><span>Vide</span></article>`;return `<article class="dxDeckSlot">${s.img?`<img src="${H(s.img)}" alt="">`:''}<b>${H(s.name||s.login)}</b><span>${H(s.game||s.title||'Live')}</span><div class="dxDeckMini"><button onclick="dxOpenDeck(${i})">Voir</button><button onclick="dxClearDeck(${i})">Vider</button></div></article>`;}).join('')}</div></section>`;}
  function followed(){return `<section class="dxFollow"><div class="dxFollowHead"><div><h2>Suivis Twitch en ligne</h2><p class="dxSmall">Séparé des recommandations.</p></div>${state.session?.twitch?'<span class="dxPill">Connecté</span>':'<button class="dxBtn primary" onclick="connectTwitch?.()">Connecter Twitch</button>'}</div><div class="dxFollowRail" id="dxFollowRail">${state.session?.twitch?'<div class="dxSmall">Chargement…</div>':'<div class="dxSmall">Connexion Twitch nécessaire.</div>'}</div></section>`;}
  async function loadFollowed(){const rail=document.getElementById('dxFollowRail');if(!rail||!state.session?.twitch)return;let items=[];try{let r=await api('/api/twitch/followed/live').catch(()=>null);items=r?.items||r?.streams||[];if(!items.length){r=await api('/followed_streams').catch(()=>null);items=r?.items||r?.streams||[];}}catch{}window.__dxFollowed=items;rail.innerHTML=items.length?items.slice(0,18).map((x,i)=>{const n=id({...x,platform:'twitch'});return `<button class="dxFollowCard" onclick="dxUseFollow(${i})">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div><b>${H(n.name)}</b><span>${H(n.game)} · ${n.viewers}</span></div></button>`;}).join(''):'<div class="dxSmall">Aucun suivi en ligne.</div>';}
  function renderMain(){const m=document.getElementById('dxMain');if(!m)return;m.innerHTML=dx.mode==='watch'?watch():preview();bindSwipe();}
  function renderAll(){const el=document.getElementById('discover');if(!el)return;document.body.classList.add('oryonDiscoverV3');document.body.classList.remove('oryonDiscoverProduct','oryonDiscoverFinalActive','oryonCleanDiscoverActive');const current=dx.mood||'petite-commu';el.innerHTML=`<div class="dxPage"><div class="dxShell"><header class="dxTop"><div><span class="dxKicker">Swap ton mood</span><h1 class="dxTitle">Swap ton mood.</h1><p class="dxLead">Choisis une ambiance. Swapp propose un live. Tu swipes, tu ouvres, tu lurk ou tu discutes.</p></div><div class="dxControls"><div class="dxMoodRail">${moods.map(([m,l])=>`<button class="dxMood ${m===current?'active':''}" onclick="dxSetMood('${m}')">${H(l)}</button>`).join('')}</div><div class="dxSearch"><input id="dxQuery" value="${H(dx.category||'')}" placeholder="jeu, créateur, ambiance" onkeydown="if(event.key==='Enter')dxFindLive()"><select id="dxMax"><option value="50">≤50 viewers</option><option value="150" selected>≤150 viewers</option><option value="300">≤300 viewers</option></select><select id="dxLang"><option value="fr" selected>FR</option><option value="en">EN</option></select><button class="dxBtn primary" onclick="dxFindLive()">Chercher un live</button></div></div></header><section class="dxStage ${dx.mode==='watch'&&!dx.chat?'noSide':''}"><div class="dxMain" id="dxMain">${dx.mode==='watch'?watch():preview()}</div>${dx.mode==='watch'&&dx.chat?`<aside class="dxChat"><div class="dxChatHead"><span>Chat du stream</span><button class="dxBtn" onclick="dxToggleChat()">Fermer</button></div><div class="dxChatFrame">${chat(cur())}</div></aside>`:side()}</section>${deck()}${followed()}</div></div>`;bindSwipe();loadFollowed();document.querySelectorAll('#discover button').forEach(b=>{if(/^URL$/i.test((b.textContent||'').trim()))b.remove();});}
  function bindSwipe(){const card=document.getElementById('dxSwipeCard');if(!card)return;let startX=0,startY=0,dxv=0,active=false;const move=x=>{dxv=x-startX;const rot=Math.max(-14,Math.min(14,dxv/14));card.style.transform=`translateX(${dxv*.24}px) rotate(${rot}deg)`;card.classList.toggle('like',dxv>38);card.classList.toggle('nope',dxv<-38);};card.addEventListener('pointerdown',e=>{active=true;startX=e.clientX;startY=e.clientY;dxv=0;card.setPointerCapture?.(e.pointerId);});card.addEventListener('pointermove',e=>{if(!active)return;if(Math.abs(e.clientY-startY)>80&&Math.abs(dxv)<35)return;move(e.clientX);});card.addEventListener('pointerup',()=>{if(!active)return;active=false;const f=dxv;card.style.transform='';card.classList.remove('like','nope');if(f>90)dxLike();else if(f<-90)dxNope();});card.addEventListener('pointercancel',()=>{active=false;card.style.transform='';card.classList.remove('like','nope');});}
  window.dxSetMood=m=>{dx.mood=m;localStorage.setItem('oryon_dx_mood',m);dx.category='';dx.mode='preview';dx.chat=false;fetchPool();};
  window.dxFindLive=()=>{dx.category=String(document.getElementById('dxQuery')?.value||'').trim();dx.mode='preview';dx.chat=false;return fetchPool({game:dx.category});};
  window.dxWatch=()=>{if(!cur())return;dx.mode='watch';dx.chat=false;renderAll();};
  window.dxToggleChat=()=>{if(dx.mode!=='watch')dx.mode='watch';dx.chat=!dx.chat;renderAll();};
  window.dxNext=()=>{const items=state.zap?.items||[];if(items.length>1){state.zap.index=(Number(state.zap.index||0)+1)%items.length;state.zap.last=cur();dx.mode='preview';dx.chat=false;renderAll();}else fetchPool();};
  window.dxNope=()=>dxNext();
  window.dxLike=()=>{try{const n=id(cur()||{});const arr=JSON.parse(localStorage.getItem('oryon_discover_likes')||'[]');if(n.login&&!arr.some(x=>x.platform===n.platform&&x.login===n.login)){arr.unshift({platform:n.platform,login:n.login,name:n.name,game:n.game,ts:Date.now()});localStorage.setItem('oryon_discover_likes',JSON.stringify(arr.slice(0,100)));}}catch{}dxNext();};
  window.dxLurk=()=>{const x=cur();if(!x)return;const d=readDeck();const n=id(x);const meta={platform:n.platform,login:n.login,name:n.name,title:n.title,game:n.game,viewers:n.viewers,img:n.img,addedAt:Date.now()};const k=meta.platform+':'+meta.login.toLowerCase();if(!d.some(s=>s&&(s.platform+':'+String(s.login).toLowerCase())===k))d.unshift(meta);writeDeck(d);toast?.('Ajouté au DeckLurker');renderAll();};
  window.dxOpenDeck=i=>{const s=readDeck()[i];if(!s)return;setItems([s,...(state.zap?.items||[])]);dx.mode='watch';dx.chat=false;renderAll();};
  window.dxClearDeck=i=>{const d=readDeck();d.splice(i,1);writeDeck(d);renderAll();};
  window.dxUseFollow=i=>{const x=(window.__dxFollowed||[])[i];if(!x)return;setItems([{...x,platform:'twitch'},...(state.zap?.items||[])]);dx.mode='watch';dx.chat=false;renderAll();};
  window.renderDiscover=renderDiscover=async function renderDiscoverV3(){document.body.classList.add('oryonDiscoverV3');renderAll();if(!(state.zap?.items||[]).length&&!dx.loading)await fetchPool();};
  window.findLive=window.dxFindLive;window.renderZap=renderMain;window.zapNext=window.dxNext;window.hfWatchCurrent=window.dxWatch;window.zapOpenCurrent=window.dxWatch;window.oryonAddCurrentToDeck=window.dxLurk;window.oryonToggleDiscoverChat=window.dxToggleChat;
  window.pickCat=async function(name,img=''){dx.category=String(name||'').trim();dx.mood='decouverte-jeu';localStorage.setItem('oryon_dx_mood',dx.mood);await setView('discover');await fetchPool({game:dx.category,autoWatch:true});};
  window.pickCatEncoded=(name,img='')=>window.pickCat(decodeURIComponent(name||''),decodeURIComponent(img||''));
  const oldSetView=window.setView;if(typeof oldSetView==='function'&&!oldSetView.__dxV3Wrapped){const wrapped=function(v){document.body.classList.toggle('oryonDiscoverV3',v==='discover');if(v!=='discover')document.body.classList.remove('oryonDiscoverV3');return oldSetView.apply(this,arguments);};wrapped.__dxV3Wrapped=true;window.setView=wrapped;}
})();

/* =========================================================
   SWAPP DISCOVER V4 — clean full-width product page
   Fixes: no URL button, no 'Pourquoi ce live', bigger PC layout,
   mood buttons actually search, Twitch followed stays separated.
   ========================================================= */
(function installOryonDiscoverV4(){
  const STYLE_ID='oryonDiscoverV4Style';
  document.getElementById(STYLE_ID)?.remove();
  const css=document.createElement('style');
  css.id=STYLE_ID;
  css.textContent=`
    body.oryonDiscoverV4{background:#05070d!important;overflow-x:hidden!important;}
    body.oryonDiscoverV4 *{-webkit-tap-highlight-color:transparent;text-decoration:none!important;}
    body.oryonDiscoverV4 #discover,body.oryonDiscoverV4 #discover *{box-sizing:border-box;user-select:none;}
    body.oryonDiscoverV4 #discover input,body.oryonDiscoverV4 #discover textarea{user-select:text;}
    body.oryonDiscoverV4 .app{width:100%!important;max-width:none!important;margin:0!important;padding:0!important;}
    body.oryonDiscoverV4 #discover.view.active{display:block!important;width:100%!important;max-width:none!important;margin:0!important;padding:0!important;background:#05070d!important;overflow-x:hidden!important;}
    #discover .v4Page{min-height:calc(100vh - 52px);padding:clamp(22px,2.4vw,48px) clamp(22px,3.2vw,72px) 128px;background:radial-gradient(circle at 0% 0%,rgba(139,92,246,.32),transparent 34%),radial-gradient(circle at 100% 0%,rgba(34,211,238,.22),transparent 34%),linear-gradient(180deg,#07131d 0%,#05070d 66%);}
    #discover .v4Shell{width:100%;max-width:2400px;margin:0 auto;display:grid;gap:clamp(22px,2vw,34px);}
    #discover .v4Hero{display:grid;grid-template-columns:minmax(360px,.58fr) minmax(640px,1fr);gap:clamp(22px,2.2vw,44px);align-items:end;}
    #discover .v4Kicker{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(139,92,246,.65);background:rgba(139,92,246,.20);border-radius:999px;padding:8px 12px;font-size:13px;font-weight:1000;color:#fff;box-shadow:0 0 22px rgba(139,92,246,.16);}
    #discover .v4Kicker:before{content:"";width:8px;height:8px;border-radius:999px;background:#22c55e;box-shadow:0 0 14px #22c55e;}
    #discover .v4Title{font-size:clamp(70px,6.7vw,142px);line-height:.82;letter-spacing:-.09em;margin:14px 0 10px;font-weight:1000;color:#fff;text-wrap:balance;}
    #discover .v4Lead{max-width:780px;color:#c6d4e8;font-size:clamp(16px,1.18vw,24px);line-height:1.35;margin:0;}
    #discover .v4Controls{border:1px solid rgba(148,163,184,.22);border-radius:28px;background:linear-gradient(180deg,rgba(15,23,42,.90),rgba(15,23,42,.58));box-shadow:0 26px 90px rgba(0,0,0,.30);padding:16px;display:grid;gap:12px;min-width:0;}
    #discover .v4MoodRail{display:flex;gap:10px;overflow:auto;padding-bottom:5px;scrollbar-width:thin;}
    #discover .v4MoodRail::-webkit-scrollbar{height:8px}#discover .v4MoodRail::-webkit-scrollbar-thumb{background:rgba(148,163,184,.36);border-radius:999px}
    #discover .v4Mood{border:1px solid rgba(255,255,255,.15);background:rgba(255,255,255,.07);color:#fff;border-radius:999px;padding:11px 15px;font-weight:1000;white-space:nowrap;cursor:pointer;outline:none;min-height:42px;}
    #discover .v4Mood.active,#discover .v4Mood:hover{border-color:rgba(34,211,238,.88);background:linear-gradient(135deg,rgba(139,92,246,.44),rgba(34,211,238,.20));box-shadow:0 10px 32px rgba(34,211,238,.08);}
    #discover .v4Search{display:grid;grid-template-columns:minmax(320px,1fr) 160px 105px auto;gap:10px;}
    #discover .v4Search input,#discover .v4Search select{min-width:0;border:1px solid rgba(148,163,184,.22);background:#070b13;color:#fff;border-radius:16px;padding:14px 15px;outline:none;font-weight:850;min-height:50px;}
    #discover .v4Btn{border:1px solid rgba(255,255,255,.14);background:linear-gradient(180deg,rgba(255,255,255,.105),rgba(255,255,255,.055));color:#fff;border-radius:17px;padding:14px 17px;font-weight:1000;cursor:pointer;white-space:nowrap;transition:transform .15s ease,border-color .15s ease,background .15s ease;outline:none;min-height:50px;}
    #discover .v4Btn:hover{border-color:rgba(139,92,246,.76);transform:translateY(-1px);}#discover .v4Btn.primary{background:linear-gradient(135deg,#8b5cf6,#d946ef);box-shadow:0 18px 60px rgba(168,85,247,.25);border:0;}
    #discover .v4Stage{display:grid;grid-template-columns:minmax(0,1fr) minmax(390px,500px);gap:clamp(18px,1.8vw,32px);align-items:stretch;}
    #discover .v4Main{min-width:0;display:grid;gap:14px;align-content:start;}
    #discover .v4Card,#discover .v4Player{position:relative;width:100%;aspect-ratio:16/9;border:1px solid rgba(34,211,238,.40);border-radius:34px;overflow:hidden;background:#02040a;box-shadow:0 34px 120px rgba(0,0,0,.42),0 0 0 1px rgba(139,92,246,.08) inset;touch-action:pan-y;transform-origin:center bottom;transition:transform .18s ease;}
    #discover .v4Card{min-height:min(72vh,820px);}#discover .v4Player{min-height:min(70vh,820px);}
    #discover .v4Card img,#discover .v4Player iframe{position:absolute;inset:0;width:100%;height:100%;border:0;display:block;background:#000;}
    #discover .v4Card img{object-fit:cover;filter:saturate(1.06) contrast(1.04);}#discover .v4Card:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(2,6,23,.10),rgba(2,6,23,.16) 32%,rgba(2,6,23,.92));}
    #discover .v4Tags{position:absolute;z-index:2;top:18px;left:18px;right:18px;display:flex;gap:8px;flex-wrap:wrap;}
    #discover .v4Tag{display:inline-flex;max-width:230px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;border:1px solid rgba(255,255,255,.16);background:rgba(3,7,18,.68);backdrop-filter:blur(12px);border-radius:999px;padding:8px 11px;color:#fff;font-weight:1000;font-size:12px;line-height:1;}
    #discover .v4CardText{position:absolute;z-index:2;left:30px;right:30px;bottom:28px;display:grid;gap:11px;}
    #discover .v4LiveTitle{font-size:clamp(42px,4.4vw,86px);line-height:.92;letter-spacing:-.068em;margin:0;max-width:1400px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-shadow:0 14px 34px rgba(0,0,0,.50);}
    #discover .v4Meta{display:flex;gap:8px;flex-wrap:wrap;color:#e5edff;font-size:clamp(16px,1.2vw,22px);font-weight:950;text-shadow:0 8px 24px rgba(0,0,0,.45);}
    #discover .v4Swipe{position:absolute;z-index:4;left:50%;top:48%;transform:translate(-50%,-50%);font-size:clamp(46px,7vw,112px);font-weight:1000;letter-spacing:-.06em;text-shadow:0 18px 50px rgba(0,0,0,.55);opacity:0;pointer-events:none;text-transform:uppercase;}
    #discover .v4Swipe.like{color:#34d399;}#discover .v4Card.like .v4Swipe.like{opacity:1;}#discover .v4Swipe.nope{color:#fb7185;}#discover .v4Card.nope .v4Swipe.nope{opacity:1;}
    #discover .v4Actions{width:100%;max-width:980px;margin:0 auto;display:grid;grid-template-columns:1fr 1.22fr 1fr;gap:12px;border:1px solid rgba(148,163,184,.18);border-radius:25px;background:rgba(15,23,42,.70);padding:12px;box-shadow:0 18px 60px rgba(0,0,0,.24);}
    #discover .v4Actions .v4Btn{min-height:62px;font-size:19px;border-radius:19px;}#discover .v4Actions .v4Btn.primary{font-size:21px;}
    #discover .v4Panel{border:1px solid rgba(148,163,184,.18);border-radius:28px;background:linear-gradient(180deg,rgba(15,23,42,.78),rgba(15,23,42,.42));padding:20px;box-shadow:0 22px 70px rgba(0,0,0,.24);min-width:0;}
    #discover .v4Panel h2{font-size:clamp(25px,1.7vw,36px);line-height:1;margin:0 0 12px;letter-spacing:-.04em;}#discover .v4Panel p{color:#b9c7dc;font-size:15px;line-height:1.45;margin:0 0 12px;}
    #discover .v4Bio{display:grid;gap:16px;align-content:start;}#discover .v4BioTitle{font-size:22px;font-weight:1000;margin:0;color:#fff;line-height:1.1;}#discover .v4BioRows{display:grid;gap:8px;}#discover .v4BioRow{display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.045);border-radius:15px;padding:11px 12px;font-weight:900;}#discover .v4BioRow span{color:#9fb0c7;font-size:12px;}
    #discover .v4Chat{display:grid;grid-template-rows:auto minmax(0,1fr);padding:0;min-height:100%;overflow:hidden;background:#111318;}
    #discover .v4ChatHead{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:13px 14px;border-bottom:1px solid rgba(255,255,255,.12);font-weight:950;background:rgba(255,255,255,.035);}
    #discover .v4ChatFrame{min-height:680px;background:#18181b;}#discover .v4ChatFrame iframe{width:100%;height:100%;border:0;display:block;background:#18181b;}
    #discover .v4Empty{min-height:520px;border:1px dashed rgba(148,163,184,.26);border-radius:30px;display:grid;place-items:center;text-align:center;color:#b7c3d7;background:rgba(15,23,42,.42);padding:32px}.v4Empty h2{font-size:clamp(34px,5vw,70px);line-height:.95;margin:0 0 10px;color:#fff;letter-spacing:-.055em}.v4Empty p{max-width:640px;margin:0 auto 18px!important}
    #discover .v4Deck,#discover .v4Follow{width:100%;border:1px solid rgba(148,163,184,.16);border-radius:26px;background:rgba(15,23,42,.48);padding:16px;}.v4DeckHead,.v4FollowHead{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:12px}.v4DeckHead h2,.v4FollowHead h2{margin:0;font-size:24px}.v4DeckHead p,.v4FollowHead p{margin:4px 0 0;color:#aab6cc}.v4Pill{display:inline-flex;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);border-radius:999px;padding:6px 9px;font-size:12px;font-weight:950;color:#fff;}
    #discover .v4DeckGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.v4DeckSlot{position:relative;min-height:126px;border:1px solid rgba(148,163,184,.18);border-radius:18px;background:rgba(0,0,0,.22);overflow:hidden;padding:12px;display:flex;flex-direction:column;justify-content:flex-end}.v4DeckSlot img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.58}.v4DeckSlot:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.88));}.v4DeckSlot>*{position:relative;z-index:1}.v4DeckSlot b{display:block}.v4DeckSlot span{display:block;color:#b7c3d7;font-size:12px;margin-top:2px}.v4DeckSlot.empty{align-items:center;justify-content:center;text-align:center;color:#6d7890}.v4DeckMini{display:flex;gap:6px;margin-top:8px}.v4DeckMini button{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:#fff;border-radius:10px;padding:6px 8px;font-weight:900;cursor:pointer;}
    #discover .v4FollowRail{display:flex;gap:11px;overflow:auto;padding:2px 0 6px}.v4FollowCard{min-width:285px;height:142px;border:1px solid rgba(255,255,255,.14);border-radius:20px;background:#080d18;overflow:hidden;position:relative;text-align:left;color:#fff;cursor:pointer}.v4FollowCard img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.70}.v4FollowCard:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent,rgba(2,6,23,.86))}.v4FollowCard div{position:absolute;left:12px;right:12px;bottom:11px;z-index:1}.v4FollowCard b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.v4FollowCard span{font-size:12px;color:#d7deee}.v4Small{font-size:13px;color:#9aa7bd}
    @media(min-width:1600px){#discover .v4Page{padding-left:clamp(64px,5vw,110px);padding-right:clamp(64px,5vw,110px)}#discover .v4Stage{grid-template-columns:minmax(0,1fr) minmax(430px,540px)}#discover .v4Player,#discover .v4Card{min-height:min(76vh,920px)}}
    @media(max-width:1150px){#discover .v4Hero{grid-template-columns:1fr}#discover .v4Stage{grid-template-columns:1fr!important}#discover .v4Search{grid-template-columns:1fr 1fr 1fr}#discover .v4ChatFrame{min-height:520px}#discover .v4DeckGrid{grid-template-columns:repeat(2,minmax(0,1fr));}}
    @media(max-width:700px){#discover .v4Page{padding:14px 14px 98px}#discover .v4Shell{gap:16px}#discover .v4Title{font-size:clamp(48px,14vw,76px)}#discover .v4Lead{font-size:13px}.v4Controls{border-radius:22px!important;padding:11px!important}#discover .v4Search{grid-template-columns:1fr}#discover .v4Card{aspect-ratio:auto;min-height:470px;border-radius:24px}#discover .v4Player{min-height:auto;aspect-ratio:16/9;border-radius:24px}#discover .v4Tags{left:12px;right:12px;top:12px;gap:6px}#discover .v4Tag{font-size:10px;padding:6px 8px;max-width:132px}#discover .v4CardText{left:14px;right:14px;bottom:14px}#discover .v4LiveTitle{font-size:clamp(28px,8vw,42px)}#discover .v4Meta{font-size:13px}#discover .v4Actions{grid-template-columns:1fr 1fr 1fr;padding:8px;gap:8px;border-radius:20px}#discover .v4Actions .v4Btn{min-height:50px;font-size:13px;padding:9px 7px;border-radius:15px}#discover .v4Actions .v4Btn.primary{font-size:14px}#discover .v4Panel{border-radius:22px;padding:15px}#discover .v4ChatFrame{min-height:430px}#discover .v4DeckGrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.v4DeckSlot{min-height:92px!important;padding:9px!important}.v4FollowCard{min-width:78vw;height:128px}.v4DeckHead,.v4FollowHead{display:block}}
  `;
  document.head.appendChild(css);
  ...
  const host=()=>location.hostname||'localhost';
  const MOODS=[['petite-commu','Petite commu',''],['chill','Chill','chill'],['discussion','Discussion','Just Chatting'],['nuit-calme','Nuit calme','chill'],['rp','RP','roleplay'],['decouverte-jeu','Découverte jeu','Minecraft']];
  const V4=window.__oryonDiscoverV4=window.__oryonDiscoverV4||{mood:localStorage.getItem('oryon_v4_mood')||'petite-commu',mode:'preview',chat:false,loading:false,category:''};
  const current=()=>{const z=state.zap||{};return (z.items||[])[Number(z.index||0)]||null};
  function norm(x={}){let a={};try{a=typeof liveIdentity==='function'?liveIdentity(x):{};}catch{}let img=a.img||x.img||x.thumbnail_url||x.image_url||'';if(img&&img.includes('{width}'))img=img.replace('{width}','1280').replace('{height}','720');const login=String(a.login||x.login||x.user_login||x.broadcaster_login||x.host_login||x.room||'').toLowerCase();const platform=String(a.platform||x.platform||x.source||(x.host_login||x.room?'oryon':'twitch')).toLowerCase();return {platform,login,name:a.name||x.display_name||x.user_name||x.broadcaster_name||x.host_name||login||'Live',title:a.title||x.title||`Live de ${login||'créateur'}`,game:a.game||x.game_name||x.game||x.category||'Live',viewers:Number(a.viewers??x.viewer_count??x.viewers??0)||0,img:img||'https://static-cdn.jtvnw.net/ttv-static/404_preview-1280x720.jpg',bio:x.description||x.bio||x.user_description||x.about||''};}
  function moodLabel(m){return MOODS.find(x=>x[0]===m)?.[1]||m}
  function moodQuery(m){return MOODS.find(x=>x[0]===m)?.[2]||''}
  ...
  function tags(x){const n=norm(x);return [...new Set([n.platform==='twitch'?'Twitch intégré':'Swapp',n.viewers?`${n.viewers} viewers`:'taille humaine',n.game,'chat lisible'].filter(Boolean))].slice(0,5)}
  function player(x){const n=norm(x);if(n.platform==='twitch'&&n.login)return `<iframe allowfullscreen="true" scrolling="no" src="https://player.twitch.tv/?channel=${E(n.login)}&parent=${E(host())}&muted=false&autoplay=true"></iframe>`;const embed=x?.embed_url||x?.oryon_local_player_url||'';if(n.platform==='oryon'&&embed)return `<iframe allowfullscreen="true" scrolling="no" src="${E(embed)}"></iframe>`;return `<div class="v4Empty"><div><h2>Live Swapp</h2><p>${n.login?'La chaîne Swapp est détectée.':'Le lecteur natif apparaît ici quand la chaîne est lancée.'}</p>${n.login?`<button class="v4Btn primary" onclick="openOryon?.('${E(n.login)}')">Ouvrir la chaîne</button>`:''}</div></div>`}
  function chat(x){const n=norm(x);if(n.platform==='twitch'&&n.login)return `<iframe src="https://www.twitch.tv/embed/${E(n.login)}/chat?parent=${E(host())}&darkpopout"></iframe>`;return `<div class="v4Empty"><div><h2>Chat Swapp</h2><p>Disponible sur la page chaîne native.</p></div></div>`}
  function deckKey(){return 'oryon_v4_deck_'+String(state.session?.local?.login||state.session?.twitch?.login||'guest').toLowerCase()}
  function readDeck(){try{const d=JSON.parse(localStorage.getItem(deckKey())||'[]');return Array.isArray(d)?d.slice(0,8):[]}catch{return []}}
  function writeDeck(d){try{localStorage.setItem(deckKey(),JSON.stringify((d||[]).slice(0,8)))}catch{}}
  function bioPanel(){const x=current(),n=x?norm(x):null;if(!n)return `<aside class="v4Panel v4Bio"><h2>Bio</h2><p>Choisis un mood ou une catégorie pour charger un live.</p></aside>`;return `<aside class="v4Panel v4Bio"><h2>Bio</h2><div class="v4BioTitle">${H(n.name)}</div><p>${H(n.bio||n.title||'Aucune bio disponible. Le titre du live sert de contexte pour l’instant.')}</p><div class="v4BioRows"><div class="v4BioRow"><b>${H(n.game)}</b><span>catégorie</span></div><div class="v4BioRow"><b>${H(n.viewers)} viewers</b><span>audience</span></div><div class="v4BioRow"><b>${H(n.platform==='twitch'?'Twitch':'Swapp')}</b><span>source</span></div><div class="v4BioRow"><b>${H(moodLabel(V4.mood))}</b><span>mood</span></div></div></aside>`}
  function preview(){const x=current();if(V4.loading)return `<div class="v4Empty"><div><h2>Recherche…</h2><p>Swapp cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="v4Empty"><div><h2>Aucun live trouvé.</h2><p>Change de mood ou lance une recherche par jeu.</p><button class="v4Btn primary" onclick="v4FindLive()">Chercher un live</button></div></div>`;const n=norm(x);return `<article id="v4SwipeCard" class="v4Card">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div class="v4Tags">${tags(x).map(t=>`<span class="v4Tag">${H(t)}</span>`).join('')}</div><div class="v4Swipe nope">PAS OUF</div><div class="v4Swipe like">J’AIME</div><div class="v4CardText"><h2 class="v4LiveTitle">${H(n.title)}</h2><div class="v4Meta"><span>${H(n.name)}</span><span>·</span><span>${H(n.game)}</span><span>·</span><span>${n.viewers} viewers</span></div></div></article><div class="v4Actions"><button class="v4Btn" onclick="v4Nope()">Pas ouf</button><button class="v4Btn primary" onclick="v4Watch()">Voir</button><button class="v4Btn" onclick="v4Like()">J’aime</button></div>`}
  function watch(){const x=current();if(!x)return preview();const n=norm(x);return `<main class="v4Main"><section class="v4Player"><div class="v4PlayerBox">${player(x)}</div></section><div class="v4Actions"><button class="v4Btn" onclick="v4Lurk()">LURK</button><button class="v4Btn" onclick="v4ToggleChat()">CHAT</button><button class="v4Btn primary" onclick="v4Next()">SUIVANT</button></div></main>`}
  function deck(){const d=readDeck();return `<section class="v4Deck"><div class="v4DeckHead"><div><h2>DeckLurker</h2><p>Garde tes lives sous la main et switch sans ouvrir dix onglets.</p></div><span class="v4Pill">${d.length}/8</span></div><div class="v4DeckGrid">${Array.from({length:8},(_,i)=>{const s=d[i];if(!s)return `<article class="v4DeckSlot empty"><b>Case ${i+1}</b><span>Vide</span></article>`;return `<article class="v4DeckSlot">${s.img?`<img src="${H(s.img)}" alt="">`:''}<b>${H(s.name||s.login)}</b><span>${H(s.game||s.title||'Live')}</span><div class="v4DeckMini"><button onclick="v4OpenDeck(${i})">Voir</button><button onclick="v4ClearDeck(${i})">Vider</button></div></article>`}).join('')}</div></section>`}
  function followed(){return `<section class="v4Follow"><div class="v4FollowHead"><div><h2>Suivis Twitch en ligne</h2><p>Séparé des recommandations. Ça n’alimente pas Swap ton mood.</p></div>${state.session?.twitch?'<span class="v4Pill">Connecté</span>':'<button class="v4Btn primary" onclick="connectTwitch?.()">Connecter Twitch</button>'}</div><div class="v4FollowRail" id="v4FollowRail">${state.session?.twitch?'<div class="v4Small">Chargement…</div>':'<div class="v4Small">Connexion Twitch nécessaire.</div>'}</div></section>`}
  async function loadFollowed(){const rail=document.getElementById('v4FollowRail');if(!rail||!state.session?.twitch)return;let items=[];try{let r=await api('/api/twitch/followed/live').catch(()=>null);items=r?.items||r?.streams||[];if(!items.length){r=await api('/followed_streams').catch(()=>null);items=r?.items||r?.streams||[];}}catch{}window.__v4Followed=items;rail.innerHTML=items.length?items.slice(0,18).map((x,i)=>{const n=norm({...x,platform:'twitch'});return `<button class="v4FollowCard" onclick="v4UseFollow(${i})">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div><b>${H(n.name)}</b><span>${H(n.game)} · ${n.viewers}</span></div></button>`}).join(''):'<div class="v4Small">Aucun suivi en ligne.</div>'}
  function renderMain(){const el=document.getElementById('v4Main');if(el){el.innerHTML=V4.mode==='watch'?watch():preview();bindSwipe();}}
  function renderAll(){const el=document.getElementById('discover');if(!el)return;document.body.classList.add('oryonDiscoverV4');document.body.classList.remove('oryonDiscoverV3','oryonDiscoverProduct','oryonDiscoverFinalActive','oryonCleanDiscoverActive');const curMood=V4.mood||'petite-commu';el.innerHTML=`<div class="v4Page"><div class="v4Shell"><header class="v4Hero"><div><span class="v4Kicker">Swap ton mood</span><h1 class="v4Title">Swap ton mood.</h1><p class="v4Lead">Choisis une ambiance. Swapp propose un live. Tu swipes, tu ouvres, tu lurk ou tu discutes.</p></div><div class="v4Controls"><div class="v4MoodRail">${MOODS.map(([m,l])=>`<button class="v4Mood ${m===curMood?'active':''}" onclick="v4SetMood('${m}')">${H(l)}</button>`).join('')}</div><div class="v4Search"><input id="v4Query" value="${H(V4.category||'')}" placeholder="jeu, créateur, ambiance" onkeydown="if(event.key==='Enter')v4FindLive()"><select id="v4Max"><option value="50">≤50 viewers</option><option value="150" selected>≤150 viewers</option><option value="300">≤300 viewers</option></select><select id="v4Lang"><option value="fr" selected>FR</option><option value="en">EN</option></select><button class="v4Btn primary" onclick="v4FindLive()">Chercher un live</button></div></div></header><section class="v4Stage"><div class="v4Main" id="v4Main">${V4.mode==='watch'?watch():preview()}</div>${V4.mode==='watch'&&V4.chat?`<aside class="v4Panel v4Chat"><div class="v4ChatHead"><span>Chat du stream</span><button class="v4Btn" onclick="v4ToggleChat()">Fermer</button></div><div class="v4ChatFrame">${chat(current())}</div></aside>`:bioPanel()}</section>${deck()}${followed()}</div></div>`;bindSwipe();loadFollowed();}
  async function fetchPool({game='',autoWatch=false}={}){V4.loading=true;renderMain();const q=String(game||V4.category||document.getElementById('v4Query')?.value||moodQuery(V4.mood)||'').trim();const max=Number(document.getElementById('v4Max')?.value||150);const lang=document.getElementById('v4Lang')?.value||'fr';let items=[];try{let r=await api('/api/oryon/discover/find-live?'+qs({q,mood:V4.mood,max,lang,source:'both',discover:'1'})).catch(()=>null);items=(r?.items||[]).filter(x=>(x.platform||'')!=='peertube');if(!items.length&&q){const rr=await api('/api/twitch/random-small-live?'+qs({game:q,max,language:lang})).catch(()=>null);if(rr?.success&&rr.target)items=[{...rr.target,platform:'twitch',login:rr.target.login||rr.target.user_login,display_name:rr.target.name||rr.target.display_name,user_name:rr.target.name,viewer_count:rr.target.viewers,game_name:rr.target.game||q,thumbnail_url:rr.target.thumbnail_url||rr.target.img,title:rr.target.title}];}if(!items.length&&!q){r=await api('/api/oryon/discover/find-live?'+qs({q:moodQuery(V4.mood)||'',mood:V4.mood,max,lang,source:'both',discover:'1'})).catch(()=>null);items=(r?.items||[]).filter(x=>(x.platform||'')!=='peertube');}items.sort((a,b)=>Math.abs((Number(norm(a).viewers)||0)-45)-Math.abs((Number(norm(b).viewers)||0)-45));setItems(items);V4.mode=autoWatch?'watch':'preview';V4.chat=false;}catch(e){console.error(e);setItems([]);V4.mode='preview';V4.chat=false;}finally{V4.loading=false;renderAll();}}
  function bindSwipe(){const card=document.getElementById('v4SwipeCard');if(!card||card.__v4bound)return;card.__v4bound=true;let sx=0,sy=0,dx=0,active=false;const reset=()=>{active=false;card.style.transform='';card.classList.remove('like','nope')};card.addEventListener('pointerdown',e=>{if(e.target.closest('button,input,select,textarea'))return;active=true;sx=e.clientX;sy=e.clientY;dx=0;card.setPointerCapture?.(e.pointerId)});card.addEventListener('pointermove',e=>{if(!active)return;dx=e.clientX-sx;const dy=e.clientY-sy;if(Math.abs(dy)>90&&Math.abs(dx)<30)return;const rot=Math.max(-14,Math.min(14,dx/15));card.style.transform=`translateX(${dx*.26}px) rotate(${rot}deg)`;card.classList.toggle('like',dx>40);card.classList.toggle('nope',dx<-40)});card.addEventListener('pointerup',()=>{if(!active)return;const f=dx;reset();if(f>95)v4Like();else if(f<-95)v4Nope()});card.addEventListener('pointercancel',reset);}
  window.v4SetMood=m=>{V4.mood=m;localStorage.setItem('oryon_v4_mood',m);V4.category='';V4.mode='preview';V4.chat=false;return fetchPool({game:moodQuery(m)});};
  window.v4FindLive=()=>{V4.category=String(document.getElementById('v4Query')?.value||'').trim();V4.mode='preview';V4.chat=false;return fetchPool({game:V4.category});};
  window.v4Watch=()=>{if(!current())return;V4.mode='watch';V4.chat=false;renderAll();};
  window.v4ToggleChat=()=>{if(V4.mode!=='watch')V4.mode='watch';V4.chat=!V4.chat;renderAll();};
  window.v4Next=()=>{const items=state.zap?.items||[];if(items.length>1){state.zap.index=(Number(state.zap.index||0)+1)%items.length;state.zap.last=current();V4.mode='preview';V4.chat=false;renderAll();}else fetchPool({game:V4.category||moodQuery(V4.mood)});};
  window.v4Nope=()=>v4Next();
  window.v4Like=()=>{try{const n=norm(current()||{});const arr=JSON.parse(localStorage.getItem('oryon_v4_likes')||'[]');if(n.login&&!arr.some(x=>x.platform===n.platform&&x.login===n.login)){arr.unshift({platform:n.platform,login:n.login,name:n.name,game:n.game,ts:Date.now()});localStorage.setItem('oryon_v4_likes',JSON.stringify(arr.slice(0,100)));}}catch{}v4Next();};
  window.v4Lurk=()=>{const x=current();if(!x)return;const d=readDeck();const n=norm(x);const meta={platform:n.platform,login:n.login,name:n.name,title:n.title,game:n.game,viewers:n.viewers,img:n.img,addedAt:Date.now()};const k=(meta.platform+':'+meta.login).toLowerCase();if(!d.some(s=>s&&(s.platform+':'+String(s.login)).toLowerCase()===k))d.unshift(meta);writeDeck(d);toast?.('Ajouté au DeckLurker');renderAll();};
  window.v4OpenDeck=i=>{const s=readDeck()[i];if(!s)return;setItems([s,...(state.zap?.items||[])]);V4.mode='watch';V4.chat=false;renderAll();};
  window.v4ClearDeck=i=>{const d=readDeck();d.splice(i,1);writeDeck(d);renderAll();};
  window.v4UseFollow=i=>{const x=(window.__v4Followed||[])[i];if(!x)return;setItems([{...x,platform:'twitch'},...(state.zap?.items||[])]);V4.mode='watch';V4.chat=false;renderAll();};
  window.renderDiscover=renderDiscover=async function renderDiscoverV4(){renderAll();if(!(state.zap?.items||[]).length&&!V4.loading)await fetchPool({game:V4.category||moodQuery(V4.mood)});};
  window.findLive=window.v4FindLive;window.renderZap=renderMain;window.zapNext=window.v4Next;window.zapOpenCurrent=window.v4Watch;window.oryonAddCurrentToDeck=window.v4Lurk;window.oryonToggleDiscoverChat=window.v4ToggleChat;
  window.pickCat=async function(name,img=''){V4.category=String(name||'').trim();V4.mood='decouverte-jeu';localStorage.setItem('oryon_v4_mood',V4.mood);await setView('discover');await fetchPool({game:V4.category,autoWatch:true});};
  window.pickCatEncoded=(name,img='')=>window.pickCat(decodeURIComponent(name||''),decodeURIComponent(img||''));
...
/* =========================================================
   SWAPP HOTFIX — Twitch connect must not break layout
...
/* =========================================================
   SWAPP CATEGORY VISUAL RESTORE — keep category page as before
...
/* =========================================================
   SWAPP DISCOVERY LEARNING BRIDGE — persistent viewer memory
    ...
    try{
      const r = await fetch('/api/oryon/viewer/choice', {
        ...
        headers:{
          ...
          'X-Swapp-Viewer-Id': ensureAnonViewerId()
        },
