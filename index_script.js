...
async function api(url,opt={}){try{const r=await fetch(url,{credentials:'include',headers:{'Content-Type':'application/json',...(opt.headers||{})},...opt});let j;try{j=await r.json()}catch{j={success:false,error:await r.text()}} if(!r.ok&&j.success!==false)j.success=false; return j}catch(e){console.warn('[Swapp] API indisponible:',url,e?.message||e);return {success:false,error:'network_error',items:[],categories:[],gifs:[],local:null,twitch:null}}}
...
function swappStartupFallback(e){try{console.error('[Swapp] démarrage UI:',e);const views=document.querySelectorAll('.view');views.forEach(v=>v.classList.toggle('active',v.id==='home'));const home=document.getElementById('home');if(home&&!home.innerHTML.trim()){home.innerHTML='<div class="swappHomeFinal"><section class="swappHomeStage"><div class="swappHomeCopy"><span class="eyebrow"><i class="dot"></i>Swapp</span><h1>Des lives à taille humaine.</h1><p>L’accueil reste disponible même si les API ne répondent pas encore. Lance le serveur Node pour activer les lives, Twitch et les comptes.</p><div class="swappHomeActions"><button class="btn" onclick="setView(\'discover\')">Découvrir</button><button class="btn secondary" onclick="setView(\'settings\')">Connexion et créer un compte</button></div></div><div class="swappLivePlaceholder"><div><h2>Swapp démarre.</h2><p>Si tu as ouvert le fichier HTML directement, démarre plutôt l’application avec npm start.</p></div></div></section></div>';}document.title='Swapp — découvre les lives qu’on ne te montre pas';}catch(_){}}
init().catch(swappStartupFallback);
...
/* =========================================================
   ORYON UNIFIED DISCOVER MODULE REPAIR — no competing modules
...
(function installOryonUnifiedDiscoverRepair(){
  ...
  function renderWatch(x){
    ...
    if(id.platform==='oryon'){
      zap.innerHTML=`<section class="oryonWatchBox"><div class="oryonWatchPlayer"><div class="discoverFallbackCard"><div><h2>${safe(id.name)}</h2><p>Live Oryon natif.</p><button class="btn" onclick="openOryon('${safe(id.login)}')">Ouvrir la chaîne</button></div></div></div></section>`;
    } else {
...
/* =========================================================
   ORYON DISCOVER STABLE V2 — final deterministic repair
...
(function installOryonDiscoverStableV2(){
  ...
  function tagsFor(x){ const id=liveId(x); const t=[]; t.push(id.platform==='oryon'?'Oryon':'Twitch intégré'); if(id.viewers)t.push(id.viewers<80?'petit live':'audience active'); t.push('entrée normale'); if(id.game)t.push(id.game); return [...new Set(t)].slice(0,4); }
...
/* =========================================================
   ORYON SAFE DISCOVER GUARD — fullwidth, no URL, no black screen
   ========================================================= */
...
/* =========================================================
   ORYON DISCOVER — FINAL REBUILD FROM ZERO
...
(function installOryonDiscoverFinalRebuild(){
  ...
  function tagList(x){const id=idOf(x);return [...new Set([id.platform==='oryon'?'Oryon':'Twitch intégré',id.viewers?`${id.viewers} viewers`:'taille humaine',id.game,'chat lisible'].filter(Boolean))].slice(0,5);}
  ...
  function playerHtml(x){const id=idOf(x);if(id.platform==='twitch')return `<iframe allowfullscreen allow="autoplay; fullscreen" src="https://player.twitch.tv/?channel=${enc(id.login)}&parent=${enc(host())}&autoplay=true&muted=false"></iframe>`;return `<div class="sdEmpty"><div><h2>${safe(id.name)}</h2><p>Live Oryon sélectionné.</p><button class="sdBtn primary" onclick="openOryon?.('${safe(id.login)}')">Ouvrir la chaîne</button></div></div>`;}
  function chatHtml(x){const id=idOf(x);if(id.platform==='twitch')return `<iframe src="https://www.twitch.tv/embed/${enc(id.login)}/chat?parent=${enc(host())}&darkpopout" title="Chat Twitch ${safe(id.login)}"></iframe>`;return `<div class="sdEmpty"><div><h2>Chat Oryon</h2><p>Le chat Oryon complet est disponible sur la page chaîne.</p></div></div>`;}
  function previewHtml(){const x=cur();if(sd.loading)return `<div class="sdEmpty"><div><h2>Recherche…</h2><p>Oryon cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="sdEmpty"><div><h2>Aucun live trouvé</h2><p>Change de mood ou relance la recherche.</p><button class="sdBtn primary" onclick="sdFindLive()">Chercher un live</button></div></div>`;const id=idOf(x);return `<article id="sdSwipeCard" class="sdCard"><img src="${safe(id.img)}" alt=""><div class="sdTags">${tagList(x).map(t=>`<span class="sdTag">${safe(t)}</span>`).join('')}</div><div class="sdSwipeText nope">PAS OUF</div><div class="sdSwipeText like">J’AIME</div><div class="sdCardText"><h2 class="sdTitle">${safe(id.title)}</h2><div class="sdMeta"><span>${safe(id.name)}</span><span>·</span><span>${safe(id.game)}</span><span>·</span><span>${id.viewers} viewers</span></div></div></article><div class="sdActions"><button class="sdBtn" onclick="sdNope()">Pas ouf</button><button class="sdBtn primary" onclick="sdWatch()">Voir</button><button class="sdBtn" onclick="sdLike()">J’aime</button></div>`;}
  function watchHtml(){const x=cur();if(!x)return previewHtml();const id=idOf(x);return `<div class="sdWatchGrid ${sd.chat?'':'noChat'}"><main class="sdPlayerShell"><div class="sdPlayerHead"><div><b>${safe(id.name)}</b><div class="sdSmall">${safe(id.game)} · ${id.viewers} viewers</div></div><span class="sdPill">${safe(id.platform==='twitch'?'Twitch':'Oryon')}</span></div><div class="sdPlayerBox">${playerHtml(x)}</div><div class="sdActions"><button class="sdBtn" onclick="sdLurk()">LURK</button><button class="sdBtn" onclick="sdToggleChat()">CHAT</button><button class="sdBtn primary" onclick="sdNext()">SUIVANT</button></div></main>${sd.chat?`<aside class="sdChatShell"><div class="sdChatHead"><span>Chat du stream</span><button class="sdBtn" onclick="sdToggleChat()">Fermer</button></div><div class="sdChatFrame">${chatHtml(x)}</div></aside>`:''}</div>`;}
...
/* =========================================================
   ORYON DISCOVER — PRODUCT GRADE OVERRIDE
...
(function installOryonDiscoverProductGrade(){
  ...
  function readDeck(){try{const d=JSON.parse(localStorage.getItem(deckKey())||'[]');return Array.isArray(d)?d.slice(0,8):[]}catch{return []}}
  function writeDeck(d){try{localStorage.setItem(deckKey(),JSON.stringify((d||[]).slice(0,8)))}catch{}}
  function tags(x){const n=id(x);return [n.platform==='twitch'?'Twitch intégré':'Oryon natif',n.viewers?`${n.viewers} viewers`:'petit live',n.game,'entrée douce'].filter(Boolean).slice(0,5)}
  ...
  function player(x){const n=id(x);return n.platform==='twitch'?`<iframe allowfullscreen allow="autoplay; fullscreen" src="https://player.twitch.tv/?channel=${E(n.login)}&parent=${E(host())}&autoplay=true&muted=false"></iframe>`:`<div class="mxEmpty"><div><h2>${H(n.name)}</h2><p>Live Oryon sélectionné.</p><button class="mxBtn primary" onclick="openOryon?.('${H(n.login)}')">Ouvrir la chaîne</button></div></div>`}
  function chat(x){const n=id(x);return n.platform==='twitch'?`<iframe title="Chat Twitch" src="https://www.twitch.tv/embed/${E(n.login)}/chat?parent=${E(host())}&darkpopout"></iframe>`:`<div class="mxEmpty"><div><h2>Chat Oryon</h2><p>Disponible sur la page chaîne.</p></div></div>`}
  function preview(){const x=cur();if(mx.loading)return `<div class="mxEmpty"><div><h2>Recherche…</h2><p>Oryon cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="mxEmpty"><div><h2>Aucun live trouvé.</h2><p>Change de mood ou relance une recherche.</p><button class="mxBtn primary" onclick="mxFindLive()">Chercher un live</button></div></div>`;const n=id(x);return `<article id="mxSwipeCard" class="mxCard">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div class="mxTags">${tags(x).map(t=>`<span class="mxTag">${H(t)}</span>`).join('')}</div><div class="mxSwipe nope">PAS OUF</div><div class="mxSwipe like">J’AIME</div><div class="mxCardText"><h2 class="mxTitle">${H(n.title)}</h2><div class="mxMeta"><span>${H(n.name)}</span><span>·</span><span>${H(n.game)}</span><span>·</span><span>${n.viewers} viewers</span></div></div></article><div class="mxActions"><button class="mxBtn" onclick="mxNope()">Pas ouf</button><button class="mxBtn primary" onclick="mxWatch()">Voir</button><button class="mxBtn" onclick="mxLike()">J’aime</button></div>`}
  function side(){const x=cur(), n=x?id(x):null;return `<aside class="mxSide"><section class="mxPanel"><h2>Pourquoi ce live ?</h2>${n?`<div class="mxSignals"><div class="mxSignal"><b>${H(moods.find(m=>m[0]===mx.mood)?.[1]||'Ambiance')}</b><span>mood</span></div><div class="mxSignal"><b>${n.viewers} viewers</b><span>taille humaine</span></div><div class="mxSignal"><b>${H(n.game)}</b><span>catégorie</span></div><div class="mxSignal"><b>${n.platform==='twitch'?'Twitch':'Oryon'}</b><span>source</span></div></div>`:'<p>Choisis un mood et lance une proposition.</p>'}</section><section class="mxPanel"><h2>Swap clair</h2><p>Avant lecture : Pas ouf / Voir / J’aime. Après Voir : Lurk / Chat / Suivant.</p></section></aside>`}
  function watch(){const x=cur();if(!x)return preview();const n=id(x);return `<main class="mxPlayer"><div class="mxPlayerHead"><div><b>${H(n.name)}</b><div class="mxSmall">${H(n.game)} · ${n.viewers} viewers</div></div><span class="mxPill">${n.platform==='twitch'?'Twitch':'Oryon'}</span></div><div class="mxPlayerBox">${player(x)}</div><div class="mxActions"><button class="mxBtn" onclick="mxLurk()">LURK</button><button class="mxBtn" onclick="mxToggleChat()">CHAT</button><button class="mxBtn primary" onclick="mxNext()">SUIVANT</button></div></main>`}
...
/* =========================================================
   ORYON DISCOVER — DESKTOP CHAT RIGHT / PRODUCT LAYOUT V3
...
(function installOryonDiscoverV3(){
  ...
  function player(x){const n=id(x);return n.platform==='twitch'?`<iframe allowfullscreen allow="autoplay; fullscreen" src="https://player.twitch.tv/?channel=${E(n.login)}&parent=${E(host())}&autoplay=true&muted=false"></iframe>`:`<div class="dxEmpty"><div><h2>${H(n.name)}</h2><p>Live Oryon sélectionné.</p></div></div>`;}
  function chat(x){const n=id(x);return n.platform==='twitch'?`<iframe title="Chat Twitch" src="https://www.twitch.tv/embed/${E(n.login)}/chat?parent=${E(host())}&darkpopout"></iframe>`:`<div class="dxEmpty"><div><h2>Chat Oryon</h2><p>Disponible sur la page chaîne.</p></div></div>`;}
  function preview(){const x=cur();if(dx.loading)return `<div class="dxEmpty"><div><h2>Recherche…</h2><p>Oryon cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="dxEmpty"><div><h2>Aucun live trouvé.</h2><p>Change de mood ou relance une recherche.</p><button class="dxBtn primary" onclick="dxFindLive()">Chercher un live</button></div></div>`;const n=id(x);return `<article id="dxSwipeCard" class="dxCard">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div class="dxTags">${tags(x).map(t=>`<span class="dxTag">${H(t)}</span>`).join('')}</div><div class="dxSwipe nope">PAS OUF</div><div class="dxSwipe like">J’AIME</div><div class="dxCardText"><h2 class="dxLiveTitle">${H(n.title)}</h2><div class="dxMeta"><span>${H(n.name)}</span><span>·</span><span>${H(n.game)}</span><span>·</span><span>${n.viewers} viewers</span></div></div></article><div class="dxActions"><button class="dxBtn" onclick="dxNope()">Pas ouf</button><button class="dxBtn primary" onclick="dxWatch()">Voir</button><button class="dxBtn" onclick="dxLike()">J’aime</button></div>`;}
  function side(){const x=cur(),n=x?id(x):null;return `<aside class="dxSide"><section class="dxPanel"><h2>Pourquoi ce live ?</h2>${n?`<div class="dxSignals"><div class="dxSignal"><b>${H(moods.find(m=>m[0]===dx.mood)?.[1]||'Ambiance')}</b><span>mood</span></div><div class="dxSignal"><b>${n.viewers} viewers</b><span>taille humaine</span></div><div class="dxSignal"><b>${H(n.game)}</b><span>catégorie</span></div><div class="dxSignal"><b>${n.platform==='twitch'?'Twitch':'Oryon'}</b><span>source</span></div></div>`:'<p>Choisis un mood et lance une proposition.</p>'}</section><section class="dxPanel"><h2>Lecture propre</h2><p>Voir ouvre le live. Chat apparaît à droite sur PC. Lurk garde le live dans DeckLurker.</p></section></aside>`;}
  function watch(){const x=cur();if(!x)return preview();const n=id(x);return `<main class="dxPlayer"><div class="dxPlayerHead"><div><b>${H(n.name)}</b><div class="dxSmall">${H(n.game)} · ${n.viewers} viewers</div></div><span class="dxPill">${n.platform==='twitch'?'Twitch':'Oryon'}</span></div><div class="dxPlayerBox">${player(x)}</div><div class="dxActions"><button class="dxBtn" onclick="dxLurk()">LURK</button><button class="dxBtn" onclick="dxToggleChat()">CHAT</button><button class="dxBtn primary" onclick="dxNext()">SUIVANT</button></div></main>`;}
...
/* =========================================================
   ORYON DISCOVER V4 — clean full-width product page
...
(function installOryonDiscoverV4(){
  ...
  function readDeck(){try{const d=JSON.parse(localStorage.getItem(deckKey())||'[]');return Array.isArray(d)?d.slice(0,8):[]}catch{return []}}
  function writeDeck(d){try{localStorage.setItem(deckKey(),JSON.stringify((d||[]).slice(0,8)))}catch{}}
  function bioPanel(){const x=current(),n=x?norm(x):null;if(!n)return `<aside class="v4Panel v4Bio"><h2>Bio</h2><p>Choisis un mood ou une catégorie pour charger un live.</p></aside>`;return `<aside class="v4Panel v4Bio"><h2>Bio</h2><div class="v4BioTitle">${H(n.name)}</div><p>${H(n.bio||n.title||'Aucune bio disponible. Le titre du live sert de contexte pour l’instant.')}</p><div class="v4BioRows"><div class="v4BioRow"><b>${H(n.game)}</b><span>catégorie</span></div><div class="v4BioRow"><b>${H(n.viewers)} viewers</b><span>audience</span></div><div class="v4BioRow"><b>${H(n.platform==='twitch'?'Twitch':'Oryon')}</b><span>source</span></div><div class="v4BioRow"><b>${H(moodLabel(V4.mood))}</b><span>mood</span></div></div></aside>`}
  function preview(){const x=current();if(V4.loading)return `<div class="v4Empty"><div><h2>Recherche…</h2><p>Oryon cherche un live qui colle à ton mood.</p></div></div>`;if(!x)return `<div class="v4Empty"><div><h2>Aucun live trouvé.</h2><p>Change de mood ou lance une recherche par jeu.</p><button class="v4Btn primary" onclick="v4FindLive()">Chercher un live</button></div></div>`;const n=norm(x);return `<article id="v4SwipeCard" class="v4Card">${n.img?`<img src="${H(n.img)}" alt="">`:''}<div class="v4Tags">${tags(x).map(t=>`<span class="v4Tag">${H(t)}</span>`).join('')}</div><div class="v4Swipe nope">PAS OUF</div><div class="v4Swipe like">J’AIME</div><div class="v4CardText"><h2 class="v4LiveTitle">${H(n.title)}</h2><div class="v4Meta"><span>${H(n.name)}</span><span>·</span><span>${H(n.game)}</span><span>·</span><span>${n.viewers} viewers</span></div></div></article><div class="v4Actions"><button class="v4Btn" onclick="v4Nope()">Pas ouf</button><button class="v4Btn primary" onclick="v4Watch()">Voir</button><button class="v4Btn" onclick="v4Like()">J’aime</button></div>`}
