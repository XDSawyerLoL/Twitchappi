...
(function(){
  ...
  window.openMarketOverlay = async function(){
    ...
    // Require Twitch login for multi-user market (server will also enforce it)
    try{
      const r = await fetch('/twitch_user_status', { credentials:'include' });
      const j = await r.json().catch(()=>({}));
      if(!j || !j.connected){
        alert("Pour utiliser le Marché du streamer, tu dois être connecté à Twitch.");
        // Try opening OAuth in a new tab (fallback)
        window.open('/auth/twitch', '_blank', 'noopener');
        return;
      }
    }catch(_){
      // If status fails, server will still block market calls.
    }
