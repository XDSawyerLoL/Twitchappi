...
// Always include cookies (important when the app runs inside an iframe)
...
nav.querySelectorAll('.u-tab-btn').forEach(b=>b.classList.remove('active'));
    ...
    async function initFirebaseStatus() {
      async function checkStatus() {
        try {
          const response = await apiFetch('/firebase_status');
    ...
    async function checkAuth() {
      const res = await apiFetch('/twitch_user_status');
      const data = await res.json();
      window.__twitchConnected = !!data.is_connected;
      if (data.is_connected) {
        currentUser = data.display_name;
        window.currentUser = data.display_name;
        window.__twitchConnected = true;
        window.__twitchUser = data;
    ...
    function logout() { apiFetch('/twitch_logout', { method:'POST' }).then(()=>location.reload()); }

    // PLAYER
    async function initPlayer() {
      const data = await apiJson('/get_default_stream');
      if (data.success) {
    ...
    async function cycle(dir){
      const res = await apiFetch('/cycle_stream',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
    ...
    async function loadFollowed(){
      ...
      try{
        const res = await apiFetch('/followed_streams');
        const data = await res.json();
    ...
    async function openGifModal(){
      ...
      async function loadTrending(){
        const r = await apiFetch('/api/gifs/trending?limit=28');
      ...
      search.oninput = () => {
        ...
        t = setTimeout(async () => {
          ...
          const r = await apiFetch('/api/gifs/search?q=${encodeURIComponent(q)}&limit=28');
          const j = await r.json();
          if (!j.success) {
    ...
    async function tfResolveTrailerId(gameName){
      ...
      try{
        ...
        const r = await apiFetch('/api/youtube/trailer?q=${encodeURIComponent(q)}');
        if (r.ok){
...
tfModalOpen = false;
    ...
    async function tfRunSearch(query){
      ...
      try{
        const r = await apiFetch('/api/categories/search?q=${encodeURIComponent(q)}');
        if (r.ok){
          const d = await r.json();
    ...
    async function tfGetPreviewChannel(gameId){
      ...
      const p = (async () => {
        try{
          const res = await apiFetch('/api/stream/by_category',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
...
// BEST TIME
    ...
    async function analyzeBestTime(){
      ...
      try{
        const response = await apiFetch('/analyze_schedule',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
    ...
    async function loadStatsDashboard(){
      try{
        const res = await apiFetch('/api/stats/global');
        const data = await res.json();
        if (data.success){
        ...
        const resGames = await apiFetch('/api/stats/top_games');
        ...
        const resLang = await apiFetch('/api/stats/languages');
    ...
    async function loadChannelProData(login){
      ...
      try{
        const res = await apiFetch('/api/analytics/channel_by_login/${encodeURIComponent(login)}?days=30');
        const data = await res.json();
    ...
    async function loadAlerts(login){
      ...
      try{
        let r = await apiFetch('/api/alerts/channel_by_login/${encodeURIComponent(login)}?limit=8');
        let data = await r.json();

        if (!data.success || !data.items || data.items.length === 0){
          await apiFetch('/api/alerts/generate',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({ login, days:30 })
          }).catch(()=>{});

          r = await apiFetch('/api/alerts/channel_by_login/${encodeURIComponent(login)}?limit=8');
          data = await r.json();
        }
    ...
    async function loadGameHours(gameId){
      try{
        const r = await apiFetch('/api/games/hours?game_id=${encodeURIComponent(gameId)}&days=7');
    ...
    async function loadAIReco(){
      ...
      try{
        const res = await apiFetch('/api/ai/reco?login=${encodeURIComponent(currentChannel)}&days=30');
        const data = await res.json();
    ...
    async function runSimulation(){
      ...
      out.innerText = '...';
      try{
        const res = await apiFetch('/api/simulate/growth?channel_id=${encodeURIComponent(currentChannelId)}&hours_per_week=${encodeURIComponent(hours)}&days=30');
        const data = await res.json();
    ...
    async function loadCoStreamer(){
      ...
      try{
        const r = await apiFetch('/api/costream/best?login=${encodeURIComponent(currentChannel)}&days=14');
        const data = await r.json();
    ...
    async function runScan(){
      ...
      try{
        const r = await apiFetch('/scan_target',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
        ...
        if (data.type === 'user'){
          ...
          const ai = await apiFetch('/critique_ia',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
    ...
    async function runRaid(){
      ...
      try{
        const r = await apiFetch('/start_raid',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
    ...
    async function runBoost(){
      ...
      msg.innerText = '...';
      try{
        const r = await apiFetch('/stream_boost',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({ channel })
        });
