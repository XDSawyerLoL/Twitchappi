...
nav.querySelectorAll('.u-tab-btn').forEach(b=>b.classList.remove('active'));
  ...
  document.body.classList.add('twitflix-open');
const modal = document.getElementById('twitflix-modal');
      ...
      // ensure we start at the top (prevents "cut" feeling after reopening)
      const body = document.getElementById('tf-body');
      if (body){
        try{ body.scrollTop = 0; }catch(_){/* noop */}
        setTimeout(() => { try{ body.scrollTop = 0; }catch(_){/* noop */} }, 0);
      }
  ...
  document.body.classList.remove('twitflix-open');
tfModalOpen = false;
